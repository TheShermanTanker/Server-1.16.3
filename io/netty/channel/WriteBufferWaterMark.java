...
public final class WriteBufferWaterMark {
    ...
    WriteBufferWaterMark(final int low, final int high, final boolean validate) {
        if (validate) {
            ...
            if (high < low) {
                throw new IllegalArgumentException(new StringBuilder().append("write buffer's high water mark cannot be less than  low water mark (").append(low).append("): ").append(high).toString());
            }
        }
        this.low = low;
        this.high = high;
