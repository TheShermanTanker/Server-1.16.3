...
public class OioSctpChannel extends AbstractOioMessageChannel implements SctpChannel {
    ...
    private static com.sun.nio.sctp.SctpChannel openChannel() {
        try {
            ...
            throw new ChannelException("Failed to open a sctp channel.", (Throwable)e);
        }
    }
    ...
    public OioSctpChannel(final Channel parent, final com.sun.nio.sctp.SctpChannel ch) {
        ...
        try {
            ...
            throw new ChannelException("failed to initialize a sctp channel", (Throwable)e);
        }
        finally {
            if (!success) {
                try {
                    ...
                    OioSctpChannel.logger.warn("Failed to close a sctp channel.", (Throwable)e2);
                }
            }
        }
    }
    ...
    protected int doReadMessages(final List<Object> msgs) throws Exception {
        ...
        try {
            ...
            final MessageInfo messageInfo = this.ch.receive(data, null, (NotificationHandler)this.notificationHandler);
            if (messageInfo == null) {
                return readMessages;
            }
            data.flip();
    ...
    protected void doWrite(final ChannelOutboundBuffer in) throws Exception {
        ...
        final Set<SelectionKey> writableKeys = (Set<SelectionKey>)this.writeSelector.selectedKeys();
        if (writableKeys.isEmpty()) {
            return;
        }
        final Iterator<SelectionKey> writableKeysIt = (Iterator<SelectionKey>)writableKeys.iterator();
        int written = 0;
        while (written != size) {
            writableKeysIt.next();
            ...
            final MessageInfo mi = MessageInfo.createOutgoing(this.association(), (SocketAddress)null, packet.streamIdentifier());
    ...
    protected SocketAddress localAddress0() {
        try {
            final Iterator<SocketAddress> i = (Iterator<SocketAddress>)this.ch.getAllLocalAddresses().iterator();
            if (i.hasNext()) {
    ...
    public Set<InetSocketAddress> allLocalAddresses() {
        try {
            final Set<SocketAddress> allLocalAddresses = (Set<SocketAddress>)this.ch.getAllLocalAddresses();
            final Set<InetSocketAddress> addresses = (Set<InetSocketAddress>)new LinkedHashSet(allLocalAddresses.size());
            ...
            return (Set<InetSocketAddress>)Collections.emptySet();
        }
    }
    
    protected SocketAddress remoteAddress0() {
        try {
            final Iterator<SocketAddress> i = (Iterator<SocketAddress>)this.ch.getRemoteAddresses().iterator();
            if (i.hasNext()) {
                return (SocketAddress)i.next();
            }
        }
    ...
    public Set<InetSocketAddress> allRemoteAddresses() {
        try {
            final Set<SocketAddress> allLocalAddresses = (Set<SocketAddress>)this.ch.getRemoteAddresses();
            final Set<InetSocketAddress> addresses = (Set<InetSocketAddress>)new LinkedHashSet(allLocalAddresses.size());
            for (final SocketAddress socketAddress : allLocalAddresses) {
                addresses.add(socketAddress);
            }
        ...
        try {
            ...
            while (!finishConnect) {
                if (this.connectSelector.select(1000L) >= 0) {
                    final Set<SelectionKey> selectionKeys = (Set<SelectionKey>)this.connectSelector.selectedKeys();
    ...
    private static void closeSelector(final String selectorName, final Selector selector) {
        try {
            ...
            OioSctpChannel.logger.warn("Failed to close a " + selectorName + " selector.", (Throwable)e);
        }
    }
    ...
    private final class OioSctpChannelConfig extends DefaultSctpChannelConfig {
        ...
        protected void autoReadCleared() {
            AbstractOioChannel.this.clearReadPending();
        }
    }
}
