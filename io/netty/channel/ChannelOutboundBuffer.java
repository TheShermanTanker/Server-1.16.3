...
public final class ChannelOutboundBuffer {
    ...
    private void clearNioBuffers() {
        ...
        if (count > 0) {
            ...
            Arrays.fill((Object[])ChannelOutboundBuffer.NIO_BUFFERS.get(), 0, count, null);
        }
    }
                ...
                if (readableBytes > 0) {
                    ...
                    if (count == 1) {
                        ...
                        for (int i = 0; i < nioBufs.length && nioBufferCount < maxCount; ++i) {
                            final ByteBuffer nioBuf2 = nioBufs[i];
                            if (nioBuf2 == null) {
                                break;
                            }
                            if (nioBuf2.hasRemaining()) {
                                nioBuffers[nioBufferCount++] = nioBuf2;
                            }
                        }
                    }
    ...
    private static int writabilityMask(final int index) {
        if (index < 1 || index > 31) {
            throw new IllegalArgumentException(new StringBuilder().append("index: ").append(index).append(" (expected: 1~31)").toString());
        }
        return 1 << index;
    }
    ...
    private void fireChannelWritabilityChanged(final boolean invokeLater) {
        ...
        if (invokeLater) {
            ...
            if (task == null) {
                task = (this.fireChannelWritabilityChangedTask = (Runnable)new Runnable() {
                    public void run() {
    ...
    void close(final Throwable cause, final boolean allowChannelOpen) {
        ...
        try {
            for (Entry e = this.unflushedEntry; e != null; e = e.recycleAndGetNext()) {
                final int size = e.pendingSize;
                ChannelOutboundBuffer.TOTAL_PENDING_SIZE_UPDATER.addAndGet(this, (long)(-size));
                if (!e.cancelled) {
    ...
    void close(final ClosedChannelException cause) {
        this.close((Throwable)cause, false);
    }
    
    private static void safeSuccess(final ChannelPromise promise) {
        PromiseNotificationUtil.trySuccess(promise, null, (promise instanceof VoidChannelPromise) ? null : ChannelOutboundBuffer.logger);
    }
    ...
    static {
        ...
        TOTAL_PENDING_SIZE_UPDATER = AtomicLongFieldUpdater.newUpdater((Class)ChannelOutboundBuffer.class, "totalPendingSize");
        UNWRITABLE_UPDATER = AtomicIntegerFieldUpdater.newUpdater((Class)ChannelOutboundBuffer.class, "unwritable");
    }
    ...
    public interface MessageProcessor {
        boolean processMessage(final Object object) throws Exception;
    }
}
