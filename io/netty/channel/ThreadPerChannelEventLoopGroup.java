...
public class ThreadPerChannelEventLoopGroup extends AbstractEventExecutorGroup implements EventLoopGroup {
    ...
    protected ThreadPerChannelEventLoopGroup(final int maxChannels, final Executor executor, final Object... args) {
        this.activeChildren = (Set<EventLoop>)Collections.newSetFromMap((Map)PlatformDependent.newConcurrentHashMap());
        this.idleChildren = (Queue<EventLoop>)new ConcurrentLinkedQueue();
        ...
        if (maxChannels < 0) {
            throw new IllegalArgumentException(String.format("maxChannels: %d (expected: >= 0)", new Object[] { maxChannels }));
        }
        if (executor == null) {
        ...
        this.tooManyChannels = ThrowableUtil.<ChannelException>unknownStackTrace(new ChannelException(new StringBuilder().append("too many channels (max: ").append(maxChannels).append(')').toString()), ThreadPerChannelEventLoopGroup.class, "nextChild()");
    }
    ...
    public Iterator<EventExecutor> iterator() {
        return (Iterator<EventExecutor>)new ReadOnlyIterator((java.util.Iterator<?>)this.activeChildren.iterator());
    }
