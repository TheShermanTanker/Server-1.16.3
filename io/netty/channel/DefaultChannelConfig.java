...
public class DefaultChannelConfig implements ChannelConfig {
    ...
    protected Map<ChannelOption<?>, Object> getOptions(Map<ChannelOption<?>, Object> result, final ChannelOption<?>... options) {
        if (result == null) {
            result = (Map<ChannelOption<?>, Object>)new IdentityHashMap();
        }
    ...
    public <T> T getOption(final ChannelOption<T> option) {
        ...
        if (option == ChannelOption.RCVBUF_ALLOCATOR) {
            return this.<T>getRecvByteBufAllocator();
        }
    ...
    public ChannelConfig setConnectTimeoutMillis(final int connectTimeoutMillis) {
        if (connectTimeoutMillis < 0) {
            throw new IllegalArgumentException(String.format("connectTimeoutMillis: %d (expected: >= 0)", new Object[] { connectTimeoutMillis }));
        }
    ...
    public int getMaxMessagesPerRead() {
        try {
            final MaxMessagesRecvByteBufAllocator allocator = this.<MaxMessagesRecvByteBufAllocator>getRecvByteBufAllocator();
            return allocator.maxMessagesPerRead();
        }
        catch (ClassCastException e) {
            throw new IllegalStateException("getRecvByteBufAllocator() must return an object of type MaxMessagesRecvByteBufAllocator", (Throwable)e);
        }
    }
    ...
    public ChannelConfig setRecvByteBufAllocator(final RecvByteBufAllocator allocator) {
        this.rcvBufAllocator = ObjectUtil.<RecvByteBufAllocator>checkNotNull(allocator, "allocator");
        return this;
    }
    ...
    public ChannelConfig setAutoRead(final boolean autoRead) {
        final boolean oldAutoRead = DefaultChannelConfig.AUTOREAD_UPDATER.getAndSet(this, (int)(autoRead ? 1 : 0)) == 1;
    ...
    public ChannelConfig setWriteBufferHighWaterMark(final int writeBufferHighWaterMark) {
        ...
        while (true) {
            ...
            if (writeBufferHighWaterMark < waterMark.low()) {
                throw new IllegalArgumentException(new StringBuilder().append("writeBufferHighWaterMark cannot be less than writeBufferLowWaterMark (").append(waterMark.low()).append("): ").append(writeBufferHighWaterMark).toString());
            }
    ...
    public ChannelConfig setWriteBufferLowWaterMark(final int writeBufferLowWaterMark) {
        ...
        while (true) {
            ...
            if (writeBufferLowWaterMark > waterMark.high()) {
                throw new IllegalArgumentException(new StringBuilder().append("writeBufferLowWaterMark cannot be greater than writeBufferHighWaterMark (").append(waterMark.high()).append("): ").append(writeBufferLowWaterMark).toString());
            }
    ...
    public ChannelConfig setWriteBufferWaterMark(final WriteBufferWaterMark writeBufferWaterMark) {
        this.writeBufferWaterMark = ObjectUtil.<WriteBufferWaterMark>checkNotNull(writeBufferWaterMark, "writeBufferWaterMark");
        return this;
    }
    ...
    static {
        ...
        AUTOREAD_UPDATER = AtomicIntegerFieldUpdater.newUpdater((Class)DefaultChannelConfig.class, "autoRead");
        WATERMARK_UPDATER = AtomicReferenceFieldUpdater.newUpdater((Class)DefaultChannelConfig.class, (Class)WriteBufferWaterMark.class, "writeBufferWaterMark");
    }
}
