...
public abstract class AbstractChannel extends DefaultAttributeMap implements Channel {
    ...
    protected abstract boolean isCompatible(final EventLoop eventLoop);
    ...
    protected abstract void doBind(final SocketAddress socketAddress) throws Exception;
    ...
    protected abstract void doWrite(final ChannelOutboundBuffer channelOutboundBuffer) throws Exception;
    ...
    static {
        ...
        FLUSH0_CLOSED_CHANNEL_EXCEPTION = ThrowableUtil.<ClosedChannelException>unknownStackTrace(new ClosedChannelException(), AbstractUnsafe.class, "flush0()");
        ENSURE_OPEN_CLOSED_CHANNEL_EXCEPTION = ThrowableUtil.<ClosedChannelException>unknownStackTrace(new ClosedChannelException(), AbstractUnsafe.class, "ensureOpen(...)");
        CLOSE_CLOSED_CHANNEL_EXCEPTION = ThrowableUtil.<ClosedChannelException>unknownStackTrace(new ClosedChannelException(), AbstractUnsafe.class, "close(...)");
        WRITE_CLOSED_CHANNEL_EXCEPTION = ThrowableUtil.<ClosedChannelException>unknownStackTrace(new ClosedChannelException(), AbstractUnsafe.class, "write(...)");
        FLUSH0_NOT_YET_CONNECTED_EXCEPTION = ThrowableUtil.<NotYetConnectedException>unknownStackTrace(new NotYetConnectedException(), AbstractUnsafe.class, "flush0()");
    }
    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        public final void bind(final SocketAddress localAddress, final ChannelPromise promise) {
            ...
            if (Boolean.TRUE.equals(AbstractChannel.this.config().<Boolean>getOption(ChannelOption.SO_BROADCAST)) && localAddress instanceof InetSocketAddress && !((InetSocketAddress)localAddress).getAddress().isAnyLocalAddress() && !PlatformDependent.isWindows() && !PlatformDependent.maybeSuperUser()) {
                AbstractChannel.logger.warn(new StringBuilder().append("A non-root user can't receive a broadcast packet if the socket is not bound to a wildcard address; binding to a non-wildcard address (").append(localAddress).append(") anyway as requested.").toString());
            }
            ...
            if (!wasActive && AbstractChannel.this.isActive()) {
                this.invokeLater((Runnable)new Runnable() {
                    public void run() {
                        AbstractChannel.this.pipeline.fireChannelActive();
                    }
                });
        ...
        public final void close(final ChannelPromise promise) {
            this.assertEventLoop();
            this.close(promise, (Throwable)AbstractChannel.CLOSE_CLOSED_CHANNEL_EXCEPTION, AbstractChannel.CLOSE_CLOSED_CHANNEL_EXCEPTION, false);
        }
        ...
        private void shutdownOutput(final ChannelPromise promise, final Throwable cause) {
            if (!promise.setUncancellable()) {
                return;
            }
            final ChannelOutboundBuffer outboundBuffer = this.outboundBuffer;
            if (outboundBuffer == null) {
                promise.setFailure((Throwable)AbstractChannel.CLOSE_CLOSED_CHANNEL_EXCEPTION);
                return;
            }
            this.outboundBuffer = null;
            final Throwable shutdownCause = (Throwable)((cause == null) ? new ChannelOutputShutdownException("Channel output shutdown") : new ChannelOutputShutdownException("Channel output shutdown", cause));
                ...
                closeExecutor.execute((Runnable)new Runnable() {
                    public void run() {
                        try {
                            ...
                            AbstractChannel.this.eventLoop().execute((Runnable)new Runnable() {
                                public void run() {
        ...
        private void closeOutboundBufferForShutdown(final ChannelPipeline pipeline, final ChannelOutboundBuffer buffer, final Throwable cause) {
                ...
                closeExecutor.execute((Runnable)new Runnable() {
                    public void run() {
                        try {
                            ...
                            AbstractUnsafe.this.invokeLater((Runnable)new Runnable() {
                                public void run() {
                                    if (outboundBuffer != null) {
        ...
        public final void write(Object msg, final ChannelPromise promise) {
            ...
            final ChannelOutboundBuffer outboundBuffer = this.outboundBuffer;
            if (outboundBuffer == null) {
                this.safeSetFailure(promise, (Throwable)AbstractChannel.WRITE_CLOSED_CHANNEL_EXCEPTION);
            ...
            if (!AbstractChannel.this.isActive()) {
                try {
                    if (AbstractChannel.this.isOpen()) {
                        outboundBuffer.failFlushed((Throwable)AbstractChannel.FLUSH0_NOT_YET_CONNECTED_EXCEPTION, true);
                    }
                    else {
                        outboundBuffer.failFlushed((Throwable)AbstractChannel.FLUSH0_CLOSED_CHANNEL_EXCEPTION, false);
                    }
                }
                finally {
                    this.inFlush0 = false;
            ...
            try {
                ...
                if (t instanceof IOException && AbstractChannel.this.config().isAutoClose()) {
                    ...
                    try {
                        ...
                        this.close(this.voidPromise(), t2, AbstractChannel.FLUSH0_CLOSED_CHANNEL_EXCEPTION, false);
                    }
                }
            }
            finally {
        ...
        protected final boolean ensureOpen(final ChannelPromise promise) {
            ...
            this.safeSetFailure(promise, (Throwable)AbstractChannel.ENSURE_OPEN_CLOSED_CHANNEL_EXCEPTION);
            return false;
        }
        ...
        protected final Throwable annotateConnectException(final Throwable cause, final SocketAddress remoteAddress) {
            if (cause instanceof ConnectException) {
                return (Throwable)new AnnotatedConnectException((ConnectException)cause, remoteAddress);
            }
            if (cause instanceof NoRouteToHostException) {
                return (Throwable)new AnnotatedNoRouteToHostException((NoRouteToHostException)cause, remoteAddress);
            }
            if (cause instanceof SocketException) {
                return (Throwable)new AnnotatedSocketException((SocketException)cause, remoteAddress);
            }
            return cause;
        }
