...
public class DefaultFileRegion extends AbstractReferenceCounted implements FileRegion {
    ...
    public DefaultFileRegion(final FileChannel file, final long position, final long count) {
        ...
        if (position < 0L) {
            throw new IllegalArgumentException(new StringBuilder().append("position must be >= 0 but was ").append(position).toString());
        }
        if (count < 0L) {
            throw new IllegalArgumentException(new StringBuilder().append("count must be >= 0 but was ").append(count).toString());
        }
        this.file = file;
    ...
    public long transferTo(final WritableByteChannel target, final long position) throws IOException {
        ...
        if (count < 0L || position < 0L) {
            throw new IllegalArgumentException(new StringBuilder().append("position out of range: ").append(position).append(" (expected: 0 - ").append(this.count - 1L).append(')').toString());
        }
        if (count == 0L) {
            return 0L;
        }
    ...
    protected void deallocate() {
        ...
        try {
            ...
            if (DefaultFileRegion.logger.isWarnEnabled()) {
                DefaultFileRegion.logger.warn("Failed to close a file.", (Throwable)e);
            }
        }
    }
