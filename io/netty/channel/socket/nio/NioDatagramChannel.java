...
public final class NioDatagramChannel extends AbstractNioMessageChannel implements DatagramChannel {
    ...
    public boolean isActive() {
        ...
        return ch.isOpen() && ((this.config.<Boolean>getOption(ChannelOption.DATAGRAM_CHANNEL_ACTIVE_ON_REGISTRATION) && this.isRegistered()) || ch.socket().isBound());
    }
        ...
        try {
            ...
            synchronized (this) {
                ...
                if (this.memberships == null) {
                    this.memberships = (Map<InetAddress, List<MembershipKey>>)new HashMap();
                }
                else {
                    keys = (List<MembershipKey>)this.memberships.get(multicastAddress);
                }
                if (keys == null) {
                    keys = (List<MembershipKey>)new ArrayList();
    ...
    public ChannelFuture leaveGroup(final InetAddress multicastAddress, final NetworkInterface networkInterface, final InetAddress source, final ChannelPromise promise) {
        ...
        synchronized (this) {
            if (this.memberships != null) {
                final List<MembershipKey> keys = (List<MembershipKey>)this.memberships.get(multicastAddress);
                if (keys != null) {
                    final Iterator<MembershipKey> keyIt = (Iterator<MembershipKey>)keys.iterator();
                    while (keyIt.hasNext()) {
                        final MembershipKey key = (MembershipKey)keyIt.next();
    ...
    protected boolean closeOnReadError(final Throwable cause) {
        return !(cause instanceof SocketException) && super.closeOnReadError(cause);
    }
