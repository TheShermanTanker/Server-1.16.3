...
public abstract class AbstractCoalescingBufferQueue {
    ...
    protected AbstractCoalescingBufferQueue(final Channel channel, final int initSize) {
        this.bufAndListenerPairs = (ArrayDeque<Object>)new ArrayDeque(initSize);
    ...
    public final ByteBuf removeFirst(final ChannelPromise aggregatePromise) {
        ...
        if (entry instanceof ChannelFutureListener) {
            aggregatePromise.addListener(entry);
    ...
    public final ByteBuf remove(final ByteBufAllocator alloc, int bytes, final ChannelPromise aggregatePromise) {
        ...
        ObjectUtil.<ChannelPromise>checkNotNull(aggregatePromise, "aggregatePromise");
    ...
    public final void copyTo(final AbstractCoalescingBufferQueue dest) {
        dest.bufAndListenerPairs.addAll((Collection)this.bufAndListenerPairs);
        ...
        while (true) {
            ...
            try {
                ...
                if (entry instanceof ByteBuf) {
                    ...
                    ctx.write(previousBuf).addListener(entry);
                    previousBuf = null;
                }
            }
            catch (Throwable t) {
    ...
    protected abstract ByteBuf compose(final ByteBufAllocator byteBufAllocator, final ByteBuf byteBuf2, final ByteBuf byteBuf3);
    ...
    private void incrementReadableBytes(final int increment) {
        ...
        if (nextReadableBytes < this.readableBytes) {
            throw new IllegalStateException(new StringBuilder().append("buffer queue length overflow: ").append(this.readableBytes).append(" + ").append(increment).toString());
        }
