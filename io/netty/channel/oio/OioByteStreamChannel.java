...
public abstract class OioByteStreamChannel extends AbstractOioByteChannel {
    ...
    private static void checkEOF(final FileRegion region) throws IOException {
        if (region.transferred() < region.count()) {
            throw new EOFException(new StringBuilder().append("Expected to be able to write ").append(region.count()).append(" bytes, but only wrote ").append(region.transferred()).toString());
        }
    }
