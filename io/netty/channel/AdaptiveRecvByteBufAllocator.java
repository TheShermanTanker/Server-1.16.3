...
public class AdaptiveRecvByteBufAllocator extends DefaultMaxMessagesRecvByteBufAllocator {
    ...
    public AdaptiveRecvByteBufAllocator(final int minimum, final int initial, final int maximum) {
        if (minimum <= 0) {
            throw new IllegalArgumentException(new StringBuilder().append("minimum: ").append(minimum).toString());
        }
        if (initial < minimum) {
            throw new IllegalArgumentException(new StringBuilder().append("initial: ").append(initial).toString());
        }
        if (maximum < initial) {
            throw new IllegalArgumentException(new StringBuilder().append("maximum: ").append(maximum).toString());
        }
    ...
    static {
        final List<Integer> sizeTable = (List<Integer>)new ArrayList();
        ...
        for (int i = 0; i < AdaptiveRecvByteBufAllocator.SIZE_TABLE.length; ++i) {
            AdaptiveRecvByteBufAllocator.SIZE_TABLE[i] = (int)sizeTable.get(i);
        }
    ...
    private final class HandleImpl extends MaxMessageHandle {
        ...
        public HandleImpl(final int minIndex, final int maxIndex, final int initial) {
            this.minIndex = minIndex;
            this.maxIndex = maxIndex;
        ...
        private void record(final int actualReadBytes) {
