...
public class LocalChannel extends AbstractChannel {
    ...
    public LocalChannel() {
        ...
        this.readTask = (Runnable)new Runnable() {
            public void run() {
        ...
        this.shutdownHook = (Runnable)new Runnable() {
            public void run() {
    ...
    protected void doRegister() throws Exception {
        if (this.peer != null && this.parent() != null) {
            ...
            peer.eventLoop().execute((Runnable)new Runnable() {
                public void run() {
    ...
    protected void doClose() throws Exception {
        ...
        try {
            if (oldState != State.CLOSED) {
                ...
                if (promise != null) {
                    promise.tryFailure((Throwable)LocalChannel.DO_CLOSE_CLOSED_CHANNEL_EXCEPTION);
            ...
            if (peer != null) {
                ...
                try {
                    peerEventLoop.execute((Runnable)new Runnable() {
                        public void run() {
                    ...
                    while (true) {
                        ...
                        try {
                            if (peer.state == State.CONNECTED) {
                                ...
                                in.remove((Throwable)LocalChannel.DO_WRITE_CLOSED_CHANNEL_EXCEPTION);
                            }
                        }
                        catch (Throwable cause) {
                            in.remove(cause);
    ...
    private void runFinishPeerReadTask(final LocalChannel peer) {
        final Runnable finishPeerReadTask = (Runnable)new Runnable() {
            public void run() {
    ...
    static {
        ...
        FINISH_READ_FUTURE_UPDATER = AtomicReferenceFieldUpdater.newUpdater((Class)LocalChannel.class, (Class)Future.class, "finishReadFuture");
        METADATA = new ChannelMetadata(false);
        DO_WRITE_CLOSED_CHANNEL_EXCEPTION = ThrowableUtil.<ClosedChannelException>unknownStackTrace(new ClosedChannelException(), LocalChannel.class, "doWrite(...)");
        DO_CLOSE_CLOSED_CHANNEL_EXCEPTION = ThrowableUtil.<ClosedChannelException>unknownStackTrace(new ClosedChannelException(), LocalChannel.class, "doClose()");
    }
    ...
    private class LocalUnsafe extends AbstractUnsafe {
        public void connect(final SocketAddress remoteAddress, SocketAddress localAddress, final ChannelPromise promise) {
            ...
            if (LocalChannel.this.state == State.CONNECTED) {
                final Exception cause = (Exception)new AlreadyConnectedException();
            ...
            if (!(boundChannel instanceof LocalServerChannel)) {
                final Exception cause2 = (Exception)new ConnectException(new StringBuilder().append("connection refused: ").append(remoteAddress).toString());
                this.safeSetFailure(promise, (Throwable)cause2);
                this.close(this.voidPromise());
                return;
            }
