...
final class DefaultChannelGroupFuture extends DefaultPromise<Void> implements ChannelGroupFuture {
    ...
    DefaultChannelGroupFuture(final ChannelGroup group, final Collection<ChannelFuture> futures, final EventExecutor executor) {
            ...
            public void operationComplete(final ChannelFuture future) throws Exception {
                ...
                if (callSetDone) {
                    if (DefaultChannelGroupFuture.this.failureCount > 0) {
                        final List<Map.Entry<Channel, Throwable>> failed = (List<Map.Entry<Channel, Throwable>>)new ArrayList(DefaultChannelGroupFuture.this.failureCount);
                        for (final ChannelFuture f : DefaultChannelGroupFuture.this.futures.values()) {
                            if (!f.isSuccess()) {
                                failed.add(new DefaultEntry(f.channel(), f.cause()));
                            }
                        }
                        DefaultChannelGroupFuture.this.setFailure0(new ChannelGroupException((Collection<Map.Entry<Channel, Throwable>>)failed));
                    }
                    else {
        ...
        this.group = group;
        final Map<Channel, ChannelFuture> futureMap = (Map<Channel, ChannelFuture>)new LinkedHashMap();
        for (final ChannelFuture f : futures) {
            futureMap.put(f.channel(), f);
        }
        this.futures = (Map<Channel, ChannelFuture>)Collections.unmodifiableMap((Map)futureMap);
    ...
    DefaultChannelGroupFuture(final ChannelGroup group, final Map<Channel, ChannelFuture> futures, final EventExecutor executor) {
        ...
        this.group = group;
        this.futures = (Map<Channel, ChannelFuture>)Collections.unmodifiableMap((Map)futures);
        for (final ChannelFuture f : this.futures.values()) {
            f.addListener(this.childListener);
        }
        if (this.futures.isEmpty()) {
    ...
    public Iterator<ChannelFuture> iterator() {
        return (Iterator<ChannelFuture>)this.futures.values().iterator();
    }
