...
abstract class AbstractKQueueChannel extends AbstractChannel implements UnixChannel {
    ...
    AbstractKQueueChannel(final Channel parent, final BsdSocket fd, final boolean active) {
        ...
        this.socket = ObjectUtil.<BsdSocket>checkNotNull(fd, "fd");
        this.active = active;
        if (active) {
    ...
    abstract class AbstractKQueueUnsafe extends AbstractUnsafe {
        ...
        AbstractKQueueUnsafe() {
            this.readReadyRunnable = (Runnable)new Runnable() {
                public void run() {
        ...
        abstract void readReady(final KQueueRecvByteAllocatorHandle kQueueRecvByteAllocatorHandle);
        ...
        private boolean doFinishConnect() throws Exception {
            if (AbstractKQueueChannel.this.socket.finishConnect()) {
                ...
                if (AbstractKQueueChannel.this.requestedRemoteAddress instanceof InetSocketAddress) {
                    AbstractKQueueChannel.this.remote = (SocketAddress)UnixChannelUtil.computeRemoteAddr((InetSocketAddress)AbstractKQueueChannel.this.requestedRemoteAddress, AbstractKQueueChannel.this.socket.remoteAddress());
                }
