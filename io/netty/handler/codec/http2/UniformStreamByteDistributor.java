...
public final class UniformStreamByteDistributor implements StreamByteDistributor {
    ...
    public UniformStreamByteDistributor(final Http2Connection connection) {
        this.queue = (Deque<State>)new ArrayDeque(4);
        ...
        connectionStream.<State>setProperty(this.stateKey, new State(connectionStream));
        connection.addListener(new Http2ConnectionAdapter() {
            @Override
            public void onStreamAdded(final Http2Stream stream) {
                stream.<State>setProperty(UniformStreamByteDistributor.this.stateKey, new State(stream));
            }
    ...
    private State state(final Http2Stream stream) {
        return ObjectUtil.<Http2Stream>checkNotNull(stream, "stream").<State>getProperty(this.stateKey);
    }
    ...
    private final class State {
        ...
        void updateStreamableBytes(final int newStreamableBytes, final boolean hasFrame, final int windowSize) {
            if (!State.$assertionsDisabled && !hasFrame && newStreamableBytes != 0) {
                throw new AssertionError(new StringBuilder().append("hasFrame: ").append(hasFrame).append(" newStreamableBytes: ").append(newStreamableBytes).toString());
            }
