...
public class DelegatingDecompressorFrameListener extends Http2FrameListenerDecorator {
    ...
    private void initDecompressor(final ChannelHandlerContext ctx, final int streamId, final Http2Headers headers, final boolean endOfStream) throws Http2Exception {
        ...
        if (decompressor == null && !endOfStream) {
            CharSequence contentEncoding = ((Headers<CharSequence, CharSequence, T>)headers).get((CharSequence)HttpHeaderNames.CONTENT_ENCODING);
            if (contentEncoding == null) {
                contentEncoding = (CharSequence)HttpHeaderValues.IDENTITY;
            }
            final EmbeddedChannel channel = this.newContentDecompressor(ctx, contentEncoding);
            if (channel != null) {
                decompressor = new Http2Decompressor(channel);
                stream.<Http2Decompressor>setProperty(this.propertyKey, decompressor);
                ...
                if (HttpHeaderValues.IDENTITY.contentEqualsIgnoreCase(targetContentEncoding)) {
                    ((Headers<CharSequence, V, T>)headers).remove((CharSequence)HttpHeaderNames.CONTENT_ENCODING);
                }
                else {
                    ((Headers<CharSequence, CharSequence, Headers>)headers).set((CharSequence)HttpHeaderNames.CONTENT_ENCODING, targetContentEncoding);
                }
            }
        }
        if (decompressor != null) {
            ((Headers<CharSequence, V, T>)headers).remove((CharSequence)HttpHeaderNames.CONTENT_LENGTH);
    ...
    private static ByteBuf nextReadableBuf(final EmbeddedChannel decompressor) {
        while (true) {
            final ByteBuf buf = decompressor.<ByteBuf>readInbound();
            if (buf == null) {
                return null;
            }
            if (buf.isReadable()) {
    ...
    private final class ConsumedBytesConverter implements Http2LocalFlowController {
        ...
        ConsumedBytesConverter(final Http2LocalFlowController flowController) {
            this.flowController = ObjectUtil.<Http2LocalFlowController>checkNotNull(flowController, "flowController");
        }
    ...
    private static final class Http2Decompressor {
        ...
        int consumeBytes(final int streamId, final int decompressedBytes) throws Http2Exception {
            if (decompressedBytes < 0) {
                throw new IllegalArgumentException(new StringBuilder().append("decompressedBytes must not be negative: ").append(decompressedBytes).toString());
            }
