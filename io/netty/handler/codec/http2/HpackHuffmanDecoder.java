...
final class HpackHuffmanDecoder {
    ...
    static {
        EOS_DECODED = ThrowableUtil.<Http2Exception>unknownStackTrace(Http2Exception.connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - EOS Decoded"), HpackHuffmanDecoder.class, "decode(..)");
        INVALID_PADDING = ThrowableUtil.<Http2Exception>unknownStackTrace(Http2Exception.connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - Invalid Padding"), HpackHuffmanDecoder.class, "decode(..)");
    ...
    private static final class DecoderProcessor implements ByteProcessor {
        ...
        private void append(final int i) {
            if (this.bytes.length == this.index) {
                final int newLength = (this.bytes.length >= 1024) ? (this.bytes.length + this.initialCapacity) : (this.bytes.length << 1);
