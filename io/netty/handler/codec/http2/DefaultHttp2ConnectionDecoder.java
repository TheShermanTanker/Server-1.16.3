...
public class DefaultHttp2ConnectionDecoder implements Http2ConnectionDecoder {
    ...
    public DefaultHttp2ConnectionDecoder(final Http2Connection connection, final Http2ConnectionEncoder encoder, final Http2FrameReader frameReader, final Http2PromisedRequestVerifier requestVerifier) {
        ...
        this.frameReader = ObjectUtil.<Http2FrameReader>checkNotNull(frameReader, "frameReader");
        this.encoder = ObjectUtil.<Http2ConnectionEncoder>checkNotNull(encoder, "encoder");
        this.requestVerifier = ObjectUtil.<Http2PromisedRequestVerifier>checkNotNull(requestVerifier, "requestVerifier");
    ...
    public void frameListener(final Http2FrameListener listener) {
        this.listener = ObjectUtil.<Http2FrameListener>checkNotNull(listener, "listener");
    }
    ...
    private final class FrameReadListener implements Http2FrameListener {
        ...
        private void applyLocalSettings(final Http2Settings settings) throws Http2Exception {
            ...
            if (maxConcurrentStreams != null) {
                DefaultHttp2ConnectionDecoder.this.connection.remote().maxActiveStreams((int)Math.min((long)maxConcurrentStreams, 2147483647L));
            }
            ...
            if (stream == null) {
                ...
                if (stream.isResetSent() || this.streamCreatedAfterGoAwaySent(streamId)) {
                    if (DefaultHttp2ConnectionDecoder.logger.isInfoEnabled()) {
                        DefaultHttp2ConnectionDecoder.logger.info("{} ignoring {} frame for stream {} {}", ctx.channel(), frameName, stream.isResetSent() ? "RST_STREAM sent." : new StringBuilder().append("Stream created after GOAWAY sent. Last known stream by peer ").append(DefaultHttp2ConnectionDecoder.this.connection.remote().lastStreamKnownByPeer()).toString());
                    }
                    return true;
                }
                return false;
