...
public final class WeightedFairQueueByteDistributor implements StreamByteDistributor {
    ...
    public WeightedFairQueueByteDistributor(final Http2Connection connection, final int maxStateOnlySize) {
        ...
        if (maxStateOnlySize < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("maxStateOnlySize: ").append(maxStateOnlySize).append(" (expected: >0)").toString());
        }
        if (maxStateOnlySize == 0) {
            this.stateOnlyMap = IntCollections.<State>emptyMap();
            this.stateOnlyRemovalQueue = EmptyPriorityQueue.instance();
        }
        else {
            this.stateOnlyMap = new IntObjectHashMap<State>(maxStateOnlySize);
            this.stateOnlyRemovalQueue = new DefaultPriorityQueue<State>((java.util.Comparator<State>)StateOnlyComparator.INSTANCE, maxStateOnlySize + 2);
        }
        ...
        connectionStream.<State>setProperty(this.stateKey, this.connectionState = new State(connectionStream, 16));
        connection.addListener(new Http2ConnectionAdapter() {
            ...
            public void onStreamAdded(final Http2Stream stream) {
                ...
                if (state == null) {
                    ...
                    final List<ParentChangedEvent> events = (List<ParentChangedEvent>)new ArrayList(1);
                ...
                stream.<State>setProperty(WeightedFairQueueByteDistributor.this.stateKey, state);
            }
    ...
    public void updateDependencyTree(final int childStreamId, final int parentStreamId, final short weight, final boolean exclusive) {
        ...
        if (newParent != state.parent || (exclusive && newParent.children.size() != 1)) {
            ...
            if (newParent.isDescendantOf(state)) {
                events = (List<ParentChangedEvent>)new ArrayList(2 + (exclusive ? newParent.children.size() : 0));
                state.parent.takeChild(newParent, false, events);
            }
            else {
                events = (List<ParentChangedEvent>)new ArrayList(1 + (exclusive ? newParent.children.size() : 0));
            }
            newParent.takeChild(state, exclusive, events);
            this.notifyParentChanged(events);
        }
        while (this.stateOnlyRemovalQueue.size() > this.maxStateOnlySize) {
            final State stateToRemove = (State)this.stateOnlyRemovalQueue.poll();
            stateToRemove.parent.removeChild(stateToRemove);
    ...
    private int distributeToChildren(final int maxBytes, final Writer writer, final State state) throws Http2Exception {
        ...
        try {
            assert nextChildState.pseudoTimeToWrite >= childState.pseudoTimeToWrite : new StringBuilder().append("nextChildState[").append(nextChildState.streamId).append("].pseudoTime(").append(nextChildState.pseudoTimeToWrite).append(") <  childState[").append(childState.streamId).append("].pseudoTime(").append(childState.pseudoTimeToWrite).append(")").toString();
    ...
    void notifyParentChanged(final List<ParentChangedEvent> events) {
        for (int i = 0; i < events.size(); ++i) {
            final ParentChangedEvent event = (ParentChangedEvent)events.get(i);
    ...
    private final class State implements PriorityQueueNode {
        ...
        State(final int streamId, final Http2Stream stream, final int initialSize) {
            this.children = IntCollections.<State>emptyMap();
            this.pseudoTimeQueueIndex = -1;
            this.stateOnlyQueueIndex = -1;
            this.weight = 16;
            this.stream = stream;
            this.streamId = streamId;
            this.pseudoTimeQueue = new DefaultPriorityQueue<State>((java.util.Comparator<State>)StatePseudoTimeComparator.INSTANCE, initialSize);
        }
        ...
        void takeChild(final Iterator<IntObjectMap.PrimitiveEntry<State>> childItr, final State child, final boolean exclusive, final List<ParentChangedEvent> events) {
            ...
            if (exclusive && !this.children.isEmpty()) {
                final Iterator<IntObjectMap.PrimitiveEntry<State>> itr = (Iterator<IntObjectMap.PrimitiveEntry<State>>)this.removeAllChildrenExcept(child).entries().iterator();
                while (itr.hasNext()) {
                    child.takeChild(itr, ((IntObjectMap.PrimitiveEntry)itr.next()).value(), false, events);
                }
            }
        }
        ...
        void removeChild(final State child) {
            if (this.children.remove(child.streamId) != null) {
                final List<ParentChangedEvent> events = (List<ParentChangedEvent>)new ArrayList(1 + child.children.size());
                events.add(new ParentChangedEvent(child, child.parent));
                child.setParent(null);
                final Iterator<IntObjectMap.PrimitiveEntry<State>> itr = (Iterator<IntObjectMap.PrimitiveEntry<State>>)child.children.entries().iterator();
                while (itr.hasNext()) {
                    this.takeChild(itr, ((IntObjectMap.PrimitiveEntry)itr.next()).value(), false, events);
                }
                WeightedFairQueueByteDistributor.this.notifyParentChanged(events);
            }
        }
        ...
        private void initChildrenIfEmpty() {
            if (this.children == IntCollections.<State>emptyMap()) {
                this.initChildren();
            }
        }
        ...
        void activeCountChangeForTree(final int increment) {
            ...
            if (this.parent != null) {
                assert !(!this.parent.pseudoTimeQueue.containsTyped(this)) : new StringBuilder().append("State[").append(this.streamId).append("].activeCountForTree changed from 0 to ").append(increment).append(" is in a pseudoTimeQueue, but not in parent[ ").append(this.parent.streamId).append("]'s pseudoTimeQueue").toString();
        ...
        State pollPseudoTimeQueue() {
            final State state = (State)this.pseudoTimeQueue.poll();
            this.totalQueuedWeights -= state.weight;
            return state;
        }
