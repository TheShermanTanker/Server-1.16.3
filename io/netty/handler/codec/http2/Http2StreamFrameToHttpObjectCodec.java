...
public class Http2StreamFrameToHttpObjectCodec extends MessageToMessageCodec<Http2StreamFrame, HttpObject> {
    ...
    protected void encode(final ChannelHandlerContext ctx, final HttpObject obj, final List<Object> out) throws Exception {
        if (obj instanceof HttpResponse) {
            ...
            if (res.status().equals(HttpResponseStatus.CONTINUE)) {
                if (res instanceof FullHttpResponse) {
                    final Http2Headers headers = this.toHttp2Headers(res);
        ...
        if (obj instanceof HttpMessage) {
            final Http2Headers headers2 = this.toHttp2Headers((HttpMessage)obj);
    ...
    private Http2Headers toHttp2Headers(final HttpMessage msg) {
        if (msg instanceof HttpRequest) {
            msg.headers().set((CharSequence)HttpConversionUtil.ExtensionHeaderNames.SCHEME.text(), this.scheme.name());
        }
    ...
    protected boolean isSsl(final ChannelHandlerContext ctx) {
        ...
        return null != connChannel.pipeline().<SslHandler>get(SslHandler.class);
    }
}
