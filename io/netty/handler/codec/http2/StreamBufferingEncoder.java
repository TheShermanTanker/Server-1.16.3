...
public class StreamBufferingEncoder extends DecoratingHttp2ConnectionEncoder {
    ...
    public StreamBufferingEncoder(final Http2ConnectionEncoder delegate, final int initialMaxConcurrentStreams) {
        super(delegate);
        this.pendingStreams = (TreeMap<Integer, PendingStream>)new TreeMap();
    ...
    public ChannelFuture writeRstStream(final ChannelHandlerContext ctx, final int streamId, final long errorCode, final ChannelPromise promise) {
        ...
        final PendingStream stream = (PendingStream)this.pendingStreams.remove(streamId);
        if (stream != null) {
            stream.close(null);
            promise.setSuccess();
        }
    ...
    public ChannelFuture writeData(final ChannelHandlerContext ctx, final int streamId, final ByteBuf data, final int padding, final boolean endOfStream, final ChannelPromise promise) {
        ...
        final PendingStream pendingStream = (PendingStream)this.pendingStreams.get(streamId);
        if (pendingStream != null) {
        ...
        try {
            if (!this.closed) {
                ...
                while (!this.pendingStreams.isEmpty()) {
                    final PendingStream stream = (PendingStream)this.pendingStreams.pollFirstEntry().getValue();
                    stream.close((Throwable)e);
                }
            }
        }
        finally {
    ...
    private void tryCreatePendingStreams() {
        while (!this.pendingStreams.isEmpty() && this.canCreateStream()) {
            final Map.Entry<Integer, PendingStream> entry = (Map.Entry<Integer, PendingStream>)this.pendingStreams.pollFirstEntry();
            final PendingStream pendingStream = (PendingStream)entry.getValue();
            try {
    ...
    private void cancelGoAwayStreams(final int lastStreamId, final long errorCode, final ByteBuf debugData) {
        final Iterator<PendingStream> iter = (Iterator<PendingStream>)this.pendingStreams.values().iterator();
        final Exception e = new Http2GoAwayException(lastStreamId, errorCode, ByteBufUtil.getBytes(debugData));
        while (iter.hasNext()) {
            final PendingStream stream = (PendingStream)iter.next();
    ...
    private static final class PendingStream {
        ...
        PendingStream(final ChannelHandlerContext ctx, final int streamId) {
            this.frames = (Queue<Frame>)new ArrayDeque(2);
            this.ctx = ctx;
    ...
    private abstract static class Frame {
        ...
        abstract void send(final ChannelHandlerContext channelHandlerContext, final int integer);
    }
