...
public class DefaultHttp2FrameReader implements Http2FrameReader, Http2FrameSizePolicy, Configuration {
    ...
    private void verifyHeadersFrame() throws Http2Exception {
        ...
        if (this.payloadLength < requiredLength) {
            throw Http2Exception.streamError(this.streamId, Http2Error.FRAME_SIZE_ERROR, new StringBuilder().append("Frame length too small.").append(this.payloadLength).toString());
        }
    }
        ...
        if (this.flags.ack()) {
            ...
            for (int index = 0; index < numSettings; ++index) {
                ...
                try {
                    ...
                    switch (id) {
                        case '\u0005': {
                            throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, (Throwable)e, e.getMessage());
                        }
                        case '\u0004': {
                            throw Http2Exception.connectionError(Http2Error.FLOW_CONTROL_ERROR, (Throwable)e, e.getMessage());
                        }
                        default: {
                            throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, (Throwable)e, e.getMessage());
                        }
    ...
    private void readPingFrame(final ChannelHandlerContext ctx, final long data, final Http2FrameListener listener) throws Http2Exception {
        if (this.flags.ack()) {
    ...
    private abstract class HeadersContinuation {
        ...
        abstract void processFragment(final boolean boolean1, final ByteBuf byteBuf, final Http2FrameListener http2FrameListener) throws Http2Exception;
