...
public class DefaultHttp2RemoteFlowController implements Http2RemoteFlowController {
    ...
    public DefaultHttp2RemoteFlowController(final Http2Connection connection, final StreamByteDistributor streamByteDistributor, final Listener listener) {
        ...
        this.streamByteDistributor = ObjectUtil.<StreamByteDistributor>checkNotNull(streamByteDistributor, "streamWriteDistributor");
        this.stateKey = connection.newKey();
        this.connectionState = new FlowState(connection.connectionStream());
        connection.connectionStream().<FlowState>setProperty(this.stateKey, this.connectionState);
        ...
        connection.addListener(new Http2ConnectionAdapter() {
            ...
            public void onStreamAdded(final Http2Stream stream) {
                stream.<FlowState>setProperty(DefaultHttp2RemoteFlowController.this.stateKey, new FlowState(stream));
            }
    ...
    public void addFlowControlled(final Http2Stream stream, final FlowControlled frame) {
        assert !(!this.ctx.executor().inEventLoop());
        ObjectUtil.<FlowControlled>checkNotNull(frame, "frame");
        try {
    ...
    private int maxUsableChannelBytes() {
        ...
        final int usableBytes = (channelWritableBytes > 0) ? Math.max(channelWritableBytes, this.minUsableChannelBytes()) : 0;
    ...
    private final class FlowState implements StreamByteDistributor.StreamState {
        ...
        FlowState(final Http2Stream stream) {
            this.stream = stream;
            this.pendingWriteQueue = (Deque<FlowControlled>)new ArrayDeque(2);
        }
        ...
        void enqueueFrame(final FlowControlled frame) {
            final FlowControlled last = (FlowControlled)this.pendingWriteQueue.peekLast();
            if (last == null) {
        ...
        void cancel(final Http2Error error, final Throwable cause) {
            ...
            if (frame != null) {
                final Http2Exception exception = Http2Exception.streamError(this.stream.id(), error, cause, "Stream closed before write could take place");
                do {
        ...
        private void decrementFlowControlWindow(final int bytes) {
            try {
                ...
                throw new IllegalStateException("Invalid window state when writing frame: " + e.getMessage(), (Throwable)e);
            }
        }
        ...
        private void writeError(final FlowControlled frame, final Http2Exception cause) {
            ...
            frame.error(DefaultHttp2RemoteFlowController.this.ctx, (Throwable)cause);
        }
    }
    ...
    private class WritabilityMonitor implements StreamByteDistributor.Writer {
        ...
        void initialWindowSize(final int newWindowSize) throws Http2Exception {
            if (newWindowSize < 0) {
                throw new IllegalArgumentException(new StringBuilder().append("Invalid initial window size: ").append(newWindowSize).toString());
            }
    ...
    private final class ListenerWritabilityMonitor extends WritabilityMonitor implements Http2StreamVisitor {
        ...
        void windowSize(final FlowState state, final int initialWindowSize) {
            ...
            try {
                ...
                throw new RuntimeException("Caught unexpected exception from window", (Throwable)e);
            }
        }
        ...
        void stateCancelled(final FlowState state) {
            try {
                ...
                throw new RuntimeException("Caught unexpected exception from checkAllWritabilityChanged", (Throwable)e);
            }
        }
