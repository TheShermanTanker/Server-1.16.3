...
public class SpdySessionHandler extends ChannelDuplexHandler {
        ...
        if (msg instanceof SpdyDataFrame) {
            ...
            if (newWindowSize < 0) {
                while (spdyDataFrame.content().readableBytes() > this.initialReceiveWindowSize) {
                    final SpdyDataFrame partialDataFrame = new DefaultSpdyDataFrame(streamId, spdyDataFrame.content().readRetainedSlice(this.initialReceiveWindowSize));
    ...
    private void handleOutboundMessage(final ChannelHandlerContext ctx, final Object msg, final ChannelPromise promise) throws Exception {
        if (msg instanceof SpdyDataFrame) {
            ...
            if (sendWindowSize < dataLength) {
                ...
                final SpdyDataFrame partialDataFrame = new DefaultSpdyDataFrame(streamId, spdyDataFrame.content().readRetainedSlice(sendWindowSize));
                this.spdySession.putPendingWrite(streamId, new SpdySession.PendingWrite(spdyDataFrame, promise));
            ...
            final SpdyPingFrame spdyPingFrame = (SpdyPingFrame)msg;
            if (this.isRemoteInitiatedId(spdyPingFrame.id())) {
                ctx.fireExceptionCaught((Throwable)new IllegalArgumentException(new StringBuilder().append("invalid PING ID: ").append(spdyPingFrame.id()).toString()));
                return;
            }
    ...
    private void updateInitialSendWindowSize(final int newInitialWindowSize) {
    ...
    private void updateInitialReceiveWindowSize(final int newInitialWindowSize) {
    ...
    private boolean acceptStream(final int streamId, final byte priority, final boolean remoteSideClosed, final boolean localSideClosed) {
    ...
    private void removeStream(final int streamId, final ChannelFuture future) {
        this.spdySession.removeStream(streamId, (Throwable)SpdySessionHandler.STREAM_CLOSED, this.isRemoteInitiatedId(streamId));
        if (this.closeSessionFutureListener != null && this.spdySession.noActiveStreams()) {
            future.addListener(this.closeSessionFutureListener);
        }
    }
    ...
    private void updateSendWindowSize(final ChannelHandlerContext ctx, final int streamId, final int deltaWindowSize) {
        ...
        while (true) {
            ...
            if (sendWindowSize < dataFrameSize) {
                ...
                this.spdySession.updateSendWindowSize(0, -1 * sendWindowSize);
                final SpdyDataFrame partialDataFrame = new DefaultSpdyDataFrame(writeStreamId, spdyDataFrame.content().readRetainedSlice(sendWindowSize));
    ...
    private ChannelFuture sendGoAwayFrame(final ChannelHandlerContext ctx, final SpdySessionStatus status) {
    ...
    static {
        PROTOCOL_EXCEPTION = ThrowableUtil.<SpdyProtocolException>unknownStackTrace(new SpdyProtocolException(), SpdySessionHandler.class, "handleOutboundMessage(...)");
        STREAM_CLOSED = ThrowableUtil.<SpdyProtocolException>unknownStackTrace(new SpdyProtocolException("Stream closed"), SpdySessionHandler.class, "removeStream(...)");
    }
