...
public class SpdyHeaderBlockRawEncoder extends SpdyHeaderBlockEncoder {
    ...
    public ByteBuf encode(final ByteBufAllocator alloc, final SpdyHeadersFrame frame) throws Exception {
        ...
        for (final CharSequence name : names) {
            ...
            for (final CharSequence value : ((Headers<CharSequence, Object, Headers>)frame.headers()).getAll(name)) {
            ...
            if (valueLength > 65535) {
                throw new IllegalArgumentException(new StringBuilder().append("header exceeds allowable length: ").append(name).toString());
            }
            if (valueLength <= 0) {
                continue;
            }
