...
public class SpdyHeaderBlockRawDecoder extends SpdyHeaderBlockDecoder {
    ...
    @Override
    void decode(final ByteBufAllocator alloc, final ByteBuf headerBlock, final SpdyHeadersFrame frame) throws Exception {
        if (headerBlock == null) {
        ...
        if (this.cumulation == null) {
            ...
            if (headerBlock.isReadable()) {
                (this.cumulation = alloc.buffer(headerBlock.readableBytes())).writeBytes(headerBlock);
            }
        }
        else {
        ...
        while (headerBlock.isReadable()) {
            switch (this.state) {
                ...
                case READ_NAME: {
                    ...
                    if (((Headers<CharSequence, V, T>)frame.headers()).contains((CharSequence)this.name)) {
                        this.state = State.ERROR;
                        frame.setInvalid();
                        continue;
                    }
                ...
                case READ_VALUE_LENGTH: {
                    ...
                    if (this.length == 0) {
                        if (!frame.isTruncated()) {
                            ((Headers<CharSequence, CharSequence, Headers>)frame.headers()).add((CharSequence)this.name, "");
                        }
                        this.name = null;
                        if (--this.numHeaders == 0) {
                            this.state = State.END_HEADER_BLOCK;
                ...
                case READ_VALUE: {
                    ...
                    while (index < this.length) {
                        ...
                        try {
                            ((Headers<CharSequence, CharSequence, Headers>)frame.headers()).add((CharSequence)this.name, (CharSequence)value);
                        }
