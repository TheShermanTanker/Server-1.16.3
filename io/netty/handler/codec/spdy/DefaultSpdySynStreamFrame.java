...
public class DefaultSpdySynStreamFrame extends DefaultSpdyHeadersFrame implements SpdySynStreamFrame {
    ...
    public SpdySynStreamFrame setAssociatedStreamId(final int associatedStreamId) {
        if (associatedStreamId < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("Associated-To-Stream-ID cannot be negative: ").append(associatedStreamId).toString());
        }
    ...
    public SpdySynStreamFrame setPriority(final byte priority) {
        if (priority < 0 || priority > 7) {
            throw new IllegalArgumentException(new StringBuilder().append("Priority must be between 0 and 7 inclusive: ").append((int)priority).toString());
        }
    ...
    public String toString() {
        ...
        buf.append("--> Priority = ").append((int)this.priority()).append(StringUtil.NEWLINE).append("--> Headers:").append(StringUtil.NEWLINE);
