...
final class SpdySession {
    ...
    Map<Integer, StreamState> activeStreams() {
        final Map<Integer, StreamState> streams = (Map<Integer, StreamState>)new TreeMap((Comparator)this.streamComparator);
        streams.putAll((Map)this.activeStreams);
        return streams;
    }
    ...
    void acceptStream(final int streamId, final byte priority, final boolean remoteSideClosed, final boolean localSideClosed, final int sendWindowSize, final int receiveWindowSize, final boolean remote) {
        if (!remoteSideClosed || !localSideClosed) {
            final StreamState state = (StreamState)this.activeStreams.put(streamId, new StreamState(priority, remoteSideClosed, localSideClosed, sendWindowSize, receiveWindowSize));
            if (state == null) {
                if (remote) {
    ...
    private StreamState removeActiveStream(final int streamId, final boolean remote) {
        final StreamState state = (StreamState)this.activeStreams.remove(streamId);
        if (state != null) {
            if (remote) {
    ...
    boolean isRemoteSideClosed(final int streamId) {
        final StreamState state = (StreamState)this.activeStreams.get(streamId);
    ...
    PendingWrite getPendingWrite(final int streamId) {
        if (streamId == 0) {
            for (final Map.Entry<Integer, StreamState> e : this.activeStreams().entrySet()) {
                final StreamState state = (StreamState)e.getValue();
        ...
        final StreamState state2 = (StreamState)this.activeStreams.get(streamId);
    ...
    private static final class StreamState {
        ...
        StreamState(final byte priority, final boolean remoteSideClosed, final boolean localSideClosed, final int sendWindowSize, final int receiveWindowSize) {
            this.pendingWriteQueue = (Queue<PendingWrite>)new ConcurrentLinkedQueue();
        ...
        void clearPendingWrites(final Throwable cause) {
            while (true) {
                final PendingWrite pendingWrite = (PendingWrite)this.pendingWriteQueue.poll();
                if (pendingWrite == null) {
                    break;
                }
    ...
    private final class StreamComparator implements Comparator<Integer> {
        ...
        public int compare(final Integer id1, final Integer id2) {
            final StreamState state1 = (StreamState)SpdySession.this.activeStreams.get(id1);
            final StreamState state2 = (StreamState)SpdySession.this.activeStreams.get(id2);
