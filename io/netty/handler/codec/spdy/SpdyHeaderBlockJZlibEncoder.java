...
class SpdyHeaderBlockJZlibEncoder extends SpdyHeaderBlockRawEncoder {
    ...
    SpdyHeaderBlockJZlibEncoder(final SpdyVersion version, final int compressionLevel, final int windowBits, final int memLevel) {
        ...
        if (resultCode != 0) {
            throw new CompressionException(new StringBuilder().append("failed to initialize an SPDY header block deflater: ").append(resultCode).toString());
        }
        resultCode = this.z.deflateSetDictionary(SpdyCodecUtil.SPDY_DICT, SpdyCodecUtil.SPDY_DICT.length);
        if (resultCode != 0) {
            throw new CompressionException(new StringBuilder().append("failed to set the SPDY dictionary: ").append(resultCode).toString());
        }
    }
    ...
    private ByteBuf encode(final ByteBufAllocator alloc) {
        ...
        try {
            ...
            if (resultCode != 0) {
                throw new CompressionException(new StringBuilder().append("compression failure: ").append(resultCode).toString());
            }
