...
public class ObjectDecoderInputStream extends InputStream implements ObjectInput {
    ...
    public ObjectDecoderInputStream(final InputStream in, final ClassLoader classLoader, final int maxObjectSize) {
        ...
        if (maxObjectSize <= 0) {
            throw new IllegalArgumentException(new StringBuilder().append("maxObjectSize: ").append(maxObjectSize).toString());
        }
    ...
    public Object readObject() throws ClassNotFoundException, IOException {
        ...
        if (dataLen <= 0) {
            throw new StreamCorruptedException(new StringBuilder().append("invalid data length: ").append(dataLen).toString());
        }
        if (dataLen > this.maxObjectSize) {
            throw new StreamCorruptedException(new StringBuilder().append("data length too big: ").append(dataLen).append(" (max: ").append(this.maxObjectSize).append(')').toString());
        }
        return new CompactObjectInputStream((InputStream)this.in, this.classResolver).readObject();
    }
