...
public class Lz4FrameEncoder extends MessageToByteEncoder<ByteBuf> {
    private static final EncoderException ENCODE_FINSHED_EXCEPTION;
    ...
    private static int compressionLevel(final int blockSize) {
        if (blockSize < 64 || blockSize > 33554432) {
            throw new IllegalArgumentException(String.format("blockSize: %d (expected: %d-%d)", new Object[] { blockSize, 64, 33554432 }));
        }
    ...
    private ByteBuf allocateBuffer(final ChannelHandlerContext ctx, final ByteBuf msg, final boolean preferDirect, final boolean allowEmptyReturn) {
        ...
        if (targetBufSize > this.maxEncodeSize || 0 > targetBufSize) {
            throw new EncoderException(String.format("requested encode buffer size (%d bytes) exceeds the maximum allowable size (%d bytes)", new Object[] { targetBufSize, this.maxEncodeSize }));
        }
    ...
    protected void encode(final ChannelHandlerContext ctx, final ByteBuf in, final ByteBuf out) throws Exception {
        ...
        if (!out.isWritable(in.readableBytes())) {
            throw Lz4FrameEncoder.ENCODE_FINSHED_EXCEPTION;
        }
        out.writeBytes(in);
    }
    ...
    static {
        ENCODE_FINSHED_EXCEPTION = ThrowableUtil.<EncoderException>unknownStackTrace(new EncoderException((Throwable)new IllegalStateException("encode finished and not enough space to write remaining data")), Lz4FrameEncoder.class, "encode");
    }
}
