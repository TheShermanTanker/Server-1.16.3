...
public final class Snappy {
    ...
    static void encodeLiteral(final ByteBuf in, final ByteBuf out, final int length) {
        if (length < 61) {
    ...
    static int decodeLiteral(final byte tag, final ByteBuf in, final ByteBuf out) {
        in.markReaderIndex();
        int length = 0;
        switch (tag >> 2 & 0x3F) {
            case 60: {
    ...
    private static int decodeCopyWith2ByteOffset(final byte tag, final ByteBuf in, final ByteBuf out, final int writtenSoFar) {
        ...
        final int offset = in.readUnsignedShortLE();
        validateOffset(offset, writtenSoFar);
        out.markReaderIndex();
        if (offset < length) {
            for (int copies = length / offset; copies > 0; --copies) {
