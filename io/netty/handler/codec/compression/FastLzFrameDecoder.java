                    ...
                    try {
                        if (this.isCompressed) {
                            ...
                            if (originalLength != decompressedBytes) {
                                throw new DecompressionException(String.format("stream corrupted: originalLength(%d) and actual length(%d) mismatch", new Object[] { originalLength, decompressedBytes }));
                            }
                        }
                        else {
                        ...
                        if (this.hasChecksum && checksum != null) {
                            ...
                            if (checksumResult != this.currentChecksum) {
                                throw new DecompressionException(String.format("stream corrupted: mismatching checksum: %d (expected: %d)", new Object[] { checksumResult, this.currentChecksum }));
                            }
                        }
