...
class Bzip2BitReader {
    ...
    boolean hasReadableBits(final int count) {
        if (count < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("count: ").append(count).append(" (expected value greater than 0)").toString());
        }
    ...
    boolean hasReadableBytes(final int count) {
        if (count < 0 || count > 268435455) {
            throw new IllegalArgumentException(new StringBuilder().append("count: ").append(count).append(" (expected: 0-").append(268435455).append(')').toString());
        }
