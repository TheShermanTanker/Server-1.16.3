                ...
                switch (this.currentState) {
                    case INIT_BLOCK: {
                        ...
                        if (compressedLength < 0 || compressedLength > 33554432) {
                            throw new DecompressionException(String.format("invalid compressedLength: %d (expected: 0-%d)", new Object[] { compressedLength, 33554432 }));
                        }
                        ...
                        if (decompressedLength < 0 || decompressedLength > maxDecompressedLength) {
                            throw new DecompressionException(String.format("invalid decompressedLength: %d (expected: 0-%d)", new Object[] { decompressedLength, maxDecompressedLength }));
                        }
                        if ((decompressedLength == 0 && compressedLength != 0) || (decompressedLength != 0 && compressedLength == 0) || (blockType == 16 && decompressedLength != compressedLength)) {
                            throw new DecompressionException(String.format("stream corrupted: compressedLength(%d) and decompressedLength(%d) mismatch", new Object[] { compressedLength, decompressedLength }));
                        }
                    ...
                    case DECOMPRESS_DATA: {
                        ...
                        try {
                            switch (blockType) {
                                ...
                                default: {
                                    throw new DecompressionException(String.format("unexpected blockType: %d (expected: %d or %d)", new Object[] { blockType, 16, 32 }));
                                }
                            }
