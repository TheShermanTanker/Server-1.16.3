...
public class RedisEncoder extends MessageToMessageEncoder<RedisMessage> {
    ...
    public RedisEncoder(final RedisMessagePool messagePool) {
        this.messagePool = ObjectUtil.<RedisMessagePool>checkNotNull(messagePool, "messagePool");
    }
    ...
    private void writeRedisMessage(final ByteBufAllocator allocator, final RedisMessage msg, final List<Object> out) {
        if (msg instanceof InlineCommandRedisMessage) {
            ...
            if (!(msg instanceof ArrayRedisMessage)) {
                throw new CodecException(new StringBuilder().append("unknown message type: ").append(msg).toString());
            }
            this.writeArrayMessage(allocator, (ArrayRedisMessage)msg, out);
        }
    }
    
    private static void writeInlineCommandMessage(final ByteBufAllocator allocator, final InlineCommandRedisMessage msg, final List<Object> out) {
    ...
    private static void writeString(final ByteBufAllocator allocator, final RedisMessageType type, final String content, final List<Object> out) {
        final ByteBuf buf = allocator.ioBuffer(type.length() + ByteBufUtil.utf8MaxBytes((CharSequence)content) + 2);
        type.writeTo(buf);
