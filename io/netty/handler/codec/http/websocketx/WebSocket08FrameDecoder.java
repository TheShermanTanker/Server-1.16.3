...
public class WebSocket08FrameDecoder extends ByteToMessageDecoder implements WebSocketFrameDecoder {
        ...
        switch (this.state) {
            ...
            case READING_SECOND: {
                ...
                if (this.frameRsv != 0 && !this.allowExtensions) {
                    this.protocolViolation(ctx, new StringBuilder().append("RSV != 0 and no extension negotiated, RSV:").append(this.frameRsv).toString());
                    return;
                }
                ...
                if (this.frameOpcode > 7) {
                    ...
                    if (this.frameOpcode != 8 && this.frameOpcode != 9 && this.frameOpcode != 10) {
                        this.protocolViolation(ctx, new StringBuilder().append("control frame using reserved opcode ").append(this.frameOpcode).toString());
                        return;
                    }
                    ...
                    if (this.frameOpcode != 0 && this.frameOpcode != 1 && this.frameOpcode != 2) {
                        this.protocolViolation(ctx, new StringBuilder().append("data frame using reserved opcode ").append(this.frameOpcode).toString());
                        return;
                    }
            ...
            case READING_SIZE: {
                ...
                if (this.framePayloadLength > this.maxFramePayloadLength) {
                    this.protocolViolation(ctx, new StringBuilder().append("Max frame length of ").append(this.maxFramePayloadLength).append(" has been exceeded.").toString());
                    return;
                }
                if (WebSocket08FrameDecoder.logger.isDebugEnabled()) {
            ...
            case PAYLOAD: {
                ...
                try {
                    ...
                    throw new UnsupportedOperationException(new StringBuilder().append("Cannot decode web socket frame with opcode: ").append(this.frameOpcode).toString());
                }
                finally {
    ...
    private static int toFrameLength(final long l) {
        if (l > 2147483647L) {
            throw new TooLongFrameException(new StringBuilder().append("Length:").append(l).toString());
        }
        return (int)l;
    }
    ...
    protected void checkCloseFrameBody(final ChannelHandlerContext ctx, final ByteBuf buffer) {
        ...
        if ((statusCode >= 0 && statusCode <= 999) || (statusCode >= 1004 && statusCode <= 1006) || (statusCode >= 1012 && statusCode <= 2999)) {
            this.protocolViolation(ctx, new StringBuilder().append("Invalid close frame getStatus code: ").append(statusCode).toString());
        }
