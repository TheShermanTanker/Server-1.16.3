...
public class WebSocketClientExtensionHandler extends ChannelDuplexHandler {
    ...
    public WebSocketClientExtensionHandler(final WebSocketClientExtensionHandshaker... extensionHandshakers) {
        ...
        this.extensionHandshakers = (List<WebSocketClientExtensionHandshaker>)Arrays.asList((Object[])extensionHandshakers);
    }
    ...
    public void write(final ChannelHandlerContext ctx, final Object msg, final ChannelPromise promise) throws Exception {
        if (msg instanceof HttpRequest && WebSocketExtensionUtil.isWebsocketUpgrade(((HttpRequest)msg).headers())) {
            ...
            for (final WebSocketClientExtensionHandshaker extensionHandshaker : this.extensionHandshakers) {
                final WebSocketExtensionData extensionData = extensionHandshaker.newRequestData();
    ...
    public void channelRead(final ChannelHandlerContext ctx, final Object msg) throws Exception {
        if (msg instanceof HttpResponse) {
            ...
            if (WebSocketExtensionUtil.isWebsocketUpgrade(response.headers())) {
                final String extensionsHeader = response.headers().getAsString((CharSequence)HttpHeaderNames.SEC_WEBSOCKET_EXTENSIONS);
                if (extensionsHeader != null) {
                    final List<WebSocketExtensionData> extensions = WebSocketExtensionUtil.extractExtensions(extensionsHeader);
                    final List<WebSocketClientExtension> validExtensions = (List<WebSocketClientExtension>)new ArrayList(extensions.size());
                    int rsv = 0;
                    for (final WebSocketExtensionData extensionData : extensions) {
                        ...
                        for (extensionHandshakersIterator = (Iterator<WebSocketClientExtensionHandshaker>)this.extensionHandshakers.iterator(), validExtension = null; validExtension == null && extensionHandshakersIterator.hasNext(); validExtension = extensionHandshaker.handshakeExtension(extensionData)) {
