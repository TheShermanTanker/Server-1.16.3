...
abstract class DeflateDecoder extends WebSocketExtensionDecoder {
    ...
    protected abstract boolean appendFrameTail(final WebSocketFrame webSocketFrame);
    
    protected abstract int newRsv(final WebSocketFrame webSocketFrame);
    ...
    protected void decode(final ChannelHandlerContext ctx, final WebSocketFrame msg, final List<Object> out) throws Exception {
        ...
        while (true) {
            final ByteBuf partUncompressedContent = this.decoder.<ByteBuf>readInbound();
    ...
    private void cleanup() {
        if (this.decoder != null) {
            if (this.decoder.finish()) {
                while (true) {
                    final ByteBuf buf = this.decoder.<ByteBuf>readOutbound();
                    if (buf == null) {
                        break;
                    }
                    buf.release();
