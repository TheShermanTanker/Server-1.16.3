...
public final class WebSocketExtensionUtil {
    ...
    static boolean isWebsocketUpgrade(final HttpHeaders headers) {
        return headers.containsValue((CharSequence)HttpHeaderNames.CONNECTION, (CharSequence)HttpHeaderValues.UPGRADE, true) && headers.contains((CharSequence)HttpHeaderNames.UPGRADE, (CharSequence)HttpHeaderValues.WEBSOCKET, true);
    }
    ...
    public static List<WebSocketExtensionData> extractExtensions(final String extensionHeader) {
        ...
        if (rawExtensions.length > 0) {
            final List<WebSocketExtensionData> extensions = (List<WebSocketExtensionData>)new ArrayList(rawExtensions.length);
            for (final String rawExtension : rawExtensions) {
                ...
                if (extensionParameters.length > 1) {
                    parameters = (Map<String, String>)new HashMap(extensionParameters.length - 1);
                    for (int i = 1; i < extensionParameters.length; ++i) {
                        final String parameter = extensionParameters[i].trim();
                        final Matcher parameterMatcher = WebSocketExtensionUtil.PARAMETER.matcher((CharSequence)parameter);
                    ...
                    parameters = (Map<String, String>)Collections.emptyMap();
                }
                extensions.add(new WebSocketExtensionData(name, parameters));
            }
            return extensions;
        }
        return (List<WebSocketExtensionData>)Collections.emptyList();
    }
