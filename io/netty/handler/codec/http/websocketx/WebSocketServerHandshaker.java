...
public abstract class WebSocketServerHandshaker {
    ...
    public Set<String> subprotocols() {
        final Set<String> ret = (Set<String>)new LinkedHashSet();
        Collections.addAll((Collection)ret, (Object[])this.subprotocols);
        return ret;
    }
    ...
    public final ChannelFuture handshake(final Channel channel, final FullHttpRequest req, final HttpHeaders responseHeaders, final ChannelPromise promise) {
        ...
        if (p.<HttpObjectAggregator>get(HttpObjectAggregator.class) != null) {
            p.<HttpObjectAggregator>remove(HttpObjectAggregator.class);
        }
        if (p.<HttpContentCompressor>get(HttpContentCompressor.class) != null) {
            p.<HttpContentCompressor>remove(HttpContentCompressor.class);
        }
    ...
    public final ChannelFuture handshake(final Channel channel, final HttpRequest req, final HttpHeaders responseHeaders, final ChannelPromise promise) {
        ...
        p.addAfter(aggregatorName, "handshaker", (ChannelHandler)new SimpleChannelInboundHandler<FullHttpRequest>() {
            ...
            public void channelInactive(final ChannelHandlerContext ctx) throws Exception {
                promise.tryFailure((Throwable)WebSocketServerHandshaker.CLOSED_CHANNEL_EXCEPTION);
                ctx.fireChannelInactive();
            }
        });
        try {
            ctx.fireChannelRead(ReferenceCountUtil.<HttpRequest>retain(req));
        }
    ...
    protected abstract FullHttpResponse newHandshakeResponse(final FullHttpRequest fullHttpRequest, final HttpHeaders httpHeaders);
    ...
    static {
        ...
        CLOSED_CHANNEL_EXCEPTION = ThrowableUtil.<ClosedChannelException>unknownStackTrace(new ClosedChannelException(), WebSocketServerHandshaker.class, "handshake(...)");
    }
}
