...
public class HttpObjectAggregator extends MessageAggregator<HttpObject, HttpMessage, HttpContent, FullHttpMessage> {
    ...
    private static Object continueResponse(final HttpMessage start, final int maxContentLength, final ChannelPipeline pipeline) {
    ...
    protected Object newContinueResponse(final HttpMessage start, final int maxContentLength, final ChannelPipeline pipeline) {
        final Object response = continueResponse(start, maxContentLength, pipeline);
        if (response != null) {
    ...
    protected void finishAggregation(final FullHttpMessage aggregated) throws Exception {
        if (!HttpUtil.isContentLengthSet(aggregated)) {
            aggregated.headers().set((CharSequence)HttpHeaderNames.CONTENT_LENGTH, String.valueOf(aggregated.content().readableBytes()));
        }
    }
    ...
    protected void handleOversizedMessage(final ChannelHandlerContext ctx, final HttpMessage oversized) throws Exception {
        if (oversized instanceof HttpRequest) {
            if (oversized instanceof FullHttpMessage || (!HttpUtil.is100ContinueExpected(oversized) && !HttpUtil.isKeepAlive(oversized))) {
                ...
                ctx.writeAndFlush(HttpObjectAggregator.TOO_LARGE.retainedDuplicate()).addListener(new ChannelFutureListener() {
                    public void operationComplete(final ChannelFuture future) throws Exception {
                        if (!future.isSuccess()) {
                            HttpObjectAggregator.logger.debug("Failed to send a 413 Request Entity Too Large.", future.cause());
                            ctx.close();
            ...
            final HttpObjectDecoder decoder = ctx.pipeline().<HttpObjectDecoder>get(HttpObjectDecoder.class);
            if (decoder != null) {
                decoder.reset();
            }
            return;
        ...
        if (oversized instanceof HttpResponse) {
            ctx.close();
            throw new TooLongFrameException(new StringBuilder().append("Response entity too large: ").append(oversized).toString());
        }
    ...
    static {
        ...
        HttpObjectAggregator.EXPECTATION_FAILED.headers().set((CharSequence)HttpHeaderNames.CONTENT_LENGTH, 0);
        HttpObjectAggregator.TOO_LARGE.headers().set((CharSequence)HttpHeaderNames.CONTENT_LENGTH, 0);
        HttpObjectAggregator.TOO_LARGE_CLOSE.headers().set((CharSequence)HttpHeaderNames.CONTENT_LENGTH, 0);
        HttpObjectAggregator.TOO_LARGE_CLOSE.headers().set((CharSequence)HttpHeaderNames.CONNECTION, HttpHeaderValues.CLOSE);
    }
    ...
    private static final class AggregatedFullHttpRequest extends AggregatedFullHttpMessage implements FullHttpRequest {
        ...
        public FullHttpRequest replace(final ByteBuf content) {
            final DefaultFullHttpRequest dup = new DefaultFullHttpRequest(this.protocolVersion(), this.method(), this.uri(), content, this.headers().copy(), this.trailingHeaders().copy());
    ...
    private static final class AggregatedFullHttpResponse extends AggregatedFullHttpMessage implements FullHttpResponse {
        ...
        public FullHttpResponse replace(final ByteBuf content) {
            final DefaultFullHttpResponse dup = new DefaultFullHttpResponse(this.getProtocolVersion(), this.getStatus(), content, this.headers().copy(), this.trailingHeaders().copy());
            dup.setDecoderResult(this.decoderResult());
            return dup;
        }
