...
public class CorsHandler extends ChannelDuplexHandler {
    ...
    public CorsHandler(final CorsConfig config) {
        this((List<CorsConfig>)Collections.singletonList(ObjectUtil.<CorsConfig>checkNotNull(config, "config")), config.isShortCircuit());
    }
    
    public CorsHandler(final List<CorsConfig> configList, final boolean isShortCircuit) {
        ObjectUtil.<List<CorsConfig>>checkNonEmpty(configList, "configList");
    ...
    public void channelRead(final ChannelHandlerContext ctx, final Object msg) throws Exception {
        if (msg instanceof HttpRequest) {
            ...
            final String origin = this.request.headers().get((CharSequence)HttpHeaderNames.ORIGIN);
    ...
    private void handlePreflight(final ChannelHandlerContext ctx, final HttpRequest request) {
        ...
        if (!response.headers().contains((CharSequence)HttpHeaderNames.CONTENT_LENGTH)) {
            response.headers().set((CharSequence)HttpHeaderNames.CONTENT_LENGTH, HttpHeaderValues.ZERO);
        }
    ...
    private static boolean isPreflightRequest(final HttpRequest request) {
        ...
        return request.method().equals(HttpMethod.OPTIONS) && headers.contains((CharSequence)HttpHeaderNames.ORIGIN) && headers.contains((CharSequence)HttpHeaderNames.ACCESS_CONTROL_REQUEST_METHOD);
    }
    ...
    private void setMaxAge(final HttpResponse response) {
        response.headers().set((CharSequence)HttpHeaderNames.ACCESS_CONTROL_MAX_AGE, this.config.maxAge());
    }
    ...
    private static void forbidden(final ChannelHandlerContext ctx, final HttpRequest request) {
        final HttpResponse response = new DefaultFullHttpResponse(request.protocolVersion(), HttpResponseStatus.FORBIDDEN);
        response.headers().set((CharSequence)HttpHeaderNames.CONTENT_LENGTH, HttpHeaderValues.ZERO);
        ReferenceCountUtil.release(request);
        respond(ctx, request, response);
    }
