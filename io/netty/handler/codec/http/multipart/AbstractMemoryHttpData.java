...
public abstract class AbstractMemoryHttpData extends AbstractHttpData {
    ...
    public void setContent(final ByteBuf buffer) throws IOException {
        ...
        if (this.definedSize > 0L && this.definedSize < localsize) {
            throw new IOException(new StringBuilder().append("Out of size: ").append(localsize).append(" > ").append(this.definedSize).toString());
        }
    ...
    public boolean renameTo(final File dest) throws IOException {
        ...
        if (!dest.createNewFile()) {
            throw new IOException(new StringBuilder().append("file exists already: ").append(dest).toString());
        }
        return true;
    }
