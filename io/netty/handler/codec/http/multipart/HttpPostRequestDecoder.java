...
public class HttpPostRequestDecoder implements InterfaceHttpPostRequestDecoder {
    ...
    public static boolean isMultipart(final HttpRequest request) {
        return request.headers().contains((CharSequence)HttpHeaderNames.CONTENT_TYPE) && getMultipartDataBoundary(request.headers().get((CharSequence)HttpHeaderNames.CONTENT_TYPE)) != null;
    }
