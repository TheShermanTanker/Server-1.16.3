...
public class HttpResponseStatus implements Comparable<HttpResponseStatus> {
    ...
    public static HttpResponseStatus valueOf(final int code) {
        final HttpResponseStatus status = valueOf0(code);
        return (status != null) ? status : new HttpResponseStatus(code);
    }
    
    private static HttpResponseStatus valueOf0(final int code) {
        switch (code) {
            case 100: {
    ...
    public static HttpResponseStatus valueOf(final int code, final String reasonPhrase) {
        final HttpResponseStatus responseStatus = valueOf0(code);
    ...
    public static HttpResponseStatus parseLine(final String line) {
        try {
    ...
    public static HttpResponseStatus parseLine(final AsciiString line) {
        try {
            final int space = line.forEachByte(ByteProcessor.FIND_ASCII_SPACE);
            return (space == -1) ? valueOf(line.parseInt()) : valueOf(line.parseInt(0, space), line.toString(space + 1));
        }
        catch (Exception e) {
            throw new IllegalArgumentException(new StringBuilder().append("malformed status line: ").append(line).toString(), (Throwable)e);
        }
    }
    
    private HttpResponseStatus(final int code) {
        this(code, new StringBuilder().append(HttpStatusClass.valueOf(code).defaultReasonPhrase()).append(" (").append(code).append(')').toString(), false);
    }
    ...
    private HttpResponseStatus(final int code, final String reasonPhrase, final boolean bytes) {
        ...
        this.code = code;
        final String codeString = Integer.toString(code);
