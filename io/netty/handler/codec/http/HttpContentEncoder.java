...
public abstract class HttpContentEncoder extends MessageToMessageCodec<HttpRequest, HttpObject> {
    ...
    public HttpContentEncoder() {
        this.acceptEncodingQueue = (Queue<CharSequence>)new ArrayDeque();
    ...
    protected void decode(final ChannelHandlerContext ctx, final HttpRequest msg, final List<Object> out) throws Exception {
        CharSequence acceptedEncoding = (CharSequence)msg.headers().get((CharSequence)HttpHeaderNames.ACCEPT_ENCODING);
        if (acceptedEncoding == null) {
            acceptedEncoding = (CharSequence)HttpContentDecoder.IDENTITY;
        }
        ...
        out.add(ReferenceCountUtil.<HttpRequest>retain(msg));
    }
            ...
            switch (this.state) {
                case AWAIT_HEADERS: {
                    ...
                    if (isPassthru(res.protocolVersion(), code, acceptEncoding)) {
                        if (isFull) {
                            out.add(ReferenceCountUtil.<HttpResponse>retain(res));
                            break;
                        }
                        out.add(res);
                ...
                case PASS_THROUGH: {
                    ensureContent(msg);
                    out.add(ReferenceCountUtil.<HttpObject>retain(msg));
    ...
    protected abstract Result beginEncode(final HttpResponse httpResponse, final String string) throws Exception;
    ...
    private void fetchEncoderOutput(final List<Object> out) {
        while (true) {
            final ByteBuf buf = this.encoder.<ByteBuf>readOutbound();
            if (buf == null) {
                break;
            }
            if (!buf.isReadable()) {
