...
public final class HttpClientCodec extends CombinedChannelDuplexHandler<HttpResponseDecoder, HttpRequestEncoder> implements HttpClientUpgradeHandler.SourceCodec {
    ...
    public HttpClientCodec(final int maxInitialLineLength, final int maxHeaderSize, final int maxChunkSize, final boolean failOnMissingResponse, final boolean validateHeaders, final int initialBufferSize, final boolean parseHttpAfterConnectRequest) {
        this.queue = (Queue<HttpMethod>)new ArrayDeque();
        this.requestResponseCounter = new AtomicLong();
        ((CombinedChannelDuplexHandler<Decoder, Encoder>)this).init(new Decoder(maxInitialLineLength, maxHeaderSize, maxChunkSize, validateHeaders, initialBufferSize), new Encoder());
        this.parseHttpAfterConnectRequest = parseHttpAfterConnectRequest;
        this.failOnMissingResponse = failOnMissingResponse;
    }
    ...
    private final class Decoder extends HttpResponseDecoder {
        ...
        Decoder(final int maxInitialLineLength, final int maxHeaderSize, final int maxChunkSize, final boolean validateHeaders, final int initialBufferSize) {
        ...
        protected boolean isContentAlwaysEmpty(final HttpMessage msg) {
            ...
            final HttpMethod method = (HttpMethod)HttpClientCodec.this.queue.poll();
        ...
        public void channelInactive(final ChannelHandlerContext ctx) throws Exception {
            ...
            if (HttpClientCodec.this.failOnMissingResponse) {
                ...
                if (missingResponses > 0L) {
                    ctx.fireExceptionCaught((Throwable)new PrematureChannelClosureException(new StringBuilder().append("channel gone inactive with ").append(missingResponses).append(" missing response(s)").toString()));
                }
            }
        }
    }
