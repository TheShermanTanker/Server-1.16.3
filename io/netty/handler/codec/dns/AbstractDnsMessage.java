...
public abstract class AbstractDnsMessage extends AbstractReferenceCounted implements DnsMessage {
    ...
    public DnsMessage setOpCode(final DnsOpCode opCode) {
        this.opCode = ObjectUtil.<DnsOpCode>checkNotNull(opCode, "opCode");
        return this;
    }
    ...
    public <T extends DnsRecord> T recordAt(final DnsSection section) {
        return this.<T>recordAt(sectionOrdinal(section));
    }
    ...
    private <T extends DnsRecord> T recordAt(final int section) {
        ...
        if (records instanceof DnsRecord) {
            return AbstractDnsMessage.<T>castRecord(records);
        }
        final List<DnsRecord> recordList = (List<DnsRecord>)records;
        if (recordList.isEmpty()) {
            return null;
        }
        return AbstractDnsMessage.<T>castRecord(recordList.get(0));
    }
    
    @Override
    public <T extends DnsRecord> T recordAt(final DnsSection section, final int index) {
        return this.<T>recordAt(sectionOrdinal(section), index);
    }
    
    private <T extends DnsRecord> T recordAt(final int section, final int index) {
        final Object records = this.sectionAt(section);
        if (records == null) {
            throw new IndexOutOfBoundsException(new StringBuilder().append("index: ").append(index).append(" (expected: none)").toString());
        }
        if (!(records instanceof DnsRecord)) {
            final List<DnsRecord> recordList = (List<DnsRecord>)records;
            return AbstractDnsMessage.<T>castRecord(recordList.get(index));
        }
        if (index == 0) {
            return AbstractDnsMessage.<T>castRecord(records);
        }
        throw new IndexOutOfBoundsException(new StringBuilder().append("index: ").append(index).append("' (expected: 0)").toString());
    }
    ...
    public <T extends DnsRecord> T setRecord(final DnsSection section, final int index, final DnsRecord record) {
        return this.<T>setRecord(sectionOrdinal(section), index, record);
    }
    ...
    private <T extends DnsRecord> T setRecord(final int section, final int index, final DnsRecord record) {
        ...
        if (!(records instanceof DnsRecord)) {
            final List<DnsRecord> recordList = (List<DnsRecord>)records;
            return AbstractDnsMessage.<T>castRecord(recordList.set(index, record));
        }
        if (index == 0) {
    ...
    private void addRecord(final int section, final DnsRecord record) {
        ...
        if (records instanceof DnsRecord) {
            final List<DnsRecord> recordList = (List<DnsRecord>)newRecordList();
            recordList.add(AbstractDnsMessage.castRecord(records));
    ...
    private void addRecord(final int section, final int index, final DnsRecord record) {
        ...
        if (records == null) {
            ...
            if (records instanceof DnsRecord) {
                ...
                if (index == 0) {
                    recordList = (List<DnsRecord>)newRecordList();
                    recordList.add(record);
                    recordList.add(AbstractDnsMessage.castRecord(records));
                }
                else {
                    if (index != 1) {
                        throw new IndexOutOfBoundsException(new StringBuilder().append("index: ").append(index).append(" (expected: 0 or 1)").toString());
                    }
                    recordList = (List<DnsRecord>)newRecordList();
                    recordList.add(AbstractDnsMessage.castRecord(records));
                    recordList.add(record);
    ...
    public <T extends DnsRecord> T removeRecord(final DnsSection section, final int index) {
        return this.<T>removeRecord(sectionOrdinal(section), index);
    }
    ...
    private <T extends DnsRecord> T removeRecord(final int section, final int index) {
        ...
        if (!(records instanceof DnsRecord)) {
            final List<DnsRecord> recordList = (List<DnsRecord>)records;
            return AbstractDnsMessage.<T>castRecord(recordList.remove(index));
        }
        if (index != 0) {
            throw new IndexOutOfBoundsException(new StringBuilder().append("index: ").append(index).append(" (expected: 0)").toString());
        }
        final T record = AbstractDnsMessage.<T>castRecord(records);
    ...
    private static int sectionOrdinal(final DnsSection section) {
        return ObjectUtil.<DnsSection>checkNotNull(section, "section").ordinal();
    }
    
    private static DnsRecord checkQuestion(final int section, final DnsRecord record) {
        if (section == AbstractDnsMessage.SECTION_QUESTION && !(ObjectUtil.<DnsRecord>checkNotNull(record, "record") instanceof DnsQuestion)) {
            throw new IllegalArgumentException(new StringBuilder().append("record: ").append(record).append(" (expected: ").append(StringUtil.simpleClassName(DnsQuestion.class)).append(')').toString());
        }
        return record;
    }
    ...
    private static ArrayList<DnsRecord> newRecordList() {
        return (ArrayList<DnsRecord>)new ArrayList(2);
    }
    
    static {
        leakDetector = ResourceLeakDetectorFactory.instance().<DnsMessage>newResourceLeakDetector(DnsMessage.class);
