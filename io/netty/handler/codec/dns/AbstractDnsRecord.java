...
public abstract class AbstractDnsRecord implements DnsRecord {
    ...
    protected AbstractDnsRecord(final String name, final DnsRecordType type, final int dnsClass, final long timeToLive) {
        if (timeToLive < 0L) {
            throw new IllegalArgumentException(new StringBuilder().append("timeToLive: ").append(timeToLive).append(" (expected: >= 0)").toString());
        }
        this.name = appendTrailingDot(IDN.toASCII((String)ObjectUtil.<String>checkNotNull(name, "name")));
        this.type = ObjectUtil.<DnsRecordType>checkNotNull(type, "type");
