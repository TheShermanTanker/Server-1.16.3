...
public class DnsRecordType implements Comparable<DnsRecordType> {
    ...
    public static DnsRecordType valueOf(final String name) {
        final DnsRecordType result = (DnsRecordType)DnsRecordType.BY_NAME.get(name);
        if (result == null) {
    ...
    public DnsRecordType(final int intValue, final String name) {
        if ((intValue & 0xFFFF) != intValue) {
            throw new IllegalArgumentException(new StringBuilder().append("intValue: ").append(intValue).append(" (expected: 0 ~ 65535)").toString());
        }
