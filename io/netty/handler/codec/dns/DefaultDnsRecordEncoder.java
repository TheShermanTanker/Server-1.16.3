...
public class DefaultDnsRecordEncoder implements DnsRecordEncoder {
    ...
    private void encodeOptEcsRecord(final DnsOptEcsRecord record, final ByteBuf out) throws Exception {
        ...
        if (addressBits < sourcePrefixLength || sourcePrefixLength < 0) {
            throw new IllegalArgumentException(new StringBuilder().append(sourcePrefixLength).append(": ").append(sourcePrefixLength).append(" (expected: 0 >= ").append(addressBits).append(')').toString());
        }
    ...
    private static byte padWithZeros(final byte b, final int lowOrderBitsToPreserve) {
        switch (lowOrderBitsToPreserve) {
            ...
            default: {
                throw new IllegalArgumentException(new StringBuilder().append("lowOrderBitsToPreserve: ").append(lowOrderBitsToPreserve).toString());
            }
        }
    }
}
