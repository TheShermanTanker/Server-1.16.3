...
public final class SocksCmdResponse extends SocksResponse {
    ...
    public SocksCmdResponse(final SocksCmdStatus cmdStatus, final SocksAddressType addressType, String host, final int port) {
        ...
        if (port < 0 || port > 65535) {
            throw new IllegalArgumentException(new StringBuilder().append(port).append(" is not in bounds 0 <= x <= 65535").toString());
        }
        ...
        switch (this.addressType) {
            ...
            case DOMAIN: {
                if (this.host != null) {
                    ...
                    byteBuf.writeByte(SocksCmdResponse.DOMAIN_ZEROED.length);
                    byteBuf.writeBytes(SocksCmdResponse.DOMAIN_ZEROED);
                }
                byteBuf.writeShort(this.port);
                break;
            }
