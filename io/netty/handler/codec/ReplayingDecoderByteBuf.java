...
final class ReplayingDecoderByteBuf extends ByteBuf {
    ...
    ReplayingDecoderByteBuf(final ByteBuf buffer) {
    ...
    public int indexOf(final int fromIndex, final int toIndex, final byte value) {
        ...
        if (Math.max(fromIndex, toIndex) > this.buffer.writerIndex()) {
            throw ReplayingDecoderByteBuf.REPLAY;
        }
    ...
    static {
        ...
        (EMPTY_BUFFER = new ReplayingDecoderByteBuf(Unpooled.EMPTY_BUFFER)).terminate();
    }
}
