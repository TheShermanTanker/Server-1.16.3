...
public class JsonObjectDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(final ChannelHandlerContext ctx, final ByteBuf in, final List<Object> out) throws Exception {
        ...
        if (wrtIdx > this.maxObjectLength) {
            in.skipBytes(in.readableBytes());
            this.reset();
            throw new TooLongFrameException(new StringBuilder().append("object length exceeds ").append(this.maxObjectLength).append(": ").append(wrtIdx).append(" bytes discarded").toString());
        }
        while (idx < wrtIdx) {
            ...
            if (this.state == 1) {
                ...
                if (!this.insideString && ((this.openBraces == 1 && c == 44) || (this.openBraces == 0 && c == 93))) {
                    for (int i = in.readerIndex(); Character.isWhitespace((int)in.getByte(i)); ++i) {
                        in.skipBytes(1);
                    }
                    int idxNoSpaces;
                    for (idxNoSpaces = idx - 1; idxNoSpaces >= in.readerIndex() && Character.isWhitespace((int)in.getByte(idxNoSpaces)); --idxNoSpaces) {}
                ...
                if (!Character.isWhitespace((int)c)) {
                    this.state = -1;
                    throw new CorruptedFrameException(new StringBuilder().append("invalid JSON received at byte position ").append(idx).append(": ").append(ByteBufUtil.hexDump(in)).toString());
                }
                in.skipBytes(1);
            }
            ++idx;
