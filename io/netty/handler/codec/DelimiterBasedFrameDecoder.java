...
public class DelimiterBasedFrameDecoder extends ByteToMessageDecoder {
    ...
    private void fail(final long frameLength) {
        if (frameLength > 0L) {
            throw new TooLongFrameException(new StringBuilder().append("frame length exceeds ").append(this.maxFrameLength).append(": ").append(frameLength).append(" - discarded").toString());
        }
        throw new TooLongFrameException(new StringBuilder().append("frame length exceeds ").append(this.maxFrameLength).append(" - discarding").toString());
    }
