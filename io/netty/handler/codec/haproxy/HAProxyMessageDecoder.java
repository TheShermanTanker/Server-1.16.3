...
public class HAProxyMessageDecoder extends ByteToMessageDecoder {
    ...
    protected final void decode(final ChannelHandlerContext ctx, final ByteBuf in, final List<Object> out) throws Exception {
        ...
        if (decoded != null) {
            ...
            try {
                ...
                this.fail(ctx, null, (Exception)e);
            }
        }
    }
    ...
    private ByteBuf decodeStruct(final ChannelHandlerContext ctx, final ByteBuf buffer) throws Exception {
        ...
        if (eoh < 0) {
            ...
            if (length > this.v2MaxHeaderSize) {
                ...
                this.failOverLimit(ctx, new StringBuilder().append("over ").append(this.discardedBytes).toString());
            }
            return null;
        }
    ...
    private void fail(final ChannelHandlerContext ctx, final String errMsg, final Exception e) {
        this.finished = true;
        ctx.close();
        HAProxyProtocolException ppex;
        if (errMsg != null && e != null) {
            ppex = new HAProxyProtocolException(errMsg, (Throwable)e);
        }
        else if (errMsg != null) {
    ...
    static {
        ...
        DETECTION_RESULT_V1 = ProtocolDetectionResult.<HAProxyProtocolVersion>detected(HAProxyProtocolVersion.V1);
        DETECTION_RESULT_V2 = ProtocolDetectionResult.<HAProxyProtocolVersion>detected(HAProxyProtocolVersion.V2);
    }
}
