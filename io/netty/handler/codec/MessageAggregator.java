...
public abstract class MessageAggregator<I, S, C extends ByteBufHolder, O extends ByteBufHolder> extends MessageToMessageDecoder<I> {
    ...
    private static void validateMaxContentLength(final int maxContentLength) {
        if (maxContentLength < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("maxContentLength: ").append(maxContentLength).append(" (expected: >= 0)").toString());
        }
    }
    ...
    protected abstract boolean isStartMessage(final I object) throws Exception;
    
    protected abstract boolean isContentMessage(final I object) throws Exception;
    
    protected abstract boolean isLastContentMessage(final C byteBufHolder) throws Exception;
    
    protected abstract boolean isAggregated(final I object) throws Exception;
    ...
    public final void setMaxCumulationBufferComponents(final int maxCumulationBufferComponents) {
        if (maxCumulationBufferComponents < 2) {
            throw new IllegalArgumentException(new StringBuilder().append("maxCumulationBufferComponents: ").append(maxCumulationBufferComponents).append(" (expected: >= 2)").toString());
        }
        if (this.ctx == null) {
    ...
    protected abstract boolean isContentLengthInvalid(final S object, final int integer) throws Exception;
    
    protected abstract Object newContinueResponse(final S object, final int integer, final ChannelPipeline channelPipeline) throws Exception;
    
    protected abstract boolean closeAfterContinueResponse(final Object object) throws Exception;
    
    protected abstract boolean ignoreContentAfterContinueResponse(final Object object) throws Exception;
    
    protected abstract O beginAggregation(final S object, final ByteBuf byteBuf) throws Exception;
    ...
    protected void handleOversizedMessage(final ChannelHandlerContext ctx, final S oversized) throws Exception {
        ctx.fireExceptionCaught((Throwable)new TooLongFrameException(new StringBuilder().append("content length exceeded ").append(this.maxContentLength()).append(" bytes.").toString()));
    }
