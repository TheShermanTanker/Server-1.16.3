...
public final class DateFormatter {
    ...
    public static Date parseHttpDate(final CharSequence txt, final int start, final int end) {
        ...
        return formatter().parse0(ObjectUtil.<CharSequence>checkNotNull(txt, "txt"), start, end);
    }
    
    public static String format(final Date date) {
        return formatter().format0(ObjectUtil.<Date>checkNotNull(date, "date"));
    }
    
    public static StringBuilder append(final Date date, final StringBuilder sb) {
        return formatter().append0(ObjectUtil.<Date>checkNotNull(date, "date"), ObjectUtil.<StringBuilder>checkNotNull(sb, "sb"));
    }
    ...
    private static boolean isDelim(final char c) {
        return DateFormatter.DELIMITERS.get((int)c);
    }
    ...
    private static boolean matchMonth(final String month, final CharSequence txt, final int tokenStart) {
        return AsciiString.regionMatchesAscii((CharSequence)month, true, 0, txt, tokenStart, 3);
    }
    ...
    static {
        ...
        for (char c = ' '; c <= '/'; ++c) {
            DateFormatter.DELIMITERS.set((int)c);
        }
        for (char c = ';'; c <= '@'; ++c) {
            DateFormatter.DELIMITERS.set((int)c);
        }
