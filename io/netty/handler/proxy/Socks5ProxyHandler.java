...
public final class Socks5ProxyHandler extends ProxyHandler {
    ...
    protected boolean handleResponse(final ChannelHandlerContext ctx, final Object response) throws Exception {
        if (response instanceof Socks5InitialResponse) {
            ...
            if (res.authMethod() != Socks5AuthMethod.NO_AUTH && res.authMethod() != authMethod) {
                throw new ProxyConnectException(this.exceptionMessage(new StringBuilder().append("unexpected authMethod: ").append(res.authMethod()).toString()));
            }
            ...
            if (res2.status() != Socks5PasswordAuthStatus.SUCCESS) {
                throw new ProxyConnectException(this.exceptionMessage(new StringBuilder().append("authStatus: ").append(res2.status()).toString()));
            }
            this.sendConnectCommand(ctx);
            return false;
        }
            ...
            if (res3.status() != Socks5CommandStatus.SUCCESS) {
                throw new ProxyConnectException(this.exceptionMessage(new StringBuilder().append("status: ").append(res3.status()).toString()));
            }
            return true;
        }
    }
    ...
    static {
        INIT_REQUEST_NO_AUTH = new DefaultSocks5InitialRequest((Iterable<Socks5AuthMethod>)Collections.singletonList(Socks5AuthMethod.NO_AUTH));
        INIT_REQUEST_PASSWORD = new DefaultSocks5InitialRequest((Iterable<Socks5AuthMethod>)Arrays.asList((Object[])new Socks5AuthMethod[] { Socks5AuthMethod.NO_AUTH, Socks5AuthMethod.PASSWORD }));
    }
}
