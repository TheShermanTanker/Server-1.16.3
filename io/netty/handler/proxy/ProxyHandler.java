...
public abstract class ProxyHandler extends ChannelDuplexHandler {
    ...
    protected abstract void addCodec(final ChannelHandlerContext channelHandlerContext) throws Exception;
    
    protected abstract void removeEncoder(final ChannelHandlerContext channelHandlerContext) throws Exception;
    
    protected abstract void removeDecoder(final ChannelHandlerContext channelHandlerContext) throws Exception;
            ...
            this.connectTimeoutFuture = ctx.executor().schedule((Runnable)new Runnable() {
                public void run() {
                    if (!ProxyHandler.this.connectPromise.isDone()) {
                        ProxyHandler.this.setConnectFailure((Throwable)new ProxyConnectException(ProxyHandler.this.exceptionMessage("timeout")));
                    }
                }
    ...
    protected abstract Object newInitialMessage(final ChannelHandlerContext channelHandlerContext) throws Exception;
    ...
    public final void channelInactive(final ChannelHandlerContext ctx) throws Exception {
        if (this.finished) {
            ...
            this.setConnectFailure((Throwable)new ProxyConnectException(this.exceptionMessage("disconnected")));
        }
    }
    ...
    protected abstract boolean handleResponse(final ChannelHandlerContext channelHandlerContext, final Object object) throws Exception;
    ...
    private void setConnectSuccess() {
        ...
        if (!this.connectPromise.isDone()) {
            ...
            if (removedCodec) {
                ...
                final Exception cause = (Exception)new ProxyConnectException("failed to remove all codec handlers added by the proxy handler; bug?");
    ...
    private boolean safeRemoveDecoder() {
        try {
            ...
            ProxyHandler.logger.warn("Failed to remove proxy decoders:", (Throwable)e);
            return false;
        }
    }
    ...
    private boolean safeRemoveEncoder() {
        try {
            ...
            ProxyHandler.logger.warn("Failed to remove proxy encoders:", (Throwable)e);
            return false;
        }
    }
    ...
    private void setConnectFailure(Throwable cause) {
        ...
        if (!this.connectPromise.isDone()) {
            if (!(cause instanceof ProxyConnectException)) {
                cause = (Throwable)new ProxyConnectException(this.exceptionMessage(cause.toString()), cause);
            }
    ...
    private void failPendingWritesAndClose(final Throwable cause) {
