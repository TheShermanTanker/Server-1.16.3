...
public class FlushConsolidationHandler extends ChannelDuplexHandler {
    ...
    public FlushConsolidationHandler(final int explicitFlushAfterFlushes, final boolean consolidateWhenNoReadInProgress) {
        if (explicitFlushAfterFlushes <= 0) {
            throw new IllegalArgumentException(new StringBuilder().append("explicitFlushAfterFlushes: ").append(explicitFlushAfterFlushes).append(" (expected: > 0)").toString());
        }
        ...
        this.flushTask = (Runnable)(consolidateWhenNoReadInProgress ? new Runnable() {
            public void run() {
