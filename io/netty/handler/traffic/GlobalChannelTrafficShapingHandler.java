...
public class GlobalChannelTrafficShapingHandler extends AbstractTrafficShapingHandler {
    ...
    public GlobalChannelTrafficShapingHandler(final ScheduledExecutorService executor, final long writeGlobalLimit, final long readGlobalLimit, final long writeChannelLimit, final long readChannelLimit, final long checkInterval, final long maxTime) {
        ...
        this.channelQueues = PlatformDependent.<Integer, PerChannel>newConcurrentHashMap();
    ...
    private PerChannel getOrSetPerChannel(final ChannelHandlerContext ctx) {
        ...
        if (perChannel == null) {
            ...
            perChannel.messagesQueue = (ArrayDeque<ToSend>)new ArrayDeque();
            perChannel.channelTrafficCounter = new TrafficCounter(this, null, new StringBuilder().append("ChannelTC").append(ctx.channel().hashCode()).toString(), this.checkInterval);
    ...
    public void handlerRemoved(final ChannelHandlerContext ctx) throws Exception {
        ...
        final Channel channel = ctx.channel();
        final Integer key = channel.hashCode();
        final PerChannel perChannel = (PerChannel)this.channelQueues.remove(key);
        if (perChannel != null) {
        ...
        if (size > 0L) {
            ...
            if (wait >= 10L) {
                ...
                if (GlobalChannelTrafficShapingHandler.logger.isDebugEnabled()) {
                    GlobalChannelTrafficShapingHandler.logger.debug(new StringBuilder().append("Read Suspend: ").append(wait).append(':').append(config.isAutoRead()).append(':').append(AbstractTrafficShapingHandler.isHandlerActive(ctx)).toString());
                }
                if (config.isAutoRead() && AbstractTrafficShapingHandler.isHandlerActive(ctx)) {
                    config.setAutoRead(false);
                    channel.<Boolean>attr(GlobalChannelTrafficShapingHandler.READ_SUSPENDED).set(true);
                    final Attribute<Runnable> attr = channel.<Runnable>attr(GlobalChannelTrafficShapingHandler.REOPEN_TASK);
                    ...
                    if (GlobalChannelTrafficShapingHandler.logger.isDebugEnabled()) {
                        GlobalChannelTrafficShapingHandler.logger.debug(new StringBuilder().append("Suspend final status => ").append(config.isAutoRead()).append(':').append(AbstractTrafficShapingHandler.isHandlerActive(ctx)).append(" will reopened at: ").append(wait).toString());
                    }
                }
            }
        }
    ...
    public Collection<TrafficCounter> channelTrafficCounters() {
        return (Collection<TrafficCounter>)new AbstractCollection<TrafficCounter>() {
            public Iterator<TrafficCounter> iterator() {
                return (Iterator<TrafficCounter>)new Iterator<TrafficCounter>() {
    ...
    public void write(final ChannelHandlerContext ctx, final Object msg, final ChannelPromise promise) throws Exception {
        ...
        if (size > 0L) {
            ...
            if (wait >= 10L) {
                if (GlobalChannelTrafficShapingHandler.logger.isDebugEnabled()) {
                    GlobalChannelTrafficShapingHandler.logger.debug(new StringBuilder().append("Write suspend: ").append(wait).append(':').append(ctx.channel().config().isAutoRead()).append(':').append(AbstractTrafficShapingHandler.isHandlerActive(ctx)).toString());
                }
