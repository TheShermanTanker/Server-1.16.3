...
public class TrafficCounter {
    ...
    public synchronized void start() {
        ...
        if (localCheckInterval > 0L && this.executor != null) {
            ...
            this.monitor = (Runnable)new TrafficMonitoringTask();
    ...
    synchronized void resetAccounting(final long newLastTime) {
        ...
        if (TrafficCounter.logger.isDebugEnabled() && interval > this.checkInterval() << 1) {
            TrafficCounter.logger.debug(new StringBuilder().append("Acct schedule not ok: ").append(interval).append(" > 2*").append(this.checkInterval()).append(" from ").append(this.name).toString());
        }
        ...
        if (interval > 10L) {
            ...
            if (time > 10L) {
                if (TrafficCounter.logger.isDebugEnabled()) {
                    TrafficCounter.logger.debug(new StringBuilder().append("Time: ").append(time).append(':').append(sum).append(':').append(interval).append(':').append(pastDelay).toString());
                }
            ...
            if (time2 > 10L) {
                if (TrafficCounter.logger.isDebugEnabled()) {
                    TrafficCounter.logger.debug(new StringBuilder().append("Time: ").append(time2).append(':').append(lastsum).append(':').append(lastinterval).append(':').append(pastDelay).toString());
                }
    ...
    private final class TrafficMonitoringTask implements Runnable {
        public void run() {
            ...
            TrafficCounter.this.scheduledFuture = TrafficCounter.this.executor.schedule((Runnable)this, TrafficCounter.this.checkInterval.get(), TimeUnit.MILLISECONDS);
        }
    }
}
