...
public final class SelfSignedCertificate {
    ...
    public SelfSignedCertificate(final String fqdn, final SecureRandom random, final int bits, final Date notBefore, final Date notAfter) throws CertificateException {
        ...
        try {
            ...
            this.cert = (X509Certificate)CertificateFactory.getInstance("X509").generateCertificate((InputStream)certificateInput);
        }
        catch (Exception e2) {
            throw new CertificateEncodingException((Throwable)e2);
        }
        finally {
            if (certificateInput != null) {
                try {
                    ...
                    SelfSignedCertificate.logger.warn(new StringBuilder().append("Failed to close a file: ").append(this.certificate).toString(), (Throwable)e3);
                }
            }
        }
    }
    ...
    static String[] newSelfSignedCertificate(final String fqdn, final PrivateKey key, final X509Certificate cert) throws IOException, CertificateEncodingException {
        ...
        OutputStream keyOut = (OutputStream)new FileOutputStream(keyFile);
        try {
        ...
        OutputStream certOut = (OutputStream)new FileOutputStream(certFile);
        try {
    ...
    private static void safeDelete(final File certFile) {
        if (!certFile.delete()) {
            SelfSignedCertificate.logger.warn(new StringBuilder().append("Failed to delete a file: ").append(certFile).toString());
        }
    }
    ...
    private static void safeClose(final File keyFile, final OutputStream keyOut) {
        try {
            ...
            SelfSignedCertificate.logger.warn(new StringBuilder().append("Failed to close a file: ").append(keyFile).toString(), (Throwable)e);
        }
    }
