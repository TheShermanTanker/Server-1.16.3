...
public final class ReferenceCountedOpenSslClientContext extends ReferenceCountedOpenSslContext {
        ...
        try {
            if (!OpenSsl.useKeyManagerFactory()) {
                ...
                if (keyManagerFactory != null) {
                    ...
                    SSLContext.setCertRequestedCallback(ctx, (CertificateRequestedCallback)new OpenSslCertificateRequestedCallback(engineMap, materialManager));
                }
            }
        }
        catch (Exception e) {
    ...
    private static final class OpenSslCertificateRequestedCallback implements CertificateRequestedCallback {
        ...
        public CertificateRequestedCallback.KeyMaterial requested(final long ssl, final byte[] keyTypeBytes, final byte[][] asn1DerEncodedPrincipals) {
            final ReferenceCountedOpenSslEngine engine = this.engineMap.get(ssl);
            try {
                final Set<String> keyTypesSet = supportedClientKeyTypes(keyTypeBytes);
                final String[] keyTypes = (String[])keyTypesSet.toArray((Object[])new String[keyTypesSet.size()]);
        ...
        private static Set<String> supportedClientKeyTypes(final byte[] clientCertificateTypes) {
            final Set<String> result = (Set<String>)new HashSet(clientCertificateTypes.length);
