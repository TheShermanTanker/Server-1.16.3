...
public class ServerBootstrap extends AbstractBootstrap<ServerBootstrap, ServerChannel> {
    ...
    public ServerBootstrap() {
        this.childOptions = (Map<ChannelOption<?>, Object>)new LinkedHashMap();
        this.childAttrs = (Map<AttributeKey<?>, Object>)new LinkedHashMap();
    ...
    private ServerBootstrap(final ServerBootstrap bootstrap) {
        ...
        synchronized (bootstrap.childOptions) {
            this.childOptions.putAll((Map)bootstrap.childOptions);
        }
        synchronized (bootstrap.childAttrs) {
            this.childAttrs.putAll((Map)bootstrap.childAttrs);
        }
    }
    ...
    void init(final Channel channel) throws Exception {
        ...
        synchronized (attrs) {
            for (final Map.Entry<AttributeKey<?>, Object> e : attrs.entrySet()) {
                final AttributeKey<Object> key = (AttributeKey<Object>)e.getKey();
        ...
        synchronized (this.childOptions) {
            currentChildOptions = this.childOptions.entrySet().toArray((Object[])newOptionArray(this.childOptions.size()));
        }
        final Map.Entry<AttributeKey<?>, Object>[] currentChildAttrs;
        synchronized (this.childAttrs) {
            currentChildAttrs = this.childAttrs.entrySet().toArray((Object[])newAttrArray(this.childAttrs.size()));
        }
        p.addLast(new ChannelInitializer<Channel>() {
            public void initChannel(final Channel ch) throws Exception {
                ...
                ch.eventLoop().execute((Runnable)new Runnable() {
                    public void run() {
    ...
    final Map<ChannelOption<?>, Object> childOptions() {
        return AbstractBootstrap.<ChannelOption<?>, Object>copiedMap(this.childOptions);
    }
    
    final Map<AttributeKey<?>, Object> childAttrs() {
        return AbstractBootstrap.<AttributeKey<?>, Object>copiedMap(this.childAttrs);
    }
    ...
    private static class ServerBootstrapAcceptor extends ChannelInboundHandlerAdapter {
        ...
        ServerBootstrapAcceptor(final Channel channel, final EventLoopGroup childGroup, final ChannelHandler childHandler, final Map.Entry<ChannelOption<?>, Object>[] childOptions, final Map.Entry<AttributeKey<?>, Object>[] childAttrs) {
            ...
            this.enableAutoReadTask = (Runnable)new Runnable() {
                public void run() {
