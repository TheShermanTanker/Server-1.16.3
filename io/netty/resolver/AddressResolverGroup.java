...
public abstract class AddressResolverGroup<T extends SocketAddress> implements Closeable {
    ...
    protected AddressResolverGroup() {
        this.resolvers = (Map<EventExecutor, AddressResolver<T>>)new IdentityHashMap();
    }
    ...
    public AddressResolver<T> getResolver(final EventExecutor executor) {
        ...
        synchronized (this.resolvers) {
            r = (AddressResolver<T>)this.resolvers.get(executor);
            if (r == null) {
                ...
                try {
                    ...
                    throw new IllegalStateException("failed to create a new resolver", (Throwable)e);
                }
    ...
    protected abstract AddressResolver<T> newResolver(final EventExecutor eventExecutor) throws Exception;
    
    public void close() {
        final AddressResolver<T>[] rArray;
        synchronized (this.resolvers) {
            rArray = this.resolvers.values().toArray((Object[])new AddressResolver[this.resolvers.size()]);
