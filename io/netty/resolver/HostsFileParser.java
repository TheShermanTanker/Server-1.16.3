...
public final class HostsFileParser {
    ...
    public static HostsFileEntries parseSilently() {
        ...
        try {
            ...
            HostsFileParser.logger.warn("Failed to load and parse hosts file at " + hostsFile.getPath(), (Throwable)e);
    ...
    public static HostsFileEntries parse(final File file) throws IOException {
        ...
        if (file.exists() && file.isFile()) {
            return parse((Reader)new BufferedReader((Reader)new FileReader(file)));
        }
        return HostsFileEntries.EMPTY;
    }
    
    public static HostsFileEntries parse(final Reader reader) throws IOException {
        ObjectUtil.<Reader>checkNotNull(reader, "reader");
        final BufferedReader buff = new BufferedReader(reader);
        try {
            final Map<String, Inet4Address> ipv4Entries = (Map<String, Inet4Address>)new HashMap();
            final Map<String, Inet6Address> ipv6Entries = (Map<String, Inet6Address>)new HashMap();
            String line;
            while ((line = buff.readLine()) != null) {
                ...
                final List<String> lineParts = (List<String>)new ArrayList();
                for (final String s : HostsFileParser.WHITESPACES.split((CharSequence)line)) {
                    if (!s.isEmpty()) {
                        lineParts.add(s);
                    }
                }
                ...
                final byte[] ipBytes = NetUtil.createByteArrayFromIpAddressString((String)lineParts.get(0));
                if (ipBytes == null) {
                    continue;
                }
                for (int i = 1; i < lineParts.size(); ++i) {
                    final String hostname = (String)lineParts.get(i);
                    ...
                    if (address instanceof Inet4Address) {
                        final Inet4Address previous = (Inet4Address)ipv4Entries.put(hostnameLower, address);
                        if (previous != null) {
                            ipv4Entries.put(hostnameLower, previous);
                        }
                    }
                    else {
                        final Inet6Address previous2 = (Inet6Address)ipv6Entries.put(hostnameLower, address);
                        if (previous2 != null) {
            ...
            try {
                ...
                HostsFileParser.logger.warn("Failed to close a reader", (Throwable)e);
            }
        }
    }
