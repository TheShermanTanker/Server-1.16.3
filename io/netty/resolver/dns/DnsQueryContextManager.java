...
final class DnsQueryContextManager {
    ...
    DnsQueryContextManager() {
        this.map = (Map<InetSocketAddress, IntObjectMap<DnsQueryContext>>)new HashMap();
    }
    ...
    int add(final DnsQueryContext qCtx) {
        ...
        synchronized (contexts) {
            while (contexts.containsKey(id)) {
                id = (id + 1 & 0xFFFF);
                if (++tries >= 131070) {
                    throw new IllegalStateException(new StringBuilder().append("query ID space exhausted: ").append(qCtx.question()).toString());
                }
            }
    ...
    private IntObjectMap<DnsQueryContext> getContextMap(final InetSocketAddress nameServerAddr) {
        synchronized (this.map) {
            return (IntObjectMap<DnsQueryContext>)this.map.get(nameServerAddr);
        }
    }
    
    private IntObjectMap<DnsQueryContext> getOrCreateContextMap(final InetSocketAddress nameServerAddr) {
        synchronized (this.map) {
            final IntObjectMap<DnsQueryContext> contexts = (IntObjectMap<DnsQueryContext>)this.map.get(nameServerAddr);
            if (contexts != null) {
                return contexts;
            }
            ...
            if (a instanceof Inet4Address) {
                ...
                if (a2.isLoopbackAddress()) {
                    this.map.put(new InetSocketAddress((InetAddress)NetUtil.LOCALHOST6, port), newContexts);
                }
                else {
                    this.map.put(new InetSocketAddress((InetAddress)toCompactAddress(a2), port), newContexts);
                }
            }
            else if (a instanceof Inet6Address) {
                final Inet6Address a3 = (Inet6Address)a;
                if (a3.isLoopbackAddress()) {
                    this.map.put(new InetSocketAddress((InetAddress)NetUtil.LOCALHOST4, port), newContexts);
                }
                else if (a3.isIPv4CompatibleAddress()) {
                    this.map.put(new InetSocketAddress((InetAddress)toIPv4Address(a3), port), newContexts);
                }
            }
            return newContexts;
        }
    ...
    private static Inet6Address toCompactAddress(final Inet4Address a4) {
