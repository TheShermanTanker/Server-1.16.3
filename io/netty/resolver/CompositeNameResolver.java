...
public final class CompositeNameResolver<T> extends SimpleNameResolver<T> {
    ...
    public CompositeNameResolver(final EventExecutor executor, final NameResolver<T>... resolvers) {
        super(executor);
        ObjectUtil.<NameResolver[]>checkNotNull((NameResolver[])resolvers, "resolvers");
        for (int i = 0; i < resolvers.length; ++i) {
            if (resolvers[i] == null) {
                throw new NullPointerException(new StringBuilder().append("resolvers[").append(i).append(']').toString());
            }
        }
        if (resolvers.length < 2) {
            throw new IllegalArgumentException(new StringBuilder().append("resolvers: ").append(Arrays.asList((Object[])resolvers)).append(" (expected: at least 2 resolvers)").toString());
        }
