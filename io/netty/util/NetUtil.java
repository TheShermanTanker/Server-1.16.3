...
public final class NetUtil {
    ...
    public static byte[] createByteArrayFromIpAddressString(String ipAddressString) {
        ...
        if (isValidIpV6Address(ipAddressString)) {
            ...
            return getIPv6ByName((CharSequence)ipAddressString, true);
        }
        return null;
    }
    ...
    public static String bytesToIpAddress(final byte[] bytes, final int offset, final int length) {
        switch (length) {
            ...
            default: {
                throw new IllegalArgumentException(new StringBuilder().append("length: ").append(length).append(" (expected: 4 or 16)").toString());
            }
        }
    }
    ...
    private static boolean isValidIpV4Address(final String ip, int from, final int toExcluded) {
        ...
        int i;
        return len <= 15 && len >= 7 && (i = ip.indexOf(46, from + 1)) > 0 && isValidIpV4Word((CharSequence)ip, from, i) && (i = ip.indexOf(46, from = i + 2)) > 0 && isValidIpV4Word((CharSequence)ip, from - 1, i) && (i = ip.indexOf(46, from = i + 2)) > 0 && isValidIpV4Word((CharSequence)ip, from - 1, i) && isValidIpV4Word((CharSequence)ip, i + 1, toExcluded);
    }
    
    private static boolean isValidIpV4Address(final AsciiString ip, int from, final int toExcluded) {
        final int len = toExcluded - from;
        int i;
        return len <= 15 && len >= 7 && (i = ip.indexOf('.', from + 1)) > 0 && isValidIpV4Word((CharSequence)ip, from, i) && (i = ip.indexOf('.', from = i + 2)) > 0 && isValidIpV4Word((CharSequence)ip, from - 1, i) && (i = ip.indexOf('.', from = i + 2)) > 0 && isValidIpV4Word((CharSequence)ip, from - 1, i) && isValidIpV4Word((CharSequence)ip, i + 1, toExcluded);
    }
    ...
    public static String toAddressString(final InetAddress ip, final boolean ipv4Mapped) {
        ...
        if (!(ip instanceof Inet6Address)) {
            throw new IllegalArgumentException(new StringBuilder().append("Unhandled type: ").append(ip).toString());
        }
    ...
    static {
        ...
        (logger = InternalLoggerFactory.getInstance(NetUtil.class)).debug("-Djava.net.preferIPv4Stack: {}", NetUtil.IPV4_PREFERRED);
        NetUtil.logger.debug("-Djava.net.preferIPv6Addresses: {}", NetUtil.IPV6_ADDRESSES_PREFERRED);
        ...
        try {
            ...
            PlatformDependent.throwException((Throwable)e2);
        }
        LOCALHOST6 = localhost5;
        final List<NetworkInterface> ifaces = (List<NetworkInterface>)new ArrayList();
        try {
            final Enumeration<NetworkInterface> interfaces = (Enumeration<NetworkInterface>)NetworkInterface.getNetworkInterfaces();
            if (interfaces != null) {
            ...
            NetUtil.logger.warn("Failed to retrieve the list of available network interfaces", (Throwable)e3);
        }
    ...
    Label_0390:
        for (final NetworkInterface iface2 : ifaces) {
            ...
            while (i.hasMoreElements()) {
                final InetAddress addr = (InetAddress)i.nextElement();
        ...
        if (loopbackIface == null) {
            try {
                ...
                NetUtil.logger.warn("Failed to find the loopback interface", (Throwable)e4);
            }
        }
        if (loopbackIface != null) {
            ...
            try {
                if (NetworkInterface.getByInetAddress((InetAddress)NetUtil.LOCALHOST6) != null) {
                    NetUtil.logger.debug("Using hard-coded IPv6 localhost address: {}", localhost5);
                    loopbackAddr = (InetAddress)localhost5;
                }
            }
            catch (Exception ex) {}
            finally {
                if (loopbackAddr == null) {
                    ...
                    loopbackAddr = (InetAddress)localhost4;
                }
            }
        }
        LOOPBACK_IF = loopbackIface;
        LOCALHOST = loopbackAddr;
        SOMAXCONN = (int)AccessController.doPrivileged((PrivilegedAction)new PrivilegedAction<Integer>() {
            public Integer run() {
                ...
                try {
                    if (file.exists()) {
                        in = new BufferedReader((Reader)new FileReader(file));
