...
public class HashedWheelTimer implements Timer {
    ...
    public HashedWheelTimer(final ThreadFactory threadFactory, final long tickDuration, final TimeUnit unit, final int ticksPerWheel, final boolean leakDetection, final long maxPendingTimeouts) {
        ...
        this.timeouts = PlatformDependent.<HashedWheelTimeout>newMpscQueue();
        this.cancelledTimeouts = PlatformDependent.<HashedWheelTimeout>newMpscQueue();
        ...
        if (tickDuration <= 0L) {
            throw new IllegalArgumentException(new StringBuilder().append("tickDuration must be greater than 0: ").append(tickDuration).toString());
        }
        if (ticksPerWheel <= 0) {
            throw new IllegalArgumentException(new StringBuilder().append("ticksPerWheel must be greater than 0: ").append(ticksPerWheel).toString());
        }
        this.wheel = createWheel(ticksPerWheel);
        this.mask = this.wheel.length - 1;
        this.tickDuration = unit.toNanos(tickDuration);
        if (this.tickDuration >= Long.MAX_VALUE / this.wheel.length) {
            throw new IllegalArgumentException(String.format("tickDuration: %d (expected: 0 < tickDuration in nanos < %d", new Object[] { tickDuration, Long.MAX_VALUE / this.wheel.length }));
        }
    ...
    private static HashedWheelBucket[] createWheel(int ticksPerWheel) {
        ...
        if (ticksPerWheel > 1073741824) {
            throw new IllegalArgumentException(new StringBuilder().append("ticksPerWheel may not be greater than 2^30: ").append(ticksPerWheel).toString());
        }
    ...
    public Set<Timeout> stop() {
        ...
        if (!HashedWheelTimer.WORKER_STATE_UPDATER.compareAndSet(this, 1, 2)) {
            ...
            return (Set<Timeout>)Collections.emptySet();
        }
        try {
    ...
    public Timeout newTimeout(final TimerTask task, final long delay, final TimeUnit unit) {
        ...
        if (this.maxPendingTimeouts > 0L && pendingTimeoutsCount > this.maxPendingTimeouts) {
            ...
            throw new RejectedExecutionException(new StringBuilder().append("Number of pending timeouts (").append(pendingTimeoutsCount).append(") is greater than or equal to maximum allowed pending timeouts (").append(this.maxPendingTimeouts).append(")").toString());
        }
        this.start();
    ...
    static {
        ...
        leakDetector = ResourceLeakDetectorFactory.instance().<HashedWheelTimer>newResourceLeakDetector(HashedWheelTimer.class, 1);
        WORKER_STATE_UPDATER = AtomicIntegerFieldUpdater.newUpdater((Class)HashedWheelTimer.class, "workerState");
    }
    ...
    private final class Worker implements Runnable {
        ...
        private Worker() {
            this.unprocessedTimeouts = (Set<Timeout>)new HashSet();
        }
        ...
        public void run() {
            ...
            while (true) {
                final HashedWheelTimeout timeout = (HashedWheelTimeout)HashedWheelTimer.this.timeouts.poll();
                if (timeout == null) {
                    break;
                }
        ...
        private void processCancelledTasks() {
            while (true) {
                final HashedWheelTimeout timeout = (HashedWheelTimeout)HashedWheelTimer.this.cancelledTimeouts.poll();
                if (timeout == null) {
                    break;
                }
                try {
        ...
        public Set<Timeout> unprocessedTimeouts() {
            return (Set<Timeout>)Collections.unmodifiableSet((Set)this.unprocessedTimeouts);
        }
    }
    ...
    private static final class HashedWheelTimeout implements Timeout {
        ...
        static {
            STATE_UPDATER = AtomicIntegerFieldUpdater.newUpdater((Class)HashedWheelTimeout.class, "state");
        }
    }
            ...
            for (HashedWheelTimeout timeout = this.head; timeout != null; timeout = next) {
                ...
                if (timeout.remainingRounds <= 0L) {
                    ...
                    if (timeout.deadline > deadline) {
                        throw new IllegalStateException(String.format("timeout.deadline (%d) > deadline (%d)", new Object[] { timeout.deadline, deadline }));
                    }
                    timeout.expire();
                }
