...
public final class DomainNameMappingBuilder<V> {
    ...
    public DomainNameMappingBuilder(final int initialCapacity, final V defaultValue) {
        ...
        this.map = (Map<String, V>)new LinkedHashMap(initialCapacity);
    }
    
    public DomainNameMappingBuilder<V> add(final String hostname, final V output) {
        this.map.put(ObjectUtil.<String>checkNotNull(hostname, "hostname"), ObjectUtil.<V>checkNotNull(output, "output"));
        return this;
    }
    
    public DomainNameMapping<V> build() {
        return new ImmutableDomainNameMapping<V>(this.defaultValue, (Map)this.map);
    }
    ...
    private static final class ImmutableDomainNameMapping<V> extends DomainNameMapping<V> {
        ...
        private ImmutableDomainNameMapping(final V defaultValue, final Map<String, V> map) {
            ...
            final Set<Map.Entry<String, V>> mappings = (Set<Map.Entry<String, V>>)map.entrySet();
            ...
            final Map<String, V> mapCopy = (Map<String, V>)new LinkedHashMap(map.size());
            int index = 0;
            for (final Map.Entry<String, V> mapping : mappings) {
                final String hostname = DomainNameMapping.normalizeHostname((String)mapping.getKey());
                final V value = (V)mapping.getValue();
                mapCopy.put((this.domainNamePatterns[index] = hostname), (this.values[index] = value));
                ++index;
            }
            this.map = (Map<String, V>)Collections.unmodifiableMap((Map)mapCopy);
        }
