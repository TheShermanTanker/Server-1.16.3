...
public final class ThreadLocalRandom extends Random {
        ...
        synchronized (ThreadLocalRandom.class) {
            ...
            while (true) {
                ...
                try {
                    Long seed;
                    if (waitTime <= 0L) {
                        seed = (Long)ThreadLocalRandom.seedQueue.poll();
                    }
                    else {
                        seed = (Long)ThreadLocalRandom.seedQueue.poll(waitTime, TimeUnit.NANOSECONDS);
                    }
                    if (seed != null) {
    ...
    private static long newSeed() {
        ...
        if (current == 0L && ThreadLocalRandom.logger.isDebugEnabled()) {
            if (ThreadLocalRandom.seedGeneratorEndTime != 0L) {
                ThreadLocalRandom.logger.debug(String.format("-Dio.netty.initialSeedUniquifier: 0x%016x (took %d ms)", new Object[] { actualCurrent, TimeUnit.NANOSECONDS.toMillis(ThreadLocalRandom.seedGeneratorEndTime - ThreadLocalRandom.seedGeneratorStartTime) }));
            }
            else {
                ThreadLocalRandom.logger.debug(String.format("-Dio.netty.initialSeedUniquifier: 0x%016x", new Object[] { actualCurrent }));
            }
        }
    ...
    static {
        ...
        if (ThreadLocalRandom.initialSeedUniquifier == 0L) {
            ...
            if (secureRandom) {
                seedQueue = (BlockingQueue)new LinkedBlockingQueue();
                ...
                ThreadLocalRandom.seedGeneratorThread.setUncaughtExceptionHandler((Thread.UncaughtExceptionHandler)new Thread.UncaughtExceptionHandler() {
