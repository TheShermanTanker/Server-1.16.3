...
public final class MacAddressUtil {
    ...
    public static byte[] bestAvailableMac() {
        ...
        InetAddress bestInetAddr = (InetAddress)NetUtil.LOCALHOST4;
        final Map<NetworkInterface, InetAddress> ifaces = (Map<NetworkInterface, InetAddress>)new LinkedHashMap();
        try {
            ...
            if (interfaces != null) {
                while (interfaces.hasMoreElements()) {
                    ...
                    if (addrs.hasMoreElements()) {
                        final InetAddress a = (InetAddress)addrs.nextElement();
            ...
            MacAddressUtil.logger.warn("Failed to retrieve the list of available network interfaces", (Throwable)e);
        }
        for (final Map.Entry<NetworkInterface, InetAddress> entry : ifaces.entrySet()) {
            final NetworkInterface iface2 = (NetworkInterface)entry.getKey();
            final InetAddress inetAddr = (InetAddress)entry.getValue();
            if (iface2.isVirtual()) {
                continue;
            }
            byte[] macAddr;
    ...
    public static byte[] parseMAC(final String value) {
        ...
        for (int i = 0; i < end; ++i, j += 3) {
            ...
            machineId[i] = StringUtil.decodeHexByte((CharSequence)value, j);
            if (value.charAt(sIndex) != separator) {
                throw new IllegalArgumentException(new StringBuilder().append("expected separator '").append(separator).append(" but got '").append(value.charAt(sIndex)).append("' at index: ").append(sIndex).toString());
            }
        }
        machineId[end] = StringUtil.decodeHexByte((CharSequence)value, j);
        return machineId;
    }
    
    private static void validateMacSeparator(final char separator) {
        if (separator != ':' && separator != '-') {
            throw new IllegalArgumentException(new StringBuilder().append("unsupported separator: ").append(separator).append(" (expected: [:-])").toString());
        }
    }
    ...
    public static String formatAddress(final byte[] addr) {
        ...
        for (final byte b : addr) {
            buf.append(String.format("%02x:", new Object[] { b & 0xFF }));
        }
