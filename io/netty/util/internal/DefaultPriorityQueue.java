...
public final class DefaultPriorityQueue<T extends PriorityQueueNode> extends AbstractQueue<T> implements PriorityQueue<T> {
    ...
    public boolean offer(final T e) {
        if (e.priorityQueueIndex(this) != -1) {
            throw new IllegalArgumentException(new StringBuilder().append("e.priorityQueueIndex(): ").append(e.priorityQueueIndex(this)).append(" (expected: ").append(-1).append(") + e: ").append(e).toString());
        }
        if (this.size >= this.queue.length) {
            this.queue = (T[])Arrays.copyOf((Object[])this.queue, this.queue.length + ((this.queue.length < 64) ? (this.queue.length + 2) : (this.queue.length >>> 1)));
        }
    ...
    public <X> X[] toArray(final X[] a) {
        if (a.length < this.size) {
            return (X[])Arrays.copyOf((Object[])this.queue, this.size, a.getClass());
        }
    ...
    public Iterator<T> iterator() {
        return (Iterator<T>)new PriorityQueueIterator();
    }
