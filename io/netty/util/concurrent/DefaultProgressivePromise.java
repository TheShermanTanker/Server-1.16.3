...
public class DefaultProgressivePromise<V> extends DefaultPromise<V> implements ProgressivePromise<V> {
    ...
    public ProgressivePromise<V> setProgress(final long progress, long total) {
        if (total < 0L) {
            total = -1L;
            if (progress < 0L) {
                throw new IllegalArgumentException(new StringBuilder().append("progress: ").append(progress).append(" (expected: >= 0)").toString());
            }
        }
        else if (progress < 0L || progress > total) {
            throw new IllegalArgumentException(new StringBuilder().append("progress: ").append(progress).append(" (expected: 0 <= progress <= total (").append(total).append("))").toString());
        }
        if (this.isDone()) {
