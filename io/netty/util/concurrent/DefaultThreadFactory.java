...
public class DefaultThreadFactory implements ThreadFactory {
    ...
    public DefaultThreadFactory(final String poolName, final boolean daemon, final int priority, final ThreadGroup threadGroup) {
        ...
        if (priority < 1 || priority > 10) {
            throw new IllegalArgumentException(new StringBuilder().append("priority: ").append(priority).append(" (expected: Thread.MIN_PRIORITY <= priority <= Thread.MAX_PRIORITY)").toString());
        }
