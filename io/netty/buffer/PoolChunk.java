...
final class PoolChunk<T> implements PoolChunkMetric {
    ...
    PoolChunk(final PoolArena<T> arena, final T memory, final int pageSize, final int maxOrder, final int pageShifts, final int chunkSize, final int offset) {
        this.unpooled = false;
        this.arena = arena;
        this.memory = memory;
        ...
        assert maxOrder < 30 : new StringBuilder().append("maxOrder should be < 30, but is: ").append(maxOrder).toString();
    ...
    PoolChunk(final PoolArena<T> arena, final T memory, final int size, final int offset) {
        this.unpooled = true;
        this.arena = arena;
        this.memory = memory;
        this.offset = offset;
    ...
    private int allocateNode(final int d) {
        ...
        assert value == d && (id & initial) == 1 << d : String.format("val = %d, id & initial = %d, d = %d", new Object[] { value, id & initial, d });
    ...
    private long allocateSubpage(final int normCapacity) {
        final PoolSubpage<T> head = this.arena.findSubpagePoolHead(normCapacity);
        synchronized (head) {
    ...
    void free(final long handle) {
        final int memoryMapIdx = memoryMapIdx(handle);
        final int bitmapIdx = bitmapIdx(handle);
        if (bitmapIdx != 0) {
            final PoolSubpage<T> subpage = this.subpages[this.subpageIdx(memoryMapIdx)];
            assert subpage != null && subpage.doNotDestroy;
            final PoolSubpage<T> head = this.arena.findSubpagePoolHead(subpage.elemSize);
            synchronized (head) {
    ...
    private static int memoryMapIdx(final long handle) {
        return (int)handle;
    }
    
    private static int bitmapIdx(final long handle) {
