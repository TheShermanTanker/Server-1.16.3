...
public class PooledByteBufAllocator extends AbstractByteBufAllocator implements ByteBufAllocatorMetricProvider {
    ...
    public PooledByteBufAllocator(final boolean preferDirect, final int nHeapArena, final int nDirectArena, final int pageSize, final int maxOrder, final int tinyCacheSize, final int smallCacheSize, final int normalCacheSize, final boolean useCacheForAllThreads, final int directMemoryCacheAlignment) {
        ...
        if (nHeapArena < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("nHeapArena: ").append(nHeapArena).append(" (expected: >= 0)").toString());
        }
        if (nDirectArena < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("nDirectArea: ").append(nDirectArena).append(" (expected: >= 0)").toString());
        }
        if (directMemoryCacheAlignment < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("directMemoryCacheAlignment: ").append(directMemoryCacheAlignment).append(" (expected: >= 0)").toString());
        }
        if (directMemoryCacheAlignment > 0 && !isDirectMemoryCacheAlignmentSupported()) {
            throw new IllegalArgumentException("directMemoryCacheAlignment is not supported");
        }
        if ((directMemoryCacheAlignment & -directMemoryCacheAlignment) != directMemoryCacheAlignment) {
            throw new IllegalArgumentException(new StringBuilder().append("directMemoryCacheAlignment: ").append(directMemoryCacheAlignment).append(" (expected: power of two)").toString());
        }
        final int pageShifts = validateAndCalculatePageShifts(pageSize);
        if (nHeapArena > 0) {
            this.heapArenas = PooledByteBufAllocator.<byte[]>newArenaArray(nHeapArena);
            final List<PoolArenaMetric> metrics = (List<PoolArenaMetric>)new ArrayList(this.heapArenas.length);
            ...
            this.heapArenaMetrics = (List<PoolArenaMetric>)Collections.unmodifiableList((List)metrics);
        }
        else {
            this.heapArenas = null;
            this.heapArenaMetrics = (List<PoolArenaMetric>)Collections.emptyList();
        }
        if (nDirectArena > 0) {
            this.directArenas = PooledByteBufAllocator.<ByteBuffer>newArenaArray(nDirectArena);
            final List<PoolArenaMetric> metrics = (List<PoolArenaMetric>)new ArrayList(this.directArenas.length);
            ...
            this.directArenaMetrics = (List<PoolArenaMetric>)Collections.unmodifiableList((List)metrics);
        }
        else {
            this.directArenas = null;
            this.directArenaMetrics = (List<PoolArenaMetric>)Collections.emptyList();
        }
    ...
    private static int validateAndCalculatePageShifts(final int pageSize) {
        if (pageSize < 4096) {
            throw new IllegalArgumentException(new StringBuilder().append("pageSize: ").append(pageSize).append(" (expected: ").append(4096).append(")").toString());
        }
        if ((pageSize & pageSize - 1) != 0x0) {
            throw new IllegalArgumentException(new StringBuilder().append("pageSize: ").append(pageSize).append(" (expected: power of 2)").toString());
        }
    ...
    private static int validateAndCalculateChunkSize(final int pageSize, final int maxOrder) {
        if (maxOrder > 14) {
            throw new IllegalArgumentException(new StringBuilder().append("maxOrder: ").append(maxOrder).append(" (expected: 0-14)").toString());
        }
        int chunkSize = pageSize;
        for (int i = maxOrder; i > 0; --i) {
            if (chunkSize > 536870912) {
                throw new IllegalArgumentException(String.format("pageSize (%d) << maxOrder (%d) must not exceed %d", new Object[] { pageSize, maxOrder, 1073741824 }));
            }
            chunkSize <<= 1;
        }
        return chunkSize;
    ...
    public int numThreadLocalCaches() {
        final PoolArena<?>[] arenas = ((this.heapArenas != null) ? this.heapArenas : this.directArenas);
        if (arenas == null) {
            return 0;
        }
        int total = 0;
    ...
    static {
        ...
        DEFAULT_NUM_HEAP_ARENA = Math.max(0, SystemPropertyUtil.getInt("io.netty.allocator.numHeapArenas", (int)Math.min((long)defaultMinNumArena, runtime.maxMemory() / defaultChunkSize / 2L / 3L)));
        DEFAULT_NUM_DIRECT_ARENA = Math.max(0, SystemPropertyUtil.getInt("io.netty.allocator.numDirectArenas", (int)Math.min((long)defaultMinNumArena, PlatformDependent.maxDirectMemory() / defaultChunkSize / 2L / 3L)));
        ...
        if (PooledByteBufAllocator.logger.isDebugEnabled()) {
            PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.numHeapArenas: {}", PooledByteBufAllocator.DEFAULT_NUM_HEAP_ARENA);
            PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.numDirectArenas: {}", PooledByteBufAllocator.DEFAULT_NUM_DIRECT_ARENA);
            if (pageSizeFallbackCause == null) {
                PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.pageSize: {}", PooledByteBufAllocator.DEFAULT_PAGE_SIZE);
            }
            else {
                PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.pageSize: {}", PooledByteBufAllocator.DEFAULT_PAGE_SIZE, pageSizeFallbackCause);
            }
            if (maxOrderFallbackCause == null) {
                PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.maxOrder: {}", PooledByteBufAllocator.DEFAULT_MAX_ORDER);
            }
            else {
                PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.maxOrder: {}", PooledByteBufAllocator.DEFAULT_MAX_ORDER, maxOrderFallbackCause);
            }
            PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.chunkSize: {}", (PooledByteBufAllocator.DEFAULT_PAGE_SIZE << PooledByteBufAllocator.DEFAULT_MAX_ORDER));
            PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.tinyCacheSize: {}", PooledByteBufAllocator.DEFAULT_TINY_CACHE_SIZE);
            PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.smallCacheSize: {}", PooledByteBufAllocator.DEFAULT_SMALL_CACHE_SIZE);
            PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.normalCacheSize: {}", PooledByteBufAllocator.DEFAULT_NORMAL_CACHE_SIZE);
            PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.maxCachedBufferCapacity: {}", PooledByteBufAllocator.DEFAULT_MAX_CACHED_BUFFER_CAPACITY);
            PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.cacheTrimInterval: {}", PooledByteBufAllocator.DEFAULT_CACHE_TRIM_INTERVAL);
            PooledByteBufAllocator.logger.debug("-Dio.netty.allocator.useCacheForAllThreads: {}", PooledByteBufAllocator.DEFAULT_USE_CACHE_FOR_ALL_THREADS);
        }
    ...
    final class PoolThreadLocalCache extends FastThreadLocal<PoolThreadCache> {
        ...
        protected synchronized PoolThreadCache initialValue() {
            final PoolArena<byte[]> heapArena = this.<byte[]>leastUsedArena(PooledByteBufAllocator.this.heapArenas);
            final PoolArena<ByteBuffer> directArena = this.<ByteBuffer>leastUsedArena(PooledByteBufAllocator.this.directArenas);
