...
public class UnpooledHeapByteBuf extends AbstractReferenceCountedByteBuf {
    ...
    public UnpooledHeapByteBuf(final ByteBufAllocator alloc, final int initialCapacity, final int maxCapacity) {
        super(maxCapacity);
        ObjectUtil.<ByteBufAllocator>checkNotNull(alloc, "alloc");
        if (initialCapacity > maxCapacity) {
            throw new IllegalArgumentException(String.format("initialCapacity(%d) > maxCapacity(%d)", new Object[] { initialCapacity, maxCapacity }));
        }
        this.alloc = alloc;
    ...
    protected UnpooledHeapByteBuf(final ByteBufAllocator alloc, final byte[] initialArray, final int maxCapacity) {
        super(maxCapacity);
        ObjectUtil.<ByteBufAllocator>checkNotNull(alloc, "alloc");
        ObjectUtil.<byte[]>checkNotNull(initialArray, "initialArray");
        if (initialArray.length > maxCapacity) {
            throw new IllegalArgumentException(String.format("initialCapacity(%d) > maxCapacity(%d)", new Object[] { initialArray.length, maxCapacity }));
        }
        this.alloc = alloc;
