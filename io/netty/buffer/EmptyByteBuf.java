...
public final class EmptyByteBuf extends ByteBuf {
    ...
    public ByteBuf ensureWritable(final int minWritableBytes) {
        if (minWritableBytes < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("minWritableBytes: ").append(minWritableBytes).append(" (expected: >= 0)").toString());
        }
        if (minWritableBytes != 0) {
    ...
    private ByteBuf checkLength(final int length) {
        if (length < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("length: ").append(length).append(" (expected: >= 0)").toString());
        }
        if (length != 0) {
            throw new IndexOutOfBoundsException();
        }
