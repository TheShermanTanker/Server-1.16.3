...
public final class ByteBufUtil {
    ...
    public static int reserveAndWriteUtf8(ByteBuf buf, final CharSequence seq, final int reserveBytes) {
        ...
        final int written = writeUtf8(byteBuf, byteBuf.writerIndex, seq, seq.length());
    ...
    public static int utf8Bytes(final CharSequence seq) {
        ...
        int seqLength;
        int i;
        for (seqLength = seq.length(), i = 0; i < seqLength && seq.charAt(i) < '\u0080'; ++i) {}
    ...
    public static byte[] getBytes(final ByteBuf buf, final int start, final int length, final boolean copy) {
        if (MathUtil.isOutOfBounds(start, length, buf.capacity())) {
            throw new IndexOutOfBoundsException(new StringBuilder().append("expected: 0 <= start(").append(start).append(") <= start + length(").append(length).append(") <= buf.capacity(").append(buf.capacity()).append(')').toString());
        }
        if (!buf.hasArray()) {
    ...
    public static void copy(final AsciiString src, final int srcIdx, final ByteBuf dst, final int dstIdx, final int length) {
        if (MathUtil.isOutOfBounds(srcIdx, length, src.length())) {
            throw new IndexOutOfBoundsException(new StringBuilder().append("expected: 0 <= srcIdx(").append(srcIdx).append(") <= srcIdx + length(").append(length).append(") <= srcLen(").append(src.length()).append(')').toString());
        }
        ObjectUtil.<ByteBuf>checkNotNull(dst, "dst").setBytes(dstIdx, src.array(), srcIdx + src.arrayOffset(), length);
    }
    ...
    public static void copy(final AsciiString src, final int srcIdx, final ByteBuf dst, final int length) {
        ...
        ObjectUtil.<ByteBuf>checkNotNull(dst, "dst").writeBytes(src.array(), srcIdx + src.arrayOffset(), length);
    }
    ...
    public static boolean isText(final ByteBuf buf, final int index, final int length, final Charset charset) {
        ObjectUtil.<ByteBuf>checkNotNull(buf, "buf");
        ObjectUtil.<Charset>checkNotNull(charset, "charset");
    ...
    static {
        ...
        ByteBufUtil.logger.debug("-Dio.netty.threadLocalDirectBufferSize: {}", ByteBufUtil.THREAD_LOCAL_BUFFER_SIZE);
        MAX_CHAR_BUFFER_SIZE = SystemPropertyUtil.getInt("io.netty.maxThreadLocalCharBufferSize", 16384);
        ByteBufUtil.logger.debug("-Dio.netty.maxThreadLocalCharBufferSize: {}", ByteBufUtil.MAX_CHAR_BUFFER_SIZE);
    ...
    private static final class HexUtil {
        ...
        private static void appendPrettyHexDump(final StringBuilder dump, final ByteBuf buf, final int offset, final int length) {
            if (MathUtil.isOutOfBounds(offset, length, buf.capacity())) {
                throw new IndexOutOfBoundsException(new StringBuilder().append("expected: 0 <= offset(").append(offset).append(") <= offset + length(").append(length).append(") <= buf.capacity(").append(buf.capacity()).append(')').toString());
            }
            if (length == 0) {
                return;
            }
