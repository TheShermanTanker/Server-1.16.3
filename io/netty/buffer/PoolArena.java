...
abstract class PoolArena<T> implements PoolArenaMetric {
    ...
    protected PoolArena(final PooledByteBufAllocator parent, final int pageSize, final int maxOrder, final int pageShifts, final int chunkSize, final int cacheAlignment) {
        ...
        final List<PoolChunkListMetric> metrics = (List<PoolChunkListMetric>)new ArrayList(6);
        ...
        this.chunkListMetrics = (List<PoolChunkListMetric>)Collections.unmodifiableList((List)metrics);
    }
    ...
    int normalizeCapacity(final int reqCapacity) {
        if (reqCapacity < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("capacity: ").append(reqCapacity).append(" (expected: 0+)").toString());
        }
    ...
    void reallocate(final PooledByteBuf<T> buf, final int newCapacity, final boolean freeOldMemory) {
        if (newCapacity < 0 || newCapacity > buf.maxCapacity()) {
            throw new IllegalArgumentException(new StringBuilder().append("newCapacity: ").append(newCapacity).toString());
        }
    ...
    private static List<PoolSubpageMetric> subPageMetricList(final PoolSubpage<?>[] pages) {
        final List<PoolSubpageMetric> metrics = (List<PoolSubpageMetric>)new ArrayList();
    ...
    public long numActiveBytes() {
        ...
        synchronized (this) {
            for (int i = 0; i < this.chunkListMetrics.size(); ++i) {
                for (final PoolChunkMetric m : (PoolChunkListMetric)this.chunkListMetrics.get(i)) {
                    val += m.chunkSize();
                }
            }
        }
    ...
    protected abstract PoolChunk<T> newChunk(final int integer1, final int integer2, final int integer3, final int integer4);
    
    protected abstract PoolChunk<T> newUnpooledChunk(final int integer);
    
    protected abstract PooledByteBuf<T> newByteBuf(final int integer);
    
    protected abstract void memoryCopy(final T object1, final int integer2, final T object3, final int integer4, final int integer5);
    
    protected abstract void destroyChunk(final PoolChunk<T> poolChunk);
