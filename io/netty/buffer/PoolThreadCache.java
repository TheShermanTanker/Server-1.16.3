...
final class PoolThreadCache {
    ...
    PoolThreadCache(final PoolArena<byte[]> heapArena, final PoolArena<ByteBuffer> directArena, final int tinyCacheSize, final int smallCacheSize, final int normalCacheSize, final int maxCachedBufferCapacity, final int freeSweepAllocationThreshold) {
        if (maxCachedBufferCapacity < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("maxCachedBufferCapacity: ").append(maxCachedBufferCapacity).append(" (expected: >= 0)").toString());
        }
        this.freeSweepAllocationThreshold = freeSweepAllocationThreshold;
        this.heapArena = heapArena;
        if ((this.directArena = directArena) != null) {
            this.tinySubPageDirectCaches = PoolThreadCache.<ByteBuffer>createSubPageCaches(tinyCacheSize, 32, PoolArena.SizeClass.Tiny);
            this.smallSubPageDirectCaches = PoolThreadCache.<ByteBuffer>createSubPageCaches(smallCacheSize, directArena.numSmallSubpagePools, PoolArena.SizeClass.Small);
            this.numShiftsNormalDirect = log2(directArena.pageSize);
            this.normalDirectCaches = PoolThreadCache.<ByteBuffer>createNormalCaches(normalCacheSize, maxCachedBufferCapacity, directArena);
        ...
        if (heapArena != null) {
            this.tinySubPageHeapCaches = PoolThreadCache.<byte[]>createSubPageCaches(tinyCacheSize, 32, PoolArena.SizeClass.Tiny);
            this.smallSubPageHeapCaches = PoolThreadCache.<byte[]>createSubPageCaches(smallCacheSize, heapArena.numSmallSubpagePools, PoolArena.SizeClass.Small);
            this.numShiftsNormalHeap = log2(heapArena.pageSize);
            this.normalHeapCaches = PoolThreadCache.<byte[]>createNormalCaches(normalCacheSize, maxCachedBufferCapacity, heapArena);
        ...
        if ((this.tinySubPageDirectCaches != null || this.smallSubPageDirectCaches != null || this.normalDirectCaches != null || this.tinySubPageHeapCaches != null || this.smallSubPageHeapCaches != null || this.normalHeapCaches != null) && freeSweepAllocationThreshold < 1) {
            throw new IllegalArgumentException(new StringBuilder().append("freeSweepAllocationThreshold: ").append(freeSweepAllocationThreshold).append(" (expected: > 0)").toString());
        }
    }
    ...
    void free() {
        ...
        if (numFreed > 0 && PoolThreadCache.logger.isDebugEnabled()) {
            PoolThreadCache.logger.debug("Freed {} thread-local buffer(s) from thread: {}", numFreed, Thread.currentThread().getName());
        }
    ...
    private abstract static class MemoryRegionCache<T> {
        ...
        MemoryRegionCache(final int size, final PoolArena.SizeClass sizeClass) {
            ...
            this.queue = PlatformDependent.<Entry<T>>newFixedMpscQueue(this.size);
            this.sizeClass = sizeClass;
        }
        
        protected abstract void initBuf(final PoolChunk<T> poolChunk, final long long2, final PooledByteBuf<T> pooledByteBuf, final int integer);
        
        public final boolean add(final PoolChunk<T> chunk, final long handle) {
            final Entry<T> entry = (Entry<T>)newEntry(chunk, handle);
        ...
        public final boolean allocate(final PooledByteBuf<T> buf, final int reqCapacity) {
            final Entry<T> entry = (Entry<T>)this.queue.poll();
            if (entry == null) {
                return false;
            }
