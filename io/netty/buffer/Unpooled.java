...
public final class Unpooled {
    ...
    public static ByteBuf wrappedBuffer(final int maxNumComponents, final byte[]... arrays) {
        switch (arrays.length) {
            ...
            default: {
                final List<ByteBuf> components = (List<ByteBuf>)new ArrayList(arrays.length);
                ...
                if (!components.isEmpty()) {
                    return new CompositeByteBuf(Unpooled.ALLOC, false, maxNumComponents, (Iterable<ByteBuf>)components);
                }
                break;
            }
        }
        ...
        switch (buffers.length) {
            ...
            default: {
                for (int i = 0; i < buffers.length; ++i) {
                    ...
                    if (buf.isReadable()) {
                        return new CompositeByteBuf(Unpooled.ALLOC, false, maxNumComponents, buffers, i, buffers.length);
                    }
                    buf.release();
                }
                break;
    ...
    public static ByteBuf wrappedBuffer(final int maxNumComponents, final ByteBuffer... buffers) {
        switch (buffers.length) {
            ...
            default: {
                final List<ByteBuf> components = (List<ByteBuf>)new ArrayList(buffers.length);
