...
public class IntBigArrayBigList extends AbstractIntBigList implements RandomAccess, Cloneable, Serializable {
    ...
    public static IntBigArrayBigList wrap(final int[][] a, final long length) {
        if (length > IntBigArrays.length(a)) {
            throw new IllegalArgumentException(new StringBuilder().append("The specified length (").append(length).append(") is greater than the array size (").append(IntBigArrays.length(a)).append(")").toString());
        }
    ...
    public void ensureCapacity(final long capacity) {
        if (capacity <= this.a.length || this.a == IntBigArrays.DEFAULT_EMPTY_BIG_ARRAY) {
            return;
        }
    ...
    private void grow(long capacity) {
        final long oldLength = IntBigArrays.length(this.a);
