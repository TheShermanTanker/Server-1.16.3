...
public class IntArrayFrontCodedList extends AbstractObjectList<int[]> implements Serializable, Cloneable, RandomAccess {
    ...
    public IntArrayFrontCodedList(final Collection<int[]> c, final int ratio) {
        this((Iterator<int[]>)c.iterator(), ratio);
    }
    
    private static int readInt(final int[][] a, final long pos) {
    ...
    private static int writeInt(final int[][] a, final int length, final long pos) {
    ...
    private int extract(final int index, final int[] a, final int offset, final int length) {
        ...
        if (currLen < length) {
            IntBigArrays.copyFromBig(this.array, pos + count(arrayLength) + count(common), a, currLen + offset, Math.min(arrayLength, length - currLen));
        }
        ...
        return new ObjectListIterator<int[]>() {
            ...
            public int[] next() {
                ...
                if (this.i % IntArrayFrontCodedList.this.ratio == 0) {
                    ...
                    length = readInt(IntArrayFrontCodedList.this.array, this.pos);
                    this.s = IntArrays.ensureCapacity(this.s, length, 0);
                    IntBigArrays.copyFromBig(IntArrayFrontCodedList.this.array, this.pos + count(length), this.s, 0, length);
                    this.pos += length + count(length);
                    this.inSync = true;
                }
                else if (this.inSync) {
                    length = readInt(IntArrayFrontCodedList.this.array, this.pos);
                    final int common = readInt(IntArrayFrontCodedList.this.array, this.pos + count(length));
                    this.s = IntArrays.ensureCapacity(this.s, length + common, common);
                    IntBigArrays.copyFromBig(IntArrayFrontCodedList.this.array, this.pos + count(length) + count(common), this.s, common, length);
