...
public class ByteArrayFrontCodedList extends AbstractObjectList<byte[]> implements Serializable, Cloneable, RandomAccess {
    ...
    public ByteArrayFrontCodedList(final Iterator<byte[]> arrays, final int ratio) {
        if (ratio < 1) {
            throw new IllegalArgumentException(new StringBuilder().append("Illegal ratio (").append(ratio).append(")").toString());
        }
    ...
    public ByteArrayFrontCodedList(final Collection<byte[]> c, final int ratio) {
        this((Iterator<byte[]>)c.iterator(), ratio);
    }
    
    private static int readInt(final byte[][] a, final long pos) {
        final byte b0 = ByteBigArrays.get(a, pos);
        if (b0 >= 0) {
            return b0;
        }
        final byte b2 = ByteBigArrays.get(a, pos + 1L);
        if (b2 >= 0) {
            return -b0 - 1 << 7 | b2;
        }
        final byte b3 = ByteBigArrays.get(a, pos + 2L);
        if (b3 >= 0) {
            return -b0 - 1 << 14 | -b2 - 1 << 7 | b3;
        }
        final byte b4 = ByteBigArrays.get(a, pos + 3L);
        if (b4 >= 0) {
            return -b0 - 1 << 21 | -b2 - 1 << 14 | -b3 - 1 << 7 | b4;
        }
        return -b0 - 1 << 28 | -b2 - 1 << 21 | -b3 - 1 << 14 | -b4 - 1 << 7 | ByteBigArrays.get(a, pos + 4L);
    }
    ...
    private static int writeInt(final byte[][] a, int length, final long pos) {
        final int count = count(length);
        ByteBigArrays.set(a, pos + count - 1L, (byte)(length & 0x7F));
        if (count != 1) {
            int i = count - 1;
            while (i-- != 0) {
                length >>>= 7;
    ...
    private int extract(final int index, final byte[] a, final int offset, final int length) {
        ...
        if (currLen < length) {
            ByteBigArrays.copyFromBig(this.array, pos + count(arrayLength) + count(common), a, currLen + offset, Math.min(arrayLength, length - currLen));
        }
        ...
        return new ObjectListIterator<byte[]>() {
            ...
            public byte[] next() {
                ...
                if (this.i % ByteArrayFrontCodedList.this.ratio == 0) {
                    ...
                    length = readInt(ByteArrayFrontCodedList.this.array, this.pos);
                    this.s = ByteArrays.ensureCapacity(this.s, length, 0);
                    ByteBigArrays.copyFromBig(ByteArrayFrontCodedList.this.array, this.pos + count(length), this.s, 0, length);
                    this.pos += length + count(length);
                    this.inSync = true;
                }
                else if (this.inSync) {
                    length = readInt(ByteArrayFrontCodedList.this.array, this.pos);
                    final int common = readInt(ByteArrayFrontCodedList.this.array, this.pos + count(length));
                    this.s = ByteArrays.ensureCapacity(this.s, length + common, common);
                    ByteBigArrays.copyFromBig(ByteArrayFrontCodedList.this.array, this.pos + count(length) + count(common), this.s, common, length);
