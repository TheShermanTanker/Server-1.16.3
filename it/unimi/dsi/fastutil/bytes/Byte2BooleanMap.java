...
public interface Byte2BooleanMap extends Byte2BooleanFunction, Map<Byte, Boolean> {
    ...
    default boolean computeIfAbsent(final byte key, final IntPredicate mappingFunction) {
    ...
    default boolean computeIfAbsentNullable(final byte key, final IntFunction<? extends Boolean> mappingFunction) {
        Objects.requireNonNull(mappingFunction);
        final boolean v = this.get(key);
        final boolean drv = this.defaultReturnValue();
        if (v != drv || this.containsKey(key)) {
    ...
    default boolean computeIfPresent(final byte key, final BiFunction<? super Byte, ? super Boolean, ? extends Boolean> remappingFunction) {
    ...
    default boolean compute(final byte key, final BiFunction<? super Byte, ? super Boolean, ? extends Boolean> remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        final boolean oldValue = this.get(key);
        final boolean drv = this.defaultReturnValue();
    ...
    default boolean merge(final byte key, final boolean value, final BiFunction<? super Boolean, ? super Boolean, ? extends Boolean> remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        final boolean oldValue = this.get(key);
        final boolean drv = this.defaultReturnValue();
        boolean newValue;
