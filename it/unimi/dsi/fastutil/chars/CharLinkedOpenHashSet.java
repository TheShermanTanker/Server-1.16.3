...
public class CharLinkedOpenHashSet extends AbstractCharSortedSet implements Serializable, Cloneable, Hash {
    ...
    private class SetIterator implements CharListIterator {
        ...
        public void remove() {
            ...
            final CharLinkedOpenHashSet this$0 = CharLinkedOpenHashSet.this;
            --this$0.size;
            if (this.prev == -1) {
                CharLinkedOpenHashSet.this.first = this.next;
            }
            else {
                final long[] link = CharLinkedOpenHashSet.this.link;
                final int prev = this.prev;
                link[prev] ^= ((CharLinkedOpenHashSet.this.link[this.prev] ^ ((long)this.next & 0xFFFFFFFFL)) & 0xFFFFFFFFL);
            }
            if (this.next == -1) {
                CharLinkedOpenHashSet.this.last = this.prev;
            }
            else {
                final long[] link2 = CharLinkedOpenHashSet.this.link;
                final int next = this.next;
                link2[next] ^= ((CharLinkedOpenHashSet.this.link[this.next] ^ ((long)this.prev & 0xFFFFFFFFL) << 32) & 0xFFFFFFFF00000000L);
            }
            int pos = this.curr;
            this.curr = -1;
        ...
        Label_0280:
            while (true) {
                pos = ((last = pos) + 1 & CharLinkedOpenHashSet.this.mask);
                char curr;
                while ((curr = key[pos]) != '\0') {
                    final int slot = HashCommon.mix(curr) & CharLinkedOpenHashSet.this.mask;
                    Label_0370: {
                        if (last <= pos) {
                            if (last >= slot) {
                                break Label_0370;
