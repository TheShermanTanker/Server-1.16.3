...
public class Arrays {
    ...
    public static void ensureFromTo(final int arrayLength, final int from, final int to) {
        ...
        if (to > arrayLength) {
            throw new ArrayIndexOutOfBoundsException(new StringBuilder().append("End index (").append(to).append(") is greater than array length (").append(arrayLength).append(")").toString());
        }
    }
    ...
    public static void ensureOffsetLength(final int arrayLength, final int offset, final int length) {
        ...
        if (offset + length > arrayLength) {
            throw new ArrayIndexOutOfBoundsException(new StringBuilder().append("Last index (").append(offset + length).append(") is greater than array length (").append(arrayLength).append(")").toString());
        }
    }
    ...
    public static void parallelQuickSort(final int from, final int to, final IntComparator comp, final Swapper swapper) {
        ...
        pool.invoke((ForkJoinTask)new ForkJoinGenericQuickSort(from, to, comp, swapper));
        pool.shutdown();
    }
    ...
    protected static class ForkJoinGenericQuickSort extends RecursiveAction {
        ...
        protected void compute() {
            ...
            final int t = d - c;
            if (s > 1 && t > 1) {
                invokeAll((ForkJoinTask)new ForkJoinGenericQuickSort(this.from, this.from + s, this.comp, this.swapper), (ForkJoinTask)new ForkJoinGenericQuickSort(this.to - t, this.to, this.comp, this.swapper));
            }
            else if (s > 1) {
                invokeAll(new ForkJoinTask[] { (ForkJoinTask)new ForkJoinGenericQuickSort(this.from, this.from + s, this.comp, this.swapper) });
            }
            else {
                invokeAll(new ForkJoinTask[] { (ForkJoinTask)new ForkJoinGenericQuickSort(this.to - t, this.to, this.comp, this.swapper) });
            }
        }
    }
}
