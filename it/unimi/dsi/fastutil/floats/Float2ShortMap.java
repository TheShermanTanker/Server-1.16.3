...
public interface Float2ShortMap extends Float2ShortFunction, Map<Float, Short> {
    ...
    default short computeIfAbsent(final float key, final DoubleToIntFunction mappingFunction) {
        Objects.requireNonNull(mappingFunction);
        final short v = this.get(key);
        if (v != this.defaultReturnValue() || this.containsKey(key)) {
            return v;
        }
        final short newValue = SafeMath.safeIntToShort(mappingFunction.applyAsInt((double)key));
        this.put(key, newValue);
        return newValue;
    }
    
    default short computeIfAbsentNullable(final float key, final DoubleFunction<? extends Short> mappingFunction) {
        Objects.requireNonNull(mappingFunction);
        final short v = this.get(key);
        final short drv = this.defaultReturnValue();
        if (v != drv || this.containsKey(key)) {
        ...
        final Short mappedValue = (Short)mappingFunction.apply((double)key);
        if (mappedValue == null) {
            return drv;
        }
    ...
    default short computeIfPresent(final float key, final BiFunction<? super Float, ? super Short, ? extends Short> remappingFunction) {
    ...
    default short compute(final float key, final BiFunction<? super Float, ? super Short, ? extends Short> remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        final short oldValue = this.get(key);
        final short drv = this.defaultReturnValue();
    ...
    default short merge(final float key, final short value, final BiFunction<? super Short, ? super Short, ? extends Short> remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        final short oldValue = this.get(key);
        final short drv = this.defaultReturnValue();
        short newValue;
