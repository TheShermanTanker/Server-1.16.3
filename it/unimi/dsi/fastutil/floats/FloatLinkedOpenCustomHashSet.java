...
public class FloatLinkedOpenCustomHashSet extends AbstractFloatSortedSet implements Serializable, Cloneable, Hash {
    ...
    public FloatLinkedOpenCustomHashSet(final int expected, final float f, final FloatHash.Strategy strategy) {
        this.first = -1;
        this.last = -1;
    ...
    public FloatLinkedOpenCustomHashSet(final int expected, final FloatHash.Strategy strategy) {
        this(expected, 0.75f, strategy);
    }
    
    public FloatLinkedOpenCustomHashSet(final FloatHash.Strategy strategy) {
        this(16, 0.75f, strategy);
    }
    
    public FloatLinkedOpenCustomHashSet(final Collection<? extends Float> c, final float f, final FloatHash.Strategy strategy) {
        this(c.size(), f, strategy);
        this.addAll(c);
    }
    
    public FloatLinkedOpenCustomHashSet(final Collection<? extends Float> c, final FloatHash.Strategy strategy) {
        this(c, 0.75f, strategy);
    }
    
    public FloatLinkedOpenCustomHashSet(final FloatCollection c, final float f, final FloatHash.Strategy strategy) {
        this(c.size(), f, strategy);
        this.addAll(c);
    }
    
    public FloatLinkedOpenCustomHashSet(final FloatCollection c, final FloatHash.Strategy strategy) {
        this(c, 0.75f, strategy);
    }
    
    public FloatLinkedOpenCustomHashSet(final FloatIterator i, final float f, final FloatHash.Strategy strategy) {
        this(16, f, strategy);
        while (i.hasNext()) {
    ...
    public FloatLinkedOpenCustomHashSet(final FloatIterator i, final FloatHash.Strategy strategy) {
        this(i, 0.75f, strategy);
    }
    
    public FloatLinkedOpenCustomHashSet(final Iterator<?> i, final float f, final FloatHash.Strategy strategy) {
        this(FloatIterators.asFloatIterator(i), f, strategy);
    }
    
    public FloatLinkedOpenCustomHashSet(final Iterator<?> i, final FloatHash.Strategy strategy) {
        this(FloatIterators.asFloatIterator(i), strategy);
    }
    
    public FloatLinkedOpenCustomHashSet(final float[] a, final int offset, final int length, final float f, final FloatHash.Strategy strategy) {
    ...
    public FloatLinkedOpenCustomHashSet(final float[] a, final int offset, final int length, final FloatHash.Strategy strategy) {
        this(a, offset, length, 0.75f, strategy);
    }
    
    public FloatLinkedOpenCustomHashSet(final float[] a, final float f, final FloatHash.Strategy strategy) {
        this(a, 0, a.length, f, strategy);
    }
    
    public FloatLinkedOpenCustomHashSet(final float[] a, final FloatHash.Strategy strategy) {
    ...
    public boolean addAndMoveToFirst(final float k) {
        ...
        if (this.strategy.equals(k, 0.0f)) {
            ...
            float[] key;
            for (key = this.key, pos = (HashCommon.mix(this.strategy.hashCode(k)) & this.mask); Float.floatToIntBits(key[pos]) != 0; pos = (pos + 1 & this.mask)) {
    ...
    protected void rehash(final int newN) {
        ...
        while (j-- != 0) {
            ...
            if (this.strategy.equals(key[i], 0.0f)) {
                ...
                for (pos = (HashCommon.mix(this.strategy.hashCode(key[i])) & mask); Float.floatToIntBits(newKey[pos]) != 0; pos = (pos + 1 & mask)) {}
            }
            newKey[pos] = key[i];
            if (prev != -1) {
    ...
    private class SetIterator implements FloatListIterator {
        ...
        SetIterator(final float from) {
            ...
            if (FloatLinkedOpenCustomHashSet.this.strategy.equals(from, 0.0f)) {
                ...
                for (int pos = HashCommon.mix(FloatLinkedOpenCustomHashSet.this.strategy.hashCode(from)) & FloatLinkedOpenCustomHashSet.this.mask; Float.floatToIntBits(key[pos]) != 0; pos = (pos + 1 & FloatLinkedOpenCustomHashSet.this.mask)) {
        ...
        public void remove() {
            ...
            final FloatLinkedOpenCustomHashSet this$0 = FloatLinkedOpenCustomHashSet.this;
            --this$0.size;
            if (this.prev == -1) {
                ...
                link[prev] ^= ((FloatLinkedOpenCustomHashSet.this.link[this.prev] ^ ((long)this.next & 0xFFFFFFFFL)) & 0xFFFFFFFFL);
            }
            if (this.next == -1) {
                FloatLinkedOpenCustomHashSet.this.last = this.prev;
            }
            else {
                final long[] link2 = FloatLinkedOpenCustomHashSet.this.link;
                final int next = this.next;
                link2[next] ^= ((FloatLinkedOpenCustomHashSet.this.link[this.next] ^ ((long)this.prev & 0xFFFFFFFFL) << 32) & 0xFFFFFFFF00000000L);
            }
            int pos = this.curr;
            this.curr = -1;
        ...
        Label_0280:
            while (true) {
                pos = ((last = pos) + 1 & FloatLinkedOpenCustomHashSet.this.mask);
                float curr;
                while (Float.floatToIntBits(curr = key[pos]) != 0) {
                    final int slot = HashCommon.mix(FloatLinkedOpenCustomHashSet.this.strategy.hashCode(curr)) & FloatLinkedOpenCustomHashSet.this.mask;
                    Label_0385: {
                        if (last <= pos) {
                            if (last >= slot) {
                                break Label_0385;
