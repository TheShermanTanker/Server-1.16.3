...
public class Float2ByteLinkedOpenHashMap extends AbstractFloat2ByteSortedMap implements Serializable, Cloneable, Hash {
    ...
    public byte computeIfAbsent(final float k, final DoubleToIntFunction mappingFunction) {
        Objects.requireNonNull(mappingFunction);
        final int pos = this.find(k);
        if (pos >= 0) {
            return this.value[pos];
        }
        final byte newValue = SafeMath.safeIntToByte(mappingFunction.applyAsInt((double)k));
        this.insert(-pos - 1, k, newValue);
        return newValue;
    }
    
    public byte computeIfAbsentNullable(final float k, final DoubleFunction<? extends Byte> mappingFunction) {
        Objects.requireNonNull(mappingFunction);
        final int pos = this.find(k);
        if (pos >= 0) {
            return this.value[pos];
        }
        final Byte newValue = (Byte)mappingFunction.apply((double)k);
        if (newValue == null) {
            return this.defRetValue;
        }
    ...
    public byte computeIfPresent(final float k, final BiFunction<? super Float, ? super Byte, ? extends Byte> remappingFunction) {
    ...
    public byte compute(final float k, final BiFunction<? super Float, ? super Byte, ? extends Byte> remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        final int pos = this.find(k);
    ...
    public byte merge(final float k, final byte v, final BiFunction<? super Byte, ? super Byte, ? extends Byte> remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        final int pos = this.find(k);
        if (pos < 0) {
            this.insert(-pos - 1, k, v);
            ...
            this.values = new AbstractByteCollection() {
                ...
                public void forEach(final IntConsumer consumer) {
                    if (Float2ByteLinkedOpenHashMap.this.containsNullKey) {
                        consumer.accept((int)Float2ByteLinkedOpenHashMap.this.value[Float2ByteLinkedOpenHashMap.this.n]);
                    }
                    int pos = Float2ByteLinkedOpenHashMap.this.n;
                    while (pos-- != 0) {
                        if (Float.floatToIntBits(Float2ByteLinkedOpenHashMap.this.key[pos]) != 0) {
                            consumer.accept((int)Float2ByteLinkedOpenHashMap.this.value[pos]);
                        }
                    }
                }
            };
    ...
    final class MapEntry implements Float2ByteMap.Entry, Map.Entry<Float, Byte> {
        ...
        public byte setValue(final byte v) {
            final byte oldValue = Float2ByteLinkedOpenHashMap.this.value[this.index];
        ...
        public boolean equals(final Object o) {
            ...
            return Float.floatToIntBits(Float2ByteLinkedOpenHashMap.this.key[this.index]) == Float.floatToIntBits((float)e.getKey()) && Float2ByteLinkedOpenHashMap.this.value[this.index] == (byte)e.getValue();
        }
        ...
        public String toString() {
            return new StringBuilder().append(Float2ByteLinkedOpenHashMap.this.key[this.index]).append("=>").append((int)Float2ByteLinkedOpenHashMap.this.value[this.index]).toString();
        }
    }
    ...
    private class MapIterator {
        ...
        private MapIterator(final float from) {
            ...
            if (Float.floatToIntBits(from) == 0) {
                ...
                for (int pos = HashCommon.mix(HashCommon.float2int(from)) & Float2ByteLinkedOpenHashMap.this.mask; Float.floatToIntBits(Float2ByteLinkedOpenHashMap.this.key[pos]) != 0; pos = (pos + 1 & Float2ByteLinkedOpenHashMap.this.mask)) {
        ...
        public void remove() {
            ...
            final Float2ByteLinkedOpenHashMap this$0 = Float2ByteLinkedOpenHashMap.this;
            --this$0.size;
            if (this.prev == -1) {
                Float2ByteLinkedOpenHashMap.this.first = this.next;
            }
            else {
                final long[] link = Float2ByteLinkedOpenHashMap.this.link;
                final int prev = this.prev;
                link[prev] ^= ((Float2ByteLinkedOpenHashMap.this.link[this.prev] ^ ((long)this.next & 0xFFFFFFFFL)) & 0xFFFFFFFFL);
            }
            if (this.next == -1) {
                Float2ByteLinkedOpenHashMap.this.last = this.prev;
            }
            else {
                final long[] link2 = Float2ByteLinkedOpenHashMap.this.link;
                final int next = this.next;
                link2[next] ^= ((Float2ByteLinkedOpenHashMap.this.link[this.next] ^ ((long)this.prev & 0xFFFFFFFFL) << 32) & 0xFFFFFFFF00000000L);
            }
            int pos = this.curr;
            this.curr = -1;
        ...
        Label_0264:
            while (true) {
                pos = ((last = pos) + 1 & Float2ByteLinkedOpenHashMap.this.mask);
                float curr;
                while (Float.floatToIntBits(curr = key[pos]) != 0) {
                    final int slot = HashCommon.mix(HashCommon.float2int(curr)) & Float2ByteLinkedOpenHashMap.this.mask;
                    Label_0360: {
                        if (last <= pos) {
                            if (last >= slot) {
                                break Label_0360;
        ...
        public void set(final Float2ByteMap.Entry ok) {
            throw new UnsupportedOperationException();
        }
        
        public void add(final Float2ByteMap.Entry ok) {
            throw new UnsupportedOperationException();
        }
    }
    ...
    private final class MapEntrySet extends AbstractObjectSortedSet<Float2ByteMap.Entry> implements Float2ByteSortedMap.FastSortedEntrySet {
        ...
        @Override
        public ObjectSortedSet<Float2ByteMap.Entry> subSet(final Float2ByteMap.Entry fromElement, final Float2ByteMap.Entry toElement) {
            throw new UnsupportedOperationException();
        }
        
        @Override
        public ObjectSortedSet<Float2ByteMap.Entry> headSet(final Float2ByteMap.Entry toElement) {
            throw new UnsupportedOperationException();
        }
        
        @Override
        public ObjectSortedSet<Float2ByteMap.Entry> tailSet(final Float2ByteMap.Entry fromElement) {
            throw new UnsupportedOperationException();
        }
        ...
        public void forEach(final Consumer<? super Float2ByteMap.Entry> consumer) {
            ...
            while (i-- != 0) {
                ...
                consumer.accept(new BasicEntry(Float2ByteLinkedOpenHashMap.this.key[curr], Float2ByteLinkedOpenHashMap.this.value[curr]));
            }
        }
    ...
    private final class KeySet extends AbstractFloatSortedSet {
        ...
        public void forEach(final DoubleConsumer consumer) {
            if (Float2ByteLinkedOpenHashMap.this.containsNullKey) {
                consumer.accept((double)Float2ByteLinkedOpenHashMap.this.key[Float2ByteLinkedOpenHashMap.this.n]);
            }
            int pos = Float2ByteLinkedOpenHashMap.this.n;
            while (pos-- != 0) {
                final float k = Float2ByteLinkedOpenHashMap.this.key[pos];
                if (Float.floatToIntBits(k) != 0) {
        ...
        public boolean remove(final float k) {
            final int oldSize = Float2ByteLinkedOpenHashMap.this.size;
