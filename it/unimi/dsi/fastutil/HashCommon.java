...
public class HashCommon {
    ...
    public static int maxFill(final int n, final float f) {
        return Math.min((int)Math.ceil((double)(n * f)), n - 1);
    }
    
    public static long maxFill(final long n, final float f) {
        return Math.min((long)Math.ceil((double)(n * f)), n - 1L);
    }
    
    public static int arraySize(final int expected, final float f) {
        final long s = Math.max(2L, nextPowerOfTwo((long)Math.ceil((double)(expected / f))));
        if (s > 1073741824L) {
            throw new IllegalArgumentException(new StringBuilder().append("Too large (").append(expected).append(" expected elements with load factor ").append(f).append(")").toString());
        }
        return (int)s;
    }
    
    public static long bigArraySize(final long expected, final float f) {
        return nextPowerOfTwo((long)Math.ceil((double)(expected / f)));
    }
}
