...
public class Long2ByteLinkedOpenHashMap extends AbstractLong2ByteSortedMap implements Serializable, Cloneable, Hash {
    ...
    public byte computeIfAbsent(final long k, final LongToIntFunction mappingFunction) {
    ...
    public byte computeIfAbsentNullable(final long k, final LongFunction<? extends Byte> mappingFunction) {
        Objects.requireNonNull(mappingFunction);
        final int pos = this.find(k);
        if (pos >= 0) {
            return this.value[pos];
    ...
    public byte computeIfPresent(final long k, final BiFunction<? super Long, ? super Byte, ? extends Byte> remappingFunction) {
    ...
    public byte compute(final long k, final BiFunction<? super Long, ? super Byte, ? extends Byte> remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        final int pos = this.find(k);
    ...
    public byte merge(final long k, final byte v, final BiFunction<? super Byte, ? super Byte, ? extends Byte> remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        final int pos = this.find(k);
        if (pos < 0) {
            this.insert(-pos - 1, k, v);
            ...
            this.values = new AbstractByteCollection() {
                ...
                public void forEach(final IntConsumer consumer) {
                    if (Long2ByteLinkedOpenHashMap.this.containsNullKey) {
                        consumer.accept((int)Long2ByteLinkedOpenHashMap.this.value[Long2ByteLinkedOpenHashMap.this.n]);
                    }
                    int pos = Long2ByteLinkedOpenHashMap.this.n;
                    while (pos-- != 0) {
                        if (Long2ByteLinkedOpenHashMap.this.key[pos] != 0L) {
                            consumer.accept((int)Long2ByteLinkedOpenHashMap.this.value[pos]);
                        }
                    }
                }
            };
    ...
    final class MapEntry implements Long2ByteMap.Entry, Map.Entry<Long, Byte> {
        ...
        public byte setValue(final byte v) {
            final byte oldValue = Long2ByteLinkedOpenHashMap.this.value[this.index];
        ...
        public String toString() {
            return new StringBuilder().append(Long2ByteLinkedOpenHashMap.this.key[this.index]).append("=>").append((int)Long2ByteLinkedOpenHashMap.this.value[this.index]).toString();
        }
    }
    ...
    private class MapIterator {
        ...
        private MapIterator(final long from) {
            ...
            if (from == 0L) {
                ...
                for (int pos = (int)HashCommon.mix(from) & Long2ByteLinkedOpenHashMap.this.mask; Long2ByteLinkedOpenHashMap.this.key[pos] != 0L; pos = (pos + 1 & Long2ByteLinkedOpenHashMap.this.mask)) {
        ...
        public void remove() {
            ...
            final Long2ByteLinkedOpenHashMap this$0 = Long2ByteLinkedOpenHashMap.this;
            --this$0.size;
            if (this.prev == -1) {
                Long2ByteLinkedOpenHashMap.this.first = this.next;
            }
            else {
                final long[] link = Long2ByteLinkedOpenHashMap.this.link;
                final int prev = this.prev;
                link[prev] ^= ((Long2ByteLinkedOpenHashMap.this.link[this.prev] ^ ((long)this.next & 0xFFFFFFFFL)) & 0xFFFFFFFFL);
            }
            if (this.next == -1) {
                Long2ByteLinkedOpenHashMap.this.last = this.prev;
            }
            else {
                final long[] link2 = Long2ByteLinkedOpenHashMap.this.link;
                final int next = this.next;
                link2[next] ^= ((Long2ByteLinkedOpenHashMap.this.link[this.next] ^ ((long)this.prev & 0xFFFFFFFFL) << 32) & 0xFFFFFFFF00000000L);
            }
            int pos = this.curr;
            this.curr = -1;
        ...
        Label_0264:
            while (true) {
                pos = ((last = pos) + 1 & Long2ByteLinkedOpenHashMap.this.mask);
                long curr;
                while ((curr = key[pos]) != 0L) {
                    final int slot = (int)HashCommon.mix(curr) & Long2ByteLinkedOpenHashMap.this.mask;
                    Label_0357: {
                        if (last <= pos) {
                            if (last >= slot) {
                                break Label_0357;
    ...
    private final class MapEntrySet extends AbstractObjectSortedSet<Long2ByteMap.Entry> implements Long2ByteSortedMap.FastSortedEntrySet {
        ...
        @Override
        public ObjectSortedSet<Long2ByteMap.Entry> subSet(final Long2ByteMap.Entry fromElement, final Long2ByteMap.Entry toElement) {
            throw new UnsupportedOperationException();
        }
        
        @Override
        public ObjectSortedSet<Long2ByteMap.Entry> headSet(final Long2ByteMap.Entry toElement) {
            throw new UnsupportedOperationException();
        }
        
        @Override
        public ObjectSortedSet<Long2ByteMap.Entry> tailSet(final Long2ByteMap.Entry fromElement) {
            throw new UnsupportedOperationException();
        }
        ...
        public void forEach(final Consumer<? super Long2ByteMap.Entry> consumer) {
            ...
            while (i-- != 0) {
                ...
                consumer.accept(new BasicEntry(Long2ByteLinkedOpenHashMap.this.key[curr], Long2ByteLinkedOpenHashMap.this.value[curr]));
            }
        }
    ...
    private final class KeySet extends AbstractLongSortedSet {
        ...
        public void forEach(final LongConsumer consumer) {
            ...
            int pos = Long2ByteLinkedOpenHashMap.this.n;
            while (pos-- != 0) {
                final long k = Long2ByteLinkedOpenHashMap.this.key[pos];
                if (k != 0L) {
                    consumer.accept(k);
                }
            }
        ...
        public boolean remove(final long k) {
            final int oldSize = Long2ByteLinkedOpenHashMap.this.size;
