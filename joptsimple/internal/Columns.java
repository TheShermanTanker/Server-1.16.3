...
class Columns {
    ...
    List<Row> fit(final Row row) {
        ...
        final List<Row> rows = (List<Row>)new ArrayList();
    ...
    private List<String> piecesOf(final String raw, final int width) {
        final List<String> pieces = (List<String>)new ArrayList();
        for (final String each : raw.trim().split(Strings.LINE_SEPARATOR)) {
            pieces.addAll((Collection)this.piecesOfEmbeddedLine(each, width));
        }
        return pieces;
    }
    ...
    private List<String> piecesOfEmbeddedLine(final String line, final int width) {
        final List<String> pieces = (List<String>)new ArrayList();
        final BreakIterator words = BreakIterator.getLineInstance();
        words.setText(line);
