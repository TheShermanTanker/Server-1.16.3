...
public class FileSystemUtils {
    ...
    long parseBytes(final String freeSpace, final String path) throws IOException {
        try {
            ...
            throw new IOException("Command line '" + FileSystemUtils.DF + "' did not return numeric data as expected " + "for path '" + path + "'- check path is valid", (Throwable)ex);
        }
    }
    
    List<String> performCommand(final String[] cmdAttribs, final int max, final long timeout) throws IOException {
        final List<String> lines = (List<String>)new ArrayList(20);
        Process proc = null;
        InputStream in = null;
        ...
        try {
            ...
            inr = new BufferedReader((Reader)new InputStreamReader(in, Charset.defaultCharset()));
            ...
            if (proc.exitValue() != 0) {
                throw new IOException(new StringBuilder().append("Command line returned OS error code '").append(proc.exitValue()).append("' for command ").append(Arrays.asList((Object[])cmdAttribs)).toString());
            }
            if (lines.isEmpty()) {
                throw new IOException(new StringBuilder().append("Command line did not return any info for command ").append(Arrays.asList((Object[])cmdAttribs)).toString());
            }
            return lines;
        }
        catch (InterruptedException ex) {
            throw new IOException(new StringBuilder().append("Command line threw an InterruptedException for command ").append(Arrays.asList((Object[])cmdAttribs)).append(" timeout=").append(timeout).toString(), (Throwable)ex);
        }
        finally {
