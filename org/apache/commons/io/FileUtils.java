...
public class FileUtils {
    ...
    public static void touch(final File file) throws IOException {
        if (!file.exists()) {
            final OutputStream out = (OutputStream)openOutputStream(file);
    ...
    public static Collection<File> listFiles(final File directory, final IOFileFilter fileFilter, final IOFileFilter dirFilter) {
        ...
        final Collection<File> files = (Collection<File>)new LinkedList();
    ...
    private static void validateListFilesParameters(final File directory, final IOFileFilter fileFilter) {
        if (!directory.isDirectory()) {
            throw new IllegalArgumentException(new StringBuilder().append("Parameter 'directory' is not a directory: ").append(directory).toString());
        }
        if (fileFilter == null) {
    ...
    public static Iterator<File> iterateFiles(final File directory, final IOFileFilter fileFilter, final IOFileFilter dirFilter) {
        return (Iterator<File>)listFiles(directory, fileFilter, dirFilter).iterator();
    }
    ...
    public static Iterator<File> iterateFilesAndDirs(final File directory, final IOFileFilter fileFilter, final IOFileFilter dirFilter) {
        return (Iterator<File>)listFilesAndDirs(directory, fileFilter, dirFilter).iterator();
    }
    ...
    public static Iterator<File> iterateFiles(final File directory, final String[] extensions, final boolean recursive) {
        return (Iterator<File>)listFiles(directory, extensions, recursive).iterator();
    }
    ...
    public static boolean contentEquals(final File file1, final File file2) throws IOException {
        ...
        try {
            input1 = (InputStream)new FileInputStream(file1);
            input2 = (InputStream)new FileInputStream(file2);
    ...
    public static boolean contentEqualsIgnoreEOL(final File file1, final File file2, final String charsetName) throws IOException {
        ...
        try {
            if (charsetName == null) {
                input1 = (Reader)new InputStreamReader((InputStream)new FileInputStream(file1), Charset.defaultCharset());
                input2 = (Reader)new InputStreamReader((InputStream)new FileInputStream(file2), Charset.defaultCharset());
            }
            else {
                input1 = (Reader)new InputStreamReader((InputStream)new FileInputStream(file1), charsetName);
                input2 = (Reader)new InputStreamReader((InputStream)new FileInputStream(file2), charsetName);
            }
    ...
    private static void doCopyFile(final File srcFile, final File destFile, final boolean preserveFileDate) throws IOException {
        ...
        try {
            ...
            while (pos < size) {
                ...
                final long bytesCopied = output.transferFrom((ReadableByteChannel)input, pos, count);
                if (bytesCopied == 0L) {
                    break;
                }
                pos += bytesCopied;
            ...
            IOUtils.closeQuietly((Closeable)output, (Closeable)fos, (Closeable)input, (Closeable)fis);
        }
        final long srcLen = srcFile.length();
        final long dstLen = destFile.length();
        if (srcLen != dstLen) {
            throw new IOException(new StringBuilder().append("Failed to copy full contents from '").append(srcFile).append("' to '").append(destFile).append("' Expected length: ").append(srcLen).append(" Actual: ").append(dstLen).toString());
        }
        if (preserveFileDate) {
    ...
    public static void copyDirectory(final File srcDir, final File destDir, final FileFilter filter, final boolean preserveFileDate) throws IOException {
        ...
        if (destDir.getCanonicalPath().startsWith(srcDir.getCanonicalPath())) {
            ...
            if (srcFiles != null && srcFiles.length > 0) {
                exclusionList = (List<String>)new ArrayList(srcFiles.length);
    ...
    public static void deleteDirectory(final File directory) throws IOException {
        ...
        if (!directory.delete()) {
            final String message = new StringBuilder().append("Unable to delete directory ").append(directory).append(".").toString();
    ...
    private static File[] verifiedListFiles(final File directory) throws IOException {
        if (!directory.exists()) {
            final String message = new StringBuilder().append(directory).append(" does not exist").toString();
            throw new IllegalArgumentException(message);
        }
        if (!directory.isDirectory()) {
            final String message = new StringBuilder().append(directory).append(" is not a directory").toString();
            throw new IllegalArgumentException(message);
        }
    ...
    public static String readFileToString(final File file, final Charset encoding) throws IOException {
        InputStream in = null;
        try {
            in = (InputStream)openInputStream(file);
    ...
    public static void writeStringToFile(final File file, final String data, final Charset encoding, final boolean append) throws IOException {
        ...
        try {
            out = (OutputStream)openOutputStream(file, append);
    ...
    public static void writeLines(final File file, final String encoding, final Collection<?> lines, final String lineEnding, final boolean append) throws IOException {
        ...
        try {
            ...
            final BufferedOutputStream buffer = new BufferedOutputStream((OutputStream)out);
    ...
    public static void forceDelete(final File file) throws IOException {
        if (file.isDirectory()) {
            ...
            if (!file.delete()) {
                ...
                final String message = new StringBuilder().append("Unable to delete file: ").append(file).toString();
                throw new IOException(message);
            }
        }
    }
    ...
    public static void forceMkdir(final File directory) throws IOException {
        if (directory.exists()) {
            if (!directory.isDirectory()) {
                final String message = new StringBuilder().append("File ").append(directory).append(" exists and is ").append("not a directory. Unable to create directory.").toString();
                throw new IOException(message);
            }
        }
        else if (!directory.mkdirs() && !directory.isDirectory()) {
            final String message = new StringBuilder().append("Unable to create directory ").append(directory).toString();
            throw new IOException(message);
        }
    }
    ...
    public static long sizeOf(final File file) {
        if (!file.exists()) {
            final String message = new StringBuilder().append(file).append(" does not exist").toString();
            throw new IllegalArgumentException(message);
        }
        if (file.isDirectory()) {
    ...
    public static Checksum checksum(final File file, final Checksum checksum) throws IOException {
        ...
        InputStream in = null;
        try {
            in = (InputStream)new CheckedInputStream((InputStream)new FileInputStream(file), checksum);
    ...
    private static boolean isBrokenSymlink(final File file) throws IOException {
        ...
        final File[] fileInDir = parentDir.listFiles((FileFilter)new FileFilter() {
