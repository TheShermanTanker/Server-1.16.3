...
public class XmlStreamWriter extends Writer {
    ...
    public void close() throws IOException {
        if (this.writer == null) {
            ...
            (this.writer = (Writer)new OutputStreamWriter(this.out, this.encoding)).write(this.xmlPrologWriter.toString());
        }
        this.writer.close();
    }
    ...
    private void detectEncoding(final char[] cbuf, final int off, final int len) throws IOException {
        ...
        if (xmlProlog.length() >= 5) {
            if (xmlProlog.substring(0, 5).equals("<?xml")) {
                ...
                if (xmlPrologEnd > 0) {
                    final Matcher m = XmlStreamWriter.ENCODING_PATTERN.matcher((CharSequence)xmlProlog.substring(0, xmlPrologEnd));
                    if (m.find()) {
            ...
            if (this.encoding != null) {
                ...
                (this.writer = (Writer)new OutputStreamWriter(this.out, this.encoding)).write(xmlProlog.toString());
                if (len > size) {
