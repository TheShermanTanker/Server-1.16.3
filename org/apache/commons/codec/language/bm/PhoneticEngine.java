...
public class PhoneticEngine {
    ...
    private static String join(final Iterable<String> strings, final String sep) {
        ...
        final Iterator<String> si = (Iterator<String>)strings.iterator();
        if (si.hasNext()) {
    ...
    private PhonemeBuilder applyFinalRules(final PhonemeBuilder phonemeBuilder, final Map<String, List<Rule>> finalRules) {
        ...
        final Map<Rule.Phoneme, Rule.Phoneme> phonemes = (Map<Rule.Phoneme, Rule.Phoneme>)new TreeMap((Comparator)Rule.Phoneme.COMPARATOR);
        for (final Rule.Phoneme phoneme : phonemeBuilder.getPhonemes()) {
            ...
            for (int i = 0; i < phonemeText.length(); i = rulesApplication.getI()) {
                rulesApplication = new RulesApplication(finalRules, (CharSequence)phonemeText, subBuilder, i, this.maxPhonemes).invoke();
    ...
    public String encode(String input, final Languages.LanguageSet languageSet) {
        ...
        final List<String> words = (List<String>)Arrays.asList((Object[])input.split("\\s+"));
        final List<String> words2 = (List<String>)new ArrayList();
        switch (this.nameType) {
            case SEPHARDIC: {
                ...
                words2.removeAll((Collection)PhoneticEngine.NAME_PREFIXES.get(this.nameType));
                break;
            }
            case ASHKENAZI: {
                words2.addAll((Collection)words);
                words2.removeAll((Collection)PhoneticEngine.NAME_PREFIXES.get(this.nameType));
                break;
            }
            case GENERIC: {
            ...
            default: {
                throw new IllegalStateException(new StringBuilder().append("Unreachable case: ").append(this.nameType).toString());
            }
        }
        if (this.concat) {
            input = join((Iterable<String>)words2, " ");
        }
        else {
            if (words2.size() != 1) {
        ...
        for (int i = 0; i < input.length(); i = rulesApplication.getI(), phonemeBuilder = rulesApplication.getPhonemeBuilder()) {
            rulesApplication = new RulesApplication(rules, (CharSequence)input, phonemeBuilder, i, this.maxPhonemes).invoke();
        }
    ...
    static {
        (NAME_PREFIXES = (Map)new EnumMap((Class)NameType.class)).put(NameType.ASHKENAZI, Collections.unmodifiableSet((Set)new HashSet((Collection)Arrays.asList((Object[])new String[] { "bar", "ben", "da", "de", "van", "von" }))));
        PhoneticEngine.NAME_PREFIXES.put(NameType.SEPHARDIC, Collections.unmodifiableSet((Set)new HashSet((Collection)Arrays.asList((Object[])new String[] { "al", "el", "da", "dal", "de", "del", "dela", "de la", "della", "des", "di", "do", "dos", "du", "van", "von" }))));
        PhoneticEngine.NAME_PREFIXES.put(NameType.GENERIC, Collections.unmodifiableSet((Set)new HashSet((Collection)Arrays.asList((Object[])new String[] { "da", "dal", "de", "del", "dela", "de la", "della", "des", "di", "do", "dos", "du", "van", "von" }))));
    }
    ...
    static final class PhonemeBuilder {
        ...
        private PhonemeBuilder(final Rule.Phoneme phoneme) {
            (this.phonemes = (Set<Rule.Phoneme>)new LinkedHashSet()).add(phoneme);
        }
        ...
        public void apply(final Rule.PhonemeExpr phonemeExpr, final int maxPhonemes) {
            final Set<Rule.Phoneme> newPhonemes = (Set<Rule.Phoneme>)new LinkedHashSet(maxPhonemes);
        Label_0153:
            ...
            this.phonemes.clear();
            this.phonemes.addAll((Collection)newPhonemes);
        }
