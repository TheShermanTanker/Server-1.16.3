...
public class Lang {
    ...
    public static Lang loadFromResource(final String languageRulesResourceName, final Languages languages) {
        final List<LangRule> rules = (List<LangRule>)new ArrayList();
        ...
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                if (inExtendedComment) {
                    ...
                    rules.add(new LangRule(pattern, (Set)new HashSet((Collection)Arrays.asList((Object[])langs)), accept));
                }
            }
        }
        finally {
    ...
    private Lang(final List<LangRule> rules, final Languages languages) {
        this.rules = (List<LangRule>)Collections.unmodifiableList((List)rules);
    ...
    public Languages.LanguageSet guessLanguages(final String input) {
        ...
        final Set<String> langs = (Set<String>)new HashSet((Collection)this.languages.getLanguages());
        for (final LangRule rule : this.rules) {
            if (rule.matches(text)) {
                if (rule.acceptOnMatch) {
                    langs.retainAll((Collection)rule.languages);
                }
                else {
                    langs.removeAll((Collection)rule.languages);
                }
            }
        }
    ...
    static {
        Langs = (Map)new EnumMap((Class)NameType.class);
        for (final NameType s : NameType.values()) {
            Lang.Langs.put(s, loadFromResource(String.format("org/apache/commons/codec/language/bm/%s_lang.txt", new Object[] { s.getName() }), Languages.getInstance(s)));
        }
    }
    ...
    private static final class LangRule {
        ...
        public boolean matches(final String txt) {
            return this.pattern.matcher((CharSequence)txt).find();
        }
    }
}
