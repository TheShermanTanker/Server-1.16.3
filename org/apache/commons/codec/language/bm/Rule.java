...
public class Rule {
    ...
    private static Scanner createScanner(final String lang) {
        final String resName = String.format("org/apache/commons/codec/language/bm/%s.txt", new Object[] { lang });
        final InputStream rulesIS = Languages.class.getClassLoader().getResourceAsStream(resName);
        if (rulesIS == null) {
            throw new IllegalArgumentException("Unable to load resource: " + resName);
        }
    ...
    public static List<Rule> getInstance(final NameType nameType, final RuleType rt, final Languages.LanguageSet langs) {
        ...
        final List<Rule> allRules = (List<Rule>)new ArrayList();
        for (final List<Rule> rules : ruleMap.values()) {
            allRules.addAll((Collection)rules);
        }
        return allRules;
    }
    
    public static List<Rule> getInstance(final NameType nameType, final RuleType rt, final String lang) {
        return getInstance(nameType, rt, Languages.LanguageSet.from((Set<String>)new HashSet((Collection)Arrays.asList((Object[])new String[] { lang }))));
    }
    ...
    private static Phoneme parsePhoneme(final String ph) {
        ...
        final Set<String> langs = (Set<String>)new HashSet((Collection)Arrays.asList((Object[])in.split("[+]")));
    ...
    private static PhonemeExpr parsePhonemeExpr(final String ph) {
        ...
        final List<Phoneme> phs = (List<Phoneme>)new ArrayList();
    ...
    private static Map<String, List<Rule>> parseRules(final Scanner scanner, final String location) {
        final Map<String, List<Rule>> lines = (Map<String, List<Rule>>)new HashMap();
        int currentLine = 0;
        ...
        while (scanner.hasNextLine()) {
            ...
            if (inMultilineComment) {
                ...
                if (line.startsWith("#include")) {
                    ...
                    lines.putAll((Map)parseRules(createScanner(incl), location + "->" + incl));
                }
                else {
                    final String[] parts = line.split("\\s+");
                    if (parts.length != 4) {
                        throw new IllegalArgumentException(new StringBuilder().append("Malformed rule statement split into ").append(parts.length).append(" parts: ").append(rawLine).append(" in ").append(location).toString());
                    }
                    try {
                        ...
                        if (rules == null) {
                            rules = (List<Rule>)new ArrayList();
                            lines.put(patternKey, rules);
                        }
                        rules.add(r);
                    }
                    catch (IllegalArgumentException e) {
                        throw new IllegalStateException(new StringBuilder().append("Problem parsing line '").append(currentLine).append("' in ").append(location).toString(), (Throwable)e);
                    }
                }
            }
        }
    ...
    private static RPattern pattern(final String regex) {
        ...
        final String content = regex.substring((int)(startsWith ? 1 : 0), endsWith ? (regex.length() - 1) : regex.length());
        ...
        if (!boxes) {
            if (startsWith && endsWith) {
                ...
                if (startsWith) {
                    return new RPattern() {
                        public boolean isMatch(final CharSequence input) {
                            return startsWith(input, (CharSequence)content);
                        }
                    };
                }
                if (endsWith) {
                    return new RPattern() {
                        public boolean isMatch(final CharSequence input) {
                            return endsWith(input, (CharSequence)content);
                        }
                    };
                }
            }
            ...
            if (startsWithBox && endsWithBox) {
                ...
                if (!boxContent.contains("[")) {
                    ...
                    if (startsWith && endsWith) {
                        return new RPattern() {
                            public boolean isMatch(final CharSequence input) {
                                return input.length() == 1 && contains((CharSequence)bContent, input.charAt(0)) == shouldMatch;
                            }
                        };
                    }
                    if (startsWith) {
                        return new RPattern() {
                            public boolean isMatch(final CharSequence input) {
                                return input.length() > 0 && contains((CharSequence)bContent, input.charAt(0)) == shouldMatch;
                            }
                        };
                    }
                    if (endsWith) {
                        return new RPattern() {
                            public boolean isMatch(final CharSequence input) {
                                return input.length() > 0 && contains((CharSequence)bContent, input.charAt(input.length() - 1)) == shouldMatch;
                            }
                        };
                    }
                }
    ...
    static {
        ...
        RULES = (Map)new EnumMap((Class)NameType.class);
        for (final NameType s : NameType.values()) {
            final Map<RuleType, Map<String, Map<String, List<Rule>>>> rts = (Map<RuleType, Map<String, Map<String, List<Rule>>>>)new EnumMap((Class)RuleType.class);
            for (final RuleType rt : RuleType.values()) {
                final Map<String, Map<String, List<Rule>>> rs = (Map<String, Map<String, List<Rule>>>)new HashMap();
                ...
                for (final String l : ls.getLanguages()) {
                    try {
                        ...
                        throw new IllegalStateException("Problem processing " + createResourceName(s, rt, l), (Throwable)e);
                    }
                }
                if (!rt.equals(RuleType.RULES)) {
                    rs.put("common", parseRules(createScanner(s, rt, "common"), createResourceName(s, rt, "common")));
                }
                rts.put(rt, Collections.unmodifiableMap((Map)rs));
            }
            Rule.RULES.put(s, Collections.unmodifiableMap((Map)rts));
        }
    }
    ...
    public static final class Phoneme implements PhonemeExpr {
        ...
        public Phoneme(final Phoneme phonemeLeft, final Phoneme phonemeRight, final Languages.LanguageSet languages) {
            this((CharSequence)phonemeLeft.phonemeText, languages);
            this.phonemeText.append((CharSequence)phonemeRight.phonemeText);
        }
        ...
        public Iterable<Phoneme> getPhonemes() {
            return (Iterable<Phoneme>)Collections.singleton(this);
        }
        
        public CharSequence getPhonemeText() {
            return (CharSequence)this.phonemeText;
        }
        ...
        static {
            COMPARATOR = (Comparator)new Comparator<Phoneme>() {
    ...
    public interface RPattern {
        boolean isMatch(final CharSequence charSequence);
    }
