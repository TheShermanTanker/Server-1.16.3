...
public class MethodUtils {
    ...
    static Object[] getVarArgs(final Object[] args, final Class<?>[] methodParameterTypes) {
        ...
        Object varArgsArray = Array.newInstance((Class)ClassUtils.primitiveToWrapper(varArgComponentType), varArgLength);
        ...
        while (cls != null) {
            ...
            for (int i = 0; i < interfaces.length; ++i) {
                if (Modifier.isPublic(interfaces[i].getModifiers())) {
                    try {
                        return interfaces[i].getDeclaredMethod(methodName, (Class[])parameterTypes);
                    }
    ...
    public static Method getMatchingMethod(final Class<?> cls, final String methodName, final Class<?>... parameterTypes) {
        Validate.<Class<?>>notNull(cls, "Null class not allowed.", new Object[0]);
        Validate.<String>notEmpty(methodName, "Null or blank methodName not allowed.", new Object[0]);
        Method[] methodArray = cls.getDeclaredMethods();
        final List<Class<?>> superclassList = ClassUtils.getAllSuperclasses(cls);
        for (final Class<?> klass : superclassList) {
            methodArray = ArrayUtils.<Method>addAll(methodArray, klass.getDeclaredMethods());
        }
    ...
    public static Set<Method> getOverrideHierarchy(final Method method, final ClassUtils.Interfaces interfacesBehavior) {
        Validate.<Method>notNull(method);
        final Set<Method> result = (Set<Method>)new LinkedHashSet();
        result.add(method);
        final Class<?>[] parameterTypes = method.getParameterTypes();
        final Class<?> declaringClass = method.getDeclaringClass();
        final Iterator<Class<?>> hierarchy = (Iterator<Class<?>>)ClassUtils.hierarchy(declaringClass, interfacesBehavior).iterator();
        hierarchy.next();
    Label_0053:
        while (hierarchy.hasNext()) {
            ...
            if (Arrays.equals((Object[])m.getParameterTypes(), (Object[])parameterTypes)) {
                ...
                final Map<TypeVariable<?>, Type> typeArguments = TypeUtils.getTypeArguments((Type)declaringClass, m.getDeclaringClass());
    ...
    public static Method[] getMethodsWithAnnotation(final Class<?> cls, final Class<? extends Annotation> annotationCls) {
        ...
        return (Method[])annotatedMethodsList.toArray((Object[])new Method[annotatedMethodsList.size()]);
    }
    ...
    public static List<Method> getMethodsListWithAnnotation(final Class<?> cls, final Class<? extends Annotation> annotationCls) {
        ...
        final List<Method> annotatedMethods = (List<Method>)new ArrayList();
        for (final Method method : allMethods) {
            if (method.getAnnotation((Class)annotationCls) != null) {
