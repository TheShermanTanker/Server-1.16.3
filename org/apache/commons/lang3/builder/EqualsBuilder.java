...
public class EqualsBuilder implements Builder<Boolean> {
    ...
    static Set<Pair<IDKey, IDKey>> getRegistry() {
        return (Set<Pair<IDKey, IDKey>>)EqualsBuilder.REGISTRY.get();
    }
    
    static Pair<IDKey, IDKey> getRegisterPair(final Object lhs, final Object rhs) {
        final IDKey left = new IDKey(lhs);
        final IDKey right = new IDKey(rhs);
        return Pair.<IDKey, IDKey>of(left, right);
    }
    
    static boolean isRegistered(final Object lhs, final Object rhs) {
        final Set<Pair<IDKey, IDKey>> registry = getRegistry();
        final Pair<IDKey, IDKey> pair = getRegisterPair(lhs, rhs);
        final Pair<IDKey, IDKey> swappedPair = Pair.<IDKey, IDKey>of(pair.getLeft(), pair.getRight());
    ...
    private static void register(final Object lhs, final Object rhs) {
        ...
        if (registry == null) {
            registry = (Set<Pair<IDKey, IDKey>>)new HashSet();
    ...
    private static void reflectionAppend(final Object lhs, final Object rhs, final Class<?> clazz, final EqualsBuilder builder, final boolean useTransients, final String[] excludeFields) {
        ...
        try {
            ...
            for (int i = 0; i < fields.length && builder.isEquals; ++i) {
                ...
                if (!ArrayUtils.contains(excludeFields, f.getName()) && !f.getName().contains("$") && (useTransients || !Modifier.isTransient(f.getModifiers())) && !Modifier.isStatic(f.getModifiers()) && !f.isAnnotationPresent((Class)EqualsExclude.class)) {
                    try {
