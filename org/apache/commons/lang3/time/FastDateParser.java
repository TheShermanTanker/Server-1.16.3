...
public class FastDateParser implements DateParser, Serializable {
    ...
    private void init(final Calendar definingCalendar) {
        this.patterns = (List<StrategyAndWidth>)new ArrayList();
        final StrategyParser fm = new StrategyParser(this.pattern, definingCalendar);
        while (true) {
    ...
    public Date parse(final String source) throws ParseException {
        ...
        if (this.locale.equals(FastDateParser.JAPANESE_IMPERIAL)) {
            throw new ParseException(new StringBuilder().append("(The ").append(this.locale).append(" locale does not support dates before 1868 AD)\nUnparseable date: \"").append(source).toString(), pp.getErrorIndex());
        }
    ...
    public boolean parse(final String source, final ParsePosition pos, final Calendar calendar) {
        final ListIterator<StrategyAndWidth> lt = (ListIterator<StrategyAndWidth>)this.patterns.listIterator();
        while (lt.hasNext()) {
            final StrategyAndWidth pattern = (StrategyAndWidth)lt.next();
    ...
    private static Map<String, Integer> appendDisplayNames(final Calendar cal, final Locale locale, final int field, final StringBuilder regex) {
        final Map<String, Integer> values = (Map<String, Integer>)new HashMap();
        final Map<String, Integer> displayNames = (Map<String, Integer>)cal.getDisplayNames(field, 0, locale);
        final TreeSet<String> sorted = (TreeSet<String>)new TreeSet((Comparator)FastDateParser.LONGER_FIRST_LOWERCASE);
    ...
    private Strategy getStrategy(final char f, final int width, final Calendar definingCalendar) {
        switch (f) {
            default: {
                throw new IllegalArgumentException(new StringBuilder().append("Format '").append(f).append("' not supported").toString());
            }
            case 'D': {
    ...
    private static ConcurrentMap<Locale, Strategy> getCache(final int field) {
        synchronized (FastDateParser.caches) {
            if (FastDateParser.caches[field] == null) {
                FastDateParser.caches[field] = (ConcurrentMap<Locale, Strategy>)new ConcurrentHashMap(3);
            }
    ...
    private Strategy getLocaleSpecificStrategy(final int field, final Calendar definingCalendar) {
        ...
        if (strategy == null) {
            ...
            final Strategy inCache = (Strategy)cache.putIfAbsent(this.locale, strategy);
            if (inCache != null) {
                return inCache;
            }
        }
    ...
    static {
        ...
        LONGER_FIRST_LOWERCASE = (Comparator)new Comparator<String>() {
    ...
    private static class StrategyAndWidth {
        ...
        int getMaxWidth(final ListIterator<StrategyAndWidth> lt) {
            ...
            final Strategy nextStrategy = ((StrategyAndWidth)lt.next()).strategy;
            lt.previous();
    ...
    private class StrategyParser {
        ...
        StrategyAndWidth getNextStrategy() {
            ...
            final char c = this.pattern.charAt(this.currentIdx);
            if (isFormatLetter(c)) {
    ...
    private abstract static class Strategy {
        ...
        abstract boolean parse(final FastDateParser fastDateParser, final Calendar calendar, final String string, final ParsePosition parsePosition, final int integer);
    }
    ...
    private abstract static class PatternStrategy extends Strategy {
        ...
        boolean parse(final FastDateParser parser, final Calendar calendar, final String source, final ParsePosition pos, final int maxWidth) {
            final Matcher matcher = this.pattern.matcher((CharSequence)source.substring(pos.getIndex()));
        ...
        abstract void setCalendar(final FastDateParser fastDateParser, final Calendar calendar, final String string);
    }
    ...
    private static class CaseInsensitiveTextStrategy extends PatternStrategy {
        ...
        void setCalendar(final FastDateParser parser, final Calendar cal, final String value) {
            final Integer iVal = (Integer)this.lKeyValues.get(value.toLowerCase(this.locale));
            cal.set(this.field, (int)iVal);
        }
    }
    ...
    static class TimeZoneStrategy extends PatternStrategy {
        ...
        TimeZoneStrategy(final Locale locale) {
            this.tzNames = (Map<String, TzInfo>)new HashMap();
            this.locale = locale;
            final StringBuilder sb = new StringBuilder();
            sb.append("((?iu)[+-]\\d{4}|GMT[+-]\\d{1,2}:\\d{2}");
            final Set<String> sorted = (Set<String>)new TreeSet(FastDateParser.LONGER_FIRST_LOWERCASE);
        ...
        void setCalendar(final FastDateParser parser, final Calendar cal, final String value) {
            if (value.charAt(0) == '+' || value.charAt(0) == '-') {
                ...
                final TzInfo tzInfo = (TzInfo)this.tzNames.get(value.toLowerCase(this.locale));
