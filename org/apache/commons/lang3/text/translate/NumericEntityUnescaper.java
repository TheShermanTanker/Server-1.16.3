...
public class NumericEntityUnescaper extends CharSequenceTranslator {
    ...
    public NumericEntityUnescaper(final OPTION... options) {
        if (options.length > 0) {
            this.options = (EnumSet<OPTION>)EnumSet.copyOf((Collection)Arrays.asList((Object[])options));
        }
        else {
            this.options = (EnumSet<OPTION>)EnumSet.copyOf((Collection)Arrays.asList((Object[])new OPTION[] { OPTION.semiColonRequired }));
        }
    }
    ...
    public int translate(final CharSequence input, final int index, final Writer out) throws IOException {
        ...
        if (input.charAt(index) == '&' && index < seqEnd - 2 && input.charAt(index + 1) == '#') {
            ...
            if (entityValue > 65535) {
                ...
                out.write((int)chrs[0]);
                out.write((int)chrs[1]);
            }
            else {
