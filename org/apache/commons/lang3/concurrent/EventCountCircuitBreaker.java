...
public class EventCountCircuitBreaker extends AbstractCircuitBreaker<Integer> {
    ...
    public EventCountCircuitBreaker(final int openingThreshold, final long openingInterval, final TimeUnit openingUnit, final int closingThreshold, final long closingInterval, final TimeUnit closingUnit) {
        this.checkIntervalData = (AtomicReference<CheckIntervalData>)new AtomicReference(new CheckIntervalData(0, 0L));
    ...
    private static StateStrategy stateStrategy(final State state) {
        final StateStrategy strategy = (StateStrategy)EventCountCircuitBreaker.STRATEGY_MAP.get(state);
        return strategy;
    }
    
    private static Map<State, StateStrategy> createStrategyMap() {
        final Map<State, StateStrategy> map = (Map<State, StateStrategy>)new EnumMap((Class)State.class);
    ...
    private abstract static class StateStrategy {
        ...
        public abstract boolean isStateTransition(final EventCountCircuitBreaker eventCountCircuitBreaker, final CheckIntervalData checkIntervalData2, final CheckIntervalData checkIntervalData3);
        
        protected abstract long fetchCheckInterval(final EventCountCircuitBreaker eventCountCircuitBreaker);
    }
