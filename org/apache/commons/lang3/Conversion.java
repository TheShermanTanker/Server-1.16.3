...
public class Conversion {
    ...
    public static int hexDigitToInt(final char hexDigit) {
        ...
        if (digit < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("Cannot interpret '").append(hexDigit).append("' as a hexadecimal digit").toString());
        }
        return digit;
    }
    ...
    public static char binaryToHexDigitMsb0_4bits(final boolean[] src, final int srcPos) {
        if (src.length > 8) {
            throw new IllegalArgumentException(new StringBuilder().append("src.length>8: src.length=").append(src.length).toString());
        }
        if (src.length - srcPos < 4) {
            throw new IllegalArgumentException(new StringBuilder().append("src.length-srcPos<4: src.length=").append(src.length).append(", srcPos=").append(srcPos).toString());
        }
        if (src[srcPos + 3]) {
            if (src[srcPos + 2]) {
                if (src[srcPos + 1]) {
    ...
    public static char intToHexDigit(final int nibble) {
        ...
        if (c == '\0') {
            throw new IllegalArgumentException(new StringBuilder().append("nibble value not between 0 and 15: ").append(nibble).toString());
        }
        return c;
    }
