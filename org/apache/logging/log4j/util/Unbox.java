...
public class Unbox {
    ...
    private static State getState() {
        State state = (State)Unbox.threadLocalState.get();
        if (state == null) {
            state = new State();
    ...
    static {
        ...
        Unbox.threadLocalState = (ThreadLocal<State>)new ThreadLocal();
    ...
    private static class WebSafeState {
        ...
        private WebSafeState() {
            this.ringBuffer = (ThreadLocal<StringBuilder[]>)new ThreadLocal();
            this.current = (ThreadLocal<int[]>)new ThreadLocal();
        }
        ...
        public StringBuilder getStringBuilder() {
            ...
            final int[] index = (int[])this.current.get();
        ...
        public boolean isBoxedPrimitive(final StringBuilder text) {
            final StringBuilder[] array = (StringBuilder[])this.ringBuffer.get();
            if (array == null) {
                return false;
            }
            for (int i = 0; i < array.length; ++i) {
