...
public final class LoaderUtil {
    ...
    public static ClassLoader getThreadContextClassLoader() {
        ...
        return (ClassLoader)((LoaderUtil.SECURITY_MANAGER == null) ? LoaderUtil.TCCL_GETTER.run() : ((ClassLoader)AccessController.doPrivileged((PrivilegedAction)LoaderUtil.TCCL_GETTER)));
    }
    ...
    public static <T> T newInstanceOf(final String className) throws ClassNotFoundException, IllegalAccessException, InstantiationException, NoSuchMethodException, InvocationTargetException {
        return LoaderUtil.<T>newInstanceOf(loadClass(className));
    }
    ...
    public static <T> T newCheckedInstanceOf(final String className, final Class<T> clazz) throws ClassNotFoundException, NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        return (T)clazz.cast(LoaderUtil.newInstanceOf(className));
    }
    ...
    public static <T> T newCheckedInstanceOfProperty(final String propertyName, final Class<T> clazz) throws ClassNotFoundException, NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        return (T)LoaderUtil.newCheckedInstanceOf(className, (java.lang.Class<Object>)clazz);
    }
    
    private static boolean isIgnoreTccl() {
        if (LoaderUtil.ignoreTCCL == null) {
            final String ignoreTccl = PropertiesUtil.getProperties().getStringProperty("log4j.ignoreTCL", (String)null);
    ...
    public static Collection<URL> findResources(final String resource) {
        ...
        final Collection<URL> resources = (Collection<URL>)new LinkedHashSet(urlResources.size());
    ...
    static Collection<UrlResource> findUrlResources(final String resource) {
        ...
        final Collection<UrlResource> resources = (Collection<UrlResource>)new LinkedHashSet();
        for (final ClassLoader cl : candidates) {
            if (cl != null) {
                try {
                    final Enumeration<URL> resourceEnum = (Enumeration<URL>)cl.getResources(resource);
                    while (resourceEnum.hasMoreElements()) {
                        resources.add(new UrlResource(cl, (URL)resourceEnum.nextElement()));
                    }
                }
                catch (IOException e) {
                    LowLevelLogUtil.logException((Throwable)e);
                }
            }
        }
        return resources;
    ...
    static {
        ...
        TCCL_GETTER = (PrivilegedAction)new ThreadContextClassLoaderGetter();
        if (LoaderUtil.SECURITY_MANAGER != null) {
            boolean getClassLoaderDisabled;
            try {
                LoaderUtil.SECURITY_MANAGER.checkPermission((Permission)new RuntimePermission("getClassLoader"));
