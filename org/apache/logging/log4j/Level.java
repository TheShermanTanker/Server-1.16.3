...
public final class Level implements Comparable<Level>, Serializable {
    ...
    public static Level forName(final String name, final int intValue) {
        final Level level = (Level)Level.LEVELS.get(name);
        if (level != null) {
            return level;
        }
        try {
            ...
            return (Level)Level.LEVELS.get(name);
        }
    }
    ...
    public static Level toLevel(final String name, final Level defaultLevel) {
        ...
        final Level level = (Level)Level.LEVELS.get(name.toUpperCase(Locale.ENGLISH));
        return (level == null) ? defaultLevel : level;
    }
    
    public static Level[] values() {
        final Collection<Level> values = (Collection<Level>)Level.LEVELS.values();
        return (Level[])values.toArray((Object[])new Level[values.size()]);
    }
    
    public static Level valueOf(final String name) {
        Objects.requireNonNull(name, "No level name given.");
        final String levelName = name.toUpperCase(Locale.ENGLISH);
        final Level level = (Level)Level.LEVELS.get(levelName);
        if (level != null) {
            return level;
        }
    ...
    public static <T extends Enum<T>> T valueOf(final Class<T> enumType, final String name) {
        return (T)Enum.valueOf((Class)enumType, name);
    }
    ...
    static {
        LEVELS = (ConcurrentMap)new ConcurrentHashMap();
