...
public class ResolverUtil {
    ...
    public ResolverUtil() {
        this.classMatches = (Set<Class<?>>)new HashSet();
        this.resourceMatches = (Set<URI>)new HashSet();
    }
    ...
    public void findInPackage(final Test test, String packageName) {
        ...
        try {
            urls = (Enumeration<URL>)loader.getResources(packageName);
        }
        catch (IOException ioe) {
            ResolverUtil.LOGGER.warn("Could not read package: " + packageName, (Throwable)ioe);
            return;
        }
        while (urls.hasMoreElements()) {
            try {
                ...
                ResolverUtil.LOGGER.warn("Could not read entries", (Throwable)ioe2);
            }
        }
    }
    ...
    String extractPath(final URL url) throws UnsupportedEncodingException, URISyntaxException {
        ...
        final List<String> neverDecode = (List<String>)Arrays.asList((Object[])new String[] { "vfs", "vfszip", "bundleresource" });
    ...
    private void loadImplementationsInJar(final Test test, final String parent, final File jarFile) {
        ...
        try {
            jarStream = new JarInputStream((InputStream)new FileInputStream(jarFile));
            this.loadImplementationsInJar(test, parent, jarFile.getPath(), jarStream);
        }
        catch (FileNotFoundException ex) {
            ResolverUtil.LOGGER.error(new StringBuilder().append("Could not search jar file '").append(jarFile).append("' for classes matching criteria: ").append(test).append(" file not found").toString(), (Throwable)ex);
        }
        catch (IOException ioe) {
            ResolverUtil.LOGGER.error(new StringBuilder().append("Could not search jar file '").append(jarFile).append("' for classes matching criteria: ").append(test).append(" due to an IOException").toString(), (Throwable)ioe);
        }
        finally {
    ...
    private void loadImplementationsInJar(final Test test, final String parent, final String path, final JarInputStream stream) {
        try {
            ...
            ResolverUtil.LOGGER.error("Could not search jar file '" + path + "' for classes matching criteria: " + test + " due to an IOException", (Throwable)ioe);
        }
    }
    ...
    public interface Test {
        boolean matches(final Class<?> class1);
