...
public class PluginBuilder implements Builder<Object> {
    ...
    public Object build() {
        ...
        try {
            ...
            PluginBuilder.LOGGER.error("Unable to inject fields into builder class for plugin type {}, element {}.", this.clazz, this.node.getName(), e);
        }
        try {
            ...
            PluginBuilder.LOGGER.error("Unable to invoke factory method in class {} for element {}.", this.clazz, this.node.getName(), e);
            return null;
        }
    }
    ...
    private static Builder<?> createBuilder(final Class<?> clazz) throws InvocationTargetException, IllegalAccessException {
        for (final Method method : clazz.getDeclaredMethods()) {
            if (method.isAnnotationPresent((Class)PluginBuilderFactory.class) && Modifier.isStatic(method.getModifiers()) && TypeUtil.isAssignable((Type)Builder.class, (Type)method.getReturnType())) {
                ReflectionUtil.<Method>makeAccessible(method);
    ...
    private void injectFields(final Builder<?> builder) throws IllegalAccessException {
        ...
        AccessibleObject.setAccessible((AccessibleObject[])fields.toArray((Object[])new Field[0]), true);
        ...
        for (final Field field : fields) {
            ...
            for (final Annotation a : annotations) {
                if (!(a instanceof PluginAliases)) {
                    ...
                    if (visitor != null) {
                        final Object value = visitor.setAliases(aliases).setAnnotation(a).setConversionType(field.getType()).setStrSubstitutor(this.configuration.getStrSubstitutor()).setMember((Member)field).visit(this.configuration, this.node, this.event, log);
                        if (value != null) {
    ...
    private static Method findFactoryMethod(final Class<?> clazz) {
        for (final Method method : clazz.getDeclaredMethods()) {
            if (method.isAnnotationPresent((Class)PluginFactory.class) && Modifier.isStatic(method.getModifiers())) {
                ReflectionUtil.<Method>makeAccessible(method);
                return method;
            }
        }
        ...
        for (int i = 0; i < annotations.length; ++i) {
            ...
            for (final Annotation a : annotations[i]) {
                if (!(a instanceof PluginAliases)) {
                    final PluginVisitor<? extends Annotation> visitor = PluginVisitors.findVisitor(a.annotationType());
                    if (visitor != null) {
                        final Object value = visitor.setAliases(aliases).setAnnotation(a).setConversionType(types[i]).setStrSubstitutor(this.configuration.getStrSubstitutor()).setMember((Member)factory).visit(this.configuration, this.node, this.event, log);
                        if (value != null) {
                            args[i] = value;
                        }
                    }
            ...
            final String argName = new StringBuilder().append("arg[").append(i).append("](").append(simpleName(value2)).append(")").toString();
            for (final ConstraintValidator<?> validator : validators) {
