...
public class PluginProcessor extends AbstractProcessor {
    ...
    public boolean process(final Set<? extends TypeElement> annotations, final RoundEnvironment roundEnv) {
        try {
            final Set<? extends Element> elements = roundEnv.getElementsAnnotatedWith((Class)Plugin.class);
            if (elements.isEmpty()) {
                return false;
            }
            this.collectPlugins(elements);
            this.writeCacheFile((Element[])elements.toArray((Object[])new Element[elements.size()]));
            return true;
        }
        catch (IOException e) {
            this.error((CharSequence)e.getMessage());
            return false;
        }
    }
    ...
    private void collectPlugins(final Iterable<? extends Element> elements) {
        ...
        final ElementVisitor<PluginEntry, Plugin> pluginVisitor = (ElementVisitor<PluginEntry, Plugin>)new PluginElementVisitor(elementUtils);
        final ElementVisitor<Collection<PluginEntry>, Plugin> pluginAliasesVisitor = (ElementVisitor<Collection<PluginEntry>, Plugin>)new PluginAliasesElementVisitor(elementUtils);
        for (final Element element : elements) {
            final Plugin plugin = (Plugin)element.getAnnotation((Class)Plugin.class);
            if (plugin == null) {
                continue;
            }
            final PluginEntry entry = (PluginEntry)element.accept((ElementVisitor)pluginVisitor, plugin);
            final Map<String, PluginEntry> category = this.pluginCache.getCategory(entry.getCategory());
            category.put(entry.getKey(), entry);
            final Collection<PluginEntry> entries = (Collection<PluginEntry>)element.accept((ElementVisitor)pluginAliasesVisitor, plugin);
    ...
    private void writeCacheFile(final Element... elements) throws IOException {
        final FileObject fileObject = this.processingEnv.getFiler().createResource((JavaFileManager.Location)StandardLocation.CLASS_OUTPUT, "", "META-INF/org/apache/logging/log4j/core/config/plugins/Log4j2Plugins.dat", elements);
    ...
    private static class PluginAliasesElementVisitor extends SimpleElementVisitor7<Collection<PluginEntry>, Plugin> {
        ...
        public Collection<PluginEntry> visitType(final TypeElement e, final Plugin plugin) {
            final PluginAliases aliases = (PluginAliases)e.getAnnotation((Class)PluginAliases.class);
            if (aliases == null) {
                return (Collection<PluginEntry>)this.DEFAULT_VALUE;
            }
            final Collection<PluginEntry> entries = (Collection<PluginEntry>)new ArrayList(aliases.value().length);
