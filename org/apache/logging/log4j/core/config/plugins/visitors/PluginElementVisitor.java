...
public class PluginElementVisitor extends AbstractPluginVisitor<PluginElement> {
    ...
    public Object visit(final Configuration configuration, final Node node, final LogEvent event, final StringBuilder log) {
        ...
        if (this.conversionType.isArray()) {
            ...
            final Collection<Node> used = (Collection<Node>)new ArrayList();
            log.append("={");
            boolean first = true;
            for (final Node child : node.getChildren()) {
                final PluginType<?> childType = child.getType();
                if (name.equalsIgnoreCase(childType.getElementName()) || this.conversionType.isAssignableFrom((Class)childType.getPluginClass())) {
                    if (!first) {
                        log.append(", ");
                    }
                    first = false;
            ...
            node.getChildren().removeAll((Collection)used);
            final Object[] array = (Object[])Array.newInstance((Class)this.conversionType, values.size());
            ...
            final Node namedNode = this.findNamedNode(name, (Iterable<Node>)node.getChildren());
            if (namedNode == null) {
