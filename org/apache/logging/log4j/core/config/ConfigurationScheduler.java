...
public class ConfigurationScheduler extends AbstractLifeCycle {
    ...
    public <V> ScheduledFuture<V> schedule(final Callable<V> callable, final long delay, final TimeUnit unit) {
        return (ScheduledFuture<V>)this.getExecutorService().schedule((Callable)callable, delay, unit);
    }
    ...
    public CronScheduledFuture<?> scheduleWithCron(final CronExpression cronExpression, final Date startDate, final Runnable command) {
        ...
        final ScheduledFuture<?> future = this.schedule((Runnable)runnable, this.nextFireInterval(fireDate), TimeUnit.MILLISECONDS);
    ...
    private ScheduledExecutorService getExecutorService() {
        if (this.executorService == null) {
            if (this.scheduledItems > 0) {
                ...
                final ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(this.scheduledItems, (ThreadFactory)Log4jThreadFactory.createDaemonThreadFactory("Scheduled"));
                executor.setContinueExistingPeriodicTasksAfterShutdownPolicy(false);
                executor.setExecuteExistingDelayedTasksAfterShutdownPolicy(false);
                this.executorService = (ScheduledExecutorService)executor;
            }
            else {
    ...
    public String toString() {
        ...
        final Queue<Runnable> queue = (Queue<Runnable>)((ScheduledThreadPoolExecutor)this.executorService).getQueue();
        boolean first = true;
    ...
    public class CronRunnable implements Runnable {
        ...
        public void run() {
            try {
                ...
                final ScheduledFuture<?> future = ConfigurationScheduler.this.schedule((Runnable)this, ConfigurationScheduler.this.nextFireInterval(fireDate), TimeUnit.MILLISECONDS);
