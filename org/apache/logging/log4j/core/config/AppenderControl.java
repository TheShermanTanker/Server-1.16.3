...
public class AppenderControl extends AbstractFilterable {
    ...
    public AppenderControl(final Appender appender, final Level level, final Filter filter) {
        super(filter);
        this.recursive = (ThreadLocal<AppenderControl>)new ThreadLocal();
    ...
    private void tryCallAppender(final LogEvent event) {
        try {
            ...
            this.handleAppenderError(new AppenderLoggingException((Throwable)ex2));
        }
    }
    
    private void handleAppenderError(final RuntimeException ex) {
        this.appender.getHandler().error(this.createErrorMsg("An exception occurred processing Appender "), (Throwable)ex);
        if (!this.appender.ignoreExceptions()) {
            throw ex;
        }
    }
