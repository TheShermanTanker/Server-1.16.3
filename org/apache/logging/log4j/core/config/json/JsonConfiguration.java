...
public class JsonConfiguration extends AbstractConfiguration implements Reconfigurable {
    ...
    public JsonConfiguration(final LoggerContext loggerContext, final ConfigurationSource configSource) {
        ...
        try {
            ...
            final InputStream is = (InputStream)new ByteArrayInputStream(buffer);
            ...
            JsonConfiguration.LOGGER.error("Error parsing " + configSource.getLocation(), (Throwable)ex);
        }
    }
    ...
    public void setup() {
        ...
        while (iter.hasNext()) {
            final Map.Entry<String, JsonNode> entry = (Map.Entry<String, JsonNode>)iter.next();
            final JsonNode n = (JsonNode)entry.getValue();
            if (n.isObject()) {
            ...
            if (n.isArray() || n.isObject()) {
                ...
                if (n.isArray()) {
                    ...
                    for (int i = 0; i < n.size(); ++i) {
                        final String pluginType = this.getType(n.get(i), (String)entry.getKey());
                        final PluginType<?> entryType = this.pluginManager.getPluginType(pluginType);
                        final Node item = new Node(node, (String)entry.getKey(), entryType);
                        ...
                        while (itemIter.hasNext()) {
                            final Map.Entry<String, JsonNode> itemEntry = (Map.Entry<String, JsonNode>)itemIter.next();
                            if (((JsonNode)itemEntry.getValue()).isObject()) {
                                ...
                                final JsonNode array = (JsonNode)itemEntry.getValue();
                                final String entryName = (String)itemEntry.getKey();
