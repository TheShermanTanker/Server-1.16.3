...
public class XmlConfiguration extends AbstractConfiguration implements Reconfigurable {
    ...
    public XmlConfiguration(final LoggerContext loggerContext, final ConfigurationSource configSource) {
        ...
        this.status = (List<Status>)new ArrayList();
        ...
        try {
            ...
            try {
                ...
                Closer.closeSilently((AutoCloseable)configStream);
            }
            final InputSource source = new InputSource((InputStream)new ByteArrayInputStream(buffer));
            ...
            try {
                ...
                final Throwable throwable = Throwables.getRootCause((Throwable)e);
            ...
            XmlConfiguration.LOGGER.error("Error parsing " + configSource.getLocation(), (Throwable)e2);
        }
        if (this.strict && this.schemaResource != null && buffer != null) {
            try (final InputStream is = Loader.getResourceAsStream(this.schemaResource, XmlConfiguration.class.getClassLoader())) {
                if (is != null) {
                    final Source src = (Source)new StreamSource(is, "Log4j-config.xsd");
                    ...
                    try {
                        ...
                        XmlConfiguration.LOGGER.error("Error parsing Log4j schema", (Throwable)ex);
                    }
                    if (schema != null) {
                        final Validator validator = schema.newValidator();
                        try {
                            validator.validate((Source)new StreamSource((InputStream)new ByteArrayInputStream(buffer)));
                        }
                        catch (IOException ioe) {
                            XmlConfiguration.LOGGER.error("Error reading configuration for validation", (Throwable)ioe);
                        }
                        catch (SAXException ex2) {
                            XmlConfiguration.LOGGER.error("Error validating configuration", (Throwable)ex2);
                        }
                    }
                }
            }
