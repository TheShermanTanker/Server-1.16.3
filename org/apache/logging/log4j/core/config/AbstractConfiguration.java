...
public abstract class AbstractConfiguration extends AbstractFilterable implements Configuration {
    ...
    protected AbstractConfiguration(final LoggerContext loggerContext, final ConfigurationSource configurationSource) {
        this.listeners = (List<ConfigurationListener>)new CopyOnWriteArrayList();
        this.pluginPackages = (List<String>)new ArrayList();
        ...
        this.appenders = (ConcurrentMap<String, Appender>)new ConcurrentHashMap();
        this.loggerConfigs = (ConcurrentMap<String, LoggerConfig>)new ConcurrentHashMap();
        this.customLevels = (List<CustomLevelConfig>)Collections.emptyList();
        this.properties = (ConcurrentMap<String, String>)new ConcurrentHashMap();
        this.tempLookup = new Interpolator((Map<String, String>)this.properties);
        this.subst = new StrSubstitutor(this.tempLookup);
        this.root = new LoggerConfig();
        this.componentMap = (ConcurrentMap<String, Object>)new ConcurrentHashMap();
        ...
        this.loggerContext = (WeakReference<LoggerContext>)new WeakReference(loggerContext);
    ...
    public Map<String, String> getProperties() {
        return (Map<String, String>)this.properties;
    }
    ...
    public void initialize() {
        AbstractConfiguration.LOGGER.debug("Initializing configuration {}", this);
        ...
        if (plugins != null) {
            for (final PluginType<?> type : plugins.values()) {
                try {
                    ...
                    AbstractConfiguration.LOGGER.error("Unable to initialize {} due to {}", type.getPluginClass().getName(), e.getClass().getSimpleName(), e);
                }
            }
        }
        this.setup();
    ...
    public void start() {
        ...
        final Set<LoggerConfig> alreadyStarted = (Set<LoggerConfig>)new HashSet();
    ...
    public boolean stop(final long timeout, final TimeUnit timeUnit) {
        ...
        final Appender[] array = (Appender[])this.appenders.values().toArray((Object[])new Appender[this.appenders.size()]);
    ...
    private List<Appender> getAsyncAppenders(final Appender[] all) {
        final List<Appender> result = (List<Appender>)new ArrayList();
    ...
    private void setupAdvertisement() {
        if (this.advertiserNode != null) {
            ...
            if (type != null) {
                final Class<? extends Advertiser> clazz = type.getPluginClass().asSubclass((Class)Advertiser.class);
                try {
    ...
    protected void preConfigure(final Node node) {
        try {
            ...
            AbstractConfiguration.LOGGER.error("Error capturing node data for node " + node.getName(), (Throwable)ex);
        }
    }
    ...
    protected void doConfigure() {
        ...
        if (this.rootNode.hasChildren() && ((Node)this.rootNode.getChildren().get(0)).getName().equalsIgnoreCase("Properties")) {
            final Node first = (Node)this.rootNode.getChildren().get(0);
            this.createConfiguration(first, null);
            if (first.getObject() != null) {
                this.subst.setVariableResolver(first.<StrLookup>getObject());
            }
        }
        else {
        ...
        for (final Node child : this.rootNode.getChildren()) {
            if (child.getName().equalsIgnoreCase("Properties")) {
                ...
                if (child.getName().equalsIgnoreCase("Scripts")) {
                    for (final AbstractScript script : child.<AbstractScript[]>getObject(AbstractScript[].class)) {
                    ...
                    this.addFilter(child.<Filter>getObject(Filter.class));
                }
                else if (child.getName().equalsIgnoreCase("Loggers")) {
                    final Loggers l = child.<Loggers>getObject();
                    ...
                    final List<CustomLevelConfig> copy = (List<CustomLevelConfig>)new ArrayList((Collection)this.customLevels);
                    copy.add(child.<CustomLevelConfig>getObject(CustomLevelConfig.class));
                    this.customLevels = copy;
                }
                else {
                    final List<String> expected = (List<String>)Arrays.asList((Object[])new String[] { "\"Appenders\"", "\"Loggers\"", "\"Properties\"", "\"Scripts\"", "\"CustomLevels\"" });
        ...
        for (final Map.Entry<String, LoggerConfig> entry : this.loggerConfigs.entrySet()) {
            final LoggerConfig loggerConfig = (LoggerConfig)entry.getValue();
            for (final AppenderRef ref : loggerConfig.getAppenderRefs()) {
                final Appender app = (Appender)this.appenders.get(ref.getRef());
                if (app != null) {
    ...
    public Map<String, Appender> getAppenders() {
        return (Map<String, Appender>)this.appenders;
    }
    ...
    public synchronized void removeAppender(final String appenderName) {
        ...
        final Appender app = (Appender)this.appenders.remove(appenderName);
        if (app != null) {
            app.stop();
        }
    }
    ...
    public List<CustomLevelConfig> getCustomLevels() {
        return (List<CustomLevelConfig>)Collections.unmodifiableList((List)this.customLevels);
    }
    ...
    public Map<String, LoggerConfig> getLoggers() {
        return (Map<String, LoggerConfig>)Collections.unmodifiableMap((Map)this.loggerConfigs);
    }
    ...
    private Object createPluginObject(final PluginType<?> type, final Node node, final LogEvent event) {
        ...
        if (Map.class.isAssignableFrom((Class)clazz)) {
            try {
    ...
    private static Collection<?> createPluginCollection(final Node node) {
        ...
        final Collection<Object> list = (Collection<Object>)new ArrayList(children.size());
    ...
    private void setParents() {
        for (final Map.Entry<String, LoggerConfig> entry : this.loggerConfigs.entrySet()) {
            final LoggerConfig logger = (LoggerConfig)entry.getValue();
