...
public abstract class ConfigurationFactory extends ConfigurationBuilderFactory {
    ...
    public static ConfigurationFactory getInstance() {
        if (ConfigurationFactory.factories == null) {
            ...
            try {
                if (ConfigurationFactory.factories == null) {
                    final List<ConfigurationFactory> list = (List<ConfigurationFactory>)new ArrayList();
                    final String factoryClass = PropertiesUtil.getProperties().getStringProperty("log4j.configurationFactory");
                    if (factoryClass != null) {
                        addFactory((Collection<ConfigurationFactory>)list, factoryClass);
                    }
                    final PluginManager manager = new PluginManager("ConfigurationFactory");
                    manager.collectPlugins();
                    final Map<String, PluginType<?>> plugins = manager.getPlugins();
                    final List<Class<? extends ConfigurationFactory>> ordered = (List<Class<? extends ConfigurationFactory>>)new ArrayList(plugins.size());
                    for (final PluginType<?> type : plugins.values()) {
                        try {
                            ordered.add(type.getPluginClass().asSubclass((Class)ConfigurationFactory.class));
                        }
                        catch (Exception ex) {
                            ConfigurationFactory.LOGGER.warn("Unable to add class {}", type.getPluginClass(), ex);
                        }
                    }
                    Collections.sort((List)ordered, (Comparator)OrderComparator.getInstance());
                    for (final Class<? extends ConfigurationFactory> clazz : ordered) {
                        addFactory((Collection<ConfigurationFactory>)list, clazz);
                    }
                    ConfigurationFactory.factories = (List<ConfigurationFactory>)Collections.unmodifiableList((List)list);
                }
            }
            finally {
    ...
    private static void addFactory(final Collection<ConfigurationFactory> list, final String factoryClass) {
        try {
            addFactory(list, LoaderUtil.loadClass(factoryClass).asSubclass((Class)ConfigurationFactory.class));
        }
        catch (Exception ex) {
    ...
    private static void addFactory(final Collection<ConfigurationFactory> list, final Class<? extends ConfigurationFactory> factoryClass) {
        try {
            list.add(ReflectionUtil.instantiate((java.lang.Class<Object>)factoryClass));
        }
        catch (Exception ex) {
    ...
    public abstract Configuration getConfiguration(final LoggerContext loggerContext, final ConfigurationSource configurationSource);
        ...
        try {
            ...
            if (source == null) {
                try {
                    ...
                    ConfigurationFactory.LOGGER.catching(Level.DEBUG, (Throwable)fnfe);
                }
            }
            return source;
        }
    ...
    protected ConfigurationSource getInputFromResource(final String resource, final ClassLoader loader) {
        ...
        try {
            ...
            ConfigurationFactory.LOGGER.catching(Level.DEBUG, (Throwable)ioe);
            return null;
        }
        if (is == null) {
            return null;
        ...
        if (FileUtils.isFile(url)) {
            try {
                ...
                ConfigurationFactory.LOGGER.catching(Level.DEBUG, (Throwable)ex);
            }
        }
    ...
    private static class Factory extends ConfigurationFactory {
        ...
        public Configuration getConfiguration(final LoggerContext loggerContext, final String name, final URI configLocation) {
            if (configLocation == null) {
                ...
                if (configLocationStr != null) {
                    ...
                    if (sources.length > 1) {
                        final List<AbstractConfiguration> configs = (List<AbstractConfiguration>)new ArrayList();
        ...
        private Configuration getConfiguration(final LoggerContext loggerContext, final String configLocationStr) {
            ...
            try {
                ...
                Factory.LOGGER.catching(Level.DEBUG, (Throwable)ex);
            }
            if (source == null) {
                final ClassLoader loader = LoaderUtil.getThreadContextClassLoader();
        ...
        private Configuration getConfiguration(final LoggerContext loggerContext, final boolean isTest, final String name) {
            final boolean named = Strings.isNotEmpty((CharSequence)name);
            final ClassLoader loader = LoaderUtil.getThreadContextClassLoader();
            for (final ConfigurationFactory factory : ConfigurationFactory.getFactories()) {
