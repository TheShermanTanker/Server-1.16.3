...
public class BundleContextSelector extends ClassLoaderContextSelector {
    ...
    private static LoggerContext locateContext(final Bundle bundle, final URI configLocation) {
        final String name = ((Bundle)Objects.requireNonNull(bundle, "No Bundle provided")).getSymbolicName();
        final AtomicReference<WeakReference<LoggerContext>> ref = (AtomicReference<WeakReference<LoggerContext>>)BundleContextSelector.CONTEXT_MAP.get(name);
        if (ref == null) {
            final LoggerContext context = new LoggerContext(name, bundle, configLocation);
            BundleContextSelector.CONTEXT_MAP.putIfAbsent(name, new AtomicReference((Object)new WeakReference((Object)context)));
            return (LoggerContext)((WeakReference)((AtomicReference)BundleContextSelector.CONTEXT_MAP.get(name)).get()).get();
        }
        ...
        if (ctx == null) {
            ...
            ref.compareAndSet(r, new WeakReference((Object)context2));
