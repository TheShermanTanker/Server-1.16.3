...
public final class RegexFilter extends AbstractFilter {
    ...
    private Filter.Result filter(final String msg) {
        ...
        final Matcher m = this.pattern.matcher((CharSequence)msg);
    ...
    private static int toPatternFlags(final String[] patternFlags) throws IllegalArgumentException, IllegalAccessException {
        ...
        final Comparator<Field> comparator = (Comparator<Field>)new Comparator<Field>() {
            public int compare(final Field f1, final Field f2) {
                return f1.getName().compareTo(f2.getName());
            }
        };
        Arrays.sort((Object[])fields, (Comparator)comparator);
        ...
        for (final String test : patternFlags) {
            final int index = Arrays.binarySearch((Object[])fieldNames, test);
            if (index >= 0) {
