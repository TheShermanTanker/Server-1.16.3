...
public final class StructuredDataFilter extends MapFilter {
    ...
    private StringBuilder getStringBuilder() {
        StringBuilder result = (StringBuilder)StructuredDataFilter.threadLocalStringBuilder.get();
        if (result == null) {
    ...
    private boolean listContainsValue(final List<String> candidates, final StringBuilder toMatch) {
        if (toMatch == null) {
            for (int i = 0; i < candidates.size(); ++i) {
                final String candidate = (String)candidates.get(i);
                if (candidate == null) {
                    return true;
                }
            }
            ...
            for (int i = 0; i < candidates.size(); ++i) {
                ...
                if (StringBuilders.equals((CharSequence)candidate, 0, candidate.length(), (CharSequence)toMatch, 0, toMatch.length())) {
                    return true;
                }
            }
        }
    ...
    public static StructuredDataFilter createFilter(@PluginElement("Pairs") final KeyValuePair[] pairs, @PluginAttribute("operator") final String oper, @PluginAttribute("onMatch") final Filter.Result match, @PluginAttribute("onMismatch") final Filter.Result mismatch) {
        ...
        final Map<String, List<String>> map = (Map<String, List<String>>)new HashMap();
    ...
    static {
        StructuredDataFilter.threadLocalStringBuilder = (ThreadLocal<StringBuilder>)new ThreadLocal();
    }
}
