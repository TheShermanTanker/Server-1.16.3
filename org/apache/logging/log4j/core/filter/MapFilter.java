...
public class MapFilter extends AbstractFilter {
    ...
    protected boolean filter(final Map<String, String> data) {
        boolean match = false;
        for (int i = 0; i < this.map.size(); ++i) {
            final String toMatch = (String)data.get(this.map.getKeyAt(i));
            match = (toMatch != null && this.map.<List>getValueAt(i).contains(toMatch));
            if (!this.isAnd && match) {
                break;
            }
    ...
    protected boolean filter(final ReadOnlyStringMap data) {
        boolean match = false;
        for (int i = 0; i < this.map.size(); ++i) {
            final String toMatch = data.<String>getValue(this.map.getKeyAt(i));
            match = (toMatch != null && this.map.<List>getValueAt(i).contains(toMatch));
            if (!this.isAnd && match) {
                break;
            }
    ...
    public String toString() {
        ...
        if (this.map.size() > 0) {
            ...
            for (int i = 0; i < this.map.size(); ++i) {
                ...
                final String value = (String)((list.size() > 1) ? list.get(0) : list.toString());
    ...
    protected Map<String, List<String>> getMap() {
        final Map<String, List<String>> result = (Map<String, List<String>>)new HashMap(this.map.size());
