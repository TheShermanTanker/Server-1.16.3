...
public class AsyncLoggerConfigDisruptor extends AbstractLifeCycle implements AsyncLoggerConfigDelegate {
    ...
    public synchronized void start() {
        ...
        AsyncLoggerConfigDisruptor.LOGGER.debug("Starting AsyncLoggerConfig disruptor for this configuration with ringbufferSize={}, waitStrategy={}, exceptionHandler={}...", this.disruptor.getRingBuffer().getBufferSize(), waitStrategy.getClass().getSimpleName(), errorHandler);
    ...
    private void enqueue(final LogEvent logEvent, final AsyncLoggerConfig asyncLoggerConfig) {
        this.disruptor.getRingBuffer().publishEvent((EventTranslatorTwoArg)this.translator, logEvent, asyncLoggerConfig);
    }
    
    @Override
    public boolean tryEnqueue(final LogEvent event, final AsyncLoggerConfig asyncLoggerConfig) {
        final LogEvent logEvent = this.prepareEvent(event);
        return this.disruptor.getRingBuffer().tryPublishEvent((EventTranslatorTwoArg)this.translator, logEvent, asyncLoggerConfig);
    }
    ...
    static {
        ...
        THREAD_FACTORY = (ThreadFactory)Log4jThreadFactory.createDaemonThreadFactory("AsyncLoggerConfig");
    }
