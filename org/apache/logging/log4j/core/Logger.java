...
public class Logger extends AbstractLogger implements Supplier<LoggerConfig> {
    ...
    protected Object writeReplace() throws ObjectStreamException {
        return new LoggerProxy(this.getName(), this.<MessageFactory>getMessageFactory());
    }
    ...
    public Logger getParent() {
        ...
        final MessageFactory messageFactory = this.<MessageFactory>getMessageFactory();
    ...
    public Iterator<Filter> getFilters() {
        ...
        if (filter == null) {
            return (Iterator<Filter>)new ArrayList().iterator();
        }
        if (filter instanceof CompositeFilter) {
            return ((CompositeFilter)filter).iterator();
        }
        final List<Filter> filters = (List<Filter>)new ArrayList();
        filters.add(filter);
        return (Iterator<Filter>)filters.iterator();
    }
    ...
    protected class PrivateConfig {
        ...
        boolean filter(final Level level, final Marker marker, final String msg) {
            ...
            if (filter != null) {
                final Filter.Result r = filter.filter(this.logger, level, marker, msg);
