...
public class ScriptManager implements FileWatcher, Serializable {
    ...
    public ScriptManager(final Configuration configuration, final WatchManager watchManager) {
        ...
        this.scriptRunners = (ConcurrentMap<String, ScriptRunner>)new ConcurrentHashMap();
        this.configuration = configuration;
        this.watchManager = watchManager;
        final List<ScriptEngineFactory> factories = (List<ScriptEngineFactory>)this.manager.getEngineFactories();
    ...
    public AbstractScript getScript(final String name) {
        final ScriptRunner runner = (ScriptRunner)this.scriptRunners.get(name);
        return (runner != null) ? runner.getScript() : null;
    }
    
    public void fileModified(final File file) {
        final ScriptRunner runner = (ScriptRunner)this.scriptRunners.get(file.toString());
        if (runner == null) {
    ...
    private class MainScriptRunner extends AbstractScriptRunner {
        ...
        public MainScriptRunner(final ScriptEngine scriptEngine, final AbstractScript script) {
            ...
            if (scriptEngine instanceof Compilable) {
                ...
                compiled = (CompiledScript)AccessController.doPrivileged((PrivilegedAction)new PrivilegedAction<CompiledScript>() {
        ...
        public Object execute(final Bindings bindings) {
            if (this.compiledScript != null) {
                try {
                    ...
                    ScriptManager.logger.error("Error running script " + this.script.getName(), (Throwable)ex);
                    return null;
                }
            }
            try {
