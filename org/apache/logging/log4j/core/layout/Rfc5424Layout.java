...
public final class Rfc5424Layout extends AbstractStringLayout {
    ...
    private Rfc5424Layout(final Configuration config, final Facility facility, final String id, final int ein, final boolean includeMDC, final boolean includeNL, final String escapeNL, final String mdcId, final String mdcPrefix, final String eventPrefix, final String appName, final String messageId, final String excludes, final String includes, final String required, final Charset charset, final String exceptionPattern, final boolean useTLSMessageFormat, final LoggerFields[] loggerFields) {
        ...
        if (excludes != null) {
            ...
            if (array.length > 0) {
                ...
                this.mdcExcludes = (List<String>)new ArrayList(array.length);
        ...
        if (includes != null) {
            ...
            if (array.length > 0) {
                ...
                this.mdcIncludes = (List<String>)new ArrayList(array.length);
                for (final String str : array) {
        ...
        if (required != null) {
            ...
            if (array.length > 0) {
                this.mdcRequired = (List<String>)new ArrayList(array.length);
                for (final String str : array) {
    ...
    private Map<String, FieldFormatter> createFieldFormatters(final LoggerFields[] loggerFields, final Configuration config) {
        final Map<String, FieldFormatter> sdIdMap = (Map<String, FieldFormatter>)new HashMap((loggerFields == null) ? 0 : loggerFields.length);
        if (loggerFields != null) {
            for (final LoggerFields loggerField : loggerFields) {
                ...
                final Map<String, List<PatternFormatter>> sdParams = (Map<String, List<PatternFormatter>>)new HashMap();
                ...
                if (!fields.isEmpty()) {
                    ...
                    for (final Map.Entry<String, String> entry : fields.entrySet()) {
                        final List<PatternFormatter> formatters = fieldParser.parse((String)entry.getValue());
    ...
    private static PatternParser createPatternParser(final Configuration config, final Class<? extends PatternConverter> filterClass) {
        ...
        PatternParser parser = config.<PatternParser>getComponent("RFC5424-Converter");
        if (parser == null) {
            parser = new PatternParser(config, "Converter", ThrowablePatternConverter.class);
            config.addComponent("RFC5424-Converter", parser);
            parser = config.<PatternParser>getComponent("RFC5424-Converter");
        }
        return parser;
    }
    ...
    private void appendStructuredElements(final StringBuilder buffer, final LogEvent event) {
        ...
        final Map<String, StructuredDataElement> sdElements = (Map<String, StructuredDataElement>)new HashMap();
        ...
        if (this.fieldFormatters != null) {
            for (final Map.Entry<String, FieldFormatter> sdElement : this.fieldFormatters.entrySet()) {
                ...
                final StructuredDataElement elem = ((FieldFormatter)sdElement.getValue()).format(event);
                sdElements.put(sdId, elem);
            }
        }
        if (this.includeMdc && contextMap.size() > 0) {
            final String mdcSdIdStr = this.mdcSdId.toString();
            final StructuredDataElement union = (StructuredDataElement)sdElements.get(mdcSdIdStr);
            if (union != null) {
        ...
        if (isStructured) {
            ...
            final String sdId2 = this.getId(id);
            if (sdElements.containsKey(sdId2)) {
                final StructuredDataElement union2 = (StructuredDataElement)sdElements.get(id.toString());
                union2.union(map);
                sdElements.put(sdId2, union2);
            }
            else {
                final StructuredDataElement formattedData = new StructuredDataElement(map, false);
                sdElements.put(sdId2, formattedData);
            }
        }
        ...
        for (final Map.Entry<String, StructuredDataElement> entry : sdElements.entrySet()) {
            this.formatStructuredElement((String)entry.getKey(), this.mdcPrefix, (StructuredDataElement)entry.getValue(), buffer, this.listChecker);
        }
    }
    ...
    private String escapeNewlines(final String text, final String replacement) {
        ...
        return Rfc5424Layout.NEWLINE_PATTERN.matcher((CharSequence)text).replaceAll(replacement);
    }
    ...
    private String computeTimeStampString(final long now) {
        ...
        final Calendar cal = (Calendar)new GregorianCalendar();
    ...
    private void appendMap(final String prefix, final Map<String, String> map, final StringBuilder sb, final ListChecker checker) {
        final SortedMap<String, String> sorted = (SortedMap<String, String>)new TreeMap((Map)map);
        for (final Map.Entry<String, String> entry : sorted.entrySet()) {
            if (checker.check((String)entry.getKey()) && entry.getValue() != null) {
                ...
                final String safeKey = this.escapeNewlines(this.escapeSDParams((String)entry.getKey()), this.escapeNewLine);
                final String safeValue = this.escapeNewlines(this.escapeSDParams((String)entry.getValue()), this.escapeNewLine);
    ...
    private String escapeSDParams(final String value) {
        return Rfc5424Layout.PARAM_VALUE_ESCAPE_PATTERN.matcher((CharSequence)value).replaceAll("\\\\$0");
    }
    ...
    private class FieldFormatter {
        ...
        public StructuredDataElement format(final LogEvent event) {
            final Map<String, String> map = (Map<String, String>)new HashMap(this.delegateMap.size());
            for (final Map.Entry<String, List<PatternFormatter>> entry : this.delegateMap.entrySet()) {
                final StringBuilder buffer = new StringBuilder();
                for (final PatternFormatter formatter : (List)entry.getValue()) {
    ...
    private class StructuredDataElement {
        ...
        void union(final Map<String, String> addFields) {
            this.fields.putAll((Map)addFields);
        }
