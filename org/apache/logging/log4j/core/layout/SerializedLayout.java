...
public final class SerializedLayout extends AbstractLayout<LogEvent> {
    ...
    public byte[] toByteArray(final LogEvent event) {
        ...
        try (final ObjectOutputStream oos = new PrivateObjectOutputStream((OutputStream)baos)) {
            oos.writeObject(event);
            oos.reset();
        }
        catch (IOException ioe) {
            SerializedLayout.LOGGER.error("Serialization of LogEvent failed.", (Throwable)ioe);
        }
    ...
    static {
        final ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try {
            new ObjectOutputStream((OutputStream)baos).close();
            SerializedLayout.serializedHeader = baos.toByteArray();
        }
        catch (Exception ex) {
            SerializedLayout.LOGGER.error("Unable to generate Object stream header", (Throwable)ex);
        }
    }
