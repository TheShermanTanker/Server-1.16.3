...
public final class DatePatternConverter extends LogEventPatternConverter implements ArrayPatternConverter {
    ...
    private DatePatternConverter(final String[] options) {
        super("Date", "date");
        this.threadLocalFormatter = (ThreadLocal<Formatter>)new ThreadLocal();
        this.options = (String[])((options == null) ? null : ((String[])Arrays.copyOf((Object[])options, options.length)));
        this.formatter = this.createFormatter(options);
        this.cachedTime = (AtomicReference<CachedTime>)new AtomicReference(new CachedTime(System.currentTimeMillis()));
    }
    ...
    private static Formatter createNonFixedFormatter(final String[] options) {
        ...
        try {
            ...
            DatePatternConverter.LOGGER.warn("Could not instantiate FastDateFormat with pattern " + pattern, (Throwable)e);
    ...
    private void formatWithoutThreadLocals(final long timestampMillis, final StringBuilder output) {
        ...
        if (timestampMillis != cached.timestampMillis) {
            ...
            if (this.cachedTime.compareAndSet(cached, newTime)) {
                ...
                cached = (CachedTime)this.cachedTime.get();
            }
        }
    ...
    private abstract static class Formatter {
        ...
        abstract String format(final long long1);
        
        abstract void formatToBuffer(final long long1, final StringBuilder stringBuilder);
    ...
    private static final class PatternFormatter extends Formatter {
        ...
        void formatToBuffer(final long timeMillis, final StringBuilder destination) {
            if (this.previousTime != timeMillis) {
                ...
                this.fastDateFormat.<StringBuilder>format(timeMillis, this.cachedBuffer);
            }
