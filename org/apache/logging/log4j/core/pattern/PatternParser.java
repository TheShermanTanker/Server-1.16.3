...
public final class PatternParser {
    ...
    public PatternParser(final Configuration config, final String converterKey, final Class<?> expectedClass, final Class<?> filterClass) {
        ...
        final Map<String, Class<PatternConverter>> converters = (Map<String, Class<PatternConverter>>)new LinkedHashMap();
        for (final PluginType<?> type : plugins.values()) {
            try {
                final Class<PatternConverter> clazz = (Class<PatternConverter>)type.getPluginClass();
                if (filterClass != null && !filterClass.isAssignableFrom((Class)clazz)) {
                    continue;
                }
                final ConverterKeys keys = (ConverterKeys)clazz.getAnnotation((Class)ConverterKeys.class);
                if (keys == null) {
                    continue;
                }
                ...
                PatternParser.LOGGER.error("Error processing plugin " + type.getElementName(), (Throwable)ex);
            }
        }
    ...
    public List<PatternFormatter> parse(final String pattern, final boolean alwaysWriteExceptions, final boolean disableAnsi, final boolean noConsoleNoAnsi) {
        final List<PatternFormatter> list = (List<PatternFormatter>)new ArrayList();
        final List<PatternConverter> converters = (List<PatternConverter>)new ArrayList();
        final List<FormattingInfo> fields = (List<FormattingInfo>)new ArrayList();
        this.parse(pattern, converters, fields, disableAnsi, noConsoleNoAnsi, true);
        final Iterator<FormattingInfo> fieldIter = (Iterator<FormattingInfo>)fields.iterator();
                ...
                case DOT_STATE: {
                    ...
                    switch (c) {
                        ...
                        default: {
                            ...
                            PatternParser.LOGGER.error(new StringBuilder().append("Error occurred in position ").append(i).append(".\n Was expecting digit, instead got char \"").append(c).append("\".").toString());
                            state = ParserState.LITERAL_STATE;
                            continue;
                        }
                    }
    ...
    private PatternConverter createConverter(final String converterId, final StringBuilder currentLiteral, final Map<String, Class<PatternConverter>> rules, final List<String> options, final boolean disableAnsi, final boolean noConsoleNoAnsi) {
        ...
        for (int i = converterId.length(); i > 0 && converterClass == null; converterClass = (Class<PatternConverter>)rules.get(converterName), --i) {
        ...
        if (AnsiConverter.class.isAssignableFrom((Class)converterClass)) {
            options.add(new StringBuilder().append("disableAnsi=").append(disableAnsi).toString());
            options.add(new StringBuilder().append("noConsoleNoAnsi=").append(noConsoleNoAnsi).toString());
        }
        ...
        for (final Method method : methods) {
            if (Modifier.isStatic(method.getModifiers()) && method.getDeclaringClass().equals(converterClass) && method.getName().equals("newInstance")) {
                if (newInstanceMethod == null) {
                    ...
                    PatternParser.LOGGER.error(new StringBuilder().append("Class ").append(converterClass).append(" cannot contain multiple static newInstance methods").toString());
                    return null;
                }
            }
        }
        if (newInstanceMethod == null) {
            PatternParser.LOGGER.error(new StringBuilder().append("Class ").append(converterClass).append(" does not contain a static newInstance method").toString());
            return null;
        }
        ...
        if (parms != null) {
            ...
            for (final Class<?> clazz : parmTypes) {
                if (clazz.isArray() && clazz.getName().equals("[Ljava.lang.String;")) {
                    final String[] optionsArray = (String[])options.toArray((Object[])new String[options.size()]);
                    parms[j] = optionsArray;
                }
                else if (clazz.isAssignableFrom((Class)Configuration.class)) {
                    parms[j] = this.config;
                }
                else {
        ...
        try {
            ...
            PatternParser.LOGGER.error("Error creating converter for " + converterId, (Throwable)ex);
        }
        return null;
    }
    ...
    private int finalizeConverter(final char c, final String pattern, final int start, final StringBuilder currentLiteral, final FormattingInfo formattingInfo, final Map<String, Class<PatternConverter>> rules, final List<PatternConverter> patternConverters, final List<FormattingInfo> formattingInfos, final boolean disableAnsi, final boolean noConsoleNoAnsi, final boolean convertBackslashes) {
        ...
        final List<String> options = (List<String>)new ArrayList();
