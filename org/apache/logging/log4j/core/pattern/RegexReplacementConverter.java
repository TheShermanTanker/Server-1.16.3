...
public final class RegexReplacementConverter extends LogEventPatternConverter {
    ...
    public static RegexReplacementConverter newInstance(final Configuration config, final String[] options) {
        if (options.length != 3) {
            RegexReplacementConverter.LOGGER.error(new StringBuilder().append("Incorrect number of options on replace. Expected 3 received ").append(options.length).toString());
            return null;
        }
        if (options[0] == null) {
    ...
    public void format(final LogEvent event, final StringBuilder toAppendTo) {
        ...
        toAppendTo.append(this.pattern.matcher((CharSequence)buf.toString()).replaceAll(this.substitution));
    }
}
