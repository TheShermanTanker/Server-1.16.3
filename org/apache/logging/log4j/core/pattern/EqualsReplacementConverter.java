...
public final class EqualsReplacementConverter extends EqualsBaseReplacementConverter {
    public static EqualsReplacementConverter newInstance(final Configuration config, final String[] options) {
        if (options.length != 3) {
            EqualsReplacementConverter.LOGGER.error(new StringBuilder().append("Incorrect number of options on equals. Expected 3 received ").append(options.length).toString());
            return null;
        }
        if (options[0] == null) {
    ...
    protected boolean equals(final String str, final StringBuilder buff, final int from, final int len) {
        return StringBuilders.equals((CharSequence)str, 0, str.length(), (CharSequence)buff, from, len);
    }
}
