...
public final class HighlightConverter extends LogEventPatternConverter implements AnsiConverter {
    ...
    private static Map<Level, String> createLevelStyleMap(final String[] options) {
        ...
        final Map<Level, String> levelStyles = (Map<Level, String>)new HashMap((Map)HighlightConverter.DEFAULT_STYLES);
        for (final Map.Entry<String, String> entry : styles.entrySet()) {
            final String key = ((String)entry.getKey()).toUpperCase(Locale.ENGLISH);
            final String value = (String)entry.getValue();
            if ("STYLE".equalsIgnoreCase(key)) {
                final Map<Level, String> enumMap = (Map<Level, String>)HighlightConverter.STYLES.get(value.toUpperCase(Locale.ENGLISH));
                if (enumMap == null) {
                    ...
                    levelStyles.putAll((Map)enumMap);
                }
            }
            else {
    ...
    public static HighlightConverter newInstance(final Configuration config, final String[] options) {
        if (options.length < 1) {
            HighlightConverter.LOGGER.error(new StringBuilder().append("Incorrect number of options on style. Expected at least 1, received ").append(options.length).toString());
            return null;
        }
        if (options[0] == null) {
        ...
        final boolean disableAnsi = Arrays.toString((Object[])options).contains("disableAnsi=true");
        final boolean noConsoleNoAnsi = Arrays.toString((Object[])options).contains("noConsoleNoAnsi=true");
    ...
    static {
        ...
        LOGBACK_STYLES = (Map)new HashMap();
        STYLES = (Map)new HashMap();
