...
public final class EqualsIgnoreCaseReplacementConverter extends EqualsBaseReplacementConverter {
    public static EqualsIgnoreCaseReplacementConverter newInstance(final Configuration config, final String[] options) {
        if (options.length != 3) {
            EqualsIgnoreCaseReplacementConverter.LOGGER.error(new StringBuilder().append("Incorrect number of options on equalsIgnoreCase. Expected 3 received ").append(options.length).toString());
            return null;
        }
        if (options[0] == null) {
    ...
    protected boolean equals(final String str, final StringBuilder buff, final int from, final int len) {
        return StringBuilders.equalsIgnoreCase((CharSequence)str, 0, str.length(), (CharSequence)buff, from, len);
    }
}
