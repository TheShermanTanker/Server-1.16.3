...
public final class JAnsiTextRenderer implements TextRenderer {
    ...
    public JAnsiTextRenderer(final String[] formats, final Map<String, AnsiRenderer.Code[]> defaultStyleMap) {
        ...
        if (formats.length > 1) {
            ...
            map = (Map<String, AnsiRenderer.Code[]>)new HashMap(allStyleAssignmentsArr.length + defaultStyleMap.size());
                    ...
                    if (codeNames.length == 0) {
                        ...
                        switch (s) {
                            ...
                            case "StyleMapName": {
                                ...
                                final Map<String, AnsiRenderer.Code[]> predefinedMap = (Map<String, AnsiRenderer.Code[]>)JAnsiTextRenderer.PrefedinedStyleMaps.get(predefinedMapName);
    ...
    private String render(final String text, final String... names) {
        ...
        for (final String name : names) {
            final AnsiRenderer.Code[] codes = (AnsiRenderer.Code[])this.styleMap.get(name);
            if (codes != null) {
    ...
    static {
        final Map<String, Map<String, AnsiRenderer.Code[]>> tempPreDefs = (Map<String, Map<String, AnsiRenderer.Code[]>>)new HashMap();
        Map<String, AnsiRenderer.Code[]> map = (Map<String, AnsiRenderer.Code[]>)new HashMap();
        ...
        tempPreDefs.put("Spock", (DefaultExceptionStyleMap = Collections.unmodifiableMap((Map)map)));
        map = (Map<String, AnsiRenderer.Code[]>)new HashMap();
        put(map, "Prefix", AnsiRenderer.Code.WHITE);
        ...
        put(map, "ExtraClassInfo.Location", AnsiRenderer.Code.YELLOW);
        put(map, "ExtraClassInfo.Version", AnsiRenderer.Code.YELLOW);
        tempPreDefs.put("Kirk", Collections.unmodifiableMap((Map)map));
        final Map<String, AnsiRenderer.Code[]> temp = (Map<String, AnsiRenderer.Code[]>)new HashMap();
        DefaultMessageStyleMap = Collections.unmodifiableMap((Map)temp);
        PrefedinedStyleMaps = Collections.unmodifiableMap((Map)tempPreDefs);
    }
}
