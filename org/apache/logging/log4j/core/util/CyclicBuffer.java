...
public final class CyclicBuffer<T> {
    ...
    public CyclicBuffer(final Class<T> clazz, final int size) throws IllegalArgumentException {
        ...
        this.numElems = 0;
        if (size < 1) {
            throw new IllegalArgumentException(new StringBuilder().append("The maxSize argument (").append(size).append(") is not a positive integer.").toString());
        }
    ...
    private T[] makeArray(final Class<T> cls, final int size) {
        return (T[])Array.newInstance((Class)cls, size);
    }
