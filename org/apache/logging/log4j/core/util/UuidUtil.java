...
public final class UuidUtil {
        ...
        try {
            ...
            try {
                ...
                if (mac == null) {
                    ...
                    for (Enumeration<NetworkInterface> networkInterfaces = (Enumeration<NetworkInterface>)NetworkInterface.getNetworkInterfaces(); networkInterfaces.hasMoreElements() && mac == null; mac = nic.getHardwareAddress()) {
                        nic = (NetworkInterface)networkInterfaces.nextElement();
                        if (isUpAndNotLoopback(nic)) {}
                    }
                }
            }
            catch (SocketException e) {
                UuidUtil.LOGGER.catching((Throwable)e);
            }
    ...
    static {
        ...
        final Random randomGenerator = (Random)new SecureRandom();
        if (mac == null || mac.length == 0) {
            mac = new byte[6];
