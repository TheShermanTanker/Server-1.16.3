...
public class Log4jThreadFactory implements ThreadFactory {
    ...
    public Log4jThreadFactory(final String threadFactoryName, final boolean daemon, final int priority) {
        this.threadNamePrefix = new StringBuilder().append("TF-").append(Log4jThreadFactory.FACTORY_NUMBER.getAndIncrement()).append("-").append(threadFactoryName).append("-").toString();
        this.daemon = daemon;
