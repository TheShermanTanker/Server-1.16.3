...
public class WatchManager extends AbstractLifeCycle {
    ...
    public WatchManager(final ConfigurationScheduler scheduler) {
        this.watchers = (ConcurrentMap<File, FileMonitor>)new ConcurrentHashMap();
    ...
    public void start() {
        ...
        if (this.intervalSeconds > 0) {
            this.future = this.scheduler.scheduleWithFixedDelay((Runnable)new WatchRunnable(), this.intervalSeconds, this.intervalSeconds, TimeUnit.SECONDS);
        }
    }
    ...
    public Map<File, FileWatcher> getWatchers() {
        final Map<File, FileWatcher> map = (Map<File, FileWatcher>)new HashMap();
        for (final Map.Entry<File, FileMonitor> entry : this.watchers.entrySet()) {
            map.put(entry.getKey(), ((FileMonitor)entry.getValue()).fileWatcher);
        }
        return map;
    }
    ...
    private class WatchRunnable implements Runnable {
        public void run() {
            for (final Map.Entry<File, FileMonitor> entry : WatchManager.this.watchers.entrySet()) {
                ...
                final FileMonitor fileMonitor = (FileMonitor)entry.getValue();
