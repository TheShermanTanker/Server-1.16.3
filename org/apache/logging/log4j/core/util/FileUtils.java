...
public final class FileUtils {
    ...
    public static void mkdir(final File dir, final boolean createDirectoryIfNotExisting) throws IOException {
        ...
        if (!dir.isDirectory()) {
            throw new IOException(new StringBuilder().append("File ").append(dir).append(" exists and is not a directory. Unable to create directory.").toString());
        }
    }
    
    public static void makeParentDirs(final File file) throws IOException {
        final File parent = ((File)Objects.requireNonNull(file, "file")).getCanonicalFile().getParentFile();
        if (parent != null) {
            mkdir(parent, true);
        }
    }
