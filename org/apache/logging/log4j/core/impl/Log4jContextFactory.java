...
public class Log4jContextFactory implements LoggerContextFactory, ShutdownCallbackRegistry {
    ...
    private static ContextSelector createContextSelector() {
        try {
            final ContextSelector selector = LoaderUtil.<ContextSelector>newCheckedInstanceOfProperty("Log4jContextSelector", ContextSelector.class);
            if (selector != null) {
                return selector;
            }
        }
        catch (Exception e) {
            Log4jContextFactory.LOGGER.error("Unable to create custom ContextSelector. Falling back to default.", (Throwable)e);
        }
    ...
    private static ShutdownCallbackRegistry createShutdownCallbackRegistry() {
        try {
            final ShutdownCallbackRegistry registry = LoaderUtil.<ShutdownCallbackRegistry>newCheckedInstanceOfProperty("log4j.shutdownCallbackRegistry", ShutdownCallbackRegistry.class);
            if (registry != null) {
                return registry;
            }
        }
        catch (Exception e) {
            Log4jContextFactory.LOGGER.error("Unable to create custom ShutdownCallbackRegistry. Falling back to default.", (Throwable)e);
        }
    ...
    private void initializeShutdownCallbackRegistry() {
        if (Log4jContextFactory.SHUTDOWN_HOOK_ENABLED && this.shutdownCallbackRegistry instanceof LifeCycle) {
            try {
                ...
                Log4jContextFactory.LOGGER.error("There was an error starting the ShutdownCallbackRegistry.", (Throwable)e2);
            }
        }
    }
    ...
    public LoggerContext getContext(final String fqcn, final ClassLoader loader, final Object externalContext, final boolean currentContext, final List<URI> configLocations, final String name) {
        ...
        if (ctx.getState() == LifeCycle.State.INITIALIZED) {
            if (configLocations != null && !configLocations.isEmpty()) {
                ContextAnchor.THREAD_CONTEXT.set(ctx);
                final List<AbstractConfiguration> configurations = (List<AbstractConfiguration>)new ArrayList(configLocations.size());
                for (final URI configLocation : configLocations) {
                    ...
                    if (currentReadConfiguration instanceof AbstractConfiguration) {
                        configurations.add(currentReadConfiguration);
                    }
                    else {
