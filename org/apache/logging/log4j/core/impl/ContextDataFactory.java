...
public class ContextDataFactory {
    ...
    private static Class<? extends StringMap> createCachedClass(final String className) {
        ...
        try {
            return LoaderUtil.loadClass(className).asSubclass((Class)StringMap.class);
        }
        catch (Exception any) {
            return null;
        }
    ...
    private static MethodHandle createDefaultConstructor(final Class<? extends StringMap> cachedClass) {
        ...
        try {
            return ContextDataFactory.LOOKUP.findConstructor((Class)cachedClass, MethodType.methodType(Void.TYPE));
        }
    ...
    private static MethodHandle createInitialCapacityConstructor(final Class<? extends StringMap> cachedClass) {
        ...
        try {
            return ContextDataFactory.LOOKUP.findConstructor((Class)cachedClass, MethodType.methodType(Void.TYPE, Integer.TYPE));
        }
        catch (NoSuchMethodException | IllegalAccessException ex2) {
            final ReflectiveOperationException ex;
            final ReflectiveOperationException ignored = ex;
