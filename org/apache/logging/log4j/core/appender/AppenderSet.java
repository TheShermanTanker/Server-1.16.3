...
public class AppenderSet {
    ...
    public Appender createAppender(final String appenderName, final String actualName) {
        final Node node = (Node)this.nodeMap.get(appenderName);
        if (node == null) {
            AppenderSet.LOGGER.error("No node named {} in {}", appenderName, this);
            return null;
        }
        node.getAttributes().put("name", actualName);
        if (!node.getType().getElementName().equals("appender")) {
            AppenderSet.LOGGER.error("No Appender was configured for name {} " + appenderName);
            return null;
        }
    ...
    public static class Builder implements org.apache.logging.log4j.core.util.Builder<AppenderSet> {
        ...
        public AppenderSet build() {
            ...
            final Map<String, Node> map = (Map<String, Node>)new HashMap(children.size());
            for (final Node childNode : children) {
                final String key = (String)childNode.getAttributes().get("name");
                if (key == null) {
