...
public final class RewriteAppender extends AbstractAppender {
    ...
    public void start() {
        for (final AppenderRef ref : this.appenderRefs) {
            ...
            final Appender appender = this.config.<Appender>getAppender(name);
            if (appender != null) {
                ...
                RewriteAppender.LOGGER.error(new StringBuilder().append("Appender ").append(ref).append(" cannot be located. Reference ignored").toString());
            }
        }
        super.start();
    }
