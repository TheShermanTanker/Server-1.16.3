...
public abstract class AbstractManager implements AutoCloseable {
    ...
    public static <M extends AbstractManager, T> M getManager(final String name, final ManagerFactory<M, T> factory, final T data) {
        ...
        try {
            ...
            if (manager == null) {
                ...
                if (manager == null) {
                    throw new IllegalStateException(new StringBuilder().append("ManagerFactory [").append(factory).append("] unable to create manager for [").append(name).append("] with data [").append(data).append("]").toString());
                }
    ...
    protected void log(final Level level, final String message, final Throwable throwable) {
        final Message m = AbstractManager.LOGGER.<MessageFactory>getMessageFactory().newMessage("{} {} {}: {}", this.getClass().getSimpleName(), this.getName(), message, throwable);
