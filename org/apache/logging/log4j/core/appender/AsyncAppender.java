...
public final class AsyncAppender extends AbstractAppender {
    ...
    public void start() {
        ...
        final List<AppenderControl> appenders = (List<AppenderControl>)new ArrayList();
        for (final AppenderRef appenderRef : this.appenderRefs) {
            final Appender appender = (Appender)map.get(appenderRef.getRef());
            if (appender != null) {
        ...
        if (this.errorRef != null) {
            final Appender appender2 = (Appender)map.get(this.errorRef);
            if (appender2 != null) {
    ...
    public boolean stop(final long timeout, final TimeUnit timeUnit) {
        ...
        AsyncAppender.LOGGER.trace("AsyncAppender stopping. Queue still has {} events.", this.queue.size());
        ...
        AsyncAppender.LOGGER.trace("AsyncAppender stopped. Queue has {} events.", this.queue.size());
    ...
    private class AsyncThread extends Log4jThread {
        ...
        public AsyncThread(final List<AppenderControl> appenders, final BlockingQueue<LogEvent> queue) {
            super(new StringBuilder().append("AsyncAppender-").append(AsyncAppender.THREAD_SEQUENCE.getAndIncrement()).toString());
            this.shutdown = false;
        ...
        public void run() {
            ...
            while (!this.queue.isEmpty()) {
                try {
                    final LogEvent event2 = (LogEvent)this.queue.take();
            ...
            AsyncAppender.LOGGER.trace("AsyncAppender.AsyncThread stopped. Queue has {} events remaining. Processed {} and ignored {} events since shutdown started.", this.queue.size(), count, ignored);
        }
