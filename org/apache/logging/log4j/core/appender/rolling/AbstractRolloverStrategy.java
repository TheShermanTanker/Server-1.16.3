...
public abstract class AbstractRolloverStrategy implements RolloverStrategy {
    ...
    protected Action merge(final Action compressAction, final List<Action> custom, final boolean stopOnError) {
        ...
        final List<Action> all = (List<Action>)new ArrayList();
        all.add(compressAction);
        all.addAll((Collection)custom);
    ...
    protected SortedMap<Integer, Path> getEligibleFiles(final String path, final String logfilePattern, final boolean isAscending) {
        final TreeMap<Integer, Path> eligibleFiles = (TreeMap<Integer, Path>)new TreeMap();
        ...
        if (!logfilePattern.contains("%i")) {
            return (SortedMap<Integer, Path>)eligibleFiles;
        }
        ...
        try (final DirectoryStream<Path> stream = (DirectoryStream<Path>)Files.newDirectoryStream(dir)) {
            for (final Path entry : stream) {
                final Matcher matcher = pattern.matcher((CharSequence)entry.toFile().getName());
                if (matcher.matches()) {
            ...
            throw new LoggingException(new StringBuilder().append("Error reading folder ").append(dir).append(" ").append(ioe.getMessage()).toString(), (Throwable)ioe);
        }
