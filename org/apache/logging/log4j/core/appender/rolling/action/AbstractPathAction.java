...
public abstract class AbstractPathAction extends AbstractAction {
    ...
    protected AbstractPathAction(final String basePath, final boolean followSymbolicLinks, final int maxDepth, final PathCondition[] pathFilters, final StrSubstitutor subst) {
        ...
        this.options = (Set<FileVisitOption>)(followSymbolicLinks ? EnumSet.of((Enum)FileVisitOption.FOLLOW_LINKS) : Collections.emptySet());
        this.maxDepth = maxDepth;
        this.pathConditions = (List<PathCondition>)Arrays.asList(Arrays.copyOf((Object[])pathFilters, pathFilters.length));
        this.subst = subst;
    }
    ...
    public boolean execute(final FileVisitor<Path> visitor) throws IOException {
        ...
        Files.walkFileTree(this.getBasePath(), (Set)this.options, this.maxDepth, (FileVisitor)visitor);
    ...
    protected abstract FileVisitor<Path> createFileVisitor(final Path path, final List<PathCondition> list);
    ...
    public Set<FileVisitOption> getOptions() {
        return (Set<FileVisitOption>)Collections.unmodifiableSet((Set)this.options);
    }
    ...
    public List<PathCondition> getPathConditions() {
        return (List<PathCondition>)Collections.unmodifiableList((List)this.pathConditions);
    }
