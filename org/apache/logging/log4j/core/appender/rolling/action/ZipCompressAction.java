...
public final class ZipCompressAction extends AbstractAction {
    ...
    public static boolean execute(final File source, final File destination, final boolean deleteSource, final int level) throws IOException {
        if (source.exists()) {
            try (final FileInputStream fis = new FileInputStream(source);
                 final ZipOutputStream zos = new ZipOutputStream((OutputStream)new FileOutputStream(destination))) {
                zos.setLevel(level);
    ...
    protected void reportException(final Exception ex) {
        ZipCompressAction.LOGGER.warn("Exception during compression of '" + this.source.toString() + "'.", (Throwable)ex);
    }
