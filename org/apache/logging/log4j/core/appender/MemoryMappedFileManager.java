...
public class MemoryMappedFileManager extends OutputStreamManager {
    ...
    protected MemoryMappedFileManager(final RandomAccessFile file, final String fileName, final OutputStream os, final boolean immediateFlush, final long position, final int regionLength, final String advertiseURI, final Layout<? extends Serializable> layout, final boolean writeHeader) throws IOException {
        ...
        this.isEndOfBatch = (ThreadLocal<Boolean>)new ThreadLocal();
        ...
        this.byteBuffer = (ByteBuffer)this.mappedBuffer;
    ...
    public static MemoryMappedFileManager getFileManager(final String fileName, final boolean append, final boolean immediateFlush, final int regionLength, final String advertiseURI, final Layout<? extends Serializable> layout) {
        return (MemoryMappedFileManager)OutputStreamManager.<FactoryData>getManager(fileName, new FactoryData(append, immediateFlush, regionLength, advertiseURI, layout), MemoryMappedFileManager.FACTORY);
    }
    ...
    private synchronized void remap() {
        ...
        try {
            ...
            this.logError("Unable to remap", (Throwable)ex);
        }
    }
    ...
    public synchronized boolean closeOutputStream() {
        ...
        try {
            ...
            this.logError("Unable to unmap MappedBuffer", (Throwable)ex);
        }
        try {
            ...
            this.logError("Unable to close MemoryMappedFile", (Throwable)ex2);
            return false;
        }
    }
    ...
    public static MappedByteBuffer mmap(final FileChannel fileChannel, final String fileName, final long start, final int size) throws IOException {
        ...
        while (true) {
            try {
                ...
                final long startNanos = System.nanoTime();
                final MappedByteBuffer map = fileChannel.map(FileChannel.MapMode.READ_WRITE, start, (long)size);
                ...
                MemoryMappedFileManager.LOGGER.debug("Remap attempt {}/{} failed. Retrying...", i, 10, e);
                if (i < 10) {
                    Thread.yield();
                }
                else {
    ...
    private static void unsafeUnmap(final MappedByteBuffer mbb) throws PrivilegedActionException {
        ...
        final long startNanos = System.nanoTime();
        AccessController.doPrivileged((PrivilegedExceptionAction)new PrivilegedExceptionAction<Object>() {
            public Object run() throws Exception {
                final Method getCleanerMethod = mbb.getClass().getMethod("cleaner", new Class[0]);
                getCleanerMethod.setAccessible(true);
                final Object cleaner = getCleanerMethod.invoke(mbb, new Object[0]);
                final Method cleanMethod = cleaner.getClass().getMethod("clean", new Class[0]);
                cleanMethod.invoke(cleaner, new Object[0]);
                return null;
            }
        });
        final float millis = (float)((System.nanoTime() - startNanos) / 1000000.0);
        MemoryMappedFileManager.LOGGER.debug("MMapAppender unmapped buffer OK in {} millis", millis);
    }
    ...
    public Map<String, String> getContentFormat() {
        final Map<String, String> result = (Map<String, String>)new HashMap((Map)super.getContentFormat());
    ...
    public ByteBuffer getByteBuffer() {
        return (ByteBuffer)this.mappedBuffer;
    }
    ...
    private static class MemoryMappedFileManagerFactory implements ManagerFactory<MemoryMappedFileManager, FactoryData> {
        public MemoryMappedFileManager createManager(final String name, final FactoryData data) {
            ...
            try {
                ...
                Closer.closeSilently((AutoCloseable)raf);
                return null;
            }
        }
    }
