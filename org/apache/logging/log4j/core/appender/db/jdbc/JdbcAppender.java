...
public final class JdbcAppender extends AbstractDatabaseAppender<JdbcDatabaseManager> {
    ...
    public static <B extends Builder<B>> JdbcAppender createAppender(final String name, final String ignore, final Filter filter, final ConnectionSource connectionSource, final String bufferSize, final String tableName, final ColumnConfig[] columnConfigs) {
        Assert.<String>requireNonEmpty(name, "Name cannot be empty");
        Objects.requireNonNull(connectionSource, "ConnectionSource cannot be null");
        Assert.<String>requireNonEmpty(tableName, "Table name cannot be empty");
        Assert.<ColumnConfig[]>requireNonEmpty(columnConfigs, "ColumnConfigs cannot be empty");
        final int bufferSizeInt = AbstractAppender.parseInt(bufferSize, 0);
        final boolean ignoreExceptions = Booleans.parseBoolean(ignore, true);
        return JdbcAppender.<Builder<Builder<Builder>>>newBuilder().setBufferSize(bufferSizeInt).setColumnConfigs(columnConfigs).setConnectionSource(connectionSource).setTableName(tableName).withName(name).withIgnoreExceptions(ignoreExceptions).withFilter(filter).build();
    }
    ...
    public static class Builder<B extends Builder<B>> extends AbstractAppender.Builder<B> implements org.apache.logging.log4j.core.util.Builder<JdbcAppender> {
        ...
        public JdbcAppender build() {
            ...
            final String managerName = "JdbcManager{name=" + this.getName() + ", bufferSize=" + this.bufferSize + ", tableName=" + this.tableName + ", columnConfigs=" + Arrays.toString((Object[])this.columnConfigs) + ", columnMappings=" + Arrays.toString((Object[])this.columnMappings) + '}';
