...
public final class InetAddresses {
    ...
    private static byte[] textToNumericFormatV4(final String ipString) {
        ...
        int i = 0;
        try {
            for (final String octet : InetAddresses.IPV4_SPLITTER.split((CharSequence)ipString)) {
    ...
    public static String toAddrString(final InetAddress ip) {
        Preconditions.<InetAddress>checkNotNull(ip);
    ...
    private static InetAddress forUriStringNoThrow(final String hostAddr) {
        Preconditions.<String>checkNotNull(hostAddr);
        String ipString;
        int expectBytes;
    ...
    public static Inet4Address getCompatIPv4Address(final Inet6Address ip) {
        Preconditions.checkArgument(isCompatIPv4Address(ip), "Address '%s' is not IPv4-compatible.", toAddrString((InetAddress)ip));
    ...
    public static Inet4Address get6to4IPv4Address(final Inet6Address ip) {
        Preconditions.checkArgument(is6to4Address(ip), "Address '%s' is not a 6to4 address.", toAddrString((InetAddress)ip));
    ...
    public static TeredoInfo getTeredoInfo(final Inet6Address ip) {
        Preconditions.checkArgument(isTeredoAddress(ip), "Address '%s' is not a Teredo address.", toAddrString((InetAddress)ip));
        final byte[] bytes = ip.getAddress();
    ...
    public static Inet4Address getIsatapIPv4Address(final Inet6Address ip) {
        Preconditions.checkArgument(isIsatapAddress(ip), "Address '%s' is not an ISATAP address.", toAddrString((InetAddress)ip));
        return getInet4Address(Arrays.copyOfRange(ip.getAddress(), 12, 16));
    }
    ...
    public static Inet4Address getEmbeddedIPv4ClientAddress(final Inet6Address ip) {
        ...
        throw formatIllegalArgumentException("'%s' has no embedded IPv4 address.", toAddrString((InetAddress)ip));
    }
    ...
    public static final class TeredoInfo {
        ...
        public TeredoInfo(@Nullable final Inet4Address server, @Nullable final Inet4Address client, final int port, final int flags) {
            ...
            this.server = MoreObjects.<Inet4Address>firstNonNull(server, InetAddresses.ANY4);
            this.client = MoreObjects.<Inet4Address>firstNonNull(client, InetAddresses.ANY4);
            this.port = port;
            this.flags = flags;
        }
