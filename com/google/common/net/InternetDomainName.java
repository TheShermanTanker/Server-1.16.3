...
public final class InternetDomainName {
    ...
    InternetDomainName(String name) {
        name = Ascii.toLowerCase(InternetDomainName.DOTS_MATCHER.replaceFrom((CharSequence)name, '.'));
        if (name.endsWith(".")) {
            name = name.substring(0, name.length() - 1);
        }
        Preconditions.checkArgument(name.length() <= 253, "Domain name too long: '%s':", name);
        this.name = name;
        this.parts = ImmutableList.<String>copyOf((java.lang.Iterable<? extends String>)InternetDomainName.DOT_SPLITTER.split((CharSequence)name));
    ...
    public static InternetDomainName from(final String domain) {
        return new InternetDomainName(Preconditions.<String>checkNotNull(domain));
    }
    ...
    private static boolean validateSyntax(final List<String> parts) {
        ...
        for (int i = 0; i < lastIndex; ++i) {
            final String part = (String)parts.get(i);
    ...
    private static boolean validatePart(final String part, final boolean isFinalPart) {
        ...
        final String asciiChars = CharMatcher.ascii().retainFrom((CharSequence)part);
        return InternetDomainName.PART_CHAR_MATCHER.matchesAllOf((CharSequence)asciiChars) && !InternetDomainName.DASH_MATCHER.matches(part.charAt(0)) && !InternetDomainName.DASH_MATCHER.matches(part.charAt(part.length() - 1)) && (!isFinalPart || !CharMatcher.digit().matches(part.charAt(0)));
    }
    ...
    public InternetDomainName child(final String leftParts) {
        return from((String)Preconditions.<String>checkNotNull(leftParts) + "." + this.name);
    }
    ...
    static {
        ...
        PART_CHAR_MATCHER = CharMatcher.javaLetterOrDigit().or(InternetDomainName.DASH_MATCHER);
    }
}
