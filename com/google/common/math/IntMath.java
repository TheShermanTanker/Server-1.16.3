...
public final class IntMath {
    ...
    public static int ceilingPowerOfTwo(final int x) {
        ...
        if (x > 1073741824) {
            throw new ArithmeticException(new StringBuilder().append("ceilingPowerOfTwo(").append(x).append(") not representable as an int").toString());
        }
    ...
    public static int divide(final int p, final int q, final RoundingMode mode) {
        Preconditions.<RoundingMode>checkNotNull(mode);
        if (q == 0) {
    ...
    public static int mod(final int x, final int m) {
        if (m <= 0) {
            throw new ArithmeticException(new StringBuilder().append("Modulus ").append(m).append(" must be > 0").toString());
        }
    ...
    public static int saturatedAdd(final int a, final int b) {
        return Ints.saturatedCast(a + (long)b);
    }
    ...
    public static int saturatedPow(int b, int k) {
        ...
        switch (b) {
            ...
            default: {
                int accum = 1;
                final int limit = Integer.MAX_VALUE + (b >>> 31 & (k & 0x1));
                while (true) {
                    switch (k) {
                        case 0: {
                            return accum;
