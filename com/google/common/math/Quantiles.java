...
public final class Quantiles {
    ...
    private static void checkIndex(final int index, final int scale) {
        if (index < 0 || index > scale) {
            throw new IllegalArgumentException(new StringBuilder().append("Quantile indexes must be between 0 and the scale, which is ").append(scale).toString());
        }
    }
    ...
    public static final class ScaleAndIndexes {
        ...
        public Map<Integer, Double> computeInPlace(final double... dataset) {
            Preconditions.checkArgument(dataset.length > 0, "Cannot calculate quantiles of an empty dataset");
            if (containsNaN(dataset)) {
                final Map<Integer, Double> nanMap = (Map<Integer, Double>)new HashMap();
                for (final int index : this.indexes) {
                    nanMap.put(index, Double.NaN);
                }
                return (Map<Integer, Double>)Collections.unmodifiableMap((Map)nanMap);
            }
            ...
            final Map<Integer, Double> ret = (Map<Integer, Double>)new HashMap();
            ...
            return (Map<Integer, Double>)Collections.unmodifiableMap((Map)ret);
        }
    }
}
