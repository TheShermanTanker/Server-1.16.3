...
public final class AtomicLongMap<K> implements Serializable {
    ...
    private AtomicLongMap(final ConcurrentHashMap<K, Long> map) {
        this.map = Preconditions.<ConcurrentHashMap<K, Long>>checkNotNull(map);
    }
    
    public static <K> AtomicLongMap<K> create() {
        return new AtomicLongMap<K>((java.util.concurrent.ConcurrentHashMap<K, Long>)new ConcurrentHashMap());
    }
    
    public static <K> AtomicLongMap<K> create(final Map<? extends K, ? extends Long> m) {
        final AtomicLongMap<K> result = AtomicLongMap.<K>create();
        result.putAll(m);
        return result;
    }
    ...
    public long updateAndGet(final K key, final LongUnaryOperator updaterFunction) {
        Preconditions.<LongUnaryOperator>checkNotNull(updaterFunction);
        return (long)this.map.compute(key, (k, value) -> updaterFunction.applyAsLong((value == null) ? 0L : ((long)value)));
    }
    ...
    public long getAndUpdate(final K key, final LongUnaryOperator updaterFunction) {
        ...
        this.map.compute(key, (k, value) -> {
            final long oldValue = (value == null) ? 0L : value;
            holder.set(oldValue);
    ...
    public long accumulateAndGet(final K key, final long x, final LongBinaryOperator accumulatorFunction) {
        Preconditions.<LongBinaryOperator>checkNotNull(accumulatorFunction);
    ...
    public long remove(final K key) {
        final Long result = (Long)this.map.remove(key);
    ...
    private Map<K, Long> createAsMap() {
        return (Map<K, Long>)Collections.unmodifiableMap((Map)this.map);
    }
    ...
    long putIfAbsent(final K key, final long newValue) {
        ...
        final Long result = (Long)this.map.compute(key, (k, oldValue) -> {
