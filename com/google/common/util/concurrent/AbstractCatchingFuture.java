...
abstract class AbstractCatchingFuture<V, X extends Throwable, F, T> extends TrustedFuture<V> implements Runnable {
    ...
    static <V, X extends Throwable> ListenableFuture<V> create(final ListenableFuture<? extends V> input, final Class<X> exceptionType, final Function<? super X, ? extends V> fallback, final Executor executor) {
        final CatchingFuture<V, X> future = new CatchingFuture<V, X>(input, exceptionType, fallback);
        input.addListener((Runnable)future, MoreExecutors.rejectionPropagatingExecutor(executor, future));
        return (ListenableFuture<V>)future;
    }
    ...
    AbstractCatchingFuture(final ListenableFuture<? extends V> inputFuture, final Class<X> exceptionType, final F fallback) {
        this.inputFuture = Preconditions.<ListenableFuture<? extends V>>checkNotNull(inputFuture);
        this.exceptionType = Preconditions.<Class<X>>checkNotNull(exceptionType);
        this.fallback = Preconditions.<F>checkNotNull(fallback);
    }
    ...
    public final void run() {
        ...
        try {
            sourceResult = Futures.<V>getDone((java.util.concurrent.Future<V>)localInputFuture);
        }
        catch (ExecutionException e) {
            throwable = Preconditions.<Throwable>checkNotNull(e.getCause());
        }
        catch (Throwable e2) {
            throwable = e2;
        }
    ...
    @Nullable
    @ForOverride
    abstract T doFallback(final F object, final X throwable) throws Exception;
    
    @ForOverride
    abstract void setResult(@Nullable final T object);
    ...
    private static final class AsyncCatchingFuture<V, X extends Throwable> extends AbstractCatchingFuture<V, X, AsyncFunction<? super X, ? extends V>, ListenableFuture<? extends V>> {
        ...
        ListenableFuture<? extends V> doFallback(final AsyncFunction<? super X, ? extends V> fallback, final X cause) throws Exception {
            final ListenableFuture<? extends V> replacement = fallback.apply(cause);
            Preconditions.<ListenableFuture<? extends V>>checkNotNull(replacement, "AsyncFunction.apply returned null instead of a Future. Did you mean to return immediateFuture(null)?");
            return replacement;
        }
