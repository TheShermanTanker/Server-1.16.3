...
final class TimeoutFuture<V> extends TrustedFuture<V> {
    ...
    static <V> ListenableFuture<V> create(final ListenableFuture<V> delegate, final long time, final TimeUnit unit, final ScheduledExecutorService scheduledExecutor) {
        ...
        result.timer = scheduledExecutor.schedule((Runnable)fire, time, unit);
        delegate.addListener((Runnable)fire, MoreExecutors.directExecutor());
        return result;
    }
    
    private TimeoutFuture(final ListenableFuture<V> delegate) {
        this.delegateRef = Preconditions.<ListenableFuture<V>>checkNotNull(delegate);
    }
    ...
    private static final class Fire<V> implements Runnable {
        ...
        public void run() {
            ...
            if (delegate.isDone()) {
                ...
                try {
                    timeoutFuture.setException((Throwable)new TimeoutException(new StringBuilder().append("Future timed out: ").append(delegate).toString()));
                }
                finally {
