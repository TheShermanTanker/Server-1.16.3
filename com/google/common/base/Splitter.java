...
public final class Splitter {
    ...
    public static Splitter on(final CharMatcher separatorMatcher) {
        Preconditions.<CharMatcher>checkNotNull(separatorMatcher);
    ...
    public Splitter trimResults(final CharMatcher trimmer) {
        Preconditions.<CharMatcher>checkNotNull(trimmer);
    ...
    public List<String> splitToList(final CharSequence sequence) {
        ...
        return (List<String>)Collections.unmodifiableList((List)result);
    }
    ...
    public static final class MapSplitter {
        ...
        public Map<String, String> split(final CharSequence sequence) {
            final Map<String, String> map = (Map<String, String>)new LinkedHashMap();
            for (final String entry : this.outerSplitter.split(sequence)) {
                final Iterator<String> entryFields = this.entrySplitter.splittingIterator((CharSequence)entry);
                Preconditions.checkArgument(entryFields.hasNext(), "Chunk [%s] is not a valid entry", entry);
                final String key = (String)entryFields.next();
                Preconditions.checkArgument(!map.containsKey(key), "Duplicate key [%s] found.", key);
                Preconditions.checkArgument(entryFields.hasNext(), "Chunk [%s] is not a valid entry", entry);
                final String value = (String)entryFields.next();
                map.put(key, value);
                Preconditions.checkArgument(!entryFields.hasNext(), "Chunk [%s] is not a valid entry", entry);
            }
            return (Map<String, String>)Collections.unmodifiableMap((Map)map);
        }
    }
    ...
    private abstract static class SplittingIterator extends AbstractIterator<String> {
        ...
        abstract int separatorStart(final int integer);
        
        abstract int separatorEnd(final int integer);
    ...
    private interface Strategy {
        Iterator<String> iterator(final Splitter splitter, final CharSequence charSequence);
    }
}
