...
public final class Hashing {
    ...
    public static HashFunction hmacMd5(final byte[] key) {
        return hmacMd5((Key)new SecretKeySpec((byte[])Preconditions.<byte[]>checkNotNull(key), "HmacMD5"));
    }
    ...
    public static HashFunction hmacSha1(final byte[] key) {
        return hmacSha1((Key)new SecretKeySpec((byte[])Preconditions.<byte[]>checkNotNull(key), "HmacSHA1"));
    }
    ...
    public static HashFunction hmacSha256(final byte[] key) {
        return hmacSha256((Key)new SecretKeySpec((byte[])Preconditions.<byte[]>checkNotNull(key), "HmacSHA256"));
    }
    ...
    public static HashFunction hmacSha512(final byte[] key) {
        return hmacSha512((Key)new SecretKeySpec((byte[])Preconditions.<byte[]>checkNotNull(key), "HmacSHA512"));
    }
    ...
    public static HashCode combineOrdered(final Iterable<HashCode> hashCodes) {
        final Iterator<HashCode> iterator = (Iterator<HashCode>)hashCodes.iterator();
        Preconditions.checkArgument(iterator.hasNext(), "Must be at least 1 hash code to combine.");
        final int bits = ((HashCode)iterator.next()).bits();
    ...
    public static HashCode combineUnordered(final Iterable<HashCode> hashCodes) {
        final Iterator<HashCode> iterator = (Iterator<HashCode>)hashCodes.iterator();
        Preconditions.checkArgument(iterator.hasNext(), "Must be at least 1 hash code to combine.");
        final byte[] resultBytes = new byte[((HashCode)iterator.next()).bits() / 8];
        for (final HashCode hashCode : hashCodes) {
            final byte[] nextBytes = hashCode.asBytes();
            Preconditions.checkArgument(nextBytes.length == resultBytes.length, "All hashcodes must have the same bit length.");
            for (int i = 0; i < nextBytes.length; ++i) {
    ...
    public static HashFunction concatenating(final HashFunction first, final HashFunction second, final HashFunction... rest) {
        final List<HashFunction> list = (List<HashFunction>)new ArrayList();
        list.add(first);
        list.add(second);
    ...
    public static HashFunction concatenating(final Iterable<HashFunction> hashFunctions) {
        Preconditions.<Iterable<HashFunction>>checkNotNull(hashFunctions);
        final List<HashFunction> list = (List<HashFunction>)new ArrayList();
