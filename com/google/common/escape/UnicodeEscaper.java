...
public abstract class UnicodeEscaper extends Escaper {
    ...
    protected abstract char[] escape(final int integer);
    ...
    public String escape(final String string) {
        Preconditions.<String>checkNotNull(string);
        final int end = string.length();
        final int index = this.nextEscapeIndex((CharSequence)string, 0, end);
    ...
    protected final String escapeSlow(final String s, int index) {
        ...
        while (index < end) {
            final int cp = codePointAt((CharSequence)s, index, end);
            if (cp < 0) {
            ...
            index = this.nextEscapeIndex((CharSequence)s, nextIndex, end);
        }
    ...
    protected static int codePointAt(final CharSequence seq, int index, final int end) {
        Preconditions.<CharSequence>checkNotNull(seq);
        if (index >= end) {
        ...
        if (c1 > '\udbff') {
            throw new IllegalArgumentException(new StringBuilder().append("Unexpected low surrogate character '").append(c1).append("' with value ").append((int)c1).append(" at index ").append(index - 1).append(" in '").append(seq).append("'").toString());
        }
        if (index == end) {
            return -c1;
        }
        ...
        throw new IllegalArgumentException(new StringBuilder().append("Expected low surrogate but got char '").append(c2).append("' with value ").append((int)c2).append(" at index ").append(index).append(" in '").append(seq).append("'").toString());
    }
