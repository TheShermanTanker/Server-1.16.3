...
public final class CharEscaperBuilder {
    ...
    public CharEscaperBuilder() {
        this.max = -1;
        this.map = (Map<Character, String>)new HashMap();
    }
    
    @CanIgnoreReturnValue
    public CharEscaperBuilder addEscape(final char c, final String r) {
        this.map.put(c, Preconditions.<String>checkNotNull(r));
        if (c > this.max) {
            this.max = c;
        }
        return this;
    ...
    public CharEscaperBuilder addEscapes(final char[] cs, final String r) {
        Preconditions.<String>checkNotNull(r);
        for (final char c : cs) {
            this.addEscape(c, r);
        }
        return this;
    ...
    public char[][] toArray() {
        ...
        for (final Map.Entry<Character, String> entry : this.map.entrySet()) {
            result[(char)entry.getKey()] = ((String)entry.getValue()).toCharArray();
        }
        return result;
    }
