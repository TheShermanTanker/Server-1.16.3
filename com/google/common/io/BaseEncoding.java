...
public abstract class BaseEncoding {
    ...
    public final String encode(final byte[] bytes, final int off, final int len) {
        ...
        try {
            this.encodeTo((Appendable)result, bytes, off, len);
        }
    ...
    @GwtIncompatible
    public abstract OutputStream encodingStream(final Writer writer);
    
    @GwtIncompatible
    public final ByteSink encodingSink(final CharSink encodedSink) {
        Preconditions.<CharSink>checkNotNull(encodedSink);
        return new ByteSink() {
            @Override
    ...
    public abstract boolean canDecode(final CharSequence charSequence);
    ...
    final byte[] decodeChecked(CharSequence chars) throws DecodingException {
        chars = (CharSequence)this.padding().trimTrailingFrom(chars);
    ...
    @GwtIncompatible
    public abstract InputStream decodingStream(final Reader reader);
    
    @GwtIncompatible
    public final ByteSource decodingSource(final CharSource encodedSource) {
        Preconditions.<CharSource>checkNotNull(encodedSource);
    ...
    abstract int maxEncodedSize(final int integer);
    
    abstract void encodeTo(final Appendable appendable, final byte[] arr, final int integer3, final int integer4) throws IOException;
    
    abstract int maxDecodedSize(final int integer);
    
    abstract int decodeTo(final byte[] arr, final CharSequence charSequence) throws DecodingException;
    
    abstract CharMatcher padding();
    
    public abstract BaseEncoding omitPadding();
    
    public abstract BaseEncoding withPadChar(final char character);
    
    public abstract BaseEncoding withSeparator(final String string, final int integer);
    ...
    static Reader ignoringReader(final Reader delegate, final CharMatcher toIgnore) {
        Preconditions.<Reader>checkNotNull(delegate);
        Preconditions.<CharMatcher>checkNotNull(toIgnore);
        return new Reader() {
    ...
    static Appendable separatingAppendable(final Appendable delegate, final String separator, final int afterEveryChars) {
        Preconditions.<Appendable>checkNotNull(delegate);
        Preconditions.<String>checkNotNull(separator);
        Preconditions.checkArgument(afterEveryChars > 0);
        return (Appendable)new Appendable() {
            int charsUntilSeparator = afterEveryChars;
            
            public Appendable append(final char c) throws IOException {
                if (this.charsUntilSeparator == 0) {
                    delegate.append((CharSequence)separator);
    ...
    static Writer separatingWriter(final Writer delegate, final String separator, final int afterEveryChars) {
        final Appendable seperatingAppendable = separatingAppendable((Appendable)delegate, separator, afterEveryChars);
        return new Writer() {
    ...
    private static final class Alphabet extends CharMatcher {
        ...
        Alphabet(final String name, final char[] chars) {
            ...
            this.chars = Preconditions.<char[]>checkNotNull(chars);
            try {
                this.bitsPerChar = IntMath.log2(chars.length, RoundingMode.UNNECESSARY);
            }
            catch (ArithmeticException e) {
                throw new IllegalArgumentException(new StringBuilder().append("Illegal alphabet length ").append(chars.length).toString(), (Throwable)e);
            }
            ...
            try {
                ...
                throw new IllegalArgumentException("Illegal alphabet " + new String(chars), (Throwable)e2);
            }
        ...
        int decode(final char ch) throws DecodingException {
            if (ch > '\u007f' || this.decodabet[ch] == -1) {
                throw new DecodingException(new StringBuilder().append("Unrecognized character: ").append(CharMatcher.invisible().matches(ch) ? ("0x" + Integer.toHexString((int)ch)) : Character.valueOf(ch)).toString());
            }
    ...
    static class StandardBaseEncoding extends BaseEncoding {
        ...
        StandardBaseEncoding(final Alphabet alphabet, @Nullable final Character paddingChar) {
            this.alphabet = Preconditions.<Alphabet>checkNotNull(alphabet);
        ...
        public OutputStream encodingStream(final Writer out) {
            Preconditions.<Writer>checkNotNull(out);
            return new OutputStream() {
                ...
                public void write(final int b) throws IOException {
                    ...
                    while (this.bitBufferLength >= StandardBaseEncoding.this.alphabet.bitsPerChar) {
                        ...
                        out.write((int)StandardBaseEncoding.this.alphabet.encode(charIndex));
                        ++this.writtenChars;
                    ...
                    if (this.bitBufferLength > 0) {
                        ...
                        if (StandardBaseEncoding.this.paddingChar != null) {
                            while (this.writtenChars % StandardBaseEncoding.this.alphabet.charsPerChunk != 0) {
                                out.write((int)StandardBaseEncoding.this.paddingChar);
                                ++this.writtenChars;
                            }
                        }
                    }
        ...
        void encodeTo(final Appendable target, final byte[] bytes, final int off, final int len) throws IOException {
            Preconditions.<Appendable>checkNotNull(target);
            Preconditions.checkPositionIndexes(off, off + len, bytes.length);
        ...
        void encodeChunkTo(final Appendable target, final byte[] bytes, final int off, final int len) throws IOException {
            ...
            if (this.paddingChar != null) {
                while (bitsProcessed < this.alphabet.bytesPerChunk * 8) {
                    target.append((char)this.paddingChar);
        ...
        int decodeTo(final byte[] target, CharSequence chars) throws DecodingException {
            Preconditions.<byte[]>checkNotNull(target);
            chars = (CharSequence)this.padding().trimTrailingFrom(chars);
            if (!this.alphabet.isValidPaddingStartPosition(chars.length())) {
                throw new DecodingException(new StringBuilder().append("Invalid input length ").append(chars.length()).toString());
            }
            int bytesWritten = 0;
                    ...
                    while (true) {
                        ...
                        if (readChar == -1) {
                            if (!this.hitPadding && !StandardBaseEncoding.this.alphabet.isValidPaddingStartPosition(this.readChars)) {
                                throw new DecodingException(new StringBuilder().append("Invalid input length ").append(this.readChars).toString());
                            }
                            return -1;
                        }
                        else {
                            ...
                            if (this.paddingMatcher.matches(ch)) {
                                if (!this.hitPadding && (this.readChars == 1 || !StandardBaseEncoding.this.alphabet.isValidPaddingStartPosition(this.readChars - 1))) {
                                    throw new DecodingException(new StringBuilder().append("Padding cannot start at index ").append(this.readChars).toString());
                                }
                                this.hitPadding = true;
                            }
                            else {
                                if (this.hitPadding) {
                                    throw new DecodingException(new StringBuilder().append("Expected padding character but found '").append(ch).append("' at index ").append(this.readChars).toString());
                                }
        ...
        public BaseEncoding withSeparator(final String separator, final int afterEveryChars) {
            Preconditions.checkArgument(this.padding().or(this.alphabet).matchesNoneOf((CharSequence)separator), "Separator (%s) cannot contain alphabet or padding characters", separator);
    ...
    static final class SeparatedBaseEncoding extends BaseEncoding {
        ...
        SeparatedBaseEncoding(final BaseEncoding delegate, final String separator, final int afterEveryChars) {
            this.delegate = Preconditions.<BaseEncoding>checkNotNull(delegate);
            ...
            this.separatorChars = CharMatcher.anyOf((CharSequence)separator).precomputed();
        }
        ...
        public boolean canDecode(final CharSequence chars) {
            return this.delegate.canDecode((CharSequence)this.separatorChars.removeFrom(chars));
        }
        
        @Override
        int decodeTo(final byte[] target, final CharSequence chars) throws DecodingException {
            return this.delegate.decodeTo(target, (CharSequence)this.separatorChars.removeFrom(chars));
        }
        ...
        public String toString() {
            return new StringBuilder().append(this.delegate).append(".withSeparator(\"").append(this.separator).append("\", ").append(this.afterEveryChars).append(")").toString();
        }
    }
}
