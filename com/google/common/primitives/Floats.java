...
public final class Floats {
    ...
    public static int indexOf(final float[] array, final float[] target) {
        Preconditions.<float[]>checkNotNull(array, "array");
        Preconditions.<float[]>checkNotNull(target, "target");
        if (target.length == 0) {
            return 0;
        }
        int i = 0;
    ...
    public static float[] toArray(final Collection<? extends Number> collection) {
        ...
        for (int i = 0; i < len; ++i) {
            array[i] = Preconditions.<Number>checkNotNull(boxedArray[i]).floatValue();
        }
        return array;
    }
    ...
    public static List<Float> asList(final float... backingArray) {
        if (backingArray.length == 0) {
            return (List<Float>)Collections.emptyList();
        }
    ...
    private static class FloatArrayAsList extends AbstractList<Float> implements RandomAccess, Serializable {
        ...
        public Float set(final int index, final Float element) {
            ...
            this.array[this.start + index] = Preconditions.<Float>checkNotNull(element);
            return oldValue;
        }
        ...
        public List<Float> subList(final int fromIndex, final int toIndex) {
            ...
            return (List<Float>)new FloatArrayAsList(this.array, this.start + fromIndex, this.start + toIndex);
        }
