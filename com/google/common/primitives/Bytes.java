...
public final class Bytes {
    ...
    public static int indexOf(final byte[] array, final byte[] target) {
        Preconditions.<byte[]>checkNotNull(array, "array");
        Preconditions.<byte[]>checkNotNull(target, "target");
        if (target.length == 0) {
            return 0;
        }
        int i = 0;
    ...
    public static byte[] toArray(final Collection<? extends Number> collection) {
        ...
        for (int i = 0; i < len; ++i) {
            array[i] = Preconditions.<Number>checkNotNull(boxedArray[i]).byteValue();
        }
        return array;
    }
    ...
    public static List<Byte> asList(final byte... backingArray) {
        if (backingArray.length == 0) {
            return (List<Byte>)Collections.emptyList();
        }
        return (List<Byte>)new ByteArrayAsList(backingArray);
    }
    ...
    private static class ByteArrayAsList extends AbstractList<Byte> implements RandomAccess, Serializable {
        ...
        public Byte set(final int index, final Byte element) {
            ...
            this.array[this.start + index] = Preconditions.<Byte>checkNotNull(element);
            return oldValue;
        }
        ...
        public List<Byte> subList(final int fromIndex, final int toIndex) {
            ...
            return (List<Byte>)new ByteArrayAsList(this.array, this.start + fromIndex, this.start + toIndex);
        }
