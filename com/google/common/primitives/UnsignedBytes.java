...
public final class UnsignedBytes {
    ...
    public static byte parseUnsignedByte(final String string, final int radix) {
        final int parse = Integer.parseInt((String)Preconditions.<String>checkNotNull(string), radix);
        if (parse >> 8 == 0) {
            return (byte)parse;
        }
        throw new NumberFormatException(new StringBuilder().append("out of range: ").append(parse).toString());
    }
    ...
    static Comparator<byte[]> lexicographicalComparatorJavaImpl() {
        return (Comparator<byte[]>)LexicographicalComparatorHolder.PureJavaComparator.INSTANCE;
    }
    ...
    static class LexicographicalComparatorHolder {
        ...
        enum UnsafeComparator implements Comparator<byte[]> {
            ...
            static {
                ...
                theUnsafe = getUnsafe();
                BYTE_ARRAY_BASE_OFFSET = UnsafeComparator.theUnsafe.arrayBaseOffset((Class)byte[].class);
                if (UnsafeComparator.theUnsafe.arrayIndexScale((Class)byte[].class) != 1) {
