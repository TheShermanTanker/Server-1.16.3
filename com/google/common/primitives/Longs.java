...
public final class Longs {
    ...
    public static int indexOf(final long[] array, final long[] target) {
        Preconditions.<long[]>checkNotNull(array, "array");
        Preconditions.<long[]>checkNotNull(target, "target");
        if (target.length == 0) {
            return 0;
        }
        int i = 0;
    ...
    private static int digit(final char c) {
        return (c < '\u0080') ? Longs.asciiDigits[c] : -1;
    }
    ...
    public static Long tryParse(final String string, final int radix) {
        if (Preconditions.<String>checkNotNull(string).isEmpty()) {
            return null;
        }
        if (radix < 2 || radix > 36) {
            throw new IllegalArgumentException(new StringBuilder().append("radix must be between MIN_RADIX and MAX_RADIX but was ").append(radix).toString());
        }
        int index;
    ...
    public static Comparator<long[]> lexicographicalComparator() {
        return (Comparator<long[]>)LexicographicalComparator.INSTANCE;
    }
    ...
    public static long[] toArray(final Collection<? extends Number> collection) {
        ...
        for (int i = 0; i < len; ++i) {
            array[i] = Preconditions.<Number>checkNotNull(boxedArray[i]).longValue();
        }
        return array;
    }
    ...
    public static List<Long> asList(final long... backingArray) {
        if (backingArray.length == 0) {
            return (List<Long>)Collections.emptyList();
        }
        return (List<Long>)new LongArrayAsList(backingArray);
    }
    ...
    private static class LongArrayAsList extends AbstractList<Long> implements RandomAccess, Serializable {
        ...
        public Long set(final int index, final Long element) {
            ...
            this.array[this.start + index] = Preconditions.<Long>checkNotNull(element);
            return oldValue;
        }
        ...
        public List<Long> subList(final int fromIndex, final int toIndex) {
            ...
            return (List<Long>)new LongArrayAsList(this.array, this.start + fromIndex, this.start + toIndex);
        }
