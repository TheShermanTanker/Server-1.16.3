...
final class CartesianList<E> extends AbstractList<List<E>> implements RandomAccess {
    ...
    static <E> List<List<E>> create(final List<? extends List<? extends E>> lists) {
        ...
        for (final List<? extends E> list : lists) {
            final List<E> copy = ImmutableList.copyOf((java.util.Collection<?>)list);
            if (copy.isEmpty()) {
                return ImmutableList.of();
            }
            axesBuilder.add(copy);
        }
        return (List<List<E>>)new CartesianList((ImmutableList<java.util.List<Object>>)axesBuilder.build());
    }
