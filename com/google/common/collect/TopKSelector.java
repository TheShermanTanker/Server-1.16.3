...
final class TopKSelector<T> {
    ...
    public static <T extends Comparable<? super T>> TopKSelector<T> least(final int k) {
        return TopKSelector.<T>least(k, (java.util.Comparator<? super T>)Ordering.<Comparable>natural());
    }
    
    public static <T extends Comparable<? super T>> TopKSelector<T> greatest(final int k) {
        return TopKSelector.<T>greatest(k, (java.util.Comparator<? super T>)Ordering.<Comparable>natural());
    }
    ...
    public static <T> TopKSelector<T> greatest(final int k, final Comparator<? super T> comparator) {
        return new TopKSelector<T>((java.util.Comparator<? super T>)Ordering.from(comparator).reverse(), k);
    }
    
    private TopKSelector(final Comparator<? super T> comparator, final int k) {
        this.comparator = Preconditions.<Comparator<? super T>>checkNotNull(comparator, "comparator");
        this.k = k;
    ...
    private void trim() {
        ...
        while (left < right) {
            ...
            if (++iterations >= maxIterations) {
                Arrays.sort((Object[])this.buffer, left, right, (Comparator)this.comparator);
                break;
            }
        }
    ...
    public List<T> topK() {
        Arrays.sort((Object[])this.buffer, 0, this.bufferSize, (Comparator)this.comparator);
        if (this.bufferSize > this.k) {
            Arrays.fill((Object[])this.buffer, this.k, this.buffer.length, null);
            this.bufferSize = this.k;
            this.threshold = this.buffer[this.k - 1];
        }
        return (List<T>)Collections.unmodifiableList(Arrays.asList(Arrays.copyOf((Object[])this.buffer, this.bufferSize)));
    }
}
