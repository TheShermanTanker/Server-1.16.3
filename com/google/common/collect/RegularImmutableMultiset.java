...
class RegularImmutableMultiset<E> extends ImmutableMultiset<E> {
    ...
    RegularImmutableMultiset(final Collection<? extends Multiset.Entry<? extends E>> entries) {
        ...
        final Multisets.ImmutableEntry<E>[] entryArray = new Multisets.ImmutableEntry[distinct];
        if (distinct == 0) {
            this.entries = entryArray;
            this.hashTable = null;
            this.size = 0;
            this.hashCode = 0;
            this.elementSet = ImmutableSet.<E>of();
        }
        else {
            final int tableSize = Hashing.closedTableSize(distinct, 1.0);
            final int mask = tableSize - 1;
            final Multisets.ImmutableEntry<E>[] hashTable = new Multisets.ImmutableEntry[tableSize];
            int index = 0;
            int hashCode = 0;
            long size = 0L;
            for (final Multiset.Entry<? extends E> entry : entries) {
                final E element = Preconditions.<E>checkNotNull((E)entry.getElement());
    ...
    static {
        EMPTY = new RegularImmutableMultiset<>((java.util.Collection<? extends Multiset.Entry<?>>)ImmutableList.of());
    }
