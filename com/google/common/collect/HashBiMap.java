...
public final class HashBiMap<K, V> extends Maps.IteratorBasedAbstractMap<K, V> implements BiMap<K, V>, Serializable {
    ...
    public static <K, V> HashBiMap<K, V> create() {
        return HashBiMap.<K, V>create(16);
    }
    ...
    public static <K, V> HashBiMap<K, V> create(final Map<? extends K, ? extends V> map) {
        final HashBiMap<K, V> bimap = HashBiMap.<K, V>create(map.size());
        bimap.putAll((java.util.Map<?, ?>)map);
        return bimap;
    }
    ...
    public V get(@Nullable final Object key) {
        return Maps.<V>valueOrNull((Map.Entry<?, V>)this.seekByKey(key, Hashing.smearedHash(key)));
    }
    ...
    private V put(@Nullable final K key, @Nullable final V value, final boolean force) {
        ...
        if (oldEntryForValue != null) {
            if (!force) {
                throw new IllegalArgumentException(new StringBuilder().append("value already present: ").append(value).toString());
            }
    ...
    private K putInverse(@Nullable final V value, @Nullable final K key, final boolean force) {
        ...
        if (oldEntryForKey != null) {
            if (!force) {
                throw new IllegalArgumentException(new StringBuilder().append("value already present: ").append(key).toString());
            }
            this.delete(oldEntryForKey);
        }
        if (oldEntryForValue != null) {
        ...
        this.rehashIfNecessary();
        return Maps.<K>keyOrNull((Map.Entry<K, ?>)oldEntryForValue);
    }
    ...
    Iterator<Map.Entry<K, V>> entryIterator() {
        return (Iterator<Map.Entry<K, V>>)new Itr<Map.Entry<K, V>>() {
            ...
            Map.Entry<K, V> output(final BiEntry<K, V> entry) {
                return (Map.Entry<K, V>)new MapEntry(entry);
            }
    ...
    private void readObject(final ObjectInputStream stream) throws IOException, ClassNotFoundException {
        ...
        Serialization.populateMap((java.util.Map<Object, Object>)this, stream, size);
    }
    ...
    abstract class Itr<T> implements Iterator<T> {
        ...
        abstract T output(final BiEntry<K, V> biEntry);
    }
    
    private final class KeySet extends Maps.KeySet<K, V> {
        KeySet() {
            super((Map)HashBiMap.this);
        }
        
        @Override
        public Iterator<K> iterator() {
            return (Iterator<K>)new Itr<K>() {
                @Override
    ...
    private final class Inverse extends Maps.IteratorBasedAbstractMap<V, K> implements BiMap<V, K>, Serializable {
        ...
        public K get(@Nullable final Object value) {
            return Maps.<K>keyOrNull((Map.Entry<K, ?>)HashBiMap.this.seekByValue(value, Hashing.smearedHash(value)));
        }
        ...
        @Override
        public Set<K> values() {
            return (Set<K>)this.forward().keySet();
        }
        
        @Override
        Iterator<Map.Entry<V, K>> entryIterator() {
            return (Iterator<Map.Entry<V, K>>)new Itr<Map.Entry<V, K>>() {
                @Override
                Map.Entry<V, K> output(final BiEntry<K, V> entry) {
                    return (Map.Entry<V, K>)new InverseEntry(entry);
                }
        ...
        public void forEach(final BiConsumer<? super V, ? super K> action) {
            Preconditions.<BiConsumer<? super V, ? super K>>checkNotNull(action);
            HashBiMap.this.forEach((k, v) -> action.accept(v, k));
        }
        
        public void replaceAll(final BiFunction<? super V, ? super K, ? extends K> function) {
            Preconditions.<BiFunction<? super V, ? super K, ? extends K>>checkNotNull(function);
