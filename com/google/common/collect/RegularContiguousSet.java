...
final class RegularContiguousSet<C extends Comparable> extends ContiguousSet<C> {
    ...
    ContiguousSet<C> headSetImpl(final C toElement, final boolean inclusive) {
        return this.intersectionInCurrentDomain(Range.<C>upTo(toElement, BoundType.forBoolean(inclusive)));
    }
    ...
    ContiguousSet<C> subSetImpl(final C fromElement, final boolean fromInclusive, final C toElement, final boolean toInclusive) {
        ...
        return this.intersectionInCurrentDomain(Range.<C>range(fromElement, BoundType.forBoolean(fromInclusive), toElement, BoundType.forBoolean(toInclusive)));
    }
    
    @Override
    ContiguousSet<C> tailSetImpl(final C fromElement, final boolean inclusive) {
        return this.intersectionInCurrentDomain(Range.<C>downTo(fromElement, BoundType.forBoolean(inclusive)));
    }
    ...
    public ContiguousSet<C> intersection(final ContiguousSet<C> other) {
        Preconditions.<ContiguousSet<C>>checkNotNull(other);
        Preconditions.checkArgument(this.domain.equals(other.domain));
        if (other.isEmpty()) {
            return other;
        }
        final C lowerEndpoint = Ordering.<Comparable>natural().<C>max(this.first(), other.first());
        final C upperEndpoint = Ordering.<Comparable>natural().<C>min(this.last(), other.last());
        return (lowerEndpoint.compareTo(upperEndpoint) <= 0) ? ContiguousSet.<C>create(Range.closed(lowerEndpoint, (C)upperEndpoint), this.domain) : new EmptyContiguousSet<C>(this.domain);
    }
    ...
    public Range<C> range(final BoundType lowerBoundType, final BoundType upperBoundType) {
        return Range.<C>create(this.range.lowerBound.withLowerBoundType(lowerBoundType, this.domain), this.range.upperBound.withUpperBoundType(upperBoundType, this.domain));
    }
    ...
    public boolean equals(@Nullable final Object object) {
        ...
        if (object instanceof RegularContiguousSet) {
            final RegularContiguousSet<?> that = object;
