...
public final class ObjectArrays {
    ...
    public static <T> T[] newArray(final T[] reference, final int length) {
        return Platform.<T>newArray(reference, length);
    }
    
    @GwtIncompatible
    public static <T> T[] concat(final T[] first, final T[] second, final Class<T> type) {
        final T[] result = ObjectArrays.<T>newArray(type, first.length + second.length);
    ...
    public static <T> T[] concat(@Nullable final T element, final T[] array) {
        final T[] result = ObjectArrays.<T>newArray(array, array.length + 1);
        result[0] = element;
    ...
    public static <T> T[] concat(final T[] array, @Nullable final T element) {
        final T[] result = (T[])Arrays.copyOf((Object[])array, array.length + 1);
    ...
    static <T> T[] toArrayImpl(final Collection<?> c, T[] array) {
        ...
        if (array.length < size) {
            array = ObjectArrays.<T>newArray(array, size);
        }
        fillArray(c, array);
        if (array.length > size) {
            array[size] = null;
    ...
    static <T> T[] toArrayImpl(final Object[] src, final int offset, final int len, T[] dst) {
        ...
        if (dst.length < len) {
            dst = ObjectArrays.<T>newArray(dst, len);
        }
    ...
    static Object checkElementNotNull(final Object element, final int index) {
        if (element == null) {
            throw new NullPointerException(new StringBuilder().append("at index ").append(index).toString());
        }
        return element;
    }
