...
abstract class AbstractBiMap<K, V> extends ForwardingMap<K, V> implements BiMap<K, V>, Serializable {
    ...
    private V putInBothMaps(@Nullable final K key, @Nullable final V value, final boolean force) {
        ...
        final V oldValue = (V)this.delegate.put(key, value);
    ...
    private V removeFromBothMaps(final Object key) {
        final V oldValue = (V)this.delegate.remove(key);
        this.removeFromInverseMap(oldValue);
        return oldValue;
    }
    ...
    public void replaceAll(final BiFunction<? super K, ? super V, ? extends V> function) {
        ...
        final Iterator<Map.Entry<K, V>> itr = (Iterator<Map.Entry<K, V>>)this.delegate.entrySet().iterator();
        while (itr.hasNext()) {
            final Map.Entry<K, V> entry = (Map.Entry<K, V>)itr.next();
            final K k = (K)entry.getKey();
            final V v = (V)entry.getValue();
            final K conflict = (K)this.inverse.delegate.putIfAbsent(v, k);
            if (conflict != null) {
                broken = entry;
                itr.remove();
            }
        ...
        if (broken != null) {
            throw new IllegalArgumentException(new StringBuilder().append("value already present: ").append(broken.getValue()).toString());
        }
    }
    ...
    public Set<K> keySet() {
        ...
        return (result == null) ? (this.keySet = (Set<K>)new KeySet()) : result;
    }
    
    @Override
    public Set<V> values() {
        final Set<V> result = this.valueSet;
        return (result == null) ? (this.valueSet = (Set<V>)new ValueSet()) : result;
    }
    
    @Override
    public Set<Map.Entry<K, V>> entrySet() {
        final Set<Map.Entry<K, V>> result = this.entrySet;
        return (result == null) ? (this.entrySet = (Set<Map.Entry<K, V>>)new EntrySet()) : result;
    }
    
    Iterator<Map.Entry<K, V>> entrySetIterator() {
        final Iterator<Map.Entry<K, V>> iterator = (Iterator<Map.Entry<K, V>>)this.delegate.entrySet().iterator();
        return (Iterator<Map.Entry<K, V>>)new Iterator<Map.Entry<K, V>>() {
            ...
            public Map.Entry<K, V> next() {
                this.entry = (Map.Entry<K, V>)iterator.next();
                return (Map.Entry<K, V>)new BiMapEntry(this.entry);
            }
            
            public void remove() {
                CollectPreconditions.checkRemove(this.entry != null);
                final V value = (V)this.entry.getValue();
                iterator.remove();
    ...
    private class KeySet extends ForwardingSet<K> {
        ...
        protected Set<K> delegate() {
            return (Set<K>)AbstractBiMap.this.delegate.keySet();
        }
        ...
        public Iterator<K> iterator() {
            return Maps.<K, Object>keyIterator((java.util.Iterator<Map.Entry<K, Object>>)AbstractBiMap.this.entrySet().iterator());
        }
    }
    ...
    private class ValueSet extends ForwardingSet<V> {
        ...
        public Iterator<V> iterator() {
            return Maps.<Object, V>valueIterator((java.util.Iterator<Map.Entry<Object, V>>)AbstractBiMap.this.entrySet().iterator());
        }
    ...
    class BiMapEntry extends ForwardingMapEntry<K, V> {
        ...
        public V setValue(final V value) {
            ...
            final V oldValue = (V)this.delegate.setValue(value);
            Preconditions.checkState(Objects.equal(value, AbstractBiMap.this.get(((ForwardingMapEntry<Object, V>)this).getKey())), "entry no longer in map");
    ...
    private class EntrySet extends ForwardingSet<Map.Entry<K, V>> {
        ...
        private EntrySet() {
            this.esDelegate = (Set<Map.Entry<K, V>>)AbstractBiMap.this.delegate.entrySet();
        }
