...
public class LinkedListMultimap<K, V> extends AbstractMultimap<K, V> implements ListMultimap<K, V>, Serializable {
    ...
    private LinkedListMultimap(final int expectedKeys) {
        this.keyToKeyList = (Map<K, KeyList<K, V>>)new HashMap(expectedKeys);
    }
    ...
    private Node<K, V> addNode(@Nullable final K key, @Nullable final V value, @Nullable final Node<K, V> nextSibling) {
        ...
        if (this.head == null) {
            ...
            this.tail = node2;
            this.head = node2;
            this.keyToKeyList.put(key, new KeyList((Node<Object, Object>)node));
            ++this.modCount;
        }
            ...
            this.tail = node;
            KeyList<K, V> keyList = (KeyList<K, V>)this.keyToKeyList.get(key);
            if (keyList == null) {
            ...
            final KeyList list2;
            final KeyList<K, V> keyList = (KeyList<K, V>)(list2 = (KeyList)this.keyToKeyList.get(key));
            ++list2.count;
            ...
            if (nextSibling.previousSibling == null) {
                ((KeyList)this.keyToKeyList.get(key)).head = (Node<K, V>)node;
            }
            else {
    ...
    private void removeNode(final Node<K, V> node) {
        ...
        if (node.previousSibling == null && node.nextSibling == null) {
            final KeyList<K, V> keyList = (KeyList<K, V>)this.keyToKeyList.remove(node.key);
            keyList.count = 0;
            ++this.modCount;
        }
        else {
            final KeyList list;
            final KeyList<K, V> keyList = (KeyList<K, V>)(list = (KeyList)this.keyToKeyList.get(node.key));
            --list.count;
    ...
    public List<V> replaceValues(@Nullable final K key, final Iterable<? extends V> values) {
        ...
        final ListIterator<V> keyValues = (ListIterator<V>)new ValueForKeyIterator(key);
    ...
    private List<V> getCopy(@Nullable final Object key) {
        return (List<V>)Collections.unmodifiableList((List)Lists.newArrayList((java.util.Iterator<?>)new ValueForKeyIterator(key)));
    }
    ...
    public List<V> get(@Nullable final K key) {
        return (List<V>)new AbstractSequentialList<V>() {
            public int size() {
                final KeyList<K, V> keyList = (KeyList<K, V>)LinkedListMultimap.this.keyToKeyList.get(key);
                return (keyList == null) ? 0 : keyList.count;
            }
            
            public ListIterator<V> listIterator(final int index) {
                return (ListIterator<V>)new ValueForKeyIterator(key, index);
            }
        };
    }
    ...
    Set<K> createKeySet() {
        class 1KeySetImpl extends Sets.ImprovedAbstractSet<K> {
            ...
            public Iterator<K> iterator() {
                return (Iterator<K>)new DistinctKeyIterator();
            }
    ...
    List<V> createValues() {
        class 1ValuesImpl extends AbstractSequentialList<V> {
            ...
            public ListIterator<V> listIterator(final int index) {
                ...
                return (ListIterator<V>)new TransformedListIterator<Map.Entry<K, V>, V>(nodeItr) {
                    @Override
    ...
    List<Map.Entry<K, V>> createEntries() {
        class 1EntriesImpl extends AbstractSequentialList<Map.Entry<K, V>> {
            public int size() {
                return LinkedListMultimap.this.size;
            }
            
            public ListIterator<Map.Entry<K, V>> listIterator(final int index) {
                return (ListIterator<Map.Entry<K, V>>)new NodeIterator(index);
            }
            
            public void forEach(final Consumer<? super Map.Entry<K, V>> action) {
                Preconditions.<Consumer<? super Map.Entry<K, V>>>checkNotNull(action);
                for (Node<K, V> node = LinkedListMultimap.this.head; node != null; node = node.next) {
                    action.accept(node);
                }
            }
        }
        return (List<Map.Entry<K, V>>)new 1EntriesImpl();
    }
