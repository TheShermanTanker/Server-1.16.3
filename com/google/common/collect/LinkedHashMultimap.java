...
public final class LinkedHashMultimap<K, V> extends AbstractSetMultimap<K, V> {
    ...
    public static <K, V> LinkedHashMultimap<K, V> create(final Multimap<? extends K, ? extends V> multimap) {
        final LinkedHashMultimap<K, V> result = LinkedHashMultimap.<K, V>create(multimap.keySet().size(), 2);
    ...
    private static <K, V> void deleteFromValueSet(final ValueSetLink<K, V> entry) {
        LinkedHashMultimap.<K, V>succeedsInValueSet(entry.getPredecessorInValueSet(), entry.getSuccessorInValueSet());
    }
    
    private static <K, V> void deleteFromMultimap(final ValueEntry<K, V> entry) {
        LinkedHashMultimap.<K, V>succeedsInMultimap(entry.getPredecessorInMultimap(), entry.getSuccessorInMultimap());
    }
    
    private LinkedHashMultimap(final int keyCapacity, final int valueSetCapacity) {
        super((Map)new LinkedHashMap(keyCapacity));
        this.valueSetCapacity = 2;
        CollectPreconditions.checkNonnegative(valueSetCapacity, "expectedValuesPerKey");
        this.valueSetCapacity = valueSetCapacity;
        LinkedHashMultimap.<K, V>succeedsInMultimap(this.multimapHeaderEntry = new ValueEntry<K, V>(null, null, 0, null), this.multimapHeaderEntry);
    }
    
    @Override
    Set<V> createCollection() {
        return (Set<V>)new LinkedHashSet(this.valueSetCapacity);
    }
    
    @Override
    Collection<V> createCollection(final K key) {
        return (Collection<V>)new ValueSet(key, this.valueSetCapacity);
    }
    ...
    Spliterator<Map.Entry<K, V>> entrySpliterator() {
        return (Spliterator<Map.Entry<K, V>>)Spliterators.spliterator((Collection)this.entries(), 17);
    }
    
    @Override
    Iterator<V> valueIterator() {
        return Maps.<Object, V>valueIterator(this.entryIterator());
    }
    
    @Override
    Spliterator<V> valueSpliterator() {
        return CollectSpliterators.<Map.Entry<K, V>, V>map(this.entrySpliterator(), (java.util.function.Function<? super Map.Entry<K, V>, ? extends V>)Map.Entry::getValue);
    }
    
    @Override
    public void clear() {
        super.clear();
        LinkedHashMultimap.<K, V>succeedsInMultimap(this.multimapHeaderEntry, this.multimapHeaderEntry);
    }
    ...
    private void readObject(final ObjectInputStream stream) throws IOException, ClassNotFoundException {
        ...
        final Map<K, Collection<V>> map = (Map<K, Collection<V>>)new LinkedHashMap();
        ...
        for (int entries = stream.readInt(), j = 0; j < entries; ++j) {
            ...
            ((Collection)map.get(key2)).add(value);
        }
        this.setMap(map);
    }
    ...
    final class ValueSet extends Sets.ImprovedAbstractSet<V> implements ValueSetLink<K, V> {
        ...
        ValueSet(final K key, final int expectedValues) {
            ...
            final ValueEntry<K, V>[] hashTable = new ValueEntry[tableSize];
        ...
        public boolean add(@Nullable final V value) {
            ...
            LinkedHashMultimap.succeedsInValueSet(this.lastEntry, (ValueSetLink<Object, Object>)newEntry);
            LinkedHashMultimap.succeedsInValueSet(newEntry, (ValueSetLink<Object, Object>)this);
            LinkedHashMultimap.succeedsInMultimap(LinkedHashMultimap.this.multimapHeaderEntry.getPredecessorInMultimap(), (ValueEntry<Object, Object>)newEntry);
            LinkedHashMultimap.succeedsInMultimap(newEntry, (ValueEntry<Object, Object>)LinkedHashMultimap.this.multimapHeaderEntry);
        ...
        private void rehashIfNecessary() {
            if (Hashing.needsResizing(this.size, this.hashTable.length, 1.0)) {
                final ValueEntry<K, V>[] hashTable = new ValueEntry[this.hashTable.length * 2];
                this.hashTable = hashTable;
        ...
        public boolean remove(@Nullable final Object o) {
            ...
            for (ValueEntry<K, V> entry = this.hashTable[bucket]; entry != null; entry = entry.nextInValueBucket) {
                if (entry.matchesValue(o, smearedHash)) {
                    ...
                    LinkedHashMultimap.deleteFromValueSet((ValueSetLink<Object, Object>)entry);
                    LinkedHashMultimap.deleteFromMultimap((ValueEntry<Object, Object>)entry);
                    --this.size;
                    ++this.modCount;
                    return true;
                }
        ...
        public void clear() {
            ...
            for (ValueSetLink<K, V> entry = this.firstEntry; entry != this; entry = entry.getSuccessorInValueSet()) {
                final ValueEntry<K, V> valueEntry = (ValueEntry<K, V>)(ValueEntry)entry;
                LinkedHashMultimap.deleteFromMultimap((ValueEntry<Object, Object>)valueEntry);
            }
            LinkedHashMultimap.succeedsInValueSet(this, (ValueSetLink<Object, Object>)this);
            ++this.modCount;
        }
    }
    ...
    private interface ValueSetLink<K, V> {
        ...
        void setPredecessorInValueSet(final ValueSetLink<K, V> valueSetLink);
        
        void setSuccessorInValueSet(final ValueSetLink<K, V> valueSetLink);
    }
}
