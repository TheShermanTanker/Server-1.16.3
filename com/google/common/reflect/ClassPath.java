...
public final class ClassPath {
    ...
    public ImmutableSet<ClassInfo> getAllClasses() {
        return FluentIterable.from((java.lang.Iterable<Object>)this.resources).<ClassInfo>filter(ClassInfo.class).toSet();
    }
    
    public ImmutableSet<ClassInfo> getTopLevelClasses() {
        return FluentIterable.from((java.lang.Iterable<Object>)this.resources).<ClassInfo>filter(ClassInfo.class).filter(ClassPath.IS_TOP_LEVEL).toSet();
    }
    
    public ImmutableSet<ClassInfo> getTopLevelClasses(final String packageName) {
        Preconditions.<String>checkNotNull(packageName);
        final ImmutableSet.Builder<ClassInfo> builder = ImmutableSet.<ClassInfo>builder();
    ...
    public static class ResourceInfo {
        ...
        ResourceInfo(final String resourceName, final ClassLoader loader) {
            this.resourceName = Preconditions.<String>checkNotNull(resourceName);
            this.loader = Preconditions.<ClassLoader>checkNotNull(loader);
        }
    ...
    abstract static class Scanner {
        ...
        protected abstract void scanDirectory(final ClassLoader classLoader, final File file) throws IOException;
        
        protected abstract void scanJarFile(final ClassLoader classLoader, final JarFile jarFile) throws IOException;
        ...
        private void scanFrom(final File file, final ClassLoader classloader) throws IOException {
            try {
                ...
                ClassPath.logger.warning(new StringBuilder().append("Cannot access ").append(file).append(": ").append(e).toString());
                return;
            }
            if (file.isDirectory()) {
        ...
        static ImmutableSet<File> getClassPathFromManifest(final File jarFile, @Nullable final Manifest manifest) {
            if (manifest == null) {
                return ImmutableSet.<File>of();
            }
            final ImmutableSet.Builder<File> builder = ImmutableSet.<File>builder();
            final String classpathAttribute = manifest.getMainAttributes().getValue(Attributes.Name.CLASS_PATH.toString());
            if (classpathAttribute != null) {
                for (final String path : ClassPath.CLASS_PATH_ATTRIBUTE_SEPARATOR.split((CharSequence)classpathAttribute)) {
                    URL url;
                    try {
        ...
        static ImmutableMap<File, ClassLoader> getClassPathEntries(final ClassLoader classloader) {
            final LinkedHashMap<File, ClassLoader> entries = Maps.<File, ClassLoader>newLinkedHashMap();
            ...
            return ImmutableMap.<File, ClassLoader>copyOf((java.util.Map<? extends File, ? extends ClassLoader>)entries);
        }
    ...
    static final class DefaultScanner extends Scanner {
        ...
        DefaultScanner() {
            this.resources = MultimapBuilder.hashKeys().linkedHashSetValues().<ClassLoader, String>build();
        }
        
        ImmutableSet<ResourceInfo> getResources() {
            final ImmutableSet.Builder<ResourceInfo> builder = ImmutableSet.<ResourceInfo>builder();
        ...
        protected void scanJarFile(final ClassLoader classloader, final JarFile file) {
            final Enumeration<JarEntry> entries = (Enumeration<JarEntry>)file.entries();
        ...
        private void scanDirectory(final File directory, final ClassLoader classloader, final String packagePrefix) throws IOException {
            ...
            if (files == null) {
                ClassPath.logger.warning(new StringBuilder().append("Cannot read directory ").append(directory).toString());
                return;
            }
