...
public class EventBus {
    ...
    EventBus(final String identifier, final Executor executor, final Dispatcher dispatcher, final SubscriberExceptionHandler exceptionHandler) {
        ...
        this.identifier = Preconditions.<String>checkNotNull(identifier);
        this.executor = Preconditions.<Executor>checkNotNull(executor);
        this.dispatcher = Preconditions.<Dispatcher>checkNotNull(dispatcher);
        this.exceptionHandler = Preconditions.<SubscriberExceptionHandler>checkNotNull(exceptionHandler);
    }
    ...
    void handleSubscriberException(final Throwable e, final SubscriberExceptionContext context) {
        ...
        Preconditions.<SubscriberExceptionContext>checkNotNull(context);
        try {
            this.exceptionHandler.handleException(e, context);
        }
        catch (Throwable e2) {
            EventBus.logger.log(Level.SEVERE, String.format(Locale.ROOT, "Exception %s thrown while handling exception: %s", new Object[] { e2, e }), e2);
        }
    }
