...
public class TypeToken<T> {
    ...
    public boolean isAssignableFrom(final Type from) {
        ...
        if (this.type instanceof Class) {
            return this.rawType.isAssignableFrom((Class)$Gson$Types.getRawType(from));
        }
        if (this.type instanceof ParameterizedType) {
            return isAssignableFrom(from, (ParameterizedType)this.type, (Map<String, Type>)new HashMap());
        }
        if (this.type instanceof GenericArrayType) {
            return this.rawType.isAssignableFrom((Class)$Gson$Types.getRawType(from)) && isAssignableFrom(from, (GenericArrayType)this.type);
        }
    ...
    private static boolean isAssignableFrom(final Type from, final GenericArrayType to) {
        ...
        if (toGenericComponentType instanceof ParameterizedType) {
            ...
            if (from instanceof GenericArrayType) {
                ...
                Class<?> classType;
                for (classType = from; classType.isArray(); classType = classType.getComponentType()) {}
                t = (Type)classType;
            }
    ...
    public static TypeToken<?> getParameterized(final Type rawType, final Type... typeArguments) {
        return new TypeToken<>((Type)$Gson$Types.newParameterizedTypeWithOwner(null, rawType, typeArguments));
    }
    
    public static TypeToken<?> getArray(final Type componentType) {
        return new TypeToken<>((Type)$Gson$Types.arrayOf(componentType));
    }
}
