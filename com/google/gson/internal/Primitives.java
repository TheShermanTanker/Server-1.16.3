...
public final class Primitives {
    ...
    public static boolean isWrapperType(final Type type) {
        return Primitives.WRAPPER_TO_PRIMITIVE_TYPE.containsKey($Gson$Preconditions.<Type>checkNotNull(type));
    }
    
    public static <T> Class<T> wrap(final Class<T> type) {
        final Class<T> wrapped = (Class<T>)Primitives.PRIMITIVE_TO_WRAPPER_TYPE.get($Gson$Preconditions.<Class<T>>checkNotNull(type));
        return (wrapped == null) ? type : wrapped;
    }
    
    public static <T> Class<T> unwrap(final Class<T> type) {
        final Class<T> unwrapped = (Class<T>)Primitives.WRAPPER_TO_PRIMITIVE_TYPE.get($Gson$Preconditions.<Class<T>>checkNotNull(type));
