...
public final class JsonTreeReader extends JsonReader {
    ...
    private void expect(final JsonToken expected) throws IOException {
        if (this.peek() != expected) {
            throw new IllegalStateException(new StringBuilder().append("Expected ").append(expected).append(" but was ").append(this.peek()).append(this.locationString()).toString());
        }
    }
    ...
    public String nextName() throws IOException {
        ...
        final Iterator<?> i = this.peekStack();
    ...
    public String nextString() throws IOException {
        ...
        if (token != JsonToken.STRING && token != JsonToken.NUMBER) {
            throw new IllegalStateException(new StringBuilder().append("Expected ").append(JsonToken.STRING).append(" but was ").append(token).append(this.locationString()).toString());
        }
    ...
    public double nextDouble() throws IOException {
        ...
        if (token != JsonToken.NUMBER && token != JsonToken.STRING) {
            throw new IllegalStateException(new StringBuilder().append("Expected ").append(JsonToken.NUMBER).append(" but was ").append(token).append(this.locationString()).toString());
        }
        ...
        if (!this.isLenient() && (Double.isNaN(result) || Double.isInfinite(result))) {
            throw new NumberFormatException(new StringBuilder().append("JSON forbids NaN and infinities: ").append(result).toString());
        }
        this.popStack();
        if (this.stackSize > 0) {
            final int[] pathIndices = this.pathIndices;
