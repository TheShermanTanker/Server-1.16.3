...
public final class ReflectiveTypeAdapterFactory implements TypeAdapterFactory {
    ...
    private List<String> getFieldNames(final Field f) {
        ...
        if (annotation == null) {
            ...
            return (List<String>)Collections.singletonList(name);
        }
        final String serializedName = annotation.value();
        final String[] alternates = annotation.alternate();
        if (alternates.length == 0) {
            return (List<String>)Collections.singletonList(serializedName);
        }
    ...
    public <T> TypeAdapter<T> create(final Gson gson, final TypeToken<T> type) {
        ...
        if (!Object.class.isAssignableFrom((Class)raw)) {
            return null;
        }
        final ObjectConstructor<T> constructor = this.constructorConstructor.<T>get(type);
            ...
            for (final Field field : declaredFields) {
                ...
                if (serialize || deserialize) {
                    ...
                    for (int i = 0; i < fieldNames.size(); ++i) {
                        ...
                        final BoundField replaced = (BoundField)result.put(name, boundField);
                        if (previous == null) {
                            previous = replaced;
                        }
                    }
                    if (previous != null) {
                        throw new IllegalArgumentException(new StringBuilder().append(declaredType).append(" declares multiple JSON fields named ").append(previous.name).toString());
                    }
                }
            }
    ...
    abstract static class BoundField {
        ...
        abstract boolean writeField(final Object object) throws IOException, IllegalAccessException;
        
        abstract void write(final JsonWriter jsonWriter, final Object object) throws IOException, IllegalAccessException;
        
        abstract void read(final JsonReader jsonReader, final Object object) throws IOException, IllegalAccessException;
    }
        ...
        public T read(final JsonReader in) throws IOException {
            ...
            try {
                ...
                while (in.hasNext()) {
                    ...
                    final BoundField field = (BoundField)this.boundFields.get(name);
