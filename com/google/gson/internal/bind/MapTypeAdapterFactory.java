...
public final class MapTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(final Gson gson, final TypeToken<T> typeToken) {
        ...
        final ObjectConstructor<T> constructor = this.constructorConstructor.<T>get(typeToken);
        final TypeAdapter<T> result = (TypeAdapter<T>)new Adapter(gson, keyAndValueTypes[0], (TypeAdapter<Object>)keyAdapter, keyAndValueTypes[1], (TypeAdapter<Object>)valueAdapter, constructor);
        return result;
    }
    ...
    private final class Adapter<K, V> extends TypeAdapter<Map<K, V>> {
        ...
        public Map<K, V> read(final JsonReader in) throws IOException {
            ...
            if (peek == JsonToken.BEGIN_ARRAY) {
                ...
                while (in.hasNext()) {
                    ...
                    final V replaced = (V)map.put(key, value);
                    if (replaced != null) {
                        throw new JsonSyntaxException(new StringBuilder().append("duplicate key: ").append(key).toString());
                    }
                    in.endArray();
                }
                in.endArray();
        ...
        public void write(final JsonWriter out, final Map<K, V> map) throws IOException {
            ...
            if (hasComplexKeys) {
                ...
                out.beginObject();
                for (int i = 0; i < keys.size(); ++i) {
                    final JsonElement keyElement2 = (JsonElement)keys.get(i);
