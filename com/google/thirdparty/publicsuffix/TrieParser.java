...
final class TrieParser {
    ...
    static ImmutableMap<String, PublicSuffixType> parseTrie(final CharSequence encoded) {
        final ImmutableMap.Builder<String, PublicSuffixType> builder = ImmutableMap.<String, PublicSuffixType>builder();
        for (int encodedLen = encoded.length(), idx = 0; idx < encodedLen; idx += doParseTrieToBuilder(Lists.newLinkedList(), encoded.subSequence(idx, encodedLen), builder)) {}
    ...
    private static CharSequence reverse(final CharSequence s) {
        return (CharSequence)new StringBuilder(s).reverse();
    }
