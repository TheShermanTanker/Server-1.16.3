...
public class LocateCommand {
    ...
    public static void register(final CommandDispatcher<CommandSourceStack> commandDispatcher) {
        LiteralArgumentBuilder<CommandSourceStack> literalArgumentBuilder2 = Commands.literal("locate").requires((java.util.function.Predicate<CommandSourceStack>)(db -> db.hasPermission(2)));
        for (final Map.Entry<String, StructureFeature<?>> entry4 : StructureFeature.STRUCTURES_REGISTRY.entrySet()) {
            literalArgumentBuilder2 = literalArgumentBuilder2.then(Commands.literal((String)entry4.getKey()).executes(commandContext -> locate(commandContext.getSource(), entry4.getValue())));
        }
    ...
    static {
        ERROR_FAILED = new SimpleCommandExceptionType(new TranslatableComponent("commands.locate.failed"));
    }
}
