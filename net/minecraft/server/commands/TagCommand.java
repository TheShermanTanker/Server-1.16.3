...
public class TagCommand {
    ...
    public static void register(final CommandDispatcher<CommandSourceStack> commandDispatcher) {
        commandDispatcher.register((LiteralArgumentBuilder<CommandSourceStack>)Commands.literal("tag").requires((java.util.function.Predicate<CommandSourceStack>)(db -> db.hasPermission(2))).then(((RequiredArgumentBuilder)Commands.argument("targets", (ArgumentType<Object>)EntityArgument.entities()).then(Commands.literal("add").then(Commands.argument("name", (ArgumentType<Object>)StringArgumentType.word()).executes(commandContext -> addTag(commandContext.getSource(), EntityArgument.getEntities(commandContext, "targets"), StringArgumentType.getString(commandContext, "name"))))).then(Commands.literal("remove").then(Commands.argument("name", (ArgumentType<Object>)StringArgumentType.word()).suggests((commandContext, suggestionsBuilder) -> SharedSuggestionProvider.suggest((Iterable<String>)getTags(EntityArgument.getEntities(commandContext, "targets")), suggestionsBuilder)).executes(commandContext -> removeTag(commandContext.getSource(), EntityArgument.getEntities(commandContext, "targets"), StringArgumentType.getString(commandContext, "name")))))).then(Commands.literal("list").executes(commandContext -> listTags(commandContext.getSource(), EntityArgument.getEntities(commandContext, "targets"))))));
    }
    ...
    static {
        ERROR_ADD_FAILED = new SimpleCommandExceptionType(new TranslatableComponent("commands.tag.add.failed"));
        ERROR_REMOVE_FAILED = new SimpleCommandExceptionType(new TranslatableComponent("commands.tag.remove.failed"));
    }
}
