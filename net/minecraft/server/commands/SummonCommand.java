...
public class SummonCommand {
    ...
    public static void register(final CommandDispatcher<CommandSourceStack> commandDispatcher) {
        commandDispatcher.register((LiteralArgumentBuilder<CommandSourceStack>)Commands.literal("summon").requires((java.util.function.Predicate<CommandSourceStack>)(db -> db.hasPermission(2))).then(Commands.argument("entity", (ArgumentType<Object>)EntitySummonArgument.id()).suggests(SuggestionProviders.SUMMONABLE_ENTITIES).executes(commandContext -> spawnEntity(commandContext.getSource(), EntitySummonArgument.getSummonableEntity(commandContext, "entity"), commandContext.getSource().getPosition(), new CompoundTag(), true)).then(Commands.argument("pos", (ArgumentType<Object>)Vec3Argument.vec3()).executes(commandContext -> spawnEntity(commandContext.getSource(), EntitySummonArgument.getSummonableEntity(commandContext, "entity"), Vec3Argument.getVec3(commandContext, "pos"), new CompoundTag(), true)).then(Commands.argument("nbt", (ArgumentType<Object>)CompoundTagArgument.compoundTag()).executes(commandContext -> spawnEntity(commandContext.getSource(), EntitySummonArgument.getSummonableEntity((CommandContext<CommandSourceStack>)commandContext, "entity"), Vec3Argument.getVec3((CommandContext<CommandSourceStack>)commandContext, "pos"), CompoundTagArgument.getCompoundTag(commandContext, "nbt"), false))))));
    }
    ...
    static {
        ERROR_FAILED = new SimpleCommandExceptionType(new TranslatableComponent("commands.summon.failed"));
        ERROR_DUPLICATE_UUID = new SimpleCommandExceptionType(new TranslatableComponent("commands.summon.failed.uuid"));
        INVALID_POSITION = new SimpleCommandExceptionType(new TranslatableComponent("commands.summon.invalidPosition"));
    }
}
