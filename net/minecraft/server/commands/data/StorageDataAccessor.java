...
public class StorageDataAccessor implements DataAccessor {
    ...
    static {
        SUGGEST_STORAGE = ((commandContext, suggestionsBuilder) -> SharedSuggestionProvider.suggestResource(getGlobalTags(commandContext).keys(), suggestionsBuilder));
        PROVIDER = (string -> new DataCommands.DataProvider() {
            ...
            public ArgumentBuilder<CommandSourceStack, ?> wrap(final ArgumentBuilder<CommandSourceStack, ?> argumentBuilder, final Function<ArgumentBuilder<CommandSourceStack, ?>, ArgumentBuilder<CommandSourceStack, ?>> function) {
                return argumentBuilder.then(Commands.literal("storage").then(function.apply(Commands.argument(string, (ArgumentType<Object>)ResourceLocationArgument.id()).suggests(StorageDataAccessor.SUGGEST_STORAGE))));
            }
        });
    }
}
