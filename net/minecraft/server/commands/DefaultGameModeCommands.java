...
public class DefaultGameModeCommands {
    public static void register(final CommandDispatcher<CommandSourceStack> commandDispatcher) {
        final LiteralArgumentBuilder<CommandSourceStack> literalArgumentBuilder2 = Commands.literal("defaultgamemode").requires((java.util.function.Predicate<CommandSourceStack>)(db -> db.hasPermission(2)));
        for (final GameType brr6 : GameType.values()) {
            if (brr6 != GameType.NOT_SET) {
                literalArgumentBuilder2.then(Commands.literal(brr6.getName()).executes(commandContext -> setMode(commandContext.getSource(), brr6)));
            }
        }
