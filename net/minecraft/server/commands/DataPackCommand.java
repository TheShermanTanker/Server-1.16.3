...
public class DataPackCommand {
    ...
    public static void register(final CommandDispatcher<CommandSourceStack> commandDispatcher) {
        commandDispatcher.register((LiteralArgumentBuilder<CommandSourceStack>)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("datapack").requires((java.util.function.Predicate<CommandSourceStack>)(db -> db.hasPermission(2))).then(Commands.literal("enable").then(((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)Commands.argument("name", (ArgumentType<Object>)StringArgumentType.string()).suggests(DataPackCommand.UNSELECTED_PACKS).executes(commandContext -> enablePack(commandContext.getSource(), getPack(commandContext, "name", true), (list, abs) -> abs.getDefaultPosition().<Pack>insert(list, abs, (java.util.function.Function<Pack, Pack>)(abs -> abs), false))).then(Commands.literal("after").then(Commands.argument("existing", (ArgumentType<Object>)StringArgumentType.string()).suggests(DataPackCommand.SELECTED_PACKS).executes(commandContext -> enablePack(commandContext.getSource(), getPack(commandContext, "name", true), (list, abs) -> list.add(list.indexOf(getPack(commandContext, "existing", false)) + 1, abs)))))).then(Commands.literal("before").then(Commands.argument("existing", (ArgumentType<Object>)StringArgumentType.string()).suggests(DataPackCommand.SELECTED_PACKS).executes(commandContext -> enablePack(commandContext.getSource(), getPack(commandContext, "name", true), (list, abs) -> list.add(list.indexOf(getPack(commandContext, "existing", false)), abs)))))).then(Commands.literal("last").executes(commandContext -> enablePack(commandContext.getSource(), getPack(commandContext, "name", true), List::add)))).then(Commands.literal("first").executes(commandContext -> enablePack(commandContext.getSource(), getPack(commandContext, "name", true), (list, abs) -> list.add(0, abs))))))).then(Commands.literal("disable").then(Commands.argument("name", (ArgumentType<Object>)StringArgumentType.string()).suggests(DataPackCommand.SELECTED_PACKS).executes(commandContext -> disablePack(commandContext.getSource(), getPack(commandContext, "name", false)))))).then(((LiteralArgumentBuilder)Commands.literal("list").executes(commandContext -> listPacks(commandContext.getSource())).then(Commands.literal("available").executes(commandContext -> listAvailablePacks(commandContext.getSource())))).then(Commands.literal("enabled").executes(commandContext -> listEnabledPacks(commandContext.getSource())))));
    }
    ...
    private static int enablePack(final CommandSourceStack db, final Pack abs, final Inserter a) throws CommandSyntaxException {
        ...
        final List<Pack> list5 = Lists.newArrayList((java.lang.Iterable<?>)abu4.getSelectedPacks());
        a.apply(list5, abs);
    ...
    private static int disablePack(final CommandSourceStack db, final Pack abs) {
        ...
        final List<Pack> list4 = Lists.newArrayList((java.lang.Iterable<?>)abu3.getSelectedPacks());
        list4.remove(abs);
    ...
    private static Pack getPack(final CommandContext<CommandSourceStack> commandContext, final String string, final boolean boolean3) throws CommandSyntaxException {
        final String string2 = StringArgumentType.getString(commandContext, string);
        final PackRepository abu5 = commandContext.getSource().getServer().getPackRepository();
    ...
    static {
        ERROR_UNKNOWN_PACK = new DynamicCommandExceptionType((Function<Object, Message>)(object -> new TranslatableComponent("commands.datapack.unknown", new Object[] { object })));
        ERROR_PACK_ALREADY_ENABLED = new DynamicCommandExceptionType((Function<Object, Message>)(object -> new TranslatableComponent("commands.datapack.enable.failed", new Object[] { object })));
        ERROR_PACK_ALREADY_DISABLED = new DynamicCommandExceptionType((Function<Object, Message>)(object -> new TranslatableComponent("commands.datapack.disable.failed", new Object[] { object })));
        SELECTED_PACKS = ((commandContext, suggestionsBuilder) -> SharedSuggestionProvider.suggest((Stream<String>)commandContext.getSource().getServer().getPackRepository().getSelectedIds().stream().map(StringArgumentType::escapeIfRequired), suggestionsBuilder));
        final PackRepository abu3;
        final Collection<String> collection4;
        UNSELECTED_PACKS = ((commandContext, suggestionsBuilder) -> {
            abu3 = commandContext.getSource().getServer().getPackRepository();
            collection4 = abu3.getSelectedIds();
