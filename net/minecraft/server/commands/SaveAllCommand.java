...
public class SaveAllCommand {
    ...
    public static void register(final CommandDispatcher<CommandSourceStack> commandDispatcher) {
        commandDispatcher.register((LiteralArgumentBuilder<CommandSourceStack>)((LiteralArgumentBuilder)Commands.literal("save-all").requires((java.util.function.Predicate<CommandSourceStack>)(db -> db.hasPermission(4))).executes(commandContext -> saveAll(commandContext.getSource(), false))).then(Commands.literal("flush").executes(commandContext -> saveAll(commandContext.getSource(), true))));
    }
    ...
    static {
        ERROR_FAILED = new SimpleCommandExceptionType(new TranslatableComponent("commands.save.failed"));
    }
}
