...
public class DebugCommand {
    ...
    public static void register(final CommandDispatcher<CommandSourceStack> commandDispatcher) {
        commandDispatcher.register((LiteralArgumentBuilder<CommandSourceStack>)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("debug").requires((java.util.function.Predicate<CommandSourceStack>)(db -> db.hasPermission(3))).then(Commands.literal("start").executes(commandContext -> start(commandContext.getSource())))).then(Commands.literal("stop").executes(commandContext -> stop(commandContext.getSource())))).then(Commands.literal("report").executes(commandContext -> report(commandContext.getSource()))));
    }
    ...
    private static int report(final CommandSourceStack db) {
        ...
        try {
            ...
            if (SharedConstants.IS_RUNNING_IN_IDE || DebugCommand.ZIP_FS_PROVIDER == null) {
                ...
                try (final FileSystem fileSystem6 = DebugCommand.ZIP_FS_PROVIDER.newFileSystem(path6, (Map)ImmutableMap.<String, String>of("create", "true"))) {
    ...
    static {
        ...
        ERROR_NOT_RUNNING = new SimpleCommandExceptionType(new TranslatableComponent("commands.debug.notRunning"));
        ERROR_ALREADY_RUNNING = new SimpleCommandExceptionType(new TranslatableComponent("commands.debug.alreadyRunning"));
