    ...
    private void load(final ServerAdvancementManager vv) {
        if (this.file.isFile()) {
            try (final JsonReader jsonReader3 = new JsonReader((Reader)new StringReader(Files.toString(this.file, StandardCharsets.UTF_8)))) {
                ...
                Dynamic<JsonElement> dynamic5 = new Dynamic<JsonElement>(JsonOps.INSTANCE, Streams.parse(jsonReader3));
                if (!dynamic5.get("DataVersion").asNumber().result().isPresent()) {
                    dynamic5 = dynamic5.set("DataVersion", dynamic5.createInt(1343));
                }
                dynamic5 = this.dataFixer.<JsonElement>update(DataFixTypes.ADVANCEMENTS.getType(), dynamic5, dynamic5.get("DataVersion").asInt(0), SharedConstants.getCurrentVersion().getWorldVersion());
                dynamic5 = dynamic5.remove("DataVersion");
                final Map<ResourceLocation, AdvancementProgress> map6 = PlayerAdvancements.GSON.<Map<ResourceLocation, AdvancementProgress>>getAdapter(PlayerAdvancements.TYPE_TOKEN).fromJsonTree(dynamic5.getValue());
                if (map6 == null) {
