...
public class ChunkTaskPriorityQueue<T> {
    ...
    protected void resortChunkTasks(final int integer1, final ChunkPos bra, final int integer3) {
        ...
        final List<Optional<T>> list6 = long2ObjectLinkedOpenHashMap5.remove(bra.toLong());
        ...
        if (list6 != null && !list6.isEmpty()) {
            ((Long2ObjectLinkedOpenHashMap)this.taskQueue.get(integer3)).computeIfAbsent(bra.toLong(), long1 -> Lists.newArrayList()).addAll((Collection)list6);
    ...
    protected void submit(final Optional<T> optional, final long long2, final int integer) {
        ((Long2ObjectLinkedOpenHashMap)this.taskQueue.get(integer)).computeIfAbsent(long2, long1 -> Lists.newArrayList()).add(optional);
    ...
    protected void release(final long long1, final boolean boolean2) {
        for (final Long2ObjectLinkedOpenHashMap<List<Optional<T>>> long2ObjectLinkedOpenHashMap6 : this.taskQueue) {
            final List<Optional<T>> list7 = long2ObjectLinkedOpenHashMap6.get(long1);
            if (list7 == null) {
                continue;
            }
            if (boolean2) {
    ...
    public Stream<Either<T, Runnable>> pop() {
        ...
        if (this.firstQueue < ChunkTaskPriorityQueue.PRIORITY_LEVEL_COUNT) {
            ...
            final List<Optional<T>> list6 = long2ObjectLinkedOpenHashMap3.removeFirst();
            while (this.firstQueue < ChunkTaskPriorityQueue.PRIORITY_LEVEL_COUNT && ((Long2ObjectLinkedOpenHashMap)this.taskQueue.get(this.firstQueue)).isEmpty()) {
                ++this.firstQueue;
            }
            return (Stream<Either<T, Runnable>>)list6.stream().map(optional -> (Either)optional.map(Either::left).orElseGet(() -> Either.<Object, Runnable>right(this.acquire(long4))));
        }
        return null;
    }
