...
public class ChunkHolder {
    ...
    public ChunkAccess getLastAvailable() {
        for (int integer2 = ChunkHolder.CHUNK_STATUSES.size() - 1; integer2 >= 0; --integer2) {
            ...
            if (!completableFuture4.isCompletedExceptionally()) {
                final Optional<ChunkAccess> optional5 = ((Either)completableFuture4.getNow(ChunkHolder.UNLOADED_CHUNK)).left();
    ...
    public void blockChanged(final BlockPos fx) {
        ...
        if (this.changedBlocksPerSection[byte4] == null) {
            ...
            this.changedBlocksPerSection[byte4] = new ShortArraySet();
        }
    ...
    private void updateChunkToSave(final CompletableFuture<? extends Either<? extends ChunkAccess, ChunkLoadingFailure>> completableFuture) {
        this.chunkToSave = (CompletableFuture<ChunkAccess>)this.chunkToSave.thenCombine((CompletionStage)completableFuture, (cft, either) -> either.<ChunkAccess>map(cft -> cft, a -> cft));
    }
    ...
    protected void updateFutures(final ChunkMap zs) {
        ...
        if (boolean5) {
            final Either<ChunkAccess, ChunkLoadingFailure> either9 = Either.right(new ChunkLoadingFailure() {
            ...
            if (completableFuture4 != null) {
                ...
                if (optional5.isPresent()) {
                    if (optional5.get() instanceof ProtoChunk) {
                        this.futures.set(integer3, CompletableFuture.completedFuture((Object)Either.<ImposterProtoChunk, Object>left(cgd)));
                    }
                }
            }
        }
        this.updateChunkToSave(CompletableFuture.completedFuture(Either.<LevelChunk, Object>left(cgd.getWrapped())));
    }
    
    static {
        UNLOADED_CHUNK = Either.<ChunkAccess, ChunkLoadingFailure>right(ChunkLoadingFailure.UNLOADED);
        UNLOADED_CHUNK_FUTURE = CompletableFuture.completedFuture(ChunkHolder.UNLOADED_CHUNK);
        UNLOADED_LEVEL_CHUNK = Either.<LevelChunk, ChunkLoadingFailure>right(ChunkLoadingFailure.UNLOADED);
