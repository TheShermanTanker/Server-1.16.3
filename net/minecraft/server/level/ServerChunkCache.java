...
public class ServerChunkCache extends ChunkSource {
    ...
    public ChunkAccess getChunk(final int integer1, final int integer2, final ChunkStatus cfx, final boolean boolean4) {
        ...
        final ChunkAccess cft10 = ((Either)completableFuture9.join()).<ChunkAccess>map(cft -> cft, a -> {
            if (boolean4) {
    ...
    public BlockGetter getChunkForLighting(final int integer1, final int integer2) {
        ...
        while (true) {
            ...
            final Optional<ChunkAccess> optional9 = ((Either)zr6.getFutureIfPresentUnchecked(cfx8).getNow(ChunkHolder.UNLOADED_CHUNK)).left();
    ...
    private void tickChunks() {
        ...
        if (!boolean7) {
            ...
            this.level.getProfiler().pop();
            final List<ChunkHolder> list13 = Lists.newArrayList((java.lang.Iterable<?>)this.chunkMap.getChunks());
            Collections.shuffle((List)list13);
            list13.forEach(zr -> {
                final Optional<LevelChunk> optional9 = ((Either)zr.getTickingChunkFuture().getNow(ChunkHolder.UNLOADED_LEVEL_CHUNK)).left();
                if (!optional9.isPresent()) {
                    return;
                }
                ...
                this.level.getProfiler().pop();
                final Optional<LevelChunk> optional10 = ((Either)zr.getEntityTickingChunkFuture().getNow(ChunkHolder.UNLOADED_LEVEL_CHUNK)).left();
