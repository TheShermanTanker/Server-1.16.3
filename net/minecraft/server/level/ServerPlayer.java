...
public class ServerPlayer extends Player implements ContainerListener {
    ...
    public void readAdditionalSaveData(final CompoundTag md) {
        ...
        if (md.contains("SpawnX", 99) && md.contains("SpawnY", 99) && md.contains("SpawnZ", 99)) {
            ...
            if (md.contains("SpawnDimension")) {
                this.respawnDimension = (ResourceKey<Level>)Level.RESOURCE_KEY_CODEC.<Tag>parse((DynamicOps<Tag>)NbtOps.INSTANCE, md.get("SpawnDimension")).resultOrPartial((Consumer<String>)ServerPlayer.LOGGER::error).orElse(Level.OVERWORLD);
            }
        }
    }
    ...
    public void addAdditionalSaveData(final CompoundTag md) {
        ...
        if (this.respawnPosition != null) {
            ...
            ResourceLocation.CODEC.encodeStart((DynamicOps<Object>)NbtOps.INSTANCE, this.respawnDimension.location()).resultOrPartial((Consumer<String>)ServerPlayer.LOGGER::error).ifPresent(mt -> md.put("SpawnDimension", mt));
        }
    }
    ...
    public void die(final DamageSource aph) {
        ...
        if (boolean3) {
            ...
            this.connection.send(new ClientboundPlayerCombatPacket(this.getCombatTracker(), ClientboundPlayerCombatPacket.Event.ENTITY_DIED, nr4), future -> {
                if (!future.isSuccess()) {
                    integer4 = 256;
                    string5 = component.getString(256);
                    new TranslatableComponent("death.attack.message_too_long", new Object[] { new TextComponent(string5).withStyle(ChatFormatting.YELLOW) });
                    nr5 = translatableComponent;
                    new TranslatableComponent("death.attack.even_more_magic", new Object[] { this.getDisplayName() });
                    nr6 = translatableComponent2.withStyle((UnaryOperator<Style>)(ob -> ob.withHoverEvent(new HoverEvent((HoverEvent.Action<T>)HoverEvent.Action.SHOW_TEXT, (T)nr5))));
                    this.connection.send(new ClientboundPlayerCombatPacket(this.getCombatTracker(), ClientboundPlayerCombatPacket.Event.ENTITY_DIED, nr6));
                }
                return;
            });
    ...
    public Either<BedSleepingProblem, Unit> startSleepInBed(final BlockPos fx) {
        ...
        if (this.isSleeping() || !this.isAlive()) {
            return Either.<BedSleepingProblem, Unit>left(BedSleepingProblem.OTHER_PROBLEM);
        }
        if (!this.level.dimensionType().natural()) {
            return Either.<BedSleepingProblem, Unit>left(BedSleepingProblem.NOT_POSSIBLE_HERE);
        }
        if (!this.bedInRange(fx, gc3)) {
            return Either.<BedSleepingProblem, Unit>left(BedSleepingProblem.TOO_FAR_AWAY);
        }
        if (this.bedBlocked(fx, gc3)) {
            return Either.<BedSleepingProblem, Unit>left(BedSleepingProblem.OBSTRUCTED);
        }
        this.setRespawnPosition(this.level.dimension(), fx, this.yRot, false, true);
        if (this.level.isDay()) {
            return Either.<BedSleepingProblem, Unit>left(BedSleepingProblem.NOT_POSSIBLE_NOW);
        }
        if (!this.isCreative()) {
            ...
            if (!list9.isEmpty()) {
                return Either.<BedSleepingProblem, Unit>left(BedSleepingProblem.NOT_SAFE);
            }
        }
        final Either<BedSleepingProblem, Unit> either4 = super.startSleepInBed(fx).ifRight((java.util.function.Consumer<? super Unit>)(afu -> {
    ...
    public void sendMessage(final Component nr, final ChatType no, final UUID uUID) {
        ...
        this.connection.send(new ClientboundChatPacket(nr, no, uUID), future -> {
            if (!future.isSuccess() && (no == ChatType.GAME_INFO || no == ChatType.SYSTEM)) {
                integer6 = 256;
                string7 = nr.getString(256);
                nr2 = new TextComponent(string7).withStyle(ChatFormatting.YELLOW);
                connection = this.connection;
                // new(net.minecraft.network.protocol.game.ClientboundChatPacket.class)
                new TranslatableComponent("multiplayer.message_not_delivered", new Object[] { nr2 });
                new ClientboundChatPacket(translatableComponent.withStyle(ChatFormatting.RED), ChatType.SYSTEM, uUID);
                connection.send(oj);
            }
        });
    }
