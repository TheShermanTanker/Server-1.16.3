...
public class PackRepository implements AutoCloseable {
    ...
    public PackRepository(final Pack.PackConstructor a, final RepositorySource... arr) {
        ...
        this.constructor = a;
        this.sources = ImmutableSet.copyOf(arr);
    }
    ...
    public void reload() {
        final List<String> list2 = (List<String>)this.selected.stream().map(Pack::getId).collect((Collector)ImmutableList.toImmutableList());
        this.close();
    ...
    private List<Pack> rebuildSelected(final Collection<String> collection) {
        ...
        return ImmutableList.copyOf((java.util.Collection<?>)list3);
    }
    ...
    public Collection<String> getSelectedIds() {
        return (Collection<String>)this.selected.stream().map(Pack::getId).collect((Collector)ImmutableSet.toImmutableSet());
    }
    ...
    public List<PackResources> openAllSelected() {
        return (List<PackResources>)this.selected.stream().map(Pack::open).collect((Collector)ImmutableList.toImmutableList());
    }
}
