...
public abstract class MinecraftServer extends ReentrantBlockableEventLoop<TickTask> implements SnooperPopulator, CommandSource, AutoCloseable {
    ...
    protected void createLevels(final ChunkProgressListener aap) {
        ...
        final List<CustomSpawner> list10 = ImmutableList.of(new PhantomSpawner(), new PatrolSpawner(), new CatSpawner(), new VillageSiege(), new WanderingTraderSpawner(cyj3));
        ...
        for (final Map.Entry<ResourceKey<LevelStem>, LevelStem> entry19 : gi11.entrySet()) {
            ...
            final ServerLevel aag16 = new ServerLevel(this, this.executor, this.storageSource, cxy24, vj21, cha15, aap, cfv13, boolean5, long7, ImmutableList.of(), false);
    ...
    public CompletableFuture<Void> reloadResources(final Collection<String> collection) {
        final CompletableFuture<Void> completableFuture3 = (CompletableFuture<Void>)CompletableFuture.supplyAsync(() -> (ImmutableList)collection.stream().map(this.packRepository::getPack).filter(Objects::nonNull).map(Pack::open).collect((Collector)ImmutableList.toImmutableList()), (Executor)this).thenCompose(immutableList -> ServerResources.loadResources((List<PackResources>)immutableList, this.isDedicatedServer() ? Commands.CommandSelection.DEDICATED : Commands.CommandSelection.INTEGRATED, this.getFunctionCompilationLevel(), this.executor, (Executor)this)).thenAcceptAsync(vz -> {
            this.resources.close();
            this.resources = vz;
    ...
    private static DataPackConfig getSelectedPacks(final PackRepository abu) {
        ...
        final List<String> list3 = ImmutableList.copyOf((java.util.Collection<?>)collection2);
        final List<String> list4 = (List<String>)abu.getAvailableIds().stream().filter(string -> !collection2.contains(string)).collect((Collector)ImmutableList.toImmutableList());
    ...
    public void kickUnlistedPlayers(final CommandSourceStack db) {
        ...
        final List<ServerPlayer> list5 = Lists.newArrayList((java.lang.Iterable<?>)acs3.getPlayers());
