...
public class TagParser {
    ...
    CompoundTag readSingleStruct() throws CommandSyntaxException {
        ...
        if (this.reader.canRead()) {
            throw TagParser.ERROR_TRAILING_DATA.createWithContext(this.reader);
        }
        return md2;
    }
    ...
    private Tag readListTag() throws CommandSyntaxException {
        ...
        while (this.reader.peek() != ']') {
            ...
            if (mv3 == null) {
                ...
                throw TagParser.ERROR_INSERT_MIXED_LIST.createWithContext(this.reader, mv4.getPrettyName(), mv3.getPrettyName());
            }
            mj2.add(mt5);
            if (!this.hasElementSeparator()) {
                break;
    ...
    private Tag readArrayTag() throws CommandSyntaxException {
        ...
        this.reader.setCursor(integer2);
        throw TagParser.ERROR_INVALID_ARRAY.createWithContext(this.reader, String.valueOf(character3));
    }
    
    private <T extends Number> List<T> readArray(final TagType<?> mv1, final TagType<?> mv2) throws CommandSyntaxException {
        final List<T> list4 = Lists.newArrayList();
        while (this.reader.peek() != ']') {
            ...
            if (mv3 != mv2) {
                ...
                throw TagParser.ERROR_INSERT_MIXED_ARRAY.createWithContext(this.reader, mv3.getPrettyName(), mv1.getPrettyName());
            }
            if (mv2 == ByteTag.TYPE) {
    ...
    static {
        ERROR_TRAILING_DATA = new SimpleCommandExceptionType(new TranslatableComponent("argument.nbt.trailing"));
        ERROR_EXPECTED_KEY = new SimpleCommandExceptionType(new TranslatableComponent("argument.nbt.expected.key"));
        ERROR_EXPECTED_VALUE = new SimpleCommandExceptionType(new TranslatableComponent("argument.nbt.expected.value"));
        final TranslatableComponent translatableComponent;
        ERROR_INSERT_MIXED_LIST = new Dynamic2CommandExceptionType((object1, object2) -> {
            new TranslatableComponent("argument.nbt.list.mixed", new Object[] { object1, object2 });
            return translatableComponent;
        });
        final TranslatableComponent translatableComponent2;
        ERROR_INSERT_MIXED_ARRAY = new Dynamic2CommandExceptionType((object1, object2) -> {
            new TranslatableComponent("argument.nbt.array.mixed", new Object[] { object1, object2 });
            return translatableComponent2;
        });
        ERROR_INVALID_ARRAY = new DynamicCommandExceptionType((Function<Object, Message>)(object -> new TranslatableComponent("argument.nbt.array.invalid", new Object[] { object })));
