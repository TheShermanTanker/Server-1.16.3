...
public class Commands {
    ...
    public Commands(final CommandSelection a) {
        AdvancementCommands.register(this.dispatcher = new CommandDispatcher<CommandSourceStack>());
        ...
        this.dispatcher.setConsumer((commandContext, boolean2, integer) -> commandContext.getSource().onCommandComplete(commandContext, boolean2, integer));
    }
    ...
    public void sendCommands(final ServerPlayer aah) {
        final Map<CommandNode<CommandSourceStack>, CommandNode<SharedSuggestionProvider>> map3 = Maps.newHashMap();
        final RootCommandNode<SharedSuggestionProvider> rootCommandNode4 = new RootCommandNode<SharedSuggestionProvider>();
        map3.put(this.dispatcher.getRoot(), rootCommandNode4);
        this.fillUsableCommands(this.dispatcher.getRoot(), rootCommandNode4, aah.createCommandSourceStack(), map3);
    ...
    private void fillUsableCommands(final CommandNode<CommandSourceStack> commandNode1, final CommandNode<SharedSuggestionProvider> commandNode2, final CommandSourceStack db, final Map<CommandNode<CommandSourceStack>, CommandNode<SharedSuggestionProvider>> map) {
        for (final CommandNode<CommandSourceStack> commandNode3 : commandNode1.getChildren()) {
            if (commandNode3.canUse(db)) {
                ...
                argumentBuilder8.requires((java.util.function.Predicate<SharedSuggestionProvider>)(dd -> true));
                ...
                if (argumentBuilder8 instanceof RequiredArgumentBuilder) {
                    final RequiredArgumentBuilder<SharedSuggestionProvider, ?> requiredArgumentBuilder9 = (RequiredArgumentBuilder)argumentBuilder8;
                    if (requiredArgumentBuilder9.getSuggestionsProvider() != null) {
                        requiredArgumentBuilder9.suggests(SuggestionProviders.safelySwap(requiredArgumentBuilder9.getSuggestionsProvider()));
                    }
                }
                if (argumentBuilder8.getRedirect() != null) {
                    argumentBuilder8.redirect((CommandNode<SharedSuggestionProvider>)map.get(argumentBuilder8.getRedirect()));
                }
                final CommandNode<SharedSuggestionProvider> commandNode4 = argumentBuilder8.build();
    ...
    public static LiteralArgumentBuilder<CommandSourceStack> literal(final String string) {
        return LiteralArgumentBuilder.<CommandSourceStack>literal(string);
    }
    ...
    public static <T> RequiredArgumentBuilder<CommandSourceStack, T> argument(final String string, final ArgumentType<T> argumentType) {
        return RequiredArgumentBuilder.<CommandSourceStack, T>argument(string, argumentType);
    }
    ...
    public static void validate() {
        final RootCommandNode<CommandSourceStack> rootCommandNode1 = new Commands(CommandSelection.ALL).getDispatcher().getRoot();
        final Set<ArgumentType<?>> set2 = ArgumentTypes.<CommandSourceStack>findUsedArgumentTypes(rootCommandNode1);
