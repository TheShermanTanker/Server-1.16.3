...
public class SuggestionProviders {
    ...
    static {
        ...
        ASK_SERVER = SuggestionProviders.<SharedSuggestionProvider>register(SuggestionProviders.DEFAULT_NAME, (commandContext, suggestionsBuilder) -> commandContext.getSource().customSuggestion(commandContext, suggestionsBuilder));
        ALL_RECIPES = SuggestionProviders.<CommandSourceStack>register(new ResourceLocation("all_recipes"), (commandContext, suggestionsBuilder) -> SharedSuggestionProvider.suggestResource(commandContext.getSource().getRecipeNames(), suggestionsBuilder));
        AVAILABLE_SOUNDS = SuggestionProviders.<CommandSourceStack>register(new ResourceLocation("available_sounds"), (commandContext, suggestionsBuilder) -> SharedSuggestionProvider.suggestResource((Iterable<ResourceLocation>)commandContext.getSource().getAvailableSoundEvents(), suggestionsBuilder));
        AVAILABLE_BIOMES = SuggestionProviders.<CommandSourceStack>register(new ResourceLocation("available_biomes"), (commandContext, suggestionsBuilder) -> SharedSuggestionProvider.suggestResource((Iterable<ResourceLocation>)commandContext.getSource().registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).keySet(), suggestionsBuilder));
        SUMMONABLE_ENTITIES = SuggestionProviders.<CommandSourceStack>register(new ResourceLocation("summonable_entities"), (commandContext, suggestionsBuilder) -> SharedSuggestionProvider.suggestResource((java.util.stream.Stream<Object>)Registry.ENTITY_TYPE.stream().filter(EntityType::canSummon), suggestionsBuilder, (java.util.function.Function<Object, ResourceLocation>)EntityType::getKey, (java.util.function.Function<Object, Message>)(aqb -> new TranslatableComponent(Util.makeDescriptionId("entity", EntityType.getKey(aqb))))));
    }
