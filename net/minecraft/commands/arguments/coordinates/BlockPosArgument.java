...
public class BlockPosArgument implements ArgumentType<Coordinates> {
    ...
    public static BlockPos getLoadedBlockPos(final CommandContext<CommandSourceStack> commandContext, final String string) throws CommandSyntaxException {
        final BlockPos fx3 = commandContext.<Coordinates>getArgument(string, Coordinates.class).getBlockPos(commandContext.getSource());
        if (!commandContext.getSource().getLevel().hasChunkAt(fx3)) {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(final CommandContext<S> commandContext, final SuggestionsBuilder suggestionsBuilder) {
        if (commandContext.getSource() instanceof SharedSuggestionProvider) {
            ...
            return SharedSuggestionProvider.suggestCoordinates(string4, collection5, suggestionsBuilder, Commands.createValidator(this::a));
        }
    ...
    static {
        ...
        ERROR_NOT_LOADED = new SimpleCommandExceptionType(new TranslatableComponent("argument.pos.unloaded"));
        ERROR_OUT_OF_WORLD = new SimpleCommandExceptionType(new TranslatableComponent("argument.pos.outofworld"));
    }
}
