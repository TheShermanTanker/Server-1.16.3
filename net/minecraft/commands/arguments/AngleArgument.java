...
public class AngleArgument implements ArgumentType<SingleAngle> {
    ...
    public static float getAngle(final CommandContext<CommandSourceStack> commandContext, final String string) {
        return commandContext.<SingleAngle>getArgument(string, SingleAngle.class).getAngle(commandContext.getSource());
    }
    
    public SingleAngle parse(final StringReader stringReader) throws CommandSyntaxException {
        if (!stringReader.canRead()) {
            throw AngleArgument.ERROR_NOT_COMPLETE.createWithContext(stringReader);
        }
    ...
    static {
        ...
        ERROR_NOT_COMPLETE = new SimpleCommandExceptionType(new TranslatableComponent("argument.angle.incomplete"));
    }
