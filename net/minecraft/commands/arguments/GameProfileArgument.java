...
public class GameProfileArgument implements ArgumentType<Result> {
    ...
    public static Collection<GameProfile> getGameProfiles(final CommandContext<CommandSourceStack> commandContext, final String string) throws CommandSyntaxException {
        return commandContext.<Result>getArgument(string, Result.class).getNames(commandContext.getSource());
    }
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(final CommandContext<S> commandContext, final SuggestionsBuilder suggestionsBuilder) {
        if (commandContext.getSource() instanceof SharedSuggestionProvider) {
            ...
            return fd5.fillSuggestions(suggestionsBuilder, (Consumer<SuggestionsBuilder>)(suggestionsBuilder -> SharedSuggestionProvider.suggest((Iterable<String>)commandContext.getSource().getOnlinePlayerNames(), suggestionsBuilder)));
        }
    ...
    static {
        ...
        ERROR_UNKNOWN_PLAYER = new SimpleCommandExceptionType(new TranslatableComponent("argument.player.unknown"));
    }
