...
public class PacketEncoder extends MessageToByteEncoder<Packet<?>> {
    ...
    protected void encode(final ChannelHandlerContext channelHandlerContext, final Packet<?> oj, final ByteBuf byteBuf) throws Exception {
        final ConnectionProtocol ne5 = channelHandlerContext.channel().<ConnectionProtocol>attr(Connection.ATTRIBUTE_PROTOCOL).get();
        if (ne5 == null) {
        ...
        if (PacketEncoder.LOGGER.isDebugEnabled()) {
            PacketEncoder.LOGGER.debug(PacketEncoder.MARKER, "OUT: [{}:{}] {}", channelHandlerContext.channel().<ConnectionProtocol>attr(Connection.ATTRIBUTE_PROTOCOL).get(), integer6, oj.getClass().getName());
        }
        if (integer6 == null) {
