...
public class PacketDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(final ChannelHandlerContext channelHandlerContext, final ByteBuf byteBuf, final List<Object> list) throws Exception {
        ...
        final Packet<?> oj7 = channelHandlerContext.channel().<ConnectionProtocol>attr(Connection.ATTRIBUTE_PROTOCOL).get().createPacket(this.flow, integer6);
        if (oj7 == null) {
            throw new IOException(new StringBuilder().append("Bad packet id ").append(integer6).toString());
        }
        oj7.read(nf5);
        if (nf5.readableBytes() > 0) {
            throw new IOException(new StringBuilder().append("Packet ").append(channelHandlerContext.channel().<ConnectionProtocol>attr(Connection.ATTRIBUTE_PROTOCOL).get().getId()).append("/").append(integer6).append(" (").append(oj7.getClass().getSimpleName()).append(") was larger than I expected, found ").append(nf5.readableBytes()).append(" bytes extra whilst reading packet ").append(integer6).toString());
        }
        list.add(oj7);
        if (PacketDecoder.LOGGER.isDebugEnabled()) {
            PacketDecoder.LOGGER.debug(PacketDecoder.MARKER, " IN: [{}:{}] {}", channelHandlerContext.channel().<ConnectionProtocol>attr(Connection.ATTRIBUTE_PROTOCOL).get(), integer6, oj7.getClass().getName());
        }
    }
