...
public class Mth {
    ...
    public static boolean isPowerOfTwo(final int integer) {
    ...
    public static long getSeed(final Vec3i gr) {
        return getSeed(gr.getX(), gr.getY(), gr.getZ());
    }
    ...
    public static double inverseLerp(final double double1, final double double2, final double double3) {
    ...
    public static int hsvToRgb(final float float1, final float float2, final float float3) {
        ...
        switch (integer4) {
            ...
            default: {
                throw new RuntimeException(new StringBuilder().append("Something went wrong when converting from HSV to RGB. Input was ").append(float1).append(", ").append(float2).append(", ").append(float3).toString());
            }
        }
    ...
    public static float square(final float float1) {
        return float1 * float1;
    }
    
    static {
        SQRT_OF_TWO = sqrt(2.0f);
        SIN = Util.<float[]>make(new float[65536], (java.util.function.Consumer<float[]>)(arr -> {
        ...
        for (int integer1 = 0; integer1 < 257; ++integer1) {
            final double double2 = integer1 / 256.0;
            final double double3 = Math.asin(double2);
            Mth.COS_TAB[integer1] = Math.cos(double3);
            Mth.ASIN_TAB[integer1] = double3;
        }
    }
}
