...
public class LevelFlatGeneratorInfoFix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("LevelFlatGeneratorInfoFix", this.getInputSchema().getType(References.LEVEL), (Function<Typed<?>, Typed<?>>)(typed -> typed.<Dynamic<?>>update(DSL.remainderFinder(), this::fix)));
    }
    
    private Dynamic<?> fix(final Dynamic<?> dynamic) {
        if (dynamic.get("generatorName").asString("").equalsIgnoreCase("flat")) {
            return dynamic.update("generatorOptions", (Function<Dynamic<?>, Dynamic<?>>)(dynamic -> DataFixUtils.<Dynamic>orElse(dynamic.asString().map(this::fixString).map(dynamic::createString).result(), dynamic)));
        }
        return dynamic;
    }
    ...
    String fixString(final String string) {
        ...
        stringBuilder7.append((String)StreamSupport.stream(LevelFlatGeneratorInfoFix.LAYER_SPLITTER.split((CharSequence)string3).spliterator(), false).map(string -> {
            final List<String> list6 = splitter8.splitToList((CharSequence)string);
            int integer2;
            String string2;
            if (list6.size() == 2) {
            ...
            final List<String> list7 = LevelFlatGeneratorInfoFix.BLOCK_SPLITTER.splitToList((CharSequence)string2);
