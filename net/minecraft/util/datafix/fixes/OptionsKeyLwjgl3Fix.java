...
public class OptionsKeyLwjgl3Fix extends DataFix {
    ...
    public OptionsKeyLwjgl3Fix(final Schema schema, final boolean boolean2) {
        super(schema, boolean2);
    }
    
    public TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("OptionsKeyLwjgl3Fix", this.getInputSchema().getType(References.OPTIONS), (Function<Typed<?>, Typed<?>>)(typed -> typed.<Dynamic<?>>update(DSL.remainderFinder(), dynamic -> (Dynamic)dynamic.getMapValues().map(map -> dynamic.createMap((Map)map.entrySet().stream().map(entry -> {
            if (!((Dynamic)entry.getKey()).asString("").startsWith("key_")) {
                return Pair.of(entry.getKey(), entry.getValue());
            }
            final int integer2 = Integer.parseInt(((Dynamic)entry.getValue()).asString(""));
            if (integer2 < 0) {
                final int integer3 = integer2 + 100;
                String string4;
                if (integer3 == 0) {
                    string4 = "key.mouse.left";
                }
                else if (integer3 == 1) {
                    string4 = "key.mouse.right";
                }
                else if (integer3 == 2) {
                    string4 = "key.mouse.middle";
                }
                else {
                    string4 = new StringBuilder().append("key.mouse.").append(integer3 + 1).toString();
                }
                return Pair.<Object, Dynamic>of(entry.getKey(), ((Dynamic)entry.getValue()).createString(string4));
            }
            final String string5 = OptionsKeyLwjgl3Fix.MAP.getOrDefault(integer2, "key.unknown");
            return Pair.<Object, Dynamic>of(entry.getKey(), ((Dynamic)entry.getValue()).createString(string5));
        }).collect(Collectors.toMap(Pair::getFirst, Pair::getSecond)))).result().orElse(dynamic))));
    }
    
    static {
        MAP = DataFixUtils.<Int2ObjectOpenHashMap<String>>make(new Int2ObjectOpenHashMap<String>(), (java.util.function.Consumer<Int2ObjectOpenHashMap<String>>)(int2ObjectOpenHashMap -> {
