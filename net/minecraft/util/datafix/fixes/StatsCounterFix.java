...
public class StatsCounterFix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("StatsCounterFix", this.getInputSchema().getType(References.STATS), type2, (Function<Typed<?>, Typed<?>>)(typed -> {
            final Dynamic<?> dynamic4 = typed.<Dynamic<?>>get(DSL.remainderFinder());
            final Map<Dynamic<?>, Dynamic<?>> map5 = Maps.newHashMap();
            ...
            if (optional6.isPresent()) {
                for (final Map.Entry<? extends Dynamic<?>, ? extends Dynamic<?>> entry8 : ((Map)optional6.get()).entrySet()) {
                    if (((Dynamic)entry8.getValue()).asNumber().result().isPresent()) {
                        ...
                        map5.put(dynamic5, dynamic6.set(string11, entry8.getValue()));
                    }
                }
            }
            return ((Pair)type2.readTyped(dynamic4.emptyMap().set("stats", dynamic4.createMap(map5))).result().orElseThrow(() -> new IllegalStateException("Could not parse new stats object."))).getFirst();
        }));
    }
