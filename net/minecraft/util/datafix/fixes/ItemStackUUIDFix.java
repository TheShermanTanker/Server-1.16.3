...
public class ItemStackUUIDFix extends AbstractUUIDFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("ItemStackUUIDFix", this.getInputSchema().getType(this.typeReference), (Function<Typed<?>, Typed<?>>)(typed -> {
            final OpticFinder<?> opticFinder2 = typed.getType().findField("tag");
            return typed.updateTyped(opticFinder2, typed3 -> typed3.<Dynamic<?>>update(DSL.remainderFinder(), dynamic -> {
    ...
    private Dynamic<?> updateAttributeModifiers(final Dynamic<?> dynamic) {
        return dynamic.update("AttributeModifiers", (Function<Dynamic<?>, Dynamic<?>>)(dynamic2 -> dynamic.createList(dynamic2.asStream().map(dynamic -> (Dynamic)AbstractUUIDFix.replaceUUIDLeastMost(dynamic, "UUID", "UUID").orElse(dynamic)))));
    }
    
    private Dynamic<?> updateSkullOwner(final Dynamic<?> dynamic) {
        return dynamic.update("SkullOwner", (Function<Dynamic<?>, Dynamic<?>>)(dynamic -> (Dynamic)AbstractUUIDFix.replaceUUIDString(dynamic, "Id", "Id").orElse(dynamic)));
    }
}
