...
public class NewVillageFix extends DataFix {
    ...
    private <SF> TypeRewriteRule cap(final CompoundList.CompoundListType<String, SF> compoundListType) {
        ...
        return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("NewVillageFix", type3, (Function<Typed<?>, Typed<?>>)(typed -> typed.updateTyped(opticFinder5, typed -> typed.updateTyped(opticFinder6, typed -> typed.updateTyped(opticFinder7, typed -> typed.update(opticFinder8, list -> (List)list.stream().filter(pair -> !Objects.equals(pair.getFirst(), "Village")).map(pair -> pair.mapFirst(string -> string.equals("New_Village") ? "Village" : string)).collect(Collectors.toList()))).update(DSL.remainderFinder(), dynamic -> dynamic.update("References", dynamic -> {
            final Optional<? extends Dynamic<?>> optional2 = dynamic.get("New_Village").result();
            return DataFixUtils.<Dynamic>orElse((java.util.Optional<? extends Dynamic>)optional2.map(dynamic2 -> dynamic.remove("New_Village").set("Village", dynamic2)), dynamic).remove("Village");
        })))))), this.fixTypeEverywhereTyped("NewVillageStartFix", type4, (Function<Typed<?>, Typed<?>>)(typed -> typed.<Dynamic<?>>update(DSL.remainderFinder(), dynamic -> dynamic.update("id", dynamic -> Objects.equals(NamespacedSchema.ensureNamespaced(dynamic.asString("")), "minecraft:new_village") ? dynamic.createString("minecraft:village") : dynamic)))));
    }
}
