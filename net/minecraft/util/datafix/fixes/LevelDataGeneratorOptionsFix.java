...
public class LevelDataGeneratorOptionsFix extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("LevelDataGeneratorOptionsFix", this.getInputSchema().getType(References.LEVEL), type2, (Function<Typed<?>, Typed<?>>)(typed -> (Typed)typed.write().flatMap(dynamic -> {
            ...
            if ("flat".equalsIgnoreCase(dynamic.get("generatorName").asString(""))) {
                ...
                dynamic2 = dynamic.set("generatorOptions", LevelDataGeneratorOptionsFix.convert(string5, dynamic.getOps()));
            }
            else if ("buffet".equalsIgnoreCase(dynamic.get("generatorName").asString("")) && optional3.isPresent()) {
                final Dynamic<JsonElement> dynamic3 = new Dynamic<JsonElement>(JsonOps.INSTANCE, GsonHelper.parse((String)optional3.get(), true));
                dynamic2 = dynamic.set("generatorOptions", dynamic3.convert(dynamic.getOps()));
            }
            else {
                dynamic2 = dynamic;
            }
            return type2.readTyped(dynamic2);
    ...
    private static <T> Dynamic<T> convert(final String string, final DynamicOps<T> dynamicOps) {
        ...
        if (!string.isEmpty() && iterator3.hasNext()) {
            ...
            list4.add(Pair.<Integer, String>of(Integer.valueOf(1), "minecraft:bedrock"));
            list4.add(Pair.<Integer, String>of(Integer.valueOf(2), "minecraft:dirt"));
            list4.add(Pair.<Integer, String>of(Integer.valueOf(1), "minecraft:grass_block"));
            map6.put("village", Maps.newHashMap());
        }
        final T object7 = dynamicOps.createList((java.util.stream.Stream<T>)list4.stream().map(pair -> dynamicOps.createMap((java.util.Map<Object, Object>)ImmutableMap.of(dynamicOps.createString("height"), dynamicOps.createInt(pair.getFirst()), dynamicOps.createString("block"), dynamicOps.createString((String)pair.getSecond())))));
        final T object8 = dynamicOps.createMap((java.util.Map<T, T>)map6.entrySet().stream().map(entry -> Pair.of(dynamicOps.createString(((String)entry.getKey()).toLowerCase(Locale.ROOT)), dynamicOps.createMap((java.util.Map<F, F>)((Map)entry.getValue()).entrySet().stream().map(entry -> Pair.of(dynamicOps.createString((String)entry.getKey()), dynamicOps.createString((String)entry.getValue()))).collect(Collectors.toMap(Pair::getFirst, Pair::getSecond))))).collect(Collectors.toMap(Pair::getFirst, Pair::getSecond)));
        return new Dynamic<T>(dynamicOps, dynamicOps.createMap((java.util.Map<T, T>)ImmutableMap.<T, T>of(dynamicOps.createString("layers"), object7, dynamicOps.createString("biome"), dynamicOps.createString(string2), dynamicOps.createString("structures"), object8)));
    }
    ...
    private static Pair<Integer, String> getLayerInfoFromString(final String string) {
        ...
        return Pair.<Integer, String>of(integer3, string2);
    }
