...
public class BitStorageAlignFix extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        final OpticFinder<?> opticFinder6 = DSL.typeFinder(type4);
        final Type<Pair<String, Dynamic<?>>> type5 = DSL.<Dynamic<?>>named(References.BLOCK_STATE.typeName(), DSL.remainderType());
        final OpticFinder<java.util.List<Pair<String, Dynamic<?>>>> opticFinder7 = DSL.<java.util.List<Pair<String, Dynamic<?>>>>fieldFinder("Palette", DSL.list(type5));
        return this.fixTypeEverywhereTyped("BitStorageAlignFix", type2, this.getOutputSchema().getType(References.CHUNK), (Function<Typed<?>, Typed<?>>)(typed -> typed.updateTyped(opticFinder4, typed -> this.updateHeightmaps(updateSections(opticFinder5, opticFinder6, opticFinder7, typed)))));
    }
    
    private Typed<?> updateHeightmaps(final Typed<?> typed) {
        return typed.<Dynamic<?>>update(DSL.remainderFinder(), (java.util.function.Function<Dynamic<?>, Dynamic<?>>)(dynamic -> dynamic.update("Heightmaps", dynamic2 -> dynamic2.updateMapValues(pair -> pair.mapSecond(dynamic2 -> updateBitStorage(dynamic, dynamic2, 256, 9))))));
    }
    ...
    private static Typed<?> updateSections(final OpticFinder<?> opticFinder1, final OpticFinder<?> opticFinder2, final OpticFinder<java.util.List<Pair<String, Dynamic<?>>>> opticFinder3, final Typed<?> typed) {
        return typed.updateTyped(opticFinder1, (Function<Typed<?>, Typed<?>>)(typed -> typed.updateTyped(opticFinder2, typed -> {
            ...
            return typed.<Dynamic<?>>update(DSL.remainderFinder(), dynamic -> dynamic.update("BlockStates", dynamic3 -> updateBitStorage(dynamic, dynamic3, 4096, integer3)));
        })));
    }
