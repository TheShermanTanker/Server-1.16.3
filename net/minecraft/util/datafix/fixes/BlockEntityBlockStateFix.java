...
public class BlockEntityBlockStateFix extends NamedEntityFix {
    ...
    protected Typed<?> fix(final Typed<?> typed) {
        ...
        final OpticFinder<?> opticFinder5 = DSL.fieldFinder("blockState", type4);
        ...
        return typed2.<Dynamic<?>>set(DSL.remainderFinder(), dynamic6).set(opticFinder5, ((Pair)type4.readTyped(dynamic7).result().orElseThrow(() -> new IllegalStateException("Could not parse newly created block state tag."))).getFirst());
    }
}
