...
public class ItemLoreFix extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("Item Lore componentize", type2, (Function<Typed<?>, Typed<?>>)(typed -> typed.updateTyped(opticFinder3, typed -> typed.<Dynamic<?>>update(DSL.remainderFinder(), dynamic -> dynamic.update("display", dynamic -> dynamic.update("Lore", dynamic -> DataFixUtils.<Dynamic>orElse(dynamic.asStreamOpt().map(ItemLoreFix::fixLoreList).map(dynamic::createList).result(), dynamic)))))));
    }
    
    private static <T> Stream<Dynamic<T>> fixLoreList(final Stream<Dynamic<T>> stream) {
        return (Stream<Dynamic<T>>)stream.map(dynamic -> DataFixUtils.<Dynamic>orElse(dynamic.asString().map(ItemLoreFix::fixLoreEntry).map(dynamic::createString).result(), dynamic));
    }
