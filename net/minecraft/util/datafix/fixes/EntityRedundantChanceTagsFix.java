...
public class EntityRedundantChanceTagsFix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("EntityRedundantChanceTagsFix", this.getInputSchema().getType(References.ENTITY), (Function<Typed<?>, Typed<?>>)(typed -> typed.<Dynamic<?>>update(DSL.remainderFinder(), dynamic -> {
    ...
    private static boolean isZeroList(final OptionalDynamic<?> optionalDynamic, final int integer) {
        return (boolean)optionalDynamic.flatMap((java.util.function.Function<? super Dynamic<?>, ? extends DataResult<Object>>)EntityRedundantChanceTagsFix.FLOAT_LIST_CODEC::parse).map((java.util.function.Function<? super Object, ?>)(list -> list.size() == integer && list.stream().allMatch(float1 -> float1 == 0.0f))).result().orElse(false);
    }
