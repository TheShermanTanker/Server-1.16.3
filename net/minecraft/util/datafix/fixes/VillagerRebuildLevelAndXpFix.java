...
public class VillagerRebuildLevelAndXpFix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        final OpticFinder<?> opticFinder3 = DSL.namedChoice("minecraft:villager", type2);
        ...
        final List.ListType<?> listType7 = (List.ListType)opticFinder5.type();
        final OpticFinder<?> opticFinder6 = listType7.getElement().finder();
        return this.fixTypeEverywhereTyped("Villager level and xp rebuild", this.getInputSchema().getType(References.ENTITY), (Function<Typed<?>, Typed<?>>)(typed -> typed.updateTyped(opticFinder3, type2, typed -> {
            ...
            final Optional<Number> optional8 = dynamic5.get("Xp").asNumber().result();
    ...
    private static Typed<?> addLevel(final Typed<?> typed, final int integer) {
        return typed.<Dynamic<?>>update(DSL.remainderFinder(), (java.util.function.Function<Dynamic<?>, Dynamic<?>>)(dynamic -> dynamic.update("VillagerData", dynamic -> dynamic.set("level", (Dynamic)dynamic.createInt(integer)))));
    }
    
    private static Typed<?> addXpFromLevel(final Typed<?> typed, final int integer) {
        final int integer2 = getMinXpPerLevel(integer);
        return typed.<Dynamic<?>>update(DSL.remainderFinder(), (java.util.function.Function<Dynamic<?>, Dynamic<?>>)(dynamic -> dynamic.set("Xp", (Dynamic)dynamic.createInt(integer2))));
    }
