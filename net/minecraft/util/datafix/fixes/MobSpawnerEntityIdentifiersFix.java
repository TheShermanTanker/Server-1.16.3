...
public class MobSpawnerEntityIdentifiersFix extends DataFix {
    ...
    private Dynamic<?> fix(Dynamic<?> dynamic) {
        ...
        final Optional<String> optional3 = dynamic.get("EntityId").asString().result();
        if (optional3.isPresent()) {
            Dynamic<?> dynamic2 = DataFixUtils.<Dynamic<Object>>orElse((java.util.Optional<? extends Dynamic<Object>>)dynamic.get("SpawnData").result(), dynamic.emptyMap());
        ...
        if (optional4.isPresent()) {
            dynamic = dynamic.set("SpawnPotentials", dynamic.createList(((Stream)optional4.get()).map(dynamic -> {
                ...
                if (optional2.isPresent()) {
                    final Dynamic<?> dynamic2 = DataFixUtils.<Dynamic<?>>orElse(dynamic.get("Properties").result(), dynamic.emptyMap()).set("id", dynamic.createString((String)optional2.get()));
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("MobSpawnerEntityIdentifiersFix", this.getInputSchema().getType(References.UNTAGGED_SPAWNER), type2, (Function<Typed<?>, Typed<?>>)(typed -> {
            Dynamic<?> dynamic4 = typed.<Dynamic<?>>get(DSL.remainderFinder());
            dynamic4 = dynamic4.set("id", dynamic4.createString("MobSpawner"));
            final DataResult<? extends Pair<? extends Typed<?>, ?>> dataResult5 = type2.readTyped(this.fix(dynamic4));
            if (!dataResult5.result().isPresent()) {
                return typed;
            }
            return ((Pair)dataResult5.result().get()).getFirst();
        }));
    }
}
