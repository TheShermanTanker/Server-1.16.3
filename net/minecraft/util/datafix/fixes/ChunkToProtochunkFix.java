...
public class ChunkToProtochunkFix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        final OpticFinder<?> opticFinder7 = DSL.fieldFinder("Level", type4);
        final OpticFinder<?> opticFinder8 = DSL.fieldFinder("TileTicks", type6);
        return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("ChunkToProtoChunkFix", type2, this.getOutputSchema().getType(References.CHUNK), (Function<Typed<?>, Typed<?>>)(typed -> typed.updateTyped(opticFinder7, type5, typed -> {
            ...
            Dynamic<?> dynamic7;
            if (boolean6) {
                final Optional<ByteBuffer> optional5 = dynamic5.get("Biomes").asByteBufferOpt().result();
                ...
                dynamic7 = DataFixUtils.<Dynamic<?>>orElse(typed.<Dynamic<?>>set(DSL.remainderFinder(), dynamic5).write().result(), dynamic5);
            }
            else {
                dynamic7 = dynamic5;
            }
            return ((Pair)type5.readTyped(dynamic7).result().orElseThrow(() -> new IllegalStateException("Could not read the new chunk"))).getFirst();
