...
public class JigsawRotationFix extends DataFix {
    ...
    private static Dynamic<?> fix(final Dynamic<?> dynamic) {
        final Optional<String> optional2 = dynamic.get("Name").asString().result();
    ...
    protected TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("jigsaw_rotation_fix", this.getInputSchema().getType(References.BLOCK_STATE), (Function<Typed<?>, Typed<?>>)(typed -> typed.<Dynamic<?>>update(DSL.remainderFinder(), JigsawRotationFix::fix)));
    }
    
    static {
        renames = (Map)ImmutableMap.<String, String>builder().put("down", "down_south").put("up", "up_north").put("north", "north_up").put("south", "south_up").put("west", "west_up").put("east", "east_up").build();
    }
}
