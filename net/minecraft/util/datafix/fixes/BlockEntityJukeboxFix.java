...
public class BlockEntityJukeboxFix extends NamedEntityFix {
    ...
    protected Typed<?> fix(final Typed<?> typed) {
        ...
        final OpticFinder<?> opticFinder5 = DSL.fieldFinder("RecordItem", type4);
        final Dynamic<?> dynamic6 = typed.<Dynamic<?>>get(DSL.remainderFinder());
        ...
        if (integer7 > 0) {
            ...
            if (string8 != null) {
                ...
                return typed.set(opticFinder5, ((Pair)type4.readTyped(dynamic7).result().orElseThrow(() -> new IllegalStateException("Could not create record item stack."))).getFirst()).<Dynamic<?>>set(DSL.remainderFinder(), dynamic6);
            }
        }
        return typed;
    }
