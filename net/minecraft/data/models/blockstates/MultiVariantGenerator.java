...
public class MultiVariantGenerator implements BlockStateGenerator {
    ...
    private MultiVariantGenerator(final Block bul, final List<Variant> list) {
        ...
        this.declaredPropertySets = Lists.newArrayList();
        this.block = bul;
    ...
    public JsonElement get() {
        Stream<Pair<Selector, List<Variant>>> stream2 = (Stream<Pair<Selector, List<Variant>>>)Stream.of(Pair.<Selector, List<Variant>>of(Selector.empty(), this.baseVariants));
        for (final PropertyDispatch ip4 : this.declaredPropertySets) {
            ...
            stream2 = (Stream<Pair<Selector, List<Variant>>>)stream2.flatMap(pair -> map5.entrySet().stream().map5(entry -> {
                final Selector iq3 = pair.getFirst().extend((Selector)entry.getKey());
                final List<Variant> list4 = mergeVariants((List<Variant>)pair.getSecond(), (List<Variant>)entry.getValue());
                return Pair.<Selector, List<Variant>>of(iq3, list4);
            }));
        }
        final Map<String, JsonElement> map6 = (Map<String, JsonElement>)new TreeMap();
        stream2.forEach(pair -> {
            final JsonElement jsonElement = (JsonElement)map6.put(pair.getFirst().getKey(), Variant.convertList((List<Variant>)pair.getSecond()));
        });
        final JsonObject jsonObject4 = new JsonObject();
        jsonObject4.add("variants", (JsonElement)Util.<JsonObject>make(new JsonObject(), (java.util.function.Consumer<JsonObject>)(jsonObject -> map6.forEach(jsonObject::add))));
        return jsonObject4;
    }
    
    private static List<Variant> mergeVariants(final List<Variant> list1, final List<Variant> list2) {
        final ImmutableList.Builder<Variant> builder3 = ImmutableList.<Variant>builder();
    ...
    public static MultiVariantGenerator multiVariant(final Block bul) {
        return new MultiVariantGenerator(bul, ImmutableList.of(Variant.variant()));
    }
    
    public static MultiVariantGenerator multiVariant(final Block bul, final Variant ir) {
        return new MultiVariantGenerator(bul, ImmutableList.of(ir));
    }
    
    public static MultiVariantGenerator multiVariant(final Block bul, final Variant... arr) {
        return new MultiVariantGenerator(bul, ImmutableList.copyOf(arr));
    }
}
