...
public class HashCache {
    ...
    public HashCache(final Path path, final String string) throws IOException {
        this.oldCache = Maps.newHashMap();
        this.newCache = Maps.newHashMap();
        this.keep = Sets.newHashSet();
        this.path = path;
    ...
    public void purgeStaleAndWrite() throws IOException {
        ...
        IOUtils.writeLines(this.newCache.entrySet().stream().map(entry -> (String)entry.getValue() + ' ' + this.path.relativize((Path)entry.getKey())).collect(Collectors.toList()), System.lineSeparator(), writer2);
        writer2.close();
