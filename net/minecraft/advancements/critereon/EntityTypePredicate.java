...
public abstract class EntityTypePredicate {
    ...
    public static EntityTypePredicate fromJson(@Nullable final JsonElement jsonElement) {
        ...
        final EntityType<?> aqb4 = Registry.ENTITY_TYPE.getOptional(vk3).orElseThrow(() -> new JsonSyntaxException(new StringBuilder().append("Unknown entity type '").append(vk3).append("', valid types are: ").append(EntityTypePredicate.COMMA_JOINER.join(Registry.ENTITY_TYPE.keySet())).toString()));
    ...
    static class TagPredicate extends EntityTypePredicate {
        ...
        @Override
        public JsonElement serializeToJson() {
            return new JsonPrimitive(new StringBuilder().append("#").append(SerializationTags.getInstance().getEntityTypes().getIdOrThrow(this.tag)).toString());
        }
    }
}
