...
public abstract class Registry<T> implements Codec<T>, Keyable, IdMap<T> {
    ...
    public <U> DataResult<Pair<T, U>> decode(final DynamicOps<U> dynamicOps, final U object) {
        if (dynamicOps.compressMaps()) {
            return dynamicOps.getNumberValue(object).flatMap((java.util.function.Function<? super Number, ? extends DataResult<Object>>)(number -> {
                ...
                return DataResult.<T>success(object3, this.lifecycle(object3));
            })).<Pair<T, U>>map((java.util.function.Function<? super Object, ? extends Pair<T, U>>)(object -> Pair.of(object, dynamicOps.empty())));
        }
        return ResourceLocation.CODEC.<U>decode(dynamicOps, object).<Pair<T, U>>flatMap((java.util.function.Function<? super Pair<Object, U>, ? extends DataResult<Pair<T, U>>>)(pair -> {
            final T object3 = this.get(pair.getFirst());
            if (object3 == null) {
                return DataResult.error(new StringBuilder().append("Unknown registry key: ").append(pair.getFirst()).toString());
            }
            return DataResult.<Pair<T, Object>>success(Pair.<T, Object>of(object3, pair.getSecond()), this.lifecycle(object3));
        }));
    }
    ...
    public <U> DataResult<U> encode(final T object1, final DynamicOps<U> dynamicOps, final U object3) {
        ...
        if (vk5 == null) {
            return DataResult.<U>error(new StringBuilder().append("Unknown registry element ").append(object1).toString());
        }
        if (dynamicOps.compressMaps()) {
            return dynamicOps.mergeToPrimitive(object3, dynamicOps.createInt(this.getId(object1))).setLifecycle(this.lifecycle);
        }
        return dynamicOps.mergeToPrimitive(object3, dynamicOps.createString(vk5.toString())).setLifecycle(this.lifecycle);
    }
