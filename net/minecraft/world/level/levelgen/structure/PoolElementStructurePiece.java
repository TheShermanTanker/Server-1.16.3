...
public class PoolElementStructurePiece extends StructurePiece {
    ...
    public PoolElementStructurePiece(final StructureManager cst, final CompoundTag md) {
        ...
        this.element = (StructurePoolElement)StructurePoolElement.CODEC.<CompoundTag>parse((DynamicOps<CompoundTag>)NbtOps.INSTANCE, md.getCompound("pool_element")).resultOrPartial((Consumer<String>)PoolElementStructurePiece.LOGGER::error).orElse(EmptyPoolElement.INSTANCE);
        ...
        mj4.forEach(mt -> this.junctions.add(JigsawJunction.deserialize(new Dynamic<>((DynamicOps<Object>)NbtOps.INSTANCE, mt))));
    }
    ...
    protected void addAdditionalSaveData(final CompoundTag md) {
        ...
        StructurePoolElement.CODEC.encodeStart((DynamicOps<Object>)NbtOps.INSTANCE, this.element).resultOrPartial((Consumer<String>)PoolElementStructurePiece.LOGGER::error).ifPresent(mt -> md.put("pool_element", mt));
        md.putString("rotation", this.rotation.name());
        final ListTag mj3 = new ListTag();
        for (final JigsawJunction coa5 : this.junctions) {
            mj3.add(coa5.serialize((DynamicOps<Object>)NbtOps.INSTANCE).getValue());
        }
