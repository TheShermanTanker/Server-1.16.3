...
public class LegacyStructureDataHandler {
    ...
    public CompoundTag updateFromLegacy(CompoundTag md) {
        ...
        for (final String string8 : this.currentKeys) {
            ...
            if (!md4.contains(string8, 12)) {
                ...
                final LongList longList11 = new LongArrayList();
    ...
    private CompoundTag updateStructureStart(final CompoundTag md, final ChunkPos bra) {
        ...
        for (final String string8 : this.currentKeys) {
            ...
            final CompoundTag md5 = long2ObjectMap9.get(long10);
            if (md5 == null) {
                continue;
            }
            md4.put(string8, md5);
    ...
    public static LegacyStructureDataHandler getLegacyStructureHandler(final ResourceKey<Level> vj, @Nullable final DimensionDataStorage cxz) {
        if (vj == Level.OVERWORLD) {
            return new LegacyStructureDataHandler(cxz, ImmutableList.of("Monument", "Stronghold", "Village", "Mineshaft", "Temple", "Mansion"), ImmutableList.of("Village", "Mineshaft", "Mansion", "Igloo", "Desert_Pyramid", "Jungle_Pyramid", "Swamp_Hut", "Stronghold", "Monument"));
        }
        if (vj == Level.NETHER) {
            final List<String> list3 = ImmutableList.of("Fortress");
            return new LegacyStructureDataHandler(cxz, list3, list3);
        }
        if (vj == Level.END) {
            final List<String> list3 = ImmutableList.of("EndCity");
            return new LegacyStructureDataHandler(cxz, list3, list3);
        }
