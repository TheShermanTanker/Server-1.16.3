...
public class SinglePoolElement extends StructurePoolElement {
    ...
    private static <T> DataResult<T> encodeTemplate(final Either<ResourceLocation, StructureTemplate> either, final DynamicOps<T> dynamicOps, final T object) {
        final Optional<ResourceLocation> optional4 = either.left();
        if (!optional4.isPresent()) {
            return DataResult.<T>error("Can not serialize a runtime pool element");
        }
        return ResourceLocation.CODEC.<T>encode((ResourceLocation)optional4.get(), dynamicOps, object);
    }
    
    protected static <E extends SinglePoolElement> RecordCodecBuilder<E, Supplier<StructureProcessorList>> processorsCodec() {
        return StructureProcessorType.LIST_CODEC.fieldOf("processors").<E>forGetter((java.util.function.Function<E, Supplier<StructureProcessorList>>)(coe -> coe.processors));
    }
    ...
    protected static <E extends SinglePoolElement> RecordCodecBuilder<E, Either<ResourceLocation, StructureTemplate>> templateCodec() {
        return SinglePoolElement.TEMPLATE_CODEC.fieldOf("location").<E>forGetter((java.util.function.Function<E, Either<ResourceLocation, StructureTemplate>>)(coe -> coe.template));
    }
    ...
    public SinglePoolElement(final StructureTemplate csy) {
        this(Either.<ResourceLocation, StructureTemplate>right(csy), (Supplier<StructureProcessorList>)(() -> ProcessorLists.EMPTY), StructureTemplatePool.Projection.RIGID);
    }
    
    private StructureTemplate getTemplate(final StructureManager cst) {
        return this.template.<StructureTemplate>map((java.util.function.Function<? super ResourceLocation, ? extends StructureTemplate>)cst::getOrCreate, (java.util.function.Function<? super StructureTemplate, ? extends StructureTemplate>)Function.identity());
    }
    ...
    public List<StructureTemplate.StructureBlockInfo> getDataMarkers(final StructureManager cst, final BlockPos fx, final Rotation bzj, final boolean boolean4) {
        ...
        final List<StructureTemplate.StructureBlockInfo> list8 = Lists.newArrayList();
    ...
    protected StructurePlaceSettings getSettings(final Rotation bzj, final BoundingBox cqx, final boolean boolean3) {
        ...
        this.getProjection().getProcessors().forEach((java.util.function.Consumer<? super StructureProcessor>)csu5::addProcessor);
        return csu5;
    }
    ...
    static {
        TEMPLATE_CODEC = Codec.<Either<ResourceLocation, StructureTemplate>>of(SinglePoolElement::encodeTemplate, ResourceLocation.CODEC.<Either<ResourceLocation, StructureTemplate>>map((java.util.function.Function<? super Object, ? extends Either<ResourceLocation, StructureTemplate>>)Either::left));
        CODEC = RecordCodecBuilder.<SinglePoolElement>create((java.util.function.Function<RecordCodecBuilder.Instance<SinglePoolElement>, ? extends App<RecordCodecBuilder.Mu<SinglePoolElement>, SinglePoolElement>>)(instance -> instance.<Either<ResourceLocation, StructureTemplate>, Supplier<StructureProcessorList>, StructureTemplatePool.Projection>group(SinglePoolElement.templateCodec(), SinglePoolElement.processorsCodec(), StructurePoolElement.projectionCodec()).<SinglePoolElement>apply(instance, SinglePoolElement::new)));
    }
}
