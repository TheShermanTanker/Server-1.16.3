...
public class SectionStorage<R> implements AutoCloseable {
    ...
    public SectionStorage(final File file, final Function<Runnable, Codec<R>> function2, final Function<Runnable, R> function3, final DataFixer dataFixer, final DataFixTypes afx, final boolean boolean6) {
        this.storage = new Long2ObjectOpenHashMap<Optional<R>>();
    ...
    protected R getOrCreate(final long long1) {
        ...
        this.storage.put(long1, (Optional<R>)Optional.of(object5));
        return object5;
    }
    
    private void readColumn(final ChunkPos bra) {
        this.<CompoundTag>readColumn(bra, (DynamicOps<CompoundTag>)NbtOps.INSTANCE, this.tryRead(bra));
    }
    ...
    private <T> void readColumn(final ChunkPos bra, final DynamicOps<T> dynamicOps, @Nullable final T object) {
        if (object == null) {
            for (int integer5 = 0; integer5 < 16; ++integer5) {
                this.storage.put(SectionPos.of(bra, integer5).asLong(), (Optional<R>)Optional.empty());
            }
        }
        else {
            final Dynamic<T> dynamic5 = new Dynamic<T>(dynamicOps, object);
            ...
            final Dynamic<T> dynamic6 = this.fixerUpper.<T>update(this.type.getType(), dynamic5, integer6, integer7);
            final OptionalDynamic<T> optionalDynamic10 = dynamic6.get("Sections");
    ...
    private void writeColumn(final ChunkPos bra) {
        final Dynamic<Tag> dynamic3 = this.<Tag>writeColumn(bra, (DynamicOps<Tag>)NbtOps.INSTANCE);
        final Tag mt4 = dynamic3.getValue();
        ...
        for (int integer5 = 0; integer5 < 16; ++integer5) {
            ...
            if (optional8 != null) {
                if (optional8.isPresent()) {
                    ...
                    dataResult9.resultOrPartial((Consumer<String>)SectionStorage.LOGGER::error).ifPresent(object -> map4.put(dynamicOps.createString(string10), object));
                }
            }
        }
        return new Dynamic<T>(dynamicOps, dynamicOps.createMap((java.util.Map<T, T>)ImmutableMap.<T, T>of(dynamicOps.createString("Sections"), dynamicOps.createMap(map4), dynamicOps.createString("DataVersion"), dynamicOps.createInt(SharedConstants.getCurrentVersion().getWorldVersion()))));
    }
