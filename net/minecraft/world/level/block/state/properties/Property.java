...
public abstract class Property<T extends Comparable<T>> {
    ...
    protected Property(final String string, final Class<T> class2) {
        this.codec = Codec.STRING.<T>comapFlatMap((java.util.function.Function<? super Object, ? extends DataResult<? extends T>>)(string -> (DataResult)this.getValue(string).map(DataResult::success).orElseGet(() -> DataResult.error(new StringBuilder().append("Unable to read property: ").append(this).append(" with value: ").append(string).toString()))), (java.util.function.Function<? super T, ?>)this::getName);
        this.valueCodec = this.codec.<Value<T>>xmap((java.util.function.Function<? super T, ? extends Value<T>>)this::value, (java.util.function.Function<? super Value<T>, ? extends T>)Value::value);
        this.clazz = class2;
        this.name = string;
    }
