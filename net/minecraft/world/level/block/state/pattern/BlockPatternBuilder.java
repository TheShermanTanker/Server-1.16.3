...
public class BlockPatternBuilder {
    ...
    private BlockPatternBuilder() {
        ...
        (this.lookup = Maps.newHashMap()).put(' ', Predicates.alwaysTrue());
    }
    
    public BlockPatternBuilder aisle(final String... arr) {
        if (ArrayUtils.isEmpty(arr) || StringUtils.isEmpty((CharSequence)arr[0])) {
    ...
    private void ensureAllCharactersMatched() {
        final List<Character> list2 = Lists.newArrayList();
        ...
        if (!list2.isEmpty()) {
            throw new IllegalStateException("Predicates for character(s) " + BlockPatternBuilder.COMMA_JOINED.join(list2) + " are missing");
        }
    }
