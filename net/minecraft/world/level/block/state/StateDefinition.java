...
public class StateDefinition<O, S extends StateHolder<O, S>> {
    ...
    protected StateDefinition(final Function<O, S> function, final O object, final Factory<O, S> b, final Map<String, Property<?>> map) {
        this.owner = object;
        this.propertiesByName = ImmutableSortedMap.<String, Property<?>>copyOf((java.util.Map<? extends String, ? extends Property<?>>)map);
        final Supplier<S> supplier6 = (Supplier<S>)(() -> (StateHolder)function.apply(object));
        MapCodec<S> mapCodec7 = MapCodec.<S>of(Encoder.<S>empty(), Decoder.unit((java.util.function.Supplier<A>)supplier6));
        ...
        final Map<Map<Property<?>, Comparable<?>>, S> map2 = Maps.newLinkedHashMap();
        final List<S> list10 = Lists.newArrayList();
        ...
        for (final Property<?> cfg13 : this.propertiesByName.values()) {
            stream11 = (Stream<List<Pair<Property<?>, Comparable<?>>>>)stream11.flatMap(list -> cfg13.getPossibleValues().stream().map(comparable -> {
                final List<Pair<Property<?>, Comparable<?>>> list2 = Lists.newArrayList((java.lang.Iterable<?>)list);
                list2.add(Pair.<Property, Comparable>of(cfg13, comparable));
                return list2;
            }));
        }
        stream11.forEach(list6 -> {
            final ImmutableMap<Property<?>, Comparable<?>> immutableMap7 = (ImmutableMap<Property<?>, Comparable<?>>)list6.stream().collect((Collector)ImmutableMap.toImmutableMap((java.util.function.Function<? super Object, ?>)Pair::getFirst, (java.util.function.Function<? super Object, ?>)Pair::getSecond));
        ...
        this.states = ImmutableList.<S>copyOf((java.util.Collection<? extends S>)list10);
    }
    ...
    private static <S extends StateHolder<?, S>, T extends Comparable<T>> MapCodec<S> appendPropertyCodec(final MapCodec<S> mapCodec, final Supplier<S> supplier, final String string, final Property<T> cfg) {
        return Codec.<S, Property.Value<T>>mapPair(mapCodec, cfg.valueCodec().fieldOf(string).setPartial((java.util.function.Supplier<Property.Value<T>>)(() -> cfg.value((StateHolder)supplier.get())))).<S>xmap((java.util.function.Function<? super Pair<S, Property.Value<T>>, ? extends S>)(pair -> pair.getFirst().<Comparable, Comparable>setValue((Property<Comparable>)cfg, ((Property.Value)pair.getSecond()).value())), (java.util.function.Function<? super S, ? extends Pair<S, Property.Value<T>>>)(ceg -> Pair.<StateHolder, Property.Value>of(ceg, cfg.value(ceg))));
    }
