...
public class StructureFeatureManager {
    ...
    public StructureStart<?> getStructureAt(final BlockPos fx, final boolean boolean2, final StructureFeature<?> ckx) {
        return DataFixUtils.<StructureStart<?>>orElse((java.util.Optional<? extends StructureStart<?>>)this.startsForFeature(SectionPos.of(fx), ckx).filter(crs -> crs.getBoundingBox().isInside(fx)).filter(crs -> !boolean2 || crs.getPieces().stream().anyMatch(crr -> crr.getBoundingBox().isInside(fx))).findFirst(), StructureStart.INVALID_START);
    }
}
