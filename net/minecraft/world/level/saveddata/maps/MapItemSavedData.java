...
public class MapItemSavedData extends SavedData {
    ...
    public void load(final CompoundTag md) {
        this.dimension = (ResourceKey<Level>)DimensionType.parseLegacy(new Dynamic<>(NbtOps.INSTANCE, md.get("dimension"))).resultOrPartial((Consumer<String>)MapItemSavedData.LOGGER::error).orElseThrow(() -> new IllegalArgumentException(new StringBuilder().append("Invalid map dimension: ").append(md.get("dimension")).toString()));
    ...
    public CompoundTag save(final CompoundTag md) {
        ResourceLocation.CODEC.encodeStart((DynamicOps<Object>)NbtOps.INSTANCE, this.dimension.location()).resultOrPartial((Consumer<String>)MapItemSavedData.LOGGER::error).ifPresent(mt -> md.put("dimension", mt));
