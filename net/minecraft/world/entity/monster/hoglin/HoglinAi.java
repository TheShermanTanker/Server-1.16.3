...
public class HoglinAi {
    ...
    private static void initCoreActivity(final Brain<Hoglin> arc) {
        arc.addActivity(Activity.CORE, 0, ImmutableList.of(new LookAtTargetSink(45, 90), new MoveToTargetSink()));
    }
    
    private static void initIdleActivity(final Brain<Hoglin> arc) {
        arc.addActivity(Activity.IDLE, 10, ImmutableList.of(new BecomePassiveIfMemoryPresent(MemoryModuleType.NEAREST_REPELLENT, 200), new AnimalMakeLove(EntityType.HOGLIN, 0.6f), SetWalkTargetAwayFrom.pos(MemoryModuleType.NEAREST_REPELLENT, 1.0f, 8, true), new Behavior((java.util.function.Function<? super Hoglin, Optional<? extends LivingEntity>>)HoglinAi::findNearestValidAttackTarget), new Behavior((java.util.function.Predicate<? super Hoglin>)Hoglin::isAdult, SetWalkTargetAwayFrom.entity(MemoryModuleType.NEAREST_VISIBLE_ADULT_PIGLIN, 0.4f, 8, false)), new Behavior(new SetEntityLookTarget(8.0f), IntRange.of(30, 60)), new BabyFollowAdult(HoglinAi.ADULT_FOLLOW_RANGE, 0.6f), createIdleMovementBehaviors()));
    }
    
    private static void initFightActivity(final Brain<Hoglin> arc) {
        arc.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, ImmutableList.of(new BecomePassiveIfMemoryPresent(MemoryModuleType.NEAREST_REPELLENT, 200), new AnimalMakeLove(EntityType.HOGLIN, 0.6f), new SetWalkTargetFromAttackTargetIfTargetOutOfReach(1.0f), new Behavior((java.util.function.Predicate<? super Hoglin>)Hoglin::isAdult, new MeleeAttack(40)), new Behavior((java.util.function.Predicate<? super Hoglin>)AgableMob::isBaby, new MeleeAttack(15)), new StopAttackingIfTargetInvalid(), new Behavior((java.util.function.Predicate<? super Hoglin>)HoglinAi::isBreeding, MemoryModuleType.ATTACK_TARGET)), MemoryModuleType.ATTACK_TARGET);
    }
    
    private static void initRetreatActivity(final Brain<Hoglin> arc) {
        arc.addActivityAndRemoveMemoryWhenStopped(Activity.AVOID, 10, ImmutableList.of(SetWalkTargetAwayFrom.entity(MemoryModuleType.AVOID_TARGET, 1.3f, 15, false), createIdleMovementBehaviors(), new Behavior(new SetEntityLookTarget(8.0f), IntRange.of(30, 60)), new Behavior((java.util.function.Predicate<? super Hoglin>)HoglinAi::wantsToStopFleeing, MemoryModuleType.AVOID_TARGET)), MemoryModuleType.AVOID_TARGET);
    }
    
    private static RunOne<Hoglin> createIdleMovementBehaviors() {
        return new RunOne<Hoglin>((java.util.List<Pair<Behavior<? super Hoglin>, Integer>>)ImmutableList.<Pair<RandomStroll, Integer>>of(Pair.<RandomStroll, Integer>of(new RandomStroll(0.4f), 2), Pair.of(new SetWalkTargetFromLookTarget(0.4f, 3), 2), Pair.of(new DoNothing(30, 60), 1)));
    }
    ...
    protected static void updateActivity(final Hoglin bej) {
        ...
        arc2.setActiveActivityToFirstValid(ImmutableList.of(Activity.FIGHT, Activity.AVOID, Activity.IDLE));
