...
public class Zoglin extends Monster implements Enemy, HoglinBase {
    ...
    protected Brain<?> makeBrain(final Dynamic<?> dynamic) {
        ...
        arc3.setCoreActivities(ImmutableSet.of(Activity.CORE));
    ...
    private static void initIdleActivity(final Brain<Zoglin> arc) {
        arc.addActivity(Activity.IDLE, 10, ImmutableList.of(new Behavior((java.util.function.Function<? super Zoglin, Optional<? extends LivingEntity>>)Zoglin::findNearestValidAttackTarget), new Behavior(new SetEntityLookTarget(8.0f), IntRange.of(30, 60)), new Behavior((java.util.List<Pair<Behavior<? super Zoglin>, Integer>>)ImmutableList.<Pair<RandomStroll, Integer>>of(Pair.<RandomStroll, Integer>of(new RandomStroll(0.4f), 2), Pair.of(new SetWalkTargetFromLookTarget(0.4f, 3), 2), Pair.of(new DoNothing(30, 60), 1)))));
    }
    
    private static void initFightActivity(final Brain<Zoglin> arc) {
        arc.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, ImmutableList.of(new SetWalkTargetFromAttackTargetIfTargetOutOfReach(1.0f), new Behavior((java.util.function.Predicate<? super Zoglin>)Zoglin::isAdult, new MeleeAttack(40)), new Behavior((java.util.function.Predicate<? super Zoglin>)Zoglin::isBaby, new MeleeAttack(15)), new StopAttackingIfTargetInvalid()), MemoryModuleType.ATTACK_TARGET);
    }
