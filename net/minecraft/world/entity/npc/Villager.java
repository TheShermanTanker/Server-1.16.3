...
public class Villager extends AbstractVillager implements ReputationEventHandler, VillagerDataHolder {
    ...
    private void registerBrainGoals(final Brain<Villager> arc) {
        ...
        if (this.isBaby()) {
            ...
            arc.addActivityWithConditions(Activity.WORK, VillagerGoalPackages.getWorkPackage(bfj3, 0.5f), ImmutableSet.of(Pair.<MemoryModuleType<GlobalPos>, MemoryStatus>of(MemoryModuleType.JOB_SITE, MemoryStatus.VALUE_PRESENT)));
        }
        arc.addActivity(Activity.CORE, VillagerGoalPackages.getCorePackage(bfj3, 0.5f));
        arc.addActivityWithConditions(Activity.MEET, VillagerGoalPackages.getMeetPackage(bfj3, 0.5f), ImmutableSet.of(Pair.<MemoryModuleType<GlobalPos>, MemoryStatus>of(MemoryModuleType.MEETING_POINT, MemoryStatus.VALUE_PRESENT)));
    ...
    public void addAdditionalSaveData(final CompoundTag md) {
        ...
        VillagerData.CODEC.encodeStart((DynamicOps<Object>)NbtOps.INSTANCE, this.getVillagerData()).resultOrPartial((Consumer<String>)Villager.LOGGER::error).ifPresent(mt -> md.put("VillagerData", mt));
        md.putByte("FoodLevel", this.foodLevel);
        md.put("Gossips", (Tag)this.gossips.<Tag>store((DynamicOps<Tag>)NbtOps.INSTANCE).getValue());
    ...
    public void readAdditionalSaveData(final CompoundTag md) {
        ...
        if (md.contains("VillagerData", 10)) {
            final DataResult<VillagerData> dataResult3 = VillagerData.CODEC.<Tag>parse(new Dynamic<Tag>(NbtOps.INSTANCE, md.get("VillagerData")));
            dataResult3.resultOrPartial((Consumer<String>)Villager.LOGGER::error).ifPresent(this::setVillagerData);
        }
        ...
        this.gossips.update(new Dynamic<>(NbtOps.INSTANCE, mj3));
        if (md.contains("Xp", 3)) {
    ...
    protected void updateTrades() {
        ...
        final VillagerTrades.ItemListing[] arr4 = int2ObjectMap3.get(bfh2.getLevel());
        if (arr4 == null) {
            return;
        }
    ...
    public void setGossips(final Tag mt) {
        this.gossips.update(new Dynamic<>(NbtOps.INSTANCE, mt));
    }
    ...
    static {
        ...
        FOOD_POINTS = (Map)ImmutableMap.<Item, Integer>of(Items.BREAD, 4, Items.POTATO, 1, Items.CARROT, 1, Items.BEETROOT, 1);
        WANTED_ITEMS = (Set)ImmutableSet.<Item>of(Items.BREAD, Items.POTATO, Items.CARROT, Items.WHEAT, Items.WHEAT_SEEDS, Items.BEETROOT, Items.BEETROOT_SEEDS);
        MEMORY_TYPES = ImmutableList.<MemoryModuleType<GlobalPos>>of(MemoryModuleType.HOME, MemoryModuleType.JOB_SITE, MemoryModuleType.POTENTIAL_JOB_SITE, MemoryModuleType.MEETING_POINT, (MemoryModuleType<GlobalPos>)MemoryModuleType.LIVING_ENTITIES, (MemoryModuleType<GlobalPos>)MemoryModuleType.VISIBLE_LIVING_ENTITIES, (MemoryModuleType<GlobalPos>)MemoryModuleType.VISIBLE_VILLAGER_BABIES, (MemoryModuleType<GlobalPos>)MemoryModuleType.NEAREST_PLAYERS, (MemoryModuleType<GlobalPos>)MemoryModuleType.NEAREST_VISIBLE_PLAYER, (MemoryModuleType<GlobalPos>)MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYER, (MemoryModuleType<GlobalPos>)MemoryModuleType.NEAREST_VISIBLE_WANTED_ITEM, (MemoryModuleType<GlobalPos>)MemoryModuleType.WALK_TARGET, MemoryModuleType.LOOK_TARGET, MemoryModuleType.INTERACTION_TARGET, MemoryModuleType.BREED_TARGET, MemoryModuleType.PATH, MemoryModuleType.DOORS_TO_CLOSE, MemoryModuleType.NEAREST_BED, MemoryModuleType.HURT_BY, MemoryModuleType.HURT_BY_ENTITY, MemoryModuleType.NEAREST_HOSTILE, MemoryModuleType.SECONDARY_JOB_SITE, MemoryModuleType.HIDING_PLACE, MemoryModuleType.HEARD_BELL_TIME, MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE, MemoryModuleType.LAST_SLEPT, MemoryModuleType.LAST_WOKEN, MemoryModuleType.LAST_WORKED_AT_POI, MemoryModuleType.GOLEM_DETECTED_RECENTLY);
        SENSOR_TYPES = ImmutableList.<SensorType<NearestLivingEntitySensor>>of(SensorType.NEAREST_LIVING_ENTITIES, (SensorType<NearestLivingEntitySensor>)SensorType.NEAREST_PLAYERS, (SensorType<NearestLivingEntitySensor>)SensorType.NEAREST_ITEMS, (SensorType<NearestLivingEntitySensor>)SensorType.NEAREST_BED, (SensorType<NearestLivingEntitySensor>)SensorType.HURT_BY, (SensorType<NearestLivingEntitySensor>)SensorType.VILLAGER_HOSTILES, (SensorType<NearestLivingEntitySensor>)SensorType.VILLAGER_BABIES, (SensorType<NearestLivingEntitySensor>)SensorType.SECONDARY_POIS, (SensorType<NearestLivingEntitySensor>)SensorType.GOLEM_DETECTED);
        POI_MEMORIES = (Map)ImmutableMap.<MemoryModuleType<GlobalPos>, Object>of(MemoryModuleType.HOME, (bfg, azo) -> azo == PoiType.HOME, MemoryModuleType.JOB_SITE, (bfg, azo) -> bfg.getVillagerData().getProfession().getJobPoiType() == azo, MemoryModuleType.POTENTIAL_JOB_SITE, (bfg, azo) -> PoiType.ALL_JOBS.test(azo), MemoryModuleType.MEETING_POINT, ((bfg, azo) -> azo == PoiType.MEETING));
    }
}
