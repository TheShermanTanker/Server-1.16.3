...
public class GossipContainer {
    ...
    private Collection<GossipEntry> selectGossipsForTransfer(final Random random, final int integer) {
        ...
        final Set<GossipEntry> set7 = Sets.<GossipEntry>newIdentityHashSet();
    ...
    public <T> Dynamic<T> store(final DynamicOps<T> dynamicOps) {
        return new Dynamic<T>(dynamicOps, dynamicOps.createList((java.util.stream.Stream<T>)this.unpack().map(b -> b.store((DynamicOps<Object>)dynamicOps)).map(Dynamic::getValue)));
    }
    ...
    static class GossipEntry {
        ...
        public <T> Dynamic<T> store(final DynamicOps<T> dynamicOps) {
            return new Dynamic<T>(dynamicOps, dynamicOps.createMap((java.util.Map<T, T>)ImmutableMap.<T, Object>of(dynamicOps.createString("Target"), SerializableUUID.CODEC.<T>encodeStart(dynamicOps, this.target).result().orElseThrow(RuntimeException::new), dynamicOps.createString("Type"), dynamicOps.createString(this.type.id), dynamicOps.createString("Value"), dynamicOps.createInt(this.value))));
        }
        
        public static DataResult<GossipEntry> load(final Dynamic<?> dynamic) {
            return DataResult.<GossipEntry>unbox(((Kind1<DataResult.Mu, Mu>)DataResult.instance()).<UUID, GossipType, Integer>group(dynamic.get("Target").read(SerializableUUID.CODEC), dynamic.get("Type").asString().map((java.util.function.Function<? super String, ?>)GossipType::byId), dynamic.get("Value").asNumber().map((java.util.function.Function<? super Number, ?>)Number::intValue)).apply(DataResult.instance(), (Function3<UUID, GossipType, Integer, R>)GossipEntry::new));
        }
    }
    ...
    static class EntityGossips {
        ...
        private EntityGossips() {
            this.entries = new Object2IntOpenHashMap<GossipType>();
        }
        ...
        public void decay() {
            final ObjectIterator<Object2IntMap.Entry<GossipType>> objectIterator2 = this.entries.object2IntEntrySet().iterator();
