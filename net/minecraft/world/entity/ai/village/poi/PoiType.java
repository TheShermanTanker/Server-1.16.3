...
public class PoiType {
    ...
    private static Set<BlockState> getBlockStates(final Block bul) {
        return ImmutableSet.copyOf((java.util.Collection<?>)bul.getStateDefinition().getPossibleStates());
    }
    ...
    private PoiType(final String string, final Set<BlockState> set, final int integer3, final Predicate<PoiType> predicate, final int integer5) {
        this.name = string;
        this.matchingStates = ImmutableSet.copyOf((java.util.Collection<?>)set);
    ...
    static {
        ALL_JOB_POI_TYPES = (Supplier)Suppliers.<Set>memoize(() -> (Set)Registry.VILLAGER_PROFESSION.stream().map(VillagerProfession::getJobPoiType).collect(Collectors.toSet()));
        ALL_JOBS = (azo -> ((Set)PoiType.ALL_JOB_POI_TYPES.get()).contains(azo));
        ALL = (azo -> true);
        BEDS = (Set)ImmutableList.<Block>of(Blocks.RED_BED, Blocks.BLACK_BED, Blocks.BLUE_BED, Blocks.BROWN_BED, Blocks.CYAN_BED, Blocks.GRAY_BED, Blocks.GREEN_BED, Blocks.LIGHT_BLUE_BED, Blocks.LIGHT_GRAY_BED, Blocks.LIME_BED, Blocks.MAGENTA_BED, Blocks.ORANGE_BED, Blocks.PINK_BED, Blocks.PURPLE_BED, Blocks.WHITE_BED, Blocks.YELLOW_BED).stream().flatMap(bul -> bul.getStateDefinition().getPossibleStates().stream()).filter(cee -> cee.<BedPart>getValue(BedBlock.PART) == BedPart.HEAD).collect((Collector)ImmutableSet.toImmutableSet());
        TYPE_BY_STATE = (Map)Maps.newHashMap();
        UNEMPLOYED = register("unemployed", ImmutableSet.of(), 1, PoiType.ALL_JOBS, 1);
        ...
        ALL_STATES = (Set)new ObjectOpenHashSet((java.util.Collection<?>)PoiType.TYPE_BY_STATE.keySet());
    }
}
