...
public class PoiSection {
    ...
    public static Codec<PoiSection> codec(final Runnable runnable) {
        return RecordCodecBuilder.<PoiSection>create((java.util.function.Function<RecordCodecBuilder.Instance<PoiSection>, ? extends App<RecordCodecBuilder.Mu<PoiSection>, PoiSection>>)(instance -> instance.<Runnable, Boolean, java.util.List<PoiRecord>>group(RecordCodecBuilder.point(runnable), Codec.BOOL.optionalFieldOf("Valid", Boolean.valueOf(false)).forGetter((java.util.function.Function<Object, Boolean>)(azn -> azn.isValid)), PoiRecord.codec(runnable).listOf().fieldOf("Records").forGetter((java.util.function.Function<Object, java.util.List<PoiRecord>>)(azn -> ImmutableList.copyOf((java.util.Collection<?>)azn.records.values())))).<PoiSection>apply(instance, PoiSection::new))).orElseGet(Util.prefix("Failed to read POI section: ", (Consumer<String>)PoiSection.LOGGER::error), (java.util.function.Supplier<? extends PoiSection>)(() -> new PoiSection(runnable, false, ImmutableList.of())));
    }
    ...
    private PoiSection(final Runnable runnable, final boolean boolean2, final List<PoiRecord> list) {
        this.records = new Short2ObjectOpenHashMap<PoiRecord>();
    ...
    public void add(final BlockPos fx, final PoiType azo) {
        if (this.add(new PoiRecord(fx, azo, this.setDirty))) {
            PoiSection.LOGGER.debug("Added POI of type {} @ {}", new org.apache.logging.log4j.util.Supplier[] { () -> azo, () -> fx });
            this.setDirty.run();
        }
    }
    ...
    private boolean add(final PoiRecord azm) {
        ...
        final PoiRecord azm2 = this.records.get(short5);
        if (azm2 == null) {
    ...
    public void remove(final BlockPos fx) {
        final PoiRecord azm3 = this.records.remove(SectionPos.sectionRelativePos(fx));
        if (azm3 == null) {
        ...
        PoiSection.LOGGER.debug("Removed POI of type {} @ {}", new org.apache.logging.log4j.util.Supplier[] { azm3::getPoiType, azm3::getPos });
        this.setDirty.run();
    }
    
    public boolean release(final BlockPos fx) {
        final PoiRecord azm3 = this.records.get(SectionPos.sectionRelativePos(fx));
        if (azm3 == null) {
    ...
    public boolean exists(final BlockPos fx, final Predicate<PoiType> predicate) {
        ...
        final PoiRecord azm5 = this.records.get(short4);
    ...
    public Optional<PoiType> getType(final BlockPos fx) {
        ...
        final PoiRecord azm4 = this.records.get(short3);
    ...
    public void refresh(final Consumer<BiConsumer<BlockPos, PoiType>> consumer) {
        if (!this.isValid) {
            final Short2ObjectMap<PoiRecord> short2ObjectMap3 = new Short2ObjectOpenHashMap<PoiRecord>(this.records);
            this.clear();
            consumer.accept(((fx, azo) -> {
                final short short5 = SectionPos.sectionRelativePos(fx);
                final PoiRecord azm6 = short2ObjectMap3.computeIfAbsent(short5, integer -> new PoiRecord(fx, azo, this.setDirty));
                this.add(azm6);
            }));
            this.isValid = true;
            this.setDirty.run();
