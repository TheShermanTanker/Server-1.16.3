...
public class Brain<E extends LivingEntity> {
    ...
    public static <E extends LivingEntity> Codec<Brain<E>> codec(final Collection<? extends MemoryModuleType<?>> collection1, final Collection<? extends SensorType<? extends Sensor<? super E>>> collection2) {
        final MutableObject<Codec<Brain<E>>> mutableObject3 = new MutableObject<Codec<Brain<E>>>();
        mutableObject3.setValue(new MapCodec<Brain<E>>() {
            ...
            public <T> DataResult<Brain<E>> decode(final DynamicOps<T> dynamicOps, final MapLike<T> mapLike) {
                final MutableObject<DataResult<ImmutableList.Builder<MemoryValue<?>>>> mutableObject4 = new MutableObject<DataResult<ImmutableList.Builder<MemoryValue<?>>>>(DataResult.success(ImmutableList.builder()));
                mapLike.entries().forEach(pair -> {
                    final DataResult<MemoryModuleType<?>> dataResult5 = Registry.MEMORY_MODULE_TYPE.parse(dynamicOps, pair.getFirst());
                    final DataResult<? extends MemoryValue<?>> dataResult6 = dataResult5.flatMap((java.util.function.Function<? super MemoryModuleType<?>, ? extends DataResult<? extends MemoryValue<?>>>)(aya -> this.captureRead((MemoryModuleType<Object>)aya, dynamicOps, pair.getSecond())));
                    mutableObject4.setValue(mutableObject4.getValue().apply2((java.util.function.BiFunction<Object, ? extends MemoryValue<?>, Object>)ImmutableList.Builder::add, dataResult6));
                });
                final ImmutableList<MemoryValue<?>> immutableList5 = (ImmutableList<MemoryValue<?>>)mutableObject4.getValue().resultOrPartial((Consumer<String>)Brain.LOGGER::error).map(ImmutableList.Builder::build).orElseGet(ImmutableList::of);
                return DataResult.<Brain<E>>success(new Brain<E>(collection1, collection2, immutableList5, (java.util.function.Supplier<Codec<Brain<E>>>)mutableObject3::getValue));
            }
            ...
            private <T, U> DataResult<MemoryValue<U>> captureRead(final MemoryModuleType<U> aya, final DynamicOps<T> dynamicOps, final T object) {
                return ((DataResult)aya.getCodec().map(DataResult::success).orElseGet(() -> DataResult.error(new StringBuilder().append("No codec for memory: ").append(aya).toString()))).flatMap(codec -> codec.parse((DynamicOps<Object>)dynamicOps, object)).<MemoryValue<U>>map((java.util.function.Function<? super Object, ? extends MemoryValue<U>>)(axz -> new MemoryValue(aya, Optional.of(axz))));
            }
    ...
    public void addActivityAndRemoveMemoryWhenStopped(final Activity bhc, final int integer, final ImmutableList<? extends Behavior<? super E>> immutableList, final MemoryModuleType<?> aya) {
        final Set<Pair<MemoryModuleType<?>, MemoryStatus>> set6 = ImmutableSet.of(Pair.<MemoryModuleType<?>, MemoryStatus>of(aya, MemoryStatus.VALUE_PRESENT));
        final Set<MemoryModuleType<?>> set7 = ImmutableSet.of(aya);
    ...
    public void addActivity(final Activity bhc, final ImmutableList<? extends Pair<Integer, ? extends Behavior<? super E>>> immutableList) {
        this.addActivityAndRemoveMemoriesWhenStopped(bhc, immutableList, ImmutableSet.of(), Sets.newHashSet());
    }
    ...
    public void addActivityWithConditions(final Activity bhc, final ImmutableList<? extends Pair<Integer, ? extends Behavior<? super E>>> immutableList, final Set<Pair<MemoryModuleType<?>, MemoryStatus>> set) {
        this.addActivityAndRemoveMemoriesWhenStopped(bhc, immutableList, set, Sets.newHashSet());
    }
    ...
    public Brain<E> copyWithoutBehaviors() {
        final Brain<E> arc2 = new Brain<E>(this.memories.keySet(), this.sensors.keySet(), ImmutableList.<MemoryValue<?>>of(), this.codec);
    ...
    private boolean activityRequirementsAreMet(final Activity bhc) {
        ...
        for (final Pair<MemoryModuleType<?>, MemoryStatus> pair4 : (Set)this.activityRequirements.get(bhc)) {
            ...
            final MemoryStatus ayb6 = pair4.getSecond();
    ...
    ImmutableList<? extends Pair<Integer, ? extends Behavior<? super E>>> createPriorityPairs(final int integer, final ImmutableList<? extends Behavior<? super E>> immutableList) {
        ...
        final ImmutableList.Builder<Pair<Integer, ? extends Behavior<? super E>>> builder5 = ImmutableList.<Pair<Integer, ? extends Behavior<? super E>>>builder();
        for (final Behavior<? super E> ars7 : immutableList) {
            builder5.add(Pair.of(integer2++, ars7));
        }
    ...
    public static final class Provider<E extends LivingEntity> {
        ...
        public Brain<E> makeBrain(final Dynamic<?> dynamic) {
            return (Brain<E>)this.codec.parse(dynamic).resultOrPartial((Consumer<String>)Brain.LOGGER::error).orElseGet(() -> new Brain(this.memoryTypes, this.sensorTypes, ImmutableList.<MemoryValue<?>>of(), (Supplier<Codec<Brain<E>>>)(() -> this.codec)));
        }
    }
    ...
    static final class MemoryValue<U> {
        ...
        public <T> void serialize(final DynamicOps<T> dynamicOps, final RecordBuilder<T> recordBuilder) {
            this.type.getCodec().ifPresent(codec -> this.value.ifPresent(axz -> recordBuilder.add(Registry.MEMORY_MODULE_TYPE.encodeStart((DynamicOps<Object>)dynamicOps, this.type), codec.encodeStart(dynamicOps, axz))));
        }
    }
}
