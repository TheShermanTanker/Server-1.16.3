...
public class InteractWithDoor extends Behavior<LivingEntity> {
    ...
    public InteractWithDoor() {
        super((Map)ImmutableMap.<MemoryModuleType<Path>, MemoryStatus>of(MemoryModuleType.PATH, MemoryStatus.VALUE_PRESENT, (MemoryModuleType<Path>)MemoryModuleType.DOORS_TO_CLOSE, MemoryStatus.REGISTERED));
    }
    ...
    private void rememberDoorToClose(final ServerLevel aag, final LivingEntity aqj, final BlockPos fx) {
        ...
        if (arc5.<Set<GlobalPos>>getMemory(MemoryModuleType.DOORS_TO_CLOSE).isPresent()) {
            ...
            arc5.<java.util.HashSet<GlobalPos>>setMemory((MemoryModuleType<java.util.HashSet<GlobalPos>>)MemoryModuleType.DOORS_TO_CLOSE, Sets.<GlobalPos>newHashSet(gf6));
        }
    }
}
