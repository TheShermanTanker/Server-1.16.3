...
public class TradeWithVillager extends Behavior<Villager> {
    ...
    public TradeWithVillager() {
        super((Map)ImmutableMap.<MemoryModuleType<LivingEntity>, MemoryStatus>of(MemoryModuleType.INTERACTION_TARGET, MemoryStatus.VALUE_PRESENT, (MemoryModuleType<LivingEntity>)MemoryModuleType.VISIBLE_LIVING_ENTITIES, MemoryStatus.VALUE_PRESENT));
    ...
    protected void tick(final ServerLevel aag, final Villager bfg, final long long3) {
        ...
        if (bfg2.getVillagerData().getProfession() == VillagerProfession.FARMER && bfg.getInventory().countItem(Items.WHEAT) > Items.WHEAT.getMaxStackSize() / 2) {
            throwHalfStack(bfg, ImmutableSet.of(Items.WHEAT), bfg2);
        }
