...
public class MoveToTargetSink extends Behavior<Mob> {
    ...
    public MoveToTargetSink(final int integer1, final int integer2) {
        super((Map)ImmutableMap.<MemoryModuleType<Long>, MemoryStatus>of(MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE, MemoryStatus.REGISTERED, (MemoryModuleType<Long>)MemoryModuleType.PATH, MemoryStatus.VALUE_ABSENT, (MemoryModuleType<Long>)MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_PRESENT), integer1, integer2);
    }
    ...
    protected void stop(final ServerLevel aag, final Mob aqk, final long long3) {
        if (aqk.getBrain().hasMemoryValue(MemoryModuleType.WALK_TARGET) && !this.reachedTarget(aqk, (WalkTarget)aqk.getBrain().<WalkTarget>getMemory(MemoryModuleType.WALK_TARGET).get()) && aqk.getNavigation().isStuck()) {
