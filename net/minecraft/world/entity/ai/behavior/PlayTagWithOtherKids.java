...
public class PlayTagWithOtherKids extends Behavior<PathfinderMob> {
    public PlayTagWithOtherKids() {
        super((Map)ImmutableMap.<MemoryModuleType<List<LivingEntity>>, MemoryStatus>of(MemoryModuleType.VISIBLE_VILLAGER_BABIES, MemoryStatus.VALUE_PRESENT, (MemoryModuleType<List<LivingEntity>>)MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT, (MemoryModuleType<List<LivingEntity>>)MemoryModuleType.LOOK_TARGET, MemoryStatus.REGISTERED, (MemoryModuleType<List<LivingEntity>>)MemoryModuleType.INTERACTION_TARGET, MemoryStatus.REGISTERED));
    }
    ...
    private Map<LivingEntity, Integer> checkHowManyChasersEachFriendHas(final PathfinderMob aqr) {
        final Map<LivingEntity, Integer> map3 = Maps.newHashMap();
