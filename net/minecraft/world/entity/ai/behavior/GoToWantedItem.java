...
public class GoToWantedItem<E extends LivingEntity> extends Behavior<E> {
    ...
    public GoToWantedItem(final Predicate<E> predicate, final float float2, final boolean boolean3, final int integer) {
        super((Map)ImmutableMap.<MemoryModuleType<PositionTracker>, MemoryStatus>of(MemoryModuleType.LOOK_TARGET, MemoryStatus.REGISTERED, (MemoryModuleType<PositionTracker>)MemoryModuleType.WALK_TARGET, boolean3 ? MemoryStatus.REGISTERED : MemoryStatus.VALUE_ABSENT, (MemoryModuleType<PositionTracker>)MemoryModuleType.NEAREST_VISIBLE_WANTED_ITEM, MemoryStatus.VALUE_PRESENT));
