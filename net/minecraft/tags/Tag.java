...
public interface Tag<T> {
    default <T> Codec<Tag<T>> codec(final Supplier<TagCollection<T>> supplier) {
        return ResourceLocation.CODEC.<Tag<T>>flatXmap((java.util.function.Function<? super ResourceLocation, ? extends DataResult<? extends Tag<T>>>)(vk -> (DataResult)Optional.ofNullable(((TagCollection)supplier.get()).getTag(vk)).map(DataResult::success).orElseGet(() -> DataResult.error(new StringBuilder().append("Unknown tag: ").append(vk).toString()))), (java.util.function.Function<? super Tag<T>, ? extends DataResult<? extends ResourceLocation>>)(aej -> (DataResult)Optional.ofNullable(((TagCollection)supplier.get()).getId(aej)).map(DataResult::success).orElseGet(() -> DataResult.error(new StringBuilder().append("Unknown tag: ").append(aej).toString()))));
    }
    ...
    public static class Builder {
        ...
        public <T> Optional<Tag<T>> build(final Function<ResourceLocation, Tag<T>> function1, final Function<ResourceLocation, T> function2) {
            final ImmutableSet.Builder<T> builder4 = ImmutableSet.<T>builder();
        ...
        public Builder addFromJson(final JsonObject jsonObject, final String string) {
            ...
            final List<Entry> list5 = Lists.newArrayList();
